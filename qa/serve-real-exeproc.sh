...
CREATE PROCEDURE PW9 RETURNS (R1 INTEGER) AS BEGIN FOR WITH RECURSIVE N AS (SELECT ID FROM T WHERE ID = 1 UNION ALL SELECT N.ID + 1 FROM N WHERE N.ID < 6) SELECT N.ID FROM N INTO :R1 DO SUSPEND; END^
CREATE PROCEDURE PWA RETURNS (R1 INTEGER, R2 INTEGER) AS BEGIN FOR WITH RECURSIVE N AS (SELECT ID, AMT FROM T WHERE ID = 2 UNION ALL SELECT N.ID + 1, N.AMT - 1 FROM N WHERE N.ID < 4) SELECT N.ID, N.AMT FROM N INTO :R1, :R2 DO SUSPEND; END^
SET TERM ; ^
COMMIT;
EOF
