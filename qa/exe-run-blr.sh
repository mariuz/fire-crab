...
refuse "RETURNS (R1 INTEGER) AS BEGIN FOR SELECT A.ID FROM T A JOIN U B ON A.ID = B.UID JOIN T C ON C.ID = B.UID INTO :R1 DO SUSPEND; END"
refuse "(P1 INTEGER) RETURNS (R1 INTEGER) AS BEGIN FOR SELECT ID FROM T WHERE ID = :P1 PLAN (T INDEX (IDX_EXE)) INTO :R1 DO SUSPEND; END"
