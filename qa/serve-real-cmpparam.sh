...
both "R7 promoted (a zoned cast converts through the session zone's rules) - H SELECT CAST(TZ AS TIMESTAMP) AS X FROM TS -> 09:30:00 / 00:00:00 / 10:30:00.5 (converted to the session zone; both binaries used to raise Conversion error) (engine [2024-01-10T09:30:00.000Z;2024-02-10T00:0], this server raises at execute; prev prepared then Conversion error from string '20)" "SELECT CAST(TZ AS TIMESTAMP) AS X FROM TS ORDER BY ID" '[]' "ERR"
both "R7 promoted (a zoned cast converts through the session zone's rules) - H SELECT CAST(TZ AS DATE) AS X FROM TS -> 2024-01-10;2024-02-10;2024-03-10 (engine [2024-01-10T00:00:00.000Z;2024-02-10T00:0], this server raises at execute; prev prepared then Conversion error from string '20)" "SELECT CAST(TZ AS DATE) AS X FROM TS ORDER BY ID" '[]' "ERR"
both "R7 promoted (a zoned cast converts through the session zone's rules) - H SELECT CAST(TZ AS TIME) AS X FROM TS -> 09:30:00;00:00:00;10:30:00.5 (engine [1970-01-01T09:30:00.000Z;1970-01-01T00:0], this server raises at execute; prev prepared then Conversion error from string '20)" "SELECT CAST(TZ AS TIME) AS X FROM TS ORDER BY ID" '[]' "ERR"
both "R7 promoted (a zoned TIME into a TIME dates its wall time today) - H SELECT CAST(TMZ AS TIME) AS X FROM TS WHERE ID < 3 -> 09:30:00;01:02:03 (offset zones convert) (engine [1970-01-01T09:30:00.000Z;1970-01-01T01:0], this server raises at execute; prev prepared then Conversion error from string '12)" "SELECT CAST(TMZ AS TIME) AS X FROM TS WHERE ID < 3 ORDER BY ID" '[]'
...
both "R7 promoted (a zoned cast converts through the session zone's rules) - H classic CAST(TZ AS TIMESTAMP) = ? ['2024-01-10 09:30:00'] -> 1 (the previous binary failed at execute) (engine [1], this server raises at execute; prev prepared then Conversion error from string '20)" "SELECT ID FROM TS WHERE CAST(TZ AS TIMESTAMP) = ?" '["2024-01-10 09:30:00"]' "ERR"
...
both "R7 promoted (a zoned cast converts through the session zone's rules) - H parameter-free WHERE CAST(TZ AS TIMESTAMP) = TIMESTAMP '2024-01-10 09:30:00' -> 1 (engine [1], this server raises at execute; prev prepared then Conversion error from string '20)" "SELECT ID FROM TS WHERE CAST(TZ AS TIMESTAMP) = TIMESTAMP '2024-01-10 09:30:00'" '[]' "ERR"
...
both "H promoted: SELECT CAST(TMZ AS TIME) FROM TS over the NAMED-zone row 3 (engine 09:30:00;01:02:03;09:30:00) - a region's TIME converts through its rules now" "SELECT CAST(TMZ AS TIME) AS X FROM TS ORDER BY ID"
both "R8 promoted (a zoned TIME into a TIMESTAMP is dated on the session's date) - H PRE-EXISTING: SELECT CAST(TMZ AS TIMESTAMP) FROM TS (engine today at 09:30:00 / 01:02:03 / 09:30:00) - fails at execute on the named zone" "SELECT CAST(TMZ AS TIMESTAMP) AS X FROM TS ORDER BY ID"
...
both "R8 promoted (a zoned TIME into a TIMESTAMP is dated on the session's date) - R6 stabilised: raises at execute (restored) - SELECT CAST(TIME '01:30:00 +03:00' AS TIMESTAMP) AS X FROM RDB\$DATABASE (engine [2026-09-18T22:30:00.000Z]; prev prepared then Conversion error from stri)" "SELECT CAST(TIME '01:30:00 +03:00' AS TIMESTAMP) AS X FROM RDB\$DATABASE"
both "R8 promoted (a zoned TIME into a TIMESTAMP is dated on the session's date) - R6 stabilised: raises at execute (restored) - SELECT CAST(TIME '23:30:00 -03:00' AS TIMESTAMP) AS X FROM RDB\$DATABASE (engine [2026-09-20T02:30:00.000Z]; prev prepared then Conversion error from stri)" "SELECT CAST(TIME '23:30:00 -03:00' AS TIMESTAMP) AS X FROM RDB\$DATABASE"
both "R8 promoted (a zoned TIME into a TIMESTAMP is dated on the session's date) - R6 stabilised: raises at execute (restored) - SELECT CAST(CAST(TIME '01:30:00 +03:00' AS TIMESTAMP) AS DATE) AS X FROM RDB\$DATABASE (engine [2026-09-18T00:00:00.000Z]; prev prepared then Conversion error from stri)" "SELECT CAST(CAST(TIME '01:30:00 +03:00' AS TIMESTAMP) AS DATE) AS X FROM RDB\$DATABASE"
both "R8 promoted (a zoned TIME into a TIMESTAMP is dated on the session's date) - R7 recorded: pre-existing - SELECT CAST(TIME '12:30:00 +03:00' AS TIMESTAMP) AS X FROM RDB\$DATABASE -> 2026-09-19T09:30:00.000Z (prev prepared then Conversion error from stri) (engine [2026-09-19T09:30:00.000Z], this server raises at execute; prev prepared then Conversion error from string '12)" "SELECT CAST(TIME '12:30:00 +03:00' AS TIMESTAMP) AS X FROM RDB\$DATABASE"
both "R7 promoted (a zoned TIME into a TIME dates its wall time today) - SELECT CAST(TIME '01:30:00 +03:00' AS TIME) AS X FROM RDB\$DATABASE -> 1970-01-01T22:30:00.000Z (prev prepared then Conversion error from stri) (engine [1970-01-01T22:30:00.000Z], this server raises at execute; prev prepared then Conversion error from string '01)" "SELECT CAST(TIME '01:30:00 +03:00' AS TIME) AS X FROM RDB\$DATABASE" '[]'
both "R8 promoted (a zoned TIME into a TIMESTAMP is dated on the session's date) - R7 recorded: pre-existing - SELECT CAST(TMZ AS TIMESTAMP) AS X FROM TS WHERE ID < 3 ORDER BY ID -> 2026-09-19T09:30:00.000Z;2026-09-19T01:0 (prev prepared then Conversion error from stri) (engine [2026-09-19T09:30:00.000Z;2026-09-19T01:0], this server raises at execute; prev prepared then Conversion error from string '12)" "SELECT CAST(TMZ AS TIMESTAMP) AS X FROM TS WHERE ID < 3 ORDER BY ID"
both "R7 promoted (a zoned TIME into a TIME dates its wall time today) - SELECT CAST(TMZ AS TIME) AS X FROM TS WHERE ID < 3 ORDER BY ID -> 1970-01-01T09:30:00.000Z;1970-01-01T01:0 (prev prepared then Conversion error from stri) (engine [1970-01-01T09:30:00.000Z;1970-01-01T01:0], this server raises at execute; prev prepared then Conversion error from string '12)" "SELECT CAST(TMZ AS TIME) AS X FROM TS WHERE ID < 3 ORDER BY ID" '[]'
...
both "R8 promoted (a zoned TIME into a TIMESTAMP is dated on the session's date) - R6 stabilised: raises at execute (restored) - SELECT CAST(TMZ AS TIMESTAMP) AS X FROM TS ORDER BY ID (engine [2026-09-19T09:30:00.000Z;2026-09-19T01:02:03]; prev prepared then Conversion error from stri)" "SELECT CAST(TMZ AS TIMESTAMP) AS X FROM TS ORDER BY ID"
both "R8 promoted (a zoned TIME into a TIMESTAMP is dated on the session's date) - R6 stabilised: raises at execute (restored) - SELECT CAST(TIME '20:00:00 -12:00' AS TIMESTAMP) AS X FROM RDB\$DATABASE (engine [2026-09-20T08:00:00.000Z]; prev prepared then Conversion error from stri)" "SELECT CAST(TIME '20:00:00 -12:00' AS TIMESTAMP) AS X FROM RDB\$DATABASE"
both "R8 promoted (a zoned TIME into a TIMESTAMP is dated on the session's date) - R7 recorded: pre-existing - SELECT CAST(TIME '00:30:00 +00:00' AS TIMESTAMP) AS X FROM RDB\$DATABASE -> 2026-09-19T00:30:00.000Z (prev prepared then Conversion error from stri) (engine [2026-09-19T00:30:00.000Z], this server raises at execute; prev prepared then Conversion error from string '00)" "SELECT CAST(TIME '00:30:00 +00:00' AS TIMESTAMP) AS X FROM RDB\$DATABASE"
both "R7 promoted (a zoned cast converts through the session zone's rules) - SELECT CAST(TZ AS TIMESTAMP) AS X FROM TS ORDER BY ID -> 2024-01-10T09:30:00.000Z;2024-02-10T00:0 (prev prepared then Conversion error from stri) (engine [2024-01-10T09:30:00.000Z;2024-02-10T00:0], this server raises at execute; prev prepared then Conversion error from string '20)" "SELECT CAST(TZ AS TIMESTAMP) AS X FROM TS ORDER BY ID" '[]' "ERR"
