#!/bin/bash
# GRANT / REVOKE USAGE ON SEQUENCE - the GRANT machinery, on a sequence.
#
# USAGE on a sequence is the same two effects a table grant has, one object
# type over: an RDB$USER_PRIVILEGES row per grantee (privilege 'X', object type
# 14, not 0) and a recompute of the SEQUENCE's security-class ACL. The ACL is
...
#
# fire-crab creates sequences, but to keep the two databases identical the engine
# writes the sequence (and the users) into both databases; then fire-crab grants on one copy and the
...
#   2. the sequence's security-class RDB$ACL is compared BYTE FOR BYTE;
#   3. after a REVOKE, the row is gone and the ACL is recomputed to match;
#   4. an unknown sequence is refused on both;
#   5. gbak round trip and gfix -v -full on fire-crab's raw file.
#
#   qa/serve-real-grantsequence.sh [port]
#
...
D=/tmp/fbhandson
WORK="$D/fc-gs-work.fdb"; REF="$D/fc-gs-ref.fdb"
FBK="$D/fc-gs-work.fbk"; RST="$D/fc-gs-rst.fdb"

command -v node >/dev/null 2>&1 || { echo "SKIP node not found"; exit 0; }
mkdir -p "$D"; rm -f "$WORK" "$REF" "$FBK" "$RST"

G_BOB="GRANT USAGE ON SEQUENCE S1 TO BOB"
G_ALICE="GRANT USAGE ON SEQUENCE S1 TO ALICE WITH GRANT OPTION"
G_PUB="GRANT USAGE ON GENERATOR S1 TO PUBLIC"
R_BOB="REVOKE USAGE ON SEQUENCE S1 FROM BOB"
R_UNK="GRANT USAGE ON SEQUENCE NOSUCH TO BOB"
...
"$FCWIRE" serve "127.0.0.1:$PORT" "$U" "$P" >/tmp/fc-serve-gs.log 2>&1 &
srv=$!
...
SELECT TRIM(RDB$USER)||'|'||TRIM(RDB$PRIVILEGE)||'|ot='||RDB$OBJECT_TYPE||'|go='||RDB$GRANT_OPTION||'|by='||TRIM(RDB$GRANTOR)
  FROM RDB$USER_PRIVILEGES WHERE RDB$RELATION_NAME='S1' ORDER BY RDB$USER;
SQL
}
aclq() { # <file>
    b=$("$ISQL" -q -b -user "$U" -pas "$P" "$1" 2>/dev/null <<'SQL' | grep -oE '[0-9a-f]+:[0-9a-f]+' | head -1
SET LIST ON;
SELECT c.RDB$ACL FROM RDB$SECURITY_CLASSES c JOIN RDB$GENERATORS p
  ON p.RDB$SECURITY_CLASS = c.RDB$SECURITY_CLASS WHERE p.RDB$GENERATOR_NAME='S1';
SQL
)
    [ -n "$b" ] || { echo "(none)"; return; }
    rm -f /tmp/fc-gs-acl.bin
    printf 'BLOBDUMP %s /tmp/fc-gs-acl.bin;\n' "$b" | "$ISQL" -q -user "$U" -pas "$P" "$1" >/dev/null 2>&1
    od -An -tu1 -v /tmp/fc-gs-acl.bin | tr '\n' ' ' | tr -s ' ' | strip
}
...
case "$eng_unk" in *[Ff]ailed*|*[Ee]rror*|*"not exist"*|*"not defined"*) echo "OK   the engine refuses the unknown sequence too" ;;
    *) echo "DIFF engine did not refuse unknown sequence"; echo "     $eng_unk"; fail=1 ;; esac

work_priv=$(privq "$WORK")
check "the RDB\$USER_PRIVILEGES 'G' rows match the engine" "$work_priv" "$(privq "$REF")"
case "$work_priv" in
    *"ALICE|G|ot=14|go=1"*"BOB|G|ot=14|go=0"*"PUBLIC|G|ot=14"*)
...
work_acl=$(aclq "$WORK")
check "the sequence's security-class ACL matches byte for byte" "$work_acl" "$(aclq "$REF")"
case "$work_acl" in
    "2 1 3 6 "*" 0 2 6 1 3 12 0 "*) echo "OK   the owner ACE carries alter/control/drop/execute (6 1 3 12)" ;;
...
# --- gbak and gfix -----------------------------------------------------
if "$GBAK" -b -g -user "$U" -pas "$P" "$WORK" "$FBK" >/tmp/fc-gs-backup.log 2>&1; then
    echo "OK   gbak backs up the database fire-crab wrote"
else
    echo "DIFF gbak backup"; cat /tmp/fc-gs-backup.log; fail=1
fi
"$GBAK" -c -user "$U" -pas "$P" "$FBK" "$RST" >/tmp/fc-gs-restore.log 2>&1
rc=$?
if [ $rc -eq 0 ] && ! grep -qiE "error" /tmp/fc-gs-restore.log; then
    echo "OK   gbak RESTORES it"
else
    echo "DIFF gbak restore (exit $rc)"; grep -iE "error|cannot" /tmp/fc-gs-restore.log | head; fail=1
fi
