...
refuses "ins-sel ROUND(FL,2) -> D16"    "INSERT INTO DX (ID, D16) SELECT ID, ROUND(FL, 2) FROM TX;"
refuses "ins-sel TRUNC(DP,2) -> D34"    "INSERT INTO DX (ID, D34) SELECT ID, TRUNC(DP, 2) FROM TX;"
refuses "UPDATE .. = (SELECT ROUND) D16" "UPDATE PBW SET D16 = (SELECT ROUND(FL, 2) FROM TX WHERE ID = 3) WHERE ID = 1;"
refuses "VALUES((SELECT ROUND)) D34"    "INSERT INTO DX (ID, D34) VALUES (1, (SELECT ROUND(FL, 2) FROM TX WHERE ID = 3));"
refuses "MERGE ROUND source -> D34"     "MERGE INTO PBW USING (SELECT ID, ROUND(FL, 2) R FROM TX) SRC ON (PBW.ID = SRC.ID) WHEN MATCHED THEN UPDATE SET D34 = SRC.R;"
...
echo "-- verify-found (round 2): a NON-RUNTIME approximate value refuses at a DECFLOAT store wherever its provenance was erased --"
...
refuses "ROUND(2.675E0, 2) const -> D34"     "INSERT INTO DX (ID, D34) VALUES (1, ROUND(2.675E0, 2));"
refuses "UPDATE D34 = ROUND(DP, 2) direct"   "UPDATE DX SET D34 = ROUND(DP, 2) WHERE ID = 1;"
refuses "UPDATE D34 = COALESCE(DP, 0)"       "UPDATE DX SET D34 = COALESCE(DP, 0) WHERE ID = 1;"
refuses "ins-sel -ROUND(DP,2) -> D16"        "INSERT INTO DX (ID, D16) SELECT ID, -ROUND(DP, 2) FROM TX;"
refuses "ins-sel ROUND(DP,2) + 0 -> D34"     "INSERT INTO DX (ID, D34) SELECT ID, ROUND(DP, 2) + 0 FROM TX;"
refuses "ins-sel ROUND via derived -> D34"   "INSERT INTO DX (ID, D34) SELECT ID, X FROM (SELECT ID, ROUND(DP, 2) AS X FROM TX) Q;"
refuses "MERGE SET D34 = ROUND(SRC.DP, 2)"   "MERGE INTO PBW USING (SELECT ID, DP FROM TX WHERE ID = 3) SRC ON (PBW.ID = 1) WHEN MATCHED THEN UPDATE SET D34 = ROUND(SRC.DP, 2);"
