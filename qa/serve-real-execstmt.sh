...
# bare variable. fc parsed only a literal operand before; now the operand
# is any expression, compiled to the engine's BLR (blr_exec_sql,
# blr_exec_into or blr_exec_stmt over the expression - byte for byte) and
# rendered by the interpreter at run time. Both servers build and run the
# procedures; the rows are compared and the ENGINE runs the BLR fc stored.
#
# Boundaries (recorded): a positional/named parameter head `(sql)(vals)`
# and the USING / ON EXTERNAL / AS USER modifiers remain a later slice.
#
...
check "concat / variable / FOR EXECUTE STATEMENT run and answer alike" "$c" "$e"
