...
DDL=(
  ...
  "CREATE TABLE TK (A INTEGER, B INTEGER)"
  "CREATE TABLE TJ (A INTEGER, B INTEGER)"
  "CREATE TABLE TL (A INTEGER, B INTEGER)"
  "CREATE TABLE TB (A INTEGER, B INTEGER, C INTEGER)"
  "CREATE TABLE TV (A INTEGER, B INTEGER, C INTEGER)"
  "CREATE TABLE TZ (A INTEGER, B INTEGER, C INTEGER)"
  "CREATE TABLE TQ (A INTEGER, B INTEGER, C INTEGER)"
  "CREATE TABLE TU (A INTEGER, B INTEGER)"
  "CREATE TABLE LOG (X INTEGER, Y INTEGER)"
  "CREATE EXCEPTION EX1 'boom'"
  "CREATE EXCEPTION EX2 'bang'"
  ...
  "CREATE TRIGGER TRWNAM FOR TK BEFORE INSERT AS BEGIN IF (NEW.A = 1) THEN EXCEPTION EX1; WHEN EXCEPTION EX1 DO NEW.B = -1; END"
  "CREATE TRIGGER TRW2 FOR TK BEFORE UPDATE AS BEGIN IF (NEW.A = 3) THEN EXCEPTION EX2; WHEN EXCEPTION EX1, EXCEPTION EX2 DO NEW.B = -5; END"
  "CREATE TRIGGER TRWMIX FOR TJ BEFORE INSERT AS BEGIN NEW.B = 100 / NEW.A; WHEN EXCEPTION EX2 DO NEW.B = -8; WHEN ANY DO NEW.B = -9; END"
  "CREATE TRIGGER TRWGDS FOR TL BEFORE INSERT AS BEGIN NEW.B = 100 / NEW.A; WHEN GDSCODE arith_except DO NEW.B = -50; END"
  "CREATE TRIGGER TRWSQL FOR TL BEFORE INSERT POSITION 1 AS BEGIN NEW.B = NEW.B + 200 / NEW.A; WHEN SQLCODE -802 DO NEW.B = -60; END"
  "CREATE TRIGGER TRNIF FOR TB BEFORE INSERT AS BEGIN IF (NEW.A > 1) THEN BEGIN NEW.B = 1; NEW.C = 2; END ELSE BEGIN NEW.B = 4; NEW.C = 5; END END"
  "CREATE TRIGGER TRNWH FOR TV BEFORE INSERT AS DECLARE VARIABLE V INTEGER; BEGIN V = 0; NEW.B = 0; WHILE (V < NEW.A) DO BEGIN V = V + 1; NEW.B = NEW.B + V; END NEW.C = V * 10; END"
  "CREATE TRIGGER TRNHB FOR TZ BEFORE INSERT AS BEGIN BEGIN NEW.B = 100 / NEW.A; WHEN ANY DO NEW.B = -1; END NEW.C = 3; END"
  "CREATE TRIGGER TRNRS FOR TQ BEFORE INSERT AS BEGIN IF (NEW.A = 9) THEN EXCEPTION EX1; BEGIN NEW.B = 200 / NEW.A; WHEN ANY DO NEW.B = -2; END END"
  "CREATE TRIGGER TRUPDEL FOR TU AFTER INSERT AS BEGIN INSERT INTO LOG (X, Y) VALUES (NEW.A, 0); UPDATE LOG SET Y = NEW.B WHERE X = NEW.A; DELETE FROM LOG WHERE X = NEW.A + 1000; END"
  "CREATE TRIGGER TRUPD2 FOR TU AFTER UPDATE AS DECLARE VARIABLE V INTEGER; BEGIN V = OLD.B + 100; UPDATE LOG SET Y = :V WHERE X = OLD.A; END"
)
...
for bad in \
  ...
  "CREATE TRIGGER BX FOR TM AFTER DELETE AS DECLARE VARIABLE V INTEGER; BEGIN V = OLD.A; INSERT INTO LOG (X, Y) VALUES (V, 1); END" \
  "CREATE TRIGGER BX FOR TK BEFORE INSERT AS BEGIN NEW.B = 1; WHEN EXCEPTION NOSUCH DO NEW.B = 0; END" \
  "CREATE TRIGGER BX FOR TK BEFORE INSERT AS BEGIN NEW.B = 1; WHEN GDSCODE NO_SUCH_CODE DO NEW.B = 0; END" \
  "CREATE TRIGGER BX FOR TB BEFORE INSERT AS BEGIN BEGIN NEW.B = 1; END; NEW.C = 2; END" \
  "CREATE TRIGGER BX FOR TU AFTER UPDATE AS DECLARE VARIABLE V INTEGER; BEGIN V = 1; UPDATE LOG SET Y = V WHERE X = 1; END" \
  "CREATE TRIGGER BX FOR TU AFTER INSERT AS BEGIN DELETE FROM NOSUCH WHERE X = 1; END"; do
...
INSERT INTO TK (A, B) VALUES (1, 5);
INSERT INTO TK (A, B) VALUES (2, 5);
INSERT INTO TJ (A) VALUES (4);
INSERT INTO TJ (A) VALUES (0);
INSERT INTO TL (A) VALUES (4);
INSERT INTO TL (A) VALUES (0);
INSERT INTO TB (A) VALUES (3);
INSERT INTO TB (A) VALUES (0);
INSERT INTO TV (A) VALUES (3);
INSERT INTO TZ (A) VALUES (4);
INSERT INTO TZ (A) VALUES (0);
INSERT INTO TQ (A) VALUES (9);
INSERT INTO TQ (A) VALUES (0);
INSERT INTO TU (A, B) VALUES (41, 7);
COMMIT;
UPDATE TU SET B = 8 WHERE A = 41;
COMMIT;
UPDATE TK SET A = 3 WHERE A = 2;
COMMIT;
...
SELECT 'TK|'||A||'|'||B FROM TK ORDER BY A;
SELECT 'TJ|'||A||'|'||B FROM TJ ORDER BY A;
SELECT 'TL|'||A||'|'||B FROM TL ORDER BY A;
SELECT 'TB|'||A||'|'||B||'|'||C FROM TB ORDER BY A;
SELECT 'TV|'||A||'|'||B||'|'||C FROM TV ORDER BY A;
SELECT 'TZ|'||A||'|'||B||'|'||C FROM TZ ORDER BY A;
SELECT 'TQ|'||A||'|'||B FROM TQ ORDER BY A;
...
check "the engine RUNS them: fold/ELSE, loop, exception, store, handler, multi-event" \
      "$work_x" "$(exec_dml "$REF")"
case "$work_x" in
    *"boom"*"TN|0|1"*"TN|3|10"*"TW|4|8"*"TH|0|-1"*"TH|4|25"*"TK|1|-1"*"TK|3|-5"*"TJ|0|-9"*"TJ|4|25"*"TL|0|-60"*"TL|4|75"*"TB|0|4|5"*"TB|3|1|2"*"TV|3|6|30"*"TZ|0|-1|3"*"TZ|4|25|3"*"TQ|0|-2"*"LOG|5|-1"*"LOG|6|12"*"LOG|41|107"*)
        echo "OK   observable: raises+folds+loop+handlers - incl NAMED catches (-1, -5) and named-miss-to-ANY (-9)" ;;
