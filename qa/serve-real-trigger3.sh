...
for bad in \
  ...
  "CREATE TRIGGER BX FOR TK BEFORE INSERT AS BEGIN NEW.B = 1; WHEN GDSCODE ARITH_EXCEPT DO NEW.B = 0; END"; do
...
check "the engine RUNS them: fold/ELSE, loop, exception, store, handler, multi-event" \
      "$work_x" "$(exec_dml "$REF")"
case "$work_x" in
    *"boom"*"TN|0|1"*"TN|3|10"*"TW|4|8"*"TH|0|-1"*"TH|4|25"*"TK|1|-1"*"TK|3|-5"*"TJ|0|-9"*"TJ|4|25"*"LOG|5|-1"*"LOG|6|12"*)
