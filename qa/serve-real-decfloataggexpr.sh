...
echo "-- MUST REFUSE: AVG/MIN/MAX where the fold over-widens vs engine df16 --"
refuses_fc "avg(d16-d16)"     "select avg(d16-d16) x from t;"
refuses_fc "min(d16-d16)"     "select min(d16-d16) x from t;"
refuses_fc "max(d16-d16)"     "select max(d16-d16) x from t;"
refuses_fc "avg(-d16)"        "select avg(-d16) x from t;"
refuses_fc "min(-d16)"        "select min(-d16) x from t;"
echo "-- but SUM of those IS answered (df34, engine agrees) --"
