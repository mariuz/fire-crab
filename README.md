...
| Record field decoding (RDB$FORMATS) | `fire-crab-ods` | next |
...
database generated by the paper's hands-on samples), and the record-version
walk **re-derives `SELECT COUNT(*)` from raw pages** (pointer pages -> data
pages -> MVCC flag classification) in agreement with the live engine on
every table tested, from 0 to 200,000 rows. See
