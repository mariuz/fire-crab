...
>
> **THE ARITHMETIC, RE-COUNTED WITHIN THE SECTION'S OWN BOUNDS (2026-09-17, after the CTE-cycle landing): 33 bullets, 25 struck, 8 live** - HIGH 8 of 8 struck, MEDIUM 11 of 15, LOW 6 of 10. (The describe-name work struck three at once: the EXISTS entry, the recursive-CTE one, and the scalar-subquery-in-a-derived-table entry whose two halves were closed by two different chunks - it had been left LIVE with both halves already noted as fixed, which is the under-counting twin of the over-counting this note warns about. Count, then read what the count means.) **The previous figures here - "32 bullets, 21 struck, 11 live, MEDIUM 11 of 15" - were counted over the WRONG RANGE**, which is the same failure as counting from memory and is why they disagreed with themselves: the total was section-scoped while the struck count swept the whole FILE, pulling in the `CONNECTION KILL` bullet that belongs to another section, and the live figure matched neither range. A tally here must bound the section (from this heading to the next `###`) before it counts, not just run a `grep -c` over the page. The total GREW because measurement added three entries it had not held before (the folded-subquery pad width, the join-driver order, and the recursive-walk follow-on); nothing was miscounted into existence.
>
