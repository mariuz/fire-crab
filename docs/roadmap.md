...
- a WILDCARD-FREE `LIKE` with a NON-ASCII pattern over a **REAL-charset column**, under a **TABLED single-byte attachment**, makes the ENGINE RAISE `22001 string right truncation` where this server answers **0 rows** - `SELECT COUNT(*) FROM T WHERE <utf8 col> LIKE 'café'` under WIN1252 or ISO8859_1. Found 2026-09-18 while verifying the expression-path carrier lift, as a CONTROL row that moved. Measured identical on `9bf97ca`, `34df15d` and the binary that fixed the lift, so none of the carrier slices caused it. **Its neighbours do NOT raise**: `LIKE '%café%'`, `STARTING WITH 'café'` and `= 'café'` all answer 0 on the engine under the same attachment, so the rule is specific to the wildcard-FREE form. This server has no raise path under a real attachment at all - [carrier_expr_like] and [param_or_typed_term]'s LIKE guard are both gated on a byte-carrier ATTACHMENT - which is why it answers instead of raising. Held by two `eng_raises` cells in `serve-real-nonecmp` that go red if either server changes behaviour.
