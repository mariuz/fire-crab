...
- **LOW, NEW (2026-09-18, measured alongside the wrong answer above, and running the OPPOSITE way)** — `SELECT COUNT(*) AS C FROM T GROUP BY N ORDER BY N` ANSWERS on the engine (`1,1`) and REFUSES here. The same statement with the alias `AS N` - shadowing the grouped column - is the WRONG ANSWER recorded in the section above, so this one pair of statements straddles both failure directions: ordering by a grouped column is refused when the select list holds an aggregate under a DIFFERENT alias, and wrongly accepted when the alias collides. Both were found by one probe that carried controls it did not need.
