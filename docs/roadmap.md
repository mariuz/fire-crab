...
- **Bind-error leftovers** (a plain column, an expression left side and the wide magnitudes are DONE - `serve-real-bindconv`): three shapes still refuse with the generic *Dynamic SQL Error*. A **JOIN ON** parameter (`P JOIN J ON P.ID = J.ID AND P.I = ?`), where the engine raises per outer row - INNER and LEFT alike, and even over an EMPTY inner table. A parameter inside an **IN / EXISTS subquery**, where the engine raises once the inner stream yields a row (an empty inner table, and an empty OUTER table under the semi-join build, both answer). And **`CAST(? AS INTEGER)`**, which is NOT value-gated - it raises over a NULL column and over `RDB$DATABASE`, suppressed only by an empty table or a false conjunct written in front - so it wants a row-independent raising term, not [Term::ExprConvErr]'s value gate; `CAST(? AS INTEGER)` refuses even for a CONVERTIBLE text ('5'), so the whole shape is unimplemented rather than mis-typed. Also still refusing: a text whose exponent underflows (`I < ?` ['1e-400'], where the engine compares against a tiny positive - a zero column value sorts BELOW it, so sign alone cannot decide it), and capital-hex '0X10' (the engine reads uninitialized memory). Law-safe, vector and timing differ.
