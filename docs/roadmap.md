...
- **Bind-error leftovers** (a plain column, an expression left side, the wide magnitudes, `CAST(? AS <type>)` and a JOIN's ON are DONE - `serve-real-bindconv`): what still refuses with the generic *Dynamic SQL Error*. A parameter inside an **IN / EXISTS / scalar SUBQUERY** - and this one is a MECHANISM, not a vector: an uncorrelated subquery is EVALUATED AT PREPARE by `eval_subquery` and folded into literal tokens ([resolve_subqueries]), which a `?` cannot survive because its value has not arrived yet; a correlated one is registered and evaluated per row instead. Supporting it means deferring that fold to execute (or routing a param-bearing uncorrelated subquery down the per-row path), not patching an error. Measured contract for when it is done: the engine answers `IN` / `NOT IN` / `EXISTS` / `= (SELECT ..)` with a bound value, raises once the inner stream yields a row, answers with no raise over an empty INNER table AND over an empty OUTER one (the semi-join build waits for the first outer row), and numbers an inner `?` after an outer one. A **TEXT cast target** (`S = CAST(? AS VARCHAR(5))`, refused for a FITTING value too): the slot's charset is the ATTACHMENT's, which [cast_target_descriptor] leaves flat, while the projection resolver types the cast source UTF8 - and [cmp_sides]' text path reconciles byte-carrier against real charsets, so admitting it would GUESS a charset for a NONE column. The engine raises *string right truncation, expected length 5, actual 10* for an over-long value, and the projection `SELECT CAST(? AS VARCHAR(5))` already answers it - only the comparison side is left. Also still refusing: a text whose exponent underflows (`I < ?` ['1e-400'], where the engine compares against a tiny positive - a zero column value sorts BELOW it, so sign alone cannot decide it), and capital-hex '0X10' (the engine reads uninitialized memory). Law-safe, vector and timing differ.
