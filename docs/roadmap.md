...
against the live FB6 engine: 469 gates under `qa/`, of which the 436
...
| every column of a derived table must have a NAME OF ITS OWN | `SELECT * FROM (SELECT 1+1 FROM T) Z` is the engine's `-104 / Invalid command / no column name specified for column number @1 in derived table @2`, and this server ANSWERED it, inventing the column's name from the node kind. Only three things name a column - a plain field reference, an explicit `AS`, or the derived table's declared list - so the check runs AFTER the declared override, which keeps `(SELECT 1+1 FROM T) Z (E)` legal; the top level is exempt. `@1` is the 1-based position of the FIRST offender and `@2` the scope AS WRITTEN, which for a nested pair is the INNERMOST. THE PREDICATE WAS WRONG TWICE AND MEASUREMENT CAUGHT IT BOTH TIMES: keying on `expr.is_some()` excluded AGGREGATE and UNION inner queries, whose plans are `Group`/`Union` with SLOT-READ columns carrying no expression, so `(SELECT COUNT(*) FROM T) Z` still answered and a union answered with blank names; the working test is the union arm's own (`plain = expr.is_none() && relation.is_some() && fname.is_some()`), sound because `relation` SURVIVES every wrapper - probed through a derived table, two nested, a CTE, a view, a view inside a derived, a union and a rename - which is also why a real column literally named `"ADD"` or `"COUNT"` still answers. A UNION takes its name from the FIRST branch only. Two roots beyond the predicate: a NESTED derived table lost its diagnosis to `output_cols_of`'s empty-vector catch-all feeding an `is_empty()` guard, not to any planner, and the JOIN SIDE lost it to a comment of mine asserting `Option` could not carry it - it answers `Option<Plan>` and `RefusedEval` IS a `Plan`. RECORDED, both still refusing: `plan_lateral` and the lone-`COUNT(*)` fast path destructure for `Plan::Join` and drop the carried diagnosis | `serve-real-derived` 175 (31 flip on `7a66881`) |
| a generator advances once per DELIVERED REFERENCE, at every level | the advance lived only in the TOP-LEVEL fetch. Every row source beneath it - a derived table, a CTE level, a VIEW, a union branch - projected the inner columns WITHOUT it, because `branch_rows_each`'s `Plan::Project` arm destructured with `..` and dropped `gen_cols`: the synthetic slot stayed unfilled, read back NULL, and rendered as 0 under the non-nullable announcement `NEXT VALUE FOR` carries (sqltype 580) or `<null>` under `GEN_ID`'s (581) - while the sequence never moved. ONE cause, TWO appearances, and the THIRD time that destructure has dropped `gen_cols` in this family (see [advance_generators]'s own note about `branch_rows`). THE LAW, measured over four rows before any code: the engine draws PER EVALUATED REFERENCE, not per row - one reference draws 4, two draw 8, three draw 12, a column the outer never delivers draws NOTHING, an outer filter does not re-evaluate but shrinks the delivered set FIRST (keeping 2 of 4 draws 2), and the value follows OUTPUT order (`ORDER BY Z.ID DESC` answers 4->1, 3->2). So materialise-then-advance is exact at ONE delivered reference and nowhere else, which is the boundary [classify_derived_gen] draws: strip the column where the outer never delivers it (the engine draws nothing there, so that shape was already right and a blanket guard would have BROKEN it), filter and sort and then advance where it is delivered once, and REFUSE every other count rather than trade one wrong answer for another. A VIEW over a generator went from refusing to answering | `serve-real-genrow` 56 (15 flip on `ad86de9`) |
| the describe's SYMBOLIC NAME is the engine's label, not this server's node kind | three defects the roadmap had recorded as "one behaviour seen three ways", which measurement split into three causes. **(1) EXISTS is BOOL** - the fold rewrites `EXISTS <marker>` to a literal and a patch restores BOOL, but it compared the item text EXACTLY, so an alias or a leading `NOT` fell through to the literal's CONSTANT; the engine says BOOL in every spelling and position. **(2) A SUBQUERY IN A DERIVED TABLE'S OR CTE'S SELECT LIST BLANKS EVERY EXPRESSION COLUMN of that scope** one level up, plain FIELD columns keeping theirs, the top level blanking nothing - and it blanks a neighbour the outer never selects, so it is a property of the SCOPE. The trigger is the SELECT LIST alone (a subquery in the inner WHERE/HAVING, an EXISTS or IN there, or a nested derived FROM all keep the name) and it is NOT flattening: DISTINCT, GROUP BY, an aggregate, FIRST, SKIP, ORDER BY and a window all keep it, and all nine are regression pins now. **(3) A RECURSIVE CTE IS A UNION** - an expression anchor blanks, a field anchor keeps its name; a plain UNION already agreed and the recursive path took the seed's columns untouched, so the rule never ran. One predicate is shared by all three sites so they cannot drift | `serve-real-describe` 146 (14 flip on `51a9068`) |
| inside a `WITH`, a name binds to the CTE - and a cycle is an error | an unqualified name in a CTE's body that matches a CTE of the same `WITH` binds to THAT CTE, never to a same-named table; if that closes a cycle the engine raises `-104 CTE '"<N>"' has cyclic dependencies`. This server resolved such a name to the TABLE wherever one existed and ANSWERED it, and gave a contentless 42000 where none did - ONE root cause producing a wrong answer in one spelling and a message gap in another, which is why the bullet's framing ("shadows a real table") was too narrow: the CYCLE is the error, table or no table. It reaches a self-reference in a SUBQUERY, in a JOIN, inside a nested DERIVED table, a MUTUAL pair, a three-CTE ring, a NON-recursive UNION body, and a cycle the main query never reaches (still an error; only a cycle-FREE unused CTE is a warning). **The message names the CTE the resolution ENTERED at** - the same three-CTE ring answers A1, A2 or A3 depending only on which the main query reads - and the LAST-DECLARED member when no part is reachable. Two sibling diagnoses come with it: `Recursive CTE (@1) must be an UNION` (a recursive body that is not a union) and `Non-recursive member is missing in CTE '@1'` (a union whose FIRST member already names the CTE). The detector walks every FROM/JOIN in a body - `bound_refs` sees only one query's top level - and skips QUALIFIED names, because `PUBLIC.C1` bypasses the CTE namespace and legally reads the table | `serve-real-cte` 56 (15 flip on `7e3a8a0`) |
| NOT NULL travels through a scope, and an expression over a wrapped column keeps it | the bit already survived a derived table, a CTE and a union - a plain pass-through copies it explicitly - but an EXPRESSION in the OUTER query over such a column was announced Nullable where the engine announces it fixed. Cause: the synthetic view's descriptors are built with `flags: 0` ([desc_of_projcol]) and [mark_not_null_cols] is keyed on a base TABLE, which a derived scope has not got, so the `is_nn` closure an expression consults answered "nullable" for every field. [plan_over_source]'s expression branch now runs the existing [expr_nullable] against the inner columns' own bits - reading the bit, never setting it, which the control pins: an expression over a NULLABLE column stays nullable on both servers. Measured across arithmetic, negation, CAST, a function, UPPER, `||` and a CASE with both branches fixed; through a union column and two wraps deep; and the GROUP BY face the roadmap listed beside it turned out to be STALE in its own shapes. The LATERAL face is NOT this cause and stays live: its outer reference is a typed `CAST(NULL AS <type>)` stand-in whose width the describe depends on | `serve-real-notnulldesc` 42 (11 flip on `6addfe3`) |
| a wrapped literal's charset is the attachment's, not 255 | a string expression over a literal carried through a derived table, a CTE or a view was announced charset 255 and, worse, VALUED as if 255 were a real set. `ATT_SUBTYPE` is `-1`; `charset_id` is `(sub_type as u16 & 0xFF)`, so the sentinel read as a ttype IS 255. [desc_of_projcol] resolves the real-charset sentinel but passes `ATT_SUBTYPE` through, and [text_form_m]'s `Expr::Col` arm built `TfCs::Ttype(-1)` from it with no negative guard, so [expr_value_charset] found 255 neither carrier nor tabled and dropped to UTF-8 semantics: under a NONE attachment `OCTET_LENGTH` over a wrapped `'a<C3A9>b'` answered 6 for the engine's 4, and `\|\|` raised a truncation the engine never raises. Fixed by mirroring [corr_text_form], the twin that already guarded it; no width moves. The describe matrix alone called `CAST` exempt - true of the announcement, FALSE of the value, which two cells first written as controls proved by flipping | `serve-real-litcs` 134 (17 flip on `b1df783`) |
| the computed-pad ceiling is the attachment's | a pad whose length is not a literal cannot be sized statically and falls back to the widest VARCHAR its charset admits - and that ceiling is the ATTACHMENT's, not a constant. Measured over four source charsets x four attachments: `chars = 65533/bpc(SOURCE)`, `bytes = min(chars, ceiling/bpc(OUT)) * bpc(OUT)`, `ceiling` 65533 under NONE and 32765 under any real attachment, so `LPAD(<utf8>, ID)` is 65532 / 32764 / 16383 / 16383 across NONE / UTF8 / WIN1252 / ISO8859_1. fire-crab announced 65533 (or 65532) everywhere. Fixed at the two places that decide it - the Lpad/Rpad arm counting the fallback in the SOURCE's characters, and [resolve_text_cs] taking the conditional ceiling - plus the arm byte carriers actually travel through, since NONE/OCTETS are announced as plain positive ttypes and never reach `cap`. A previous attempt moved the ceiling unconditionally and was reverted for regressing every NONE cell; the difference is that the whole matrix was measured first, ISO8859_1 included, so "real attachment" is a measured generalisation rather than a guess from one column | `serve-real-funcwidth` 115 (21 flip on `c05b375`); the gate gained the attachment dimension it never had |
| a byte carrier meeting a real charset reconciles at the FUNCTION's operands | the engine treats a byte carrier as BYTES, so a function mixing a carrier operand with a real one runs in the carrier's octet space. Eleven wrong VALUES: `POSITION('é' IN N)` 4/0, `POSITION(N IN 'xcafé')` 2/0, `REPLACE(N,'é','e')` 4/7, `TRIM(TRAILING 'é' FROM N)` 3/5, the same four over an OCTETS CAST, and column-vs-column `POSITION(N IN U)` 1/0, `POSITION(U IN N)` 1/0, `TRIM(TRAILING N FROM U)` 0/5, `REPLACE(U,N,'x')` 1/5. Two earlier attempts re-keyed the LITERAL paths and were withdrawn - one inert, one turning a CORRECT `LIKE` wrong; this one is OPERAND-keyed on the FUNCTION path, wrapping the REAL operands in the existing [Expr::CarrierEnc] as [cmp_sides] already does for comparisons, so the shared predicate route is untouched. The DESCRIBES were already the engine's (22 cells) and had to stay that way, so [text_form_m] gained a transparent `CarrierEnc` arm - it was the only prepare-time reader of that wrapper without one. A first draft counted LITERALS in its gate and turned `POSITION('é' IN U)` under a NONE attachment from 4 into 0; the owning gate caught it with nine controls where my own probe had one | `serve-real-nonecmp` 86 (13 flip on `c49807a`); the gate gained the `ran` counter and floor it never had |
| `CONTAINING` over a byte carrier compares in byte space | `WHERE N CONTAINING 'é'` over a NONE column under a UTF8 attachment answered 0 where the engine answers 1 - a qualifying row silently dropped - and the same for an OCTETS CAST. The pair is what diagnosed it: `nn LIKE '%é%'` AGREED in the identical shape, so LIKE had a working route and CONTAINING did not; a 26-cell probe narrowed it to ONE diverging cell, with `STARTING WITH` on the same non-ASCII needle agreeing too. Cause: the same one-directional, attachment-gated guard for the THIRD time, with CONTAINING reaching [containing_term] by TWO routes - an expression (OCTETS cast, blob) and a plain column through [param_or_typed_term]'s fast path - so fixing one left the headline cell unmoved. The conversion was already right in both directions; only the condition changed. The shared [adopt_carrier_literal] was left alone on purpose. **The verification found MORE than it checked**: probing a fourth attachment showed `LIKE` and `STARTING WITH` dropping rows over a byte carrier under WIN1252 and ISO8859_1 - pre-existing, measured identical on `302c4d1`, and unreachable by any gate in the suite, now a new HIGH bullet held by four `differs` cells | `serve-real-nonecmp` 96 (2 flip on `302c4d1`); floor 90 -> 96, derived by counting and confirmed by the run |
| a tabled single-byte attachment is not UTF8 | over a byte-carrier column, EVERY predicate with a non-ASCII literal - `=`, `LIKE`, `STARTING WITH` - answered 0 where the engine answers 1 under a WIN1252 or ISO8859_1 attachment: a qualifying row silently dropped. Found by the four-attachment probe that VERIFIED the previous slice rather than by planning, and the bullet was filed, WIDENED (equality was missing from the first framing) and struck within the day. The cause was one word in an existing step: the carrier lift used [intl::to_carrier], which re-spells a string's UTF-8 BYTES, on a literal a tabled attachment had already decoded one char per byte - so it lifted twice. Now lifted through [transcode_text] from the attachment's charset: identity under a tabled attachment, unchanged under UTF8. Five hypotheses about the literal's decode, the value's decode and the encode tables died before `FC_SRV_TRACE` showed two BYTE-IDENTICAL prepares answering 1 and 0, and a projection-versus-WHERE fork exonerated the values | `serve-real-nonecmp` 102 (6 flip on `9bf97ca`); the gate's attachment cells were all but two `@UTF8` - the blind spot that hid it |
| the same lift, at the expression path's three sites | the previous slice fixed the COLUMN path's carrier lift; the EXPRESSION path has three of its own - `LIKE`, `SIMILAR TO`, `STARTING WITH` - and all three were wrong **in opposite directions at once**. [intl::to_carrier] spells a string's UTF-8 bytes, so under a TABLED attachment it lifted TWICE (an OCTETS operand answered 0 under NONE/WIN1252/ISO8859_1 and 1 under UTF8), while the `expr_is_octets` guard meant a NONE carrier was never lifted at all (a `CAST(.. AS .. NONE)` answered 0 under UTF8 alone). Six wrong-answer cells, two mirror-image symptoms, one root. Each site now lifts via [transcode_text] from the attachment's charset for any byte carrier, while `octets_like_term`'s wildcard-free equality MODEL stays keyed on OCTETS - conflating the lift with the model would have traded one wrong answer for another, since over a NONE carrier `LIKE` keeps its wildcards and the engine matches them. A 120-cell matrix (5 operand kinds x 6 predicates x 4 attachments) reads 118 SAME / 2 DIFF, the two being a newly found PRE-EXISTING raise: `<utf8 col> LIKE 'café'` under a tabled attachment has the engine raise 22001 where this server answers 0, recorded with a new `eng_raises` helper because `differs` would have scored it vacuous | `serve-real-nonecmp` 113 (9 flip on `34df15d`); the gate had **no `SIMILAR TO` cell at all** and no LIKE-over-OCTETS cell, and a wildcard LIKE over OCTETS is 0-on-both BY DESIGN - only the wildcard-free form can catch anything |
| a wildcard-free `LIKE` with a non-ASCII pattern raises 22001 | over a MULTI-BYTE operand under a TABLED single-byte attachment (WIN1252, ISO8859_1 - never NONE or UTF8) the engine raises `string right truncation, expected length 0, actual 1` where this server answered 0 rows. The counts are CONSTANT, which is what says the engine is not measuring the data: it is not value-gated at all, firing over an empty table, over a NULL and under `1 = 0 AND`, so it is a statement-level [PREPARE_REFUSAL] at TWO call sites (a column carries a Descriptor into [param_or_typed_term]; an expression takes [resolve_expr_term]). Recorded as a gap and implemented the same night. **The first cut of it REFUSED `B LIKE 'café'`, which the engine answers 0** - a wrong answer replacing a right one, TWELVE blob shapes wide, caught by a gate cell added on a hunch rather than by either of the two probe matrices that read 0 DIFF beside it; excluded with [blob_result] | `serve-real-nonecmp` 138 (6 flip on `ae84769`); the boundary cells are one per condition in the guard - `NOT LIKE`, `ESCAPE`, `%`, `_`, an ASCII pattern, a 1-byte column, a carrier column, an untabled attachment, the projection form and the four quiet neighbours - so deleting any single condition turns exactly one cell red |
| a RIGHT JOIN drives from the preserved side | the engine emits a RIGHT join in the PRESERVED side's storage order - each right row followed by its matches in the accumulated side's order, and an unmatched right row padded IN ITS STORAGE POSITION - where this server streamed the syntactic left and appended the mirror at the end. **Not an order nicety: every top-level slice took a DIFFERENT SET OF ROWS**, silently and with no error (`FIRST 1` answered the engine's `[4]` as `[1]`; six of six slice shapes). Pre-existing on three binaries. Fixed at THREE sites, where the plan named two: [join_step] tags each row with the right-row index and stable-sorts for a `Right` part; the streaming arm narrowed to `Full`; and - found only because fixing the first two repaired every SLICE while leaving every unsliced LISTING wrong - [JoinCursor] now declines a bare `Right` last part, because it expands one base row at a time and its mirror phase runs last, so its unmatched rows can only ever trail. `FULL` and the ORDER BY path keep the cursor, both measured | `serve-real-jointypes` 78 (9 flip on `bd5e39d`); the gate had **no counter at all**, and its own `FIRST 1 over a RIGHT join` cell was blind to this because its preserved side was two rows in ASCENDING order - the new cells use a preserved side stored out of order with duplicate keys and an unmatched row in the middle |

## Stale claims retired

- "DATEADD on any TIMESTAMP WITH TIME ZONE returns a fixed zeroed buffer (1858-11-16 00:01:00.0000 -23:59) for every part" / "DATEDIFF between two TIMESTAMP WITH TIME ZONE values always returns 0" / "CAST to a narrower NUMERIC silently wraps modulo 2^32 / 2^16 instead of raising 22003" (all three from `### Confirmed WRONG ANSWERS still open`) — ALL THREE FALSE, measured 2026-09-17 against the live engine — and so are BOTH window-frame claims below, which makes FIVE of the five entries probed from this list stale. Every one of the seven DATEADD parts over `TIMESTAMP '2020-01-15 10:20:30.1234 +02:00'` agrees exactly and keeps its offset (`2 YEAR` → `2022-01-15 10:20:30.1234 +02:00`, and so on through MILLISECOND); all seven DATEDIFF parts agree (1, 14, 431, 10339, 620365, 37221870, 37221869876.6), nowhere near a constant 0; and `dateadd_impl` takes a `zone: u16` argument, so zoned values are handled by design. The narrowing casts all raise `22003 arithmetic exception, numeric value out of range` on BOTH servers (`CAST(70000 AS SMALLINT)`, `CAST(5000000000 AS INTEGER)`, `CAST(99999999999 AS NUMERIC(9,0))`, `CAST(1e20 AS NUMERIC(9,2))`), with `CAST(5 AS SMALLINT)` answering 5 on both as the control.
  **AND THE LIST THEY CAME FROM NEEDS RE-MEASURING BEFORE ANYONE PLANS FROM IT.** It carries 30 live bullets and not one strikethrough, yet three of the first four probed are stale — the fourth (the window `RANGE` framing claim) was probed with a cell naming a table the fixture does not have, so it agreed on `42S02` and proved nothing, and remains genuinely unmeasured. Treat the remaining entries as a hunt's output, not as a defect list: each is cheap to re-probe, and a probe that rejects a candidate costs far less than a chunk spent "fixing" what already works.
  **THE WINDOW PAIR, measured without a wrapper:** `SUM(ID) OVER (ORDER BY DEPT_ID RANGE BETWEEN UNBOUNDED PRECEDING AND 1 FOLLOWING)` over EMP's first four rows is 6, 6, 10, 10 on BOTH servers and `RANGE BETWEEN 1 PRECEDING AND UNBOUNDED FOLLOWING` is 10, 10, 10, 7 on both, with a frameless window, `OVER ()`, `ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW` (1,3,6,10) and `RANGE CURRENT ROW` (3,3,3,4) all agreeing too, and EMP's single NULL `DEPT_ID` row present as the claim's own peer group requires.
  **AND ONE GENUINELY NEW FINDING, stumbled on while MISDESIGNING that probe - a REFUSAL, so it ranks below every wrong answer:** a window function wrapped in a DERIVED TABLE is refused outright. `SELECT SUM(W) FROM (SELECT SUM(ID) OVER (ORDER BY DEPT_ID RANGE BETWEEN UNBOUNDED PRECEDING AND 1 FOLLOWING) AS W FROM EMP) Z` answers 57 on the engine and a bare *Dynamic SQL Error* here; the same wrapper over `RANGE BETWEEN 1 PRECEDING AND UNBOUNDED FOLLOWING` is 52, over `ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW` 45, and over `RANGE CURRENT ROW` 18. The frame is not what is refused - every one of those frames answers correctly unwrapped - so this is the derived/aggregate path's `OVER`-present guard, not the window machinery. Unmeasured beyond these four shapes and ungated; recorded here rather than swept into an unrelated slice. **FIXED 2026-09-17 - and ~~the derived/aggregate path's `OVER`-present guard~~ WAS THE WRONG SUSPECT, named from reading rather than measuring.** Those guards are in `corr_lone_aggregate` / `corr_inject_first` and they are OPTIMISATION DECLINES, not refusals. A trace overturned the whole premise: the statement PREPARES, BOTH levels PLAN, the fetch is ENTERED, and there is no `prepare refused` line anywhere - it died INSIDE the fetch, in `branch_rows_res`, the row-source spine, which refused a windowed `Plan::Project` outright. See the entry below; the shape is now gated, at 16 agreeing shapes and 30 flipped cells.
  **THE PROBE LESSON, paid for twice on this one candidate:** both earlier attempts were VACUOUS because the CONTROL errored instead of answering - first a cell naming `EMP` in a fixture that has only `T` (both sides `42S02`), then the derived-table wrapper above (both sides `42000`). A control that agrees BY ERRORING is not a control, and two of four "controls" refusing is the tell that the harness, not the server, is what is being measured.

- "This is NOT a one-spot change: it lands in the transliteration/collation core (`coll_value_cmp`, the comparison term, index-key building)" and "a raise/answer predicate that also turns on the column's byte width, not cheaply reproduced" (roadmap's NONE-charset entry; [adopt_carrier_literal]'s doc block) — BOTH FALSE, measured 2026-09-17. `coll_value_cmp` keys only PXW_INTL and the ICU family, and a byte-carrier operand never reaches it; the LIKE fix landed in `adopt_carrier_literal` plus an expression-path twin, with `textcolcmp` (359) and fourteen more charset/index gates unmoved. And the raise is decided by PATTERN SHAPE alone — the leading literal segment ending in a multi-byte character — identically over VARCHAR(20), VARCHAR(5), VARCHAR(4) and CHAR(5), so the column's width never entered into it. The half that WAS true is that it deserved its own measured chunk.
...
- an unnamed derived-table column on a LATERAL side, or under the lone-`COUNT(*)` join fast path, refuses WITHOUT the engine's four-line diagnosis: both callers destructure for `Plan::Join` and drop the carried `RefusedEval` (`plan_lateral` 40049, the fast path 50577). Both still REFUSE, so no wrong answer is served, and the `COUNT(*) ... GROUP BY` form - which bypasses that fast path - carries the message in full, which is what pins the cause (`serve-real-derived`, `narrower()` cells that announce their own retirement);
- `COUNT(*)` over a LATERAL, and `RDB$DB_KEY` even when aliased, refuse here where the engine answers - measured identical on `7a66881`, so neither belongs to the derived-column work (`serve-real-derived`);
- an uncorrelated LATERAL delivers its rows in a different order from the engine's when the statement has no `ORDER BY`, where no order is guaranteed to begin with; the ordered twin agrees on both binaries.
- a generator column of a derived table or CTE refuses whenever the outer delivers it anything other than EXACTLY ONCE, because the engine's draw count differs in each case and one-draw-per-delivered-row would answer a different wrong number: two references draw 8 and three draw 12 (against four rows), an outer `WHERE` on the column draws 6, an `ORDER BY` or `DISTINCT` on it draws 8 (the engine evaluates the list once to materialise and again to deliver, answering the SECOND batch), `GROUP BY` on it draws 15, a CTE referenced twice draws 8, and an aggregate over it draws 4. `FIRST`/`SKIP` over one, a UNION branch, a JOIN side, a scalar subquery, a nested derived table, `INSERT ... SELECT`, and an EXPRESSION over the generator (`(NEXT VALUE FOR S) + 100`, whose slot hides inside the column's expression and would have to be filled in the RECORD before that expression computes) refuse for the same reason. All REFUSE, so none serves a wrong answer (`serve-real-genrow`, 13 recorded cells).
- a RECURSIVE CTE WRAPPED IN A DERIVED TABLE - `SELECT * FROM (WITH RECURSIVE R AS (...) SELECT L FROM R) Z` - answers nothing here where the engine answers the walk. Measured identical before and after the describe-name work, so it belongs to neither; a refusal, ranked below every wrong answer, and recorded so it is not re-discovered as new.
- a QUOTED CTE NAME IS NOT PARSED AT ALL: `WITH "Q1" AS (SELECT ID FROM T) SELECT ID FROM "Q1"` - entirely legal, no cycle - answers `-204 Table unknown "Q1"` here where the engine answers the rows, and the same for a literal body and for a quoted declaration referenced unquoted. `parse_with`'s name scanner accepts only `[A-Za-z0-9_$]`, so the whole CTE dispatch is skipped. An UNQUOTED declaration referenced with quotes agrees, which localises it to the DECLARATION. It is also why the cycle check cannot fire for a quoted name (`serve-real-cte`, measured 2026-09-17);
- an EARLIER CTE referencing a LATER one is legal and the engine answers it (`WITH C2 AS (SELECT ID FROM C3), C3 AS (...) SELECT ID FROM C2`); this server refuses, because each body is spliced with only the CTEs declared BEFORE it;
- `WITH RECURSIVE` declaring TWO CTEs, one with a proper anchor, is legal and the engine answers; this server refuses it ("a recursive WITH may declare one CTE here"), which the measurement shows is not the engine's rule;
- the engine emits a WARNING - `CTE "@1" is not used in query` - and then answers, for a cycle-free CTE the main query never references; this server answers the same rows without the warning, having no warning channel on the prepare path.

- a NON-ASCII LITERAL INSIDE A CTE refuses at PREPARE with a contentless 42000 - `WITH Q AS (SELECT 'a<C3A9>b' AS C FROM RDB$DATABASE) SELECT OCTET_LENGTH(Q.C) FROM Q`, where the engine answers 4. It is the LITERAL'S BYTES and not the CTE: with an ASCII literal every CTE shape agrees (plain column, `||`, OCTET_LENGTH, CHAR_LENGTH, CAST to OCTETS, CAST to VARCHAR, UPPER, qualified and unqualified), and the DERIVED-TABLE twin of the same statement answers correctly. Measured identical on `b1df783`, so it is older than the wrapped-literal charset fix and not caused by it; held by two self-retiring `refuses` cells in `serve-real-litcs` that go red the day fire-crab answers.

- `CONTAINING` over a DERIVED-TABLE or CTE **literal** column refuses where the engine answers - `SELECT COUNT(*) FROM (SELECT 'café' AS C FROM T) Z WHERE Z.C CONTAINING 'é'` is 1 on the engine and a refusal here, on BOTH attachments and **for an ASCII needle too** (`CONTAINING 'caf'` refuses identically), so it is not the byte-space law. Measured IDENTICAL on `302c4d1` and on the binary that fixed the carrier mirror, so it is older than that slice and not its doing. Two controls pin what it is really about: the same wrapper over a REAL column (`SELECT U AS C ...`) answers 1 on both servers, and the same wrapped literal under `LIKE` answers 1 on both - so the refusal belongs to the WRAPPED LITERAL reaching `CONTAINING`, not to derived tables and not to the predicate. Held by two self-retiring `refuses` cells in `serve-real-nonecmp` that go red the day fire-crab answers.

- a RECURSIVE CTE JOINED TO ANOTHER CTE refuses at PREPARE with a contentless `42000 Dynamic SQL Error` where the engine answers - `WITH RECURSIVE C AS (<walk>), Q AS (SELECT MID, ID FROM JM) SELECT Q.MID FROM C R JOIN Q ON Q.ID = R.ID`, and identically with the two written the other way round. The same walk joined to a TABLE, a DERIVED TABLE or a VIEW answers fine, so it is the second CTE that is not servable, not the join. Found 2026-09-18 while measuring the recursive-CTE join-driver law (it surfaced as a cell whose "divergence" was really this refusal, which a one-sided vacuity guard had scored as an order difference); measured identical on `ae84769`, so pre-existing and caused by neither that chunk nor this one. Held by two `refuses` cells in `serve-real-recursive`.
- **DONE 2026-09-18** (`qa/serve-real-nonecmp.sh` 102 -> 113, 9 flip on `34df15d`) — **and the entry below was right about the SITE and wrong about the ROOT.** It called this "a cause of its own" because it failed under a NONE attachment, where the COLUMN path's lift never runs. True, and beside the point: it is the THIRD and FOURTH call site of ONE mistake. The EXPRESSION path has three `to_carrier` lifts of its own (LIKE, SIMILAR TO, STARTING WITH) and all three were wrong **in opposite directions at once** - `to_carrier` spells a string's UTF-8 BYTES, so under a TABLED attachment it lifted TWICE (OCTETS answered 0 under NONE/WIN1252/ISO8859_1, 1 under UTF8), while the `expr_is_octets` guard meant a NONE carrier was NEVER lifted (the NONE cast answered 0 under UTF8 alone). Each site now lifts through [transcode_text] from the ATTACHMENT's charset for ANY byte carrier, while `octets_like_term`'s wildcard-free equality MODEL stays keyed on OCTETS - the lift and the model are two decisions that one test was making. **Had I asked "where else does `to_carrier` appear?" the night I filed this, one grep would have found all three.** Original entry: an **OCTETS CAST under `STARTING WITH`** answers 0 where the engine answers 1 - `SELECT COUNT(*) FROM T WHERE CAST(<utf8 col> AS VARCHAR(20) CHARACTER SET OCTETS) STARTING WITH 'café'`, under a NONE, WIN1252 *or* ISO8859_1 attachment, while its **UTF8 twin agrees** and the same cast under `LIKE` agrees on all four. It is NOT the carrier-lift law that the tabled-attachment slice fixed: it fails under a NONE attachment too, where that lift never runs at all, so it has a cause of its own. Measured identical before and after that fix (same three attachments failing, same UTF8 cell passing), so neither caused nor repaired by it. Held by two self-retiring `differs` cells in `serve-real-nonecmp` with the UTF8 twin beside them as the contrast that pins it.

- ~~a WILDCARD-FREE `LIKE` with a NON-ASCII pattern over a **REAL-charset column**, under a **TABLED single-byte attachment**, makes the ENGINE RAISE `22001 string right truncation` where this server answers **0 rows** - `SELECT COUNT(*) FROM T WHERE <utf8 col> LIKE 'café'` under WIN1252 or ISO8859_1. Found 2026-09-18 while verifying the expression-path carrier lift, as a CONTROL row that moved. Measured identical on `9bf97ca`, `34df15d` and the binary that fixed the lift, so none of the carrier slices caused it. **Its neighbours do NOT raise**: `LIKE '%café%'`, `STARTING WITH 'café'` and `= 'café'` all answer 0 on the engine under the same attachment, so the rule is specific to the wildcard-FREE form. This server has no raise path under a real attachment at all - [carrier_expr_like] and [param_or_typed_term]'s LIKE guard are both gated on a byte-carrier ATTACHMENT - which is why it answers instead of raising. Held by two `eng_raises` cells in `serve-real-nonecmp` that go red if either server changes behaviour.~~ **DONE 2026-09-18** (`qa/serve-real-nonecmp.sh` 113 -> 138, 6 flip on `ae84769`) — implemented as [like_truncation_raise], a statement-level [PREPARE_REFUSAL] rather than a per-row `Term`, because the raise is **not value-gated**: it fires over an EMPTY table, over a row whose value is NULL, and under `1 = 0 AND`, and its two counts never move (`expected length 0, actual 1`) across pattern lengths and across VARCHAR(5), VARCHAR(20) and CHAR(8) alike. TWO call sites, because a COLUMN reaches [param_or_typed_term] carrying a Descriptor while an EXPRESSION reaches [resolve_expr_term]: `U || '' LIKE 'café'` and `UPPER(U) LIKE 'CAFÉ'` raise as well, and a fix to one leaves the other answering 0. The vector matches the engine's text word for word, both numbers included - which `agree` could never have held, since `sig()` collapses every failure to the single word REFUSE, so the cells use a new `both_raise` helper that fails if either side stops raising OR the texts diverge. **AND THE FIRST VERSION OF IT WAS A WRONG ANSWER.** With no blob guard, `B LIKE 'café'` REFUSED a statement the engine answers 0. Measured afterwards rather than assumed: the engine raises for a plain VARCHAR operand and stays quiet on SIXTEEN blob-typed shapes, while the broken binary raised on TWELVE of them - `B`, `UPPER(B)`, `LOWER(B)`, `TRIM(B)`, both concat directions, `COALESCE`, `CASE`, `IIF`, `SUBSTRING` and the mixed `U||B`/`B||U`. Excluded at the call site with [blob_result] - the predicate CONTAINING already uses - which walks Concat/Iif/Coalesce/Case and every text-answering function and so covers all twelve without naming one of them. **A gate cell added on a hunch caught it; neither probe matrix did**: the 60-cell shape matrix and the 30-cell boundary sweep both read 0 DIFF while that regression was live, because neither contained a blob.
...
- **TEMPORAL SUBTRACTION IS TYPE-DISCIPLINED, AND A TIMESTAMP DIFFERENCE ROUNDS DONE (2026-09-08, `serve-real-tempdiff`):** two silent wrong answers on date/time arithmetic, both from a third eight-surface differential hunt and pinned against FB6. (1) The engine allows a temporal difference only WITHIN a family (`add_datettime`): `DATE - DATE` (integer days), `TIME - TIME` (scale 4), `TIMESTAMP - TIMESTAMP` (scale 9, the `WITH TIME ZONE` spelling mixing freely with the plain one on the UTC instant). Every MIXED pair - a DATE against a TIMESTAMP either order and zoned or not, a TIME against anything but a TIME - is the engine's *expression evaluation not supported / -Invalid data type in DATE/TIME/TIMESTAMP addition or subtraction in add_datettime()* (42000). fire-crab's `Expr::type_of` for a temporal `Sub` had a catch-all `_ => Numeric` that INVENTED a fractional-day number for a DATE-vs-TIMESTAMP difference (`DATE - TIMESTAMP` answered `-0.416...`); the arm now types only the same-family pairs and refuses the rest at prepare, with `temporal_diff_shape` tightened to agree (the rank/width cascade must not contradict the type) and the eval arm carrying a fail-closed guard. (2) The scale-9 `TIMESTAMP - TIMESTAMP` value was computed by a TRUNCATING divide to nanodays where the engine rounds HALF AWAY FROM ZERO - a 10-hour difference is `0.416666667`, not the `...666` truncation gave (a stale note here had even called it truncation, from a case where the two coincide). TIME differences (exact) and `temporal + number` shifts were and stay correct; `temporal + temporal` was already refused.

- **NONE/OCTETS TEXT MEETING A REAL CHARSET: A CAST CARRIES THE RAW BYTES, AND AN EQUALITY/ORDERING COMPARISON WORKS IN BYTE SPACE DONE (2026-09-08, `serve-real-castnone` + `serve-real-nonecmp`):** the third hunt's top find, a silent wrong answer that DROPPED ROWS. isql's default attachment is effectively NONE, so a text literal arrives one char per octet (`'café'` is five carrier octets). Settled by a research workflow (6 differential slices + a code map) and then VERIFIED directly, including the crux the workflow argued: `WHERE u = _NONE x'FF'` is 0 rows with NO error while `CAST(_NONE x'FF' AS UTF8)` RAISES 22000 - so a comparison byte-compares and never validates, a CAST validates; two distinct models. **Stage A (`castnone`):** an unqualified `CAST(x AS VARCHAR(n))` takes the ATTACHMENT charset (measured: charset 4 under a UTF8 attachment, 0 under NONE); under a byte-carrier attachment the value moves into it via the engine's CVT_move ([transcode_text]) so the source's stored bytes pass through - `CAST(<utf8 'café'> AS VARCHAR(20))` is 5 octets / `char_length` 5, where fire-crab's carrier encoder had dropped é to one byte (4). The synthetic pad-wrap cast is excluded (its charset is the branches'); a real attachment is untouched. **Stage B (`nonecmp`):** a real-charset column compared to a byte-carrier literal now reinterprets the literal's octets in the column's charset ONCE at prepare (`adopt_carrier_literal` -> [transcode_text] from the byte-carrier attachment to the column set), turning it into a real-vs-real compare the existing machinery runs for `=`, `<>`, `<`, `>`, `<=`, `>=`, BETWEEN and IN; bytes that do not spell the set stay a carrier string that equals no real value - the engine's no-match / no-raise. `WHERE u = 'café'` over a UTF8 column now returns the row (was 0). No regressions: the 359-cell `textcolcmp`, `collate`, `index`, `join`, `orderby`/`groupby`/`distinct` and the cast/charset families all stay green, because the comparison change fires ONLY when one side is a byte carrier and the other a real charset, leaving same-charset and real-vs-real paths verbatim. **Recorded, NOT done (each its own path / measured contract):** LIKE / STARTING / SIMILAR / CONTAINING of a real column with a byte-carrier pattern (the engine's per-operator semantics differ - `<utf8> LIKE '<accented>%'` under NONE RAISES 22000, it does not answer); a hex-literal (`x'..'`, the `Rhs::Oct` expression path) equal to a real column's bytes; a text-BLOB column compared to a carrier literal (the `BlobText` expression path); a NONE/OCTETS COLUMN compared to a carrier literal (fc still returns 0 where the engine matches); column-vs-column cross-charset; the `_UTF8'..'` charset introducers (still cleanly REFUSED); and Stage D, teaching the index band to probe a real key from a reinterpreted carrier literal (a PLAN-only cosmetic today, rows already correct via scan).

- **CROSS-CHARSET COMPARISON EXTENDED TO THE EXPRESSION-PATH SHAPES DONE (2026-09-08, `serve-real-xcharcmp`):** the byte-space model, previously only on the literal fast path, now covers every operand shape that reaches the EXPRESSION comparison path - closing more dropped-row silent wrong answers. Settled by a research workflow (5 differential slices + a code map), verified directly incl. the crux that a comparison NEVER raises (`u = x'..FF'` is 0 rows, no error) where the same bytes through a CAST raise. **(1) NONE-column double-lift:** `param_or_typed_term` re-applied `to_carrier` to a literal a byte-carrier attachment had ALREADY delivered as a carrier string, so `n = 'café'` over a NONE column answered 0; the lift is now for a real-attachment literal only. **(2)** a new `Expr::CarrierEnc(inner, cs)` re-spells a REAL text value as the carrier of its OWN charset bytes (café -> its five UTF-8 octets); a byte carrier already is such a string. **(3)** `cmp_sides` (Text,Text) gained a byte-space block BEFORE the octets/collation branches: when EXACTLY ONE operand is a byte carrier and the other a real charset, wrap the real side in `CarrierEnc` and compare octets (through `OctKey` when OCTETS is involved, 0x00 pad; else `value_cmp`, 0x20 pad) - fixing a text BLOB vs a literal/column (`b='café'`), a hex literal (`u = x'636166C3A9'` matches; bad bytes 0/no-raise; reversed too), an OCTETS column vs a real literal, and column-vs-column (`u=o`, `u=n`, `u=b`). Two real charsets or two byte carriers fall through untouched, so the 359-cell `textcolcmp`, `collate`, `octets`, `index`, `join`, `orderby`/`groupby`/`distinct` all stay green. **(4)** `adopt_carrier_literal` now also reinterprets the pattern for STARTING WITH / CONTAINING / SIMILAR TO of a real text COLUMN, which the engine ANSWERS. **Recorded, NOT done:** LIKE with a carrier pattern (the engine SPLITS - `<utf8> LIKE '<accented>%'` RAISES 22000 while `LIKE '%<accented>%'` and exact `LIKE '<accented>'` ANSWER, a predicate that also turns on the column byte width); the BLOB pattern operators (`b CONTAINING '<accented>'`, a distinct pattern path); cross-charset JOIN key + mixed-carrier UNION (hashing/set-type, the riskiest); a text BLOB compared to a NUMBER (engine answers empty, fc still raises 22018); and the `_UTF8'..'` introducers (still refused). The wire 345 tests and every text/compare/index/join gate stay green.

- **A COMPUTED COLUMN TAKES ITS DECLARED FIELD TYPE, NOT THE EXPRESSION'S DONE (2026-09-08, `serve-real-computedtype`):** the fourth hunt's top find - one root cause behind three silent wrong answers. The engine wraps a `COMPUTED BY` expression in a CAST to the column's `RDB$RELATION_FIELDS` descriptor (blr_cast to the field format), so the value is ROUNDED to the column's scale, RANGE-CHECKED against its width, and DESCRIBED by its own wire type. fire-crab typed and evaluated the raw expression, so `INTEGER COMPUTED BY (ID/3.0)` answered `6.6` as INT64 scale -1 (should be `7`, LONG scale 0); `SMALLINT COMPUTED BY (ID*1000)` returned `100000` with NO error (should raise 22003); and `DOUBLE PRECISION COMPUTED BY (ID/3.0)` described as INT64, corrupting a prepared client's binding. The projection now injects `Expr::Cast(<resolved expr>, <declared target>)` and describes by the declared descriptor (`computed_cast_target` maps the field's dtype/scale/sub_type to an Int / Numeric / Approx cast; text/temporal/blob/DECFLOAT/FLOAT computed columns fall back to the expression's form, recorded). A neighbouring accuracy bug surfaced by the DOUBLE case is also fixed: the scaled->double conversion in the CAST, UNION and approx-arithmetic paths now DIVIDES by the power of ten (`exact_to_f64`, mirroring `CVT_get_double`) instead of multiplying by its reciprocal, so `20/3.0` as a DOUBLE is `6.600000000000000` to the last bit, not `...001`. **Recorded, NOT done:** a computed column used in a WHERE predicate or inside an expression (`WHERE c_int=7`, `id||c_int`) still REFUSES (42000) - the predicate/expression resolvers decline a computed field id; extending them to expand it to the same cast expression is a broader, separate chunk. No regressions: the wire 345 + ods 161 unit tests and the computed/double/union/numeric/cast/aggregate/index/join gates stay green.

- **A CAST OF AN OUT-OF-RANGE VALUE TO AN INT128-BACKED NUMERIC RAISES, AND AN IN-RANGE ONE KEEPS CLEAN DIGITS DONE (2026-09-08, `serve-real-int128cast`):** two silent wrong answers on the approximate-source cast to a NUMERIC/DECIMAL of precision 19..38 (an INT128 backing), from the fourth hunt and a numeric-typing research pass. The overflow bound is the BACKING INT128 range (`|scaled| <= 2^127-1`), independent of the declared precision. fire-crab (1) SATURATED to 2^127-1 instead of raising 22003 - `CAST(1e39 AS NUMERIC(38,0))` answered `170141183460469231731687303715884105727` where the engine raises numeric-value-out-of-range; and (2) stored the double's EXACT BINARY expansion for an in-range value - `CAST(1.5e38 AS NUMERIC(38,0))` came back `150000000000000006067947700923341471744`. The `CastTarget::Numeric` approximate branch now, for the 16-byte backing only, raises with the exact 2^127 gate the integer-target cast already uses, and converts the double through its SHORTEST round-trip decimal (its decimal significance) so `1.5e38` is `1.5x10^38` with clean low digits. The narrower backings (2/4/8) are untouched - a double large enough to reach them already overflows to 22003. **Recorded, NOT done:** the exponent-form literal typing (fire-crab types every `<sig>e<exp>` DOUBLE; the engine types a significand past i64 as an EXACT type, but the INT128-vs-DECFLOAT(34) boundary needs more measurement - `9223372036854775808e0` is INT128 while `12345678901234567890e0` is DECFLOAT(34) despite fitting i128, so a naive "fits i128 -> INT128" rule is wrong); DECFLOAT precedence in COALESCE/CASE; and a double literal with >16 significant digits cast to a wide NUMERIC (the engine constant-folds the literal's extended decimal digits, beyond a double's precision). No regressions: the wire 345 tests and the castrange / cast / numeric / computed / double / union gates stay green.

- **EXPONENT-FORM NUMERIC LITERAL TYPING MATCHES THE ENGINE DONE (2026-09-09, `serve-real-explit`):** fire-crab typed EVERY `<mantissa>e<exp>` literal DOUBLE, so a high-significand one lost precision AND announced the wrong SQLDA type (`12345678901234567890e0` came back as a lossy `1.234567890123457e+19` DOUBLE where the engine keeps the exact `12345678901234567890` DECFLOAT(34)). Settled by a focused research workflow (dense boundary walk + downstream-safety audit + code map) and VERIFIED directly. The rule is by the SIGNIFICAND alone (mantissa digits, point removed, leading zeros dropped) as a magnitude M - NOT the net value or the exponent: `M <= 2^63-1` -> DOUBLE (so `1e40`, significand 1, stays DOUBLE); `M == 2^63` EXACTLY -> INT128 (a lone quirk - `9223372036854775808e0/e1/e5`, whole scale-0 only); `M >= 2^63+1` -> DECFLOAT(34) (`9999999999999999999e0`, `1.2345678901234567890e5`, a 40-digit significand). One shared `classify_exp_literal` feeds BOTH the SELECT lexer (`expr_atom_bare`) and the WHERE lexer (`numeric_tok`), so they cannot disagree; the DOUBLE branch parses the ORIGINAL text (byte-identical to before), INT128 reuses `RawExpr::Int128`/`Tok::Int128`, DECFLOAT(34) `text_to_dec128` + `RawExpr::DecFloat34`/`Tok::DecFloat34` (with `tok_lhs_raw` and `texpr_atom_bare` gaining the DecFloat34 arm). **Recorded, NOT done:** a DECFLOAT literal (exponent OR plain) in a WHERE COMPARISON, a CAST-to-another-type, or a concat still REFUSES 42000 - DECFLOAT is not yet a comparison/merge type (its own type-system chunk); this is the SAME limitation a plain large literal already has, gated here as a known-refuse rather than a wrong value. And a 2^63 significand that would need a SCALED or ROUNDED INT128 (`9223372036854775808e-1`, `9.223372036854775808e18`) is REFUSED (no Int128 literal carries a scale; the engine types it INT128). No regressions: the wire 345 tests and the literal / numeric / decfloat / cast / castrange / double / union / arith gates stay green.

- **UTF8 LOWER/UPPER USE THE ENGINE'S PER-CHARACTER SIMPLE CASE MAPPING DONE (2026-09-09, `serve-real-utf8case`):** a fourth-hunt HIGH that silently DROPPED ROWS. fire-crab cased a UTF8 string with Rust's `to_lowercase`/`to_uppercase` (the FULL, sometimes multi-char Unicode mapping) and kept a character UNCHANGED whenever that mapping was multi-char - right for `'ß'` UPPER and for a ligature, but WRONG for U+0130 LATIN CAPITAL LETTER I WITH DOT ABOVE: its full lowercase is `'i'` + COMBINING DOT ABOVE (two chars) while its SIMPLE lowercase - what the engine applies per character - is a single `'i'` (U+0069). So `LOWER('İ')` stayed `'İ'` (2 octets) where the engine answers `'i'` (1 octet), and `WHERE LOWER(nm)='istanbul'` silently dropped an `'İSTANBUL'` row. Fixed in `fire_crab_ods::intl::simple_case` by mapping U+0130 to `'i'` on lowercasing; every other casing edge already matched the engine (probed: `ß`, `ẞ`, the ligatures, dotless `ı`, `İ` under UPPER, the accented Latin-1 letters). No regressions: the wire 345 + ods 161 unit tests and the case / collate / collation / textcolcmp / charset gates stay green.

- **A CONDITIONAL WITH A DECFLOAT BRANCH NO LONGER RENDERS A CONFIDENT WRONG VALUE DONE (2026-09-09, `serve-real-dfcond`):** the DECFLOAT type-system's first, safe stage - closing the silent wrong answers that a research workflow (3 contract slices + type-system map) surfaced, without the full `ExprType::DecFloat` fan-out. fire-crab has no `ExprType` for DECFLOAT, so `conditional_type`'s fold DROPPED a DECFLOAT branch (its `type_of` is None) and COALESCE/CASE/IIF/DECODE took the SIBLING's type - `COALESCE(<decfloat>, <int>)` came back `0`, and `COALESCE(<int>, <decfloat>)` / `NULLIF(<decfloat>, <int>)` were latently wrong the same way (a NULL int, or a mismatched NULLIF, would render the decfloat truncated). The engine types such a mix DECFLOAT and coerces the other branch into it. Until fire-crab has a real DECFLOAT result type (the larger chunk), `conditional_type` now REFUSES a conditional that contains a DECFLOAT branch (`is_decfloat_arith`), and `type_of` REFUSES `NULLIF` with a DECFLOAT first operand rather than borrowing the second operand's type - a clean 42000 instead of a wrong value (match-or-refuse). A pure-decfloat conditional already refused; non-decfloat conditionals, and `NULLIF(<int>, <decfloat>)` (int first operand -> INTEGER), are unchanged. DECFLOAT COLUMN comparisons already work (via `decfloat_term`); a UNION with a DECFLOAT leg already refused. **Recorded, NOT done (the full DECFLOAT type-system):** a real DECFLOAT result type for a conditional (correct DECFLOAT value+SQLDA, not a refuse); a DECFLOAT LITERAL in `=`/`IN`/concat; `CAST` to/from DECFLOAT; DECFLOAT-vs-DOUBLE comparison (width-follows-the-decfloat-operand); SUM/AVG over DECFLOAT; and a decimal64 `round_to_dec16` top-exponent encoder bug (`1E+384` -> garbage; `>= 1E+385` should raise 22003). No regressions: the wire 345 unit tests and the condtext / decfloat / numeric / cast / union gates stay green.

- **A MERGE READS ITS STATEMENT-START SNAPSHOT, NOT ITS OWN WRITES DONE (2026-09-09, `serve-real-mergesnap`):** a fourth-hunt HIGH that CORRUPTED both the rowset and the persisted state - a Halloween problem. A MERGE is one request over one image: every subquery that reads the target (in a `WHEN MATCHED ... UPDATE SET`, a `WHEN MATCHED ... AND <cond>`, or a `WHEN NOT MATCHED ... INSERT VALUES`) evaluates against the target as it stood at statement start, exactly as a plain searched UPDATE does. fire-crab's `merge_exec` desugared each matched row into a per-row UPDATE run against the LIVE, growing image, so `WHEN MATCHED THEN UPDATE SET n = (SELECT SUM(n) FROM t)` grew `60,110,200` (each row reading rows written before it) where the engine writes `60,60,60`. Settled by a research workflow (MERGE + UPDATE-OR-INSERT stability slices + execution map) and verified directly. The fix arms the existing `SubqImageGuard` (the pre-statement-image override `Database::bytes` already honors for RETURNING) around ONLY the per-branch `plan_update`/`plan_delete`/`plan_insert` FOLD - where the SET/condition subqueries are evaluated and the correlated lookups built - and drops it before `execute_dml_collecting`, so the WRITE still lands on the live image and earlier rows' writes accumulate (a first draft that armed the guard over the whole write phase re-based every row off the frozen image and lost all but the last install - caught by direct measurement). A plain UPDATE / DELETE / UPDATE OR INSERT never enters `merge_exec` and is untouched (already stable). Verified: SUM/COUNT/MAX/`n+SUM`/correlated/`set from source`/`n=n+1` matched updates, a NOT MATCHED insert whose VALUES read the start-of-statement sum (both new rows 60), and the plain-UPDATE and delete-below-avg controls. No regressions: the wire 345 unit tests and the merge / mergeparam / returning / corrsub / insert / update / trigger / updateorinsert gates stay green.

- **LPAD / RPAD ENFORCE THE VARCHAR-RESULT BYTE LIMIT DONE (2026-09-09, `serve-real-padlimit`):** a fifth-hunt HIGH - a confident value with the wrong cardinality where the engine refuses. An LPAD/RPAD result is a VARCHAR whose declared byte width is the pad length N times the SOURCE charset's bytes-per-character; when that exceeds 65535 the engine raises 54000 *Implementation limit exceeded* AT PREPARE, even wrapped in `CHAR_LENGTH` (which otherwise consumes the string). fire-crab built the over-long string and answered a value - `CHAR_LENGTH(LPAD('Hi',16384,'*'))` returned 16384 under a UTF8 attachment where the engine errors, and announced an impossible VARYING width. The boundary is charset-exact (measured): a NONE/single-byte source caps at N=65535, UTF8 (4 bytes/char) at N=16383 (16383x4=65532 answers, 16384x4=65536 raises). Fixed at the LPAD/RPAD resolution: when a LITERAL length N makes `N x bytes_per_char(source charset) > 65535`, resolve refuses, so the refusal propagates through any enclosing expression. A non-literal length keeps the current fallback (the rare dynamic case). No regressions: the wire 345 unit tests and the string / textwidth / charform / cast gates stay green.

- **TWO SILENT-WRONG SUBQUERY/RECURSION SHAPES REFUSED DONE (2026-09-09, `serve-real-subqarity` + `serve-real-recorder`):** two fifth-hunt finds, settled by a research workflow (traversal-order + arity contract + executor map) and verified directly. **(1) Subquery arity:** a subquery used as a SCALAR operand, an IN/NOT IN right side, or a quantified (ANY/ALL/SOME) operand must project exactly ONE column - the engine raises 07002/-104 "count of column list and variable list do not match" at prepare, independent of cardinality. fire-crab folded the multi-column inner away and then took COLUMN 1, silently answering a WRONG ROWSET (`WHERE a = (SELECT x, x*10 FROM t2)` returned the a=1 row). Fixed at the single choke point `corr_describe` with a `scalar_arity` gate (refuse when the operand degree != 1); EXISTS is exempt (passes `scalar_arity=false`) and a single-column operand is unchanged. **(2) Recursive-CTE FIRST/SKIP without ORDER BY:** the engine walks a WITH RECURSIVE depth-first in RECORD-NUMBER (storage) order, fire-crab breadth-first (a level queue) - the SAME SET, a different ORDER - so a top-level `FIRST`/`SKIP`/`ROWS` slices a DIFFERENT SET OF ROWS (measured: `FIRST 3` of a tree walk is the engine's {1,1/2,1/2/4} vs fc's {1,1/2,1/3}). fire-crab has no record-number model and structurally cannot reproduce the engine's sibling order (a stack would trade one wrong order for another), so it now REFUSES a recursive CTE with FIRST/SKIP/ROWS and no top-level ORDER BY (which is exactly where the ROWSET, not just the order, diverges); a top-level ORDER BY makes the two byte-identical and stays open, and the unlimited walk (order-only, same set) and a linear numbers series are left answering. Both are refuse-not-wrong fixes; a DISTINCT (order-independent) and a partial-key ORDER BY are noted narrowings. No regressions: the wire 345 unit tests and the recursive / cte / corrsub / subquery / view gates stay green.

- **DECFLOAT(16) ENCODES A HIGH-EXPONENT VALUE CORRECTLY AND RAISES ON OVERFLOW DONE (2026-09-10, `serve-real-dec16emax`):** a recorded confident-wrong-value (garbage) from the DECFLOAT type-system research. A decimal64's stored exponent must be <= 369 (biased <= 767), but `encode_dec64` was handed a value with a higher exponent and a short coefficient - a cast of `1E+384` arrives as coefficient 1, exponent 384 - and the out-of-range `biased` overflowed the two combination-field exponent bits, garbling the whole word: `CAST('1E+384' AS DECFLOAT(16))` answered `8.000000000000001E-369` where the engine answers `1.000000000000000E+384`. And a magnitude past the format's range (adjusted exponent > emax 384, `1E+385`) must raise 22003 *Decimal float overflow*; fire-crab produced more garbage. `encode_dec64` now NORMALISES - it pads the coefficient with trailing zeros to bring the stored exponent down to 369 while it stays within 16 digits - and a new `fit_dec64` (round to 16 sig, normalise, then REFUSE what still will not fit) is routed from the DECFLOAT(16) cast (raising 22003) and the decimal64 store paths (`round_to_dec16_of`). Mid-range and small values are byte-identical. No regressions: the wire 345 + ods 161 unit tests and the decfloat / numeric / cast / int128cast / explit gates stay green.

- **UNARY MINUS OF A SMALLINT / INTEGER MINIMUM RAISES, IT DOES NOT WRAP DONE (2026-09-10, `serve-real-negoverflow`):** a sixth-hunt HIGH - a confident wrong scalar. Negation is `0 - x`; when `x` is the minimum of its type the positive result overflows the type width, so the engine raises 22003 *Integer overflow* (`-CAST(-2147483648 AS INTEGER)` is +2147483648, one past INT32_MAX). fire-crab computed +2147483648 correctly but announced the result in a 2/4-byte SMALLINT/INTEGER wire slot, so the client read a WRAPPED value back (`-32768` / `-2147483648`). The i64 (BIGINT) and i128 negations already carried their `checked_neg`; only the i16/i32 backing lacked a check. Added a narrow-integer range check at the projection choke point `ProjCol::value_of` - beside its existing "an INT128 value in a non-INT128 wire raises" guard - so an exact result whose backing integer leaves the announced SMALLINT/INTEGER range raises 22003 before the wire slot truncates it. It runs only for COMPUTED columns (a stored value always fits), and tests the BACKING integer so a `NUMERIC(p,0)` / `NUMERIC(p,s)` over the same width is covered too. (First draft added the check to the wrong `match` arm - the stored-column path - and was inert; caught by a trace.) No regressions: the wire 345 unit tests and the arith / numeric / cast / computedtype / index / join gates stay green.
...
- **DONE SINCE 2026-09-10, one line each (the commit message carries the measured contract; the gate is the proof):**
  ...
  - ROUND over an approximate value carries its EXACT result (Value::Rounded) while describing DOUBLE / FLOAT: a CAST to text is '2.68' (was '2.680000000000000'), `||`, a derived column and every store see the exact value (DECFLOAT 2.68, INTEGER 3), arithmetic / comparison / conditionals / UNION see the double; an approximate operand converted to text announces width 24 / 15 (`serve-real-roundexact`, 2026-09-15).
  - A bound TEXT that cannot convert against a base-table INTEGER / SMALLINT / BIGINT / INT128 / NUMERIC / DOUBLE / FLOAT / BOOLEAN column raises *Conversion error from string "<text>"* PER ROW and value-gated - an empty table, a NULL column and a dead conjunct in front answer with no raise - through OR, IN, BETWEEN, FIRST, COUNT, a join's WHERE, a derived table, HAVING, UPDATE and DELETE; a DECFLOAT(34) column raises *Decimal float invalid operation* then the conversion error; a lowercase hex text is the same raise (`serve-real-bindconv`, 2026-09-15). fire-crab refused all of these at execute with a generic *Dynamic SQL Error*.
  - The same raise under an EXPRESSION left side (`DP + 0 = ?`, `ABS(I) = ?`, `I * 2 = ?`, `CAST(I AS NUMERIC(9,2)) = ?`, `COALESCE(I, 0) = ?`), gated on the EXPRESSION's own value - a NULL column and an empty table answer, a false conjunct in front silences it, either side of an OR raises (`Term::ExprConvErr`); and a bound text whose magnitude no `Rhs::Num` holds compares instead of refusing - an exact column exactly (`I < ?` ['1e400'], a 41-digit spelling, i128's own limits: every non-NULL row; `I = ?`: none), an approximate one against +-INFINITY (`DP > ?` ['-1e400']), through an expression side too (`I + 0 < ?`) (`serve-real-bindconv`, 2026-09-16).
  - **`CAST(? AS <type>)` in a comparison, answered instead of refused for EVERY bind** - the shape was unsupported, not mis-typed: the whole call lexes as ONE token, so its `?` never reached [parse_value]'s numbering and [resolve_expr_inner] refused `RawExpr::Param` outright. The `?` is now numbered where the token is consumed (its place in the TEXT: `I = ? AND N = CAST(? AS NUMERIC(9,2))` numbers 0 then 1), the slot describes as the CAST TARGET (measured: LONG for INTEGER, VARYING(5) for VARCHAR(5), DOUBLE for DOUBLE PRECISION - the engine's own describe), and the value is substituted under the cast at bind so the CAST converts PER ROW. That gives the engine's gate for free - NOT value-gated (a NULL column raises, `SELECT 1 FROM RDB$DATABASE WHERE 1 = CAST(? AS INTEGER)` raises), an empty table answers, a FALSE conjunct in front silences it - and each target's OWN vector: *numeric value is out of range* for `CAST(? AS SMALLINT)` ['99999'], the DECFLOAT two-part vector, '1.9' rounding to 2. Also here: a DECFLOAT(34) COLUMN against an unconvertible text LITERAL raises *Decimal float invalid operation* then the conversion error, where a DECFLOAT(16) column raises the conversion error ALONE (both measured), and a CAST to either width raises the two-part vector (`serve-real-bindconv`, 2026-09-16).
  - **A `?` in a JOIN's ON, answered instead of refused for EVERY bind** - `parse_on` parsed with a LOCAL counter and refused any placeholder ("a `?` in an ON is not a shape this server binds"). The counter and the descriptor sink are the caller's now, so an ON slot numbers between the select list and the WHERE - the engine's text order (probed: `ON P.I = ? WHERE P.ID = ?` bound [5, 1] answers, [1, 5] does not) - and every step's `on` is BOUND at execute ([bind_parts], at all six executor entries: the four `join_rows` sites, the three `join_rowsource` ones and `JoinCursor::open_inner`, where it runs BEFORE the parts are split so no unbound ON reaches a mirror side). INNER and LEFT, either operand order, a text bind converting, NULL as UNKNOWN, and the per-OUTER-row raise the engine gives even over an EMPTY inner side (`serve-real-bindconv`, 2026-09-16).
  - **... and so is a CONSTANT LITERAL SOURCE through `INSERT .. SELECT`** - `INSERT INTO T (DF) SELECT 1E+200 FROM RDB$DATABASE` stores 1E+200, `SELECT 1E+3 * 2` stores 2E+3, `0.1E0 * 3` stores 0.3, and the DECFLOAT(16) twin follows. The planner RE-PLANS the select text with its exponent literals read as decimals (the [decfloat_const_expr] parse mode) when [approx_source_into_decfloat] would have refused it: the column then TYPES decfloat, so that gate stops firing on its own rather than being weakened. A **MERGE's NOT MATCHED INSERT desugars to exactly this statement** (`INSERT INTO PBW (ID, D16) SELECT 88, 1E+200 FROM RDB$DATABASE`, seen in the trace), so both MERGE literal branches came along with it. The runtime paths are untouched and gated: `SELECT ROUND(FL, 2)` still stores the exact 2.68 and a plain DOUBLE column still stores 2.6749999999999998, a DOUBLE destination still 1.000000000000000e+200 and a NUMERIC one 2000.0000. A ROUND / TRUNC / conditional source keeps the refusal - it has no literal spelling to re-read ([decfloat_source_is_const] is what tells the two apart) (`serve-real-dfparambind`, 2026-09-16).
  - **A MERGE's marker INSIDE A CALL takes the CALL's own type** - `WHEN MATCHED THEN UPDATE SET DF = COALESCE(?, 0)` bound 1.1 stores the engine's 1. This was a deliberate refusal: MERGE typed a marker from its DESTINATION COLUMN pushed down the value TEXT, which reproduces the destination rule ONLY, and admitting the shape that way stored 1.1000000000000001. The value text is now read back as a TREE - its `FC$P` markers become the `?` they were made from ([merge_marker_descs]) - and typed by [resolve_dest_param_expr], the same resolver the plain INSERT and UPDATE use, so the engine's own overrides apply where the text could not see them. Measured, both branches, one statement per fresh row: **COALESCE types from its OTHER arguments** (`DF = COALESCE(?, 0)` is a plain INTEGER slot storing 1; `I = COALESCE(?, 0)` bound 2^40 OVERFLOWS that slot and answers 22003 with the row untouched), **the sibling's own SCALE wins over the column's** (`N = COALESCE(?, 0.5)` bound 1.115 describes INT64 scale -1 and stores 1.10, where the column's scale 2 would have kept 1.12), and **a CAST types its own operand** (`DF = CAST(? AS INTEGER)` bound 1.6 stores 2). A value the resolver cannot read as a tree - a SOURCE alias reference, `SET DF = S.K + ?` - keeps destination-typing under [merge_marker_shape_ok], and a paren-free value cannot hide an override, so no shape that already typed changed (`serve-real-slottype`, 2026-09-16).
    **AND A WRONG ANSWER THE SAME WORK EXPOSED, fixed in the shared resolver:** COALESCE pushes NO type into its arguments, so a parameter under a MULTIPLY, a DIVIDE or a unary MINUS is still UNKNOWN when that node is made and the ENGINE RAISES - `COALESCE(? * 2, 0.5)` is "-expression evaluation not supported / Invalid data type for multiplication in dialect 3", with `2 * ?`, `? / 2`, `2 / ?` and `? * 1.0` the same and `-?` carrying negation's own message. fire-crab ANSWERED all of them, storing an invented 2.00 for a bound 1.115 - in plain `INSERT` and `UPDATE` too, so this was pre-existing and the MERGE work is only what surfaced it. All three paths refuse now. `? + 0`, `? - 0` and `0 - ?` are fine and take the sibling's scale, as do a CAST and a nested COALESCE; NULLIF, IIF and CASE are NOT this rule - they push the destination down and take `? * 2` at its type, so they are checked against the engine's value rather than refused.
    **RECORDED (gate hygiene):** `serve-real-slottype` carried two `refuses` lines for the MERGE shapes above and NEVER DEFINED that helper - bash printed "refuses: command not found" on stderr while the gate still reported PASS, so both lines asserted nothing for as long as they existed. They are real checks now, and the helper is defined beside `both` rather than after its call sites.
  - **A TEXT VALUE TOO LONG FOR ITS COLUMN RAISES THE ENGINE'S 22001** - on every write path, carrying the two counts the client needs: `string right truncation, expected length 5, actual 7`. fire-crab answered a bare Dynamic SQL Error (42000) for ALL of them. Two roots, one law: at EXECUTE, [encode_wire_value] returns a plain `None` when the value does not fit and each caller mapped that to its own generic message; at PLAN, a LITERAL fails in [encode_set_value], so `UPDATE T SET S = 'abcdefg'` was refused as a statement this server cannot plan - and a MERGE inherited that refusal through the UPDATE its branch desugars to. Measured on both sides, cell by cell: UPDATE and INSERT with a literal, a concatenation (actual 8), `INSERT .. SELECT`, MERGE, `UPDATE .. RETURNING` and a bound `?`, into VARCHAR and CHAR alike.
    `expected` is the column's declared CHARACTER count and `actual` the value's own **in the DESTINATION's characters** - one byte IS one character in a byte carrier, which is why five two-byte characters into a `VARCHAR(5) CHARACTER SET NONE` report 10 while seven of them into a `VARCHAR(5) UTF8` report 7. CHAR takes the same rule and its blank padding is NOT counted here (unlike the OUT-capacity rule, where a CHAR counts its padding); a value that fits still pads silently.
    The fix keeps ONE implementation of the fit rule, because a copy of it went stale once already: [text_bytes_for] now reports WHY it failed (`TextFit::TooLong` with the counts, or `Other`) instead of a bare `None`, the five execute paths ask [fit_or] and the two planners ask [set_value_fit_error] over the SAME wire value [encode_set_value] builds. A too-long literal becomes `Plan::RefusedEval(StringTruncation)` - a plan that RAISES rather than a statement that cannot be planned - which is what carries the vector into MERGE's per-row branches with no change there at all. A character with no image in the destination set stays the engine's OWN 22018 *Cannot transliterate character between character sets*, which is why the fit probe answers a reason rather than a bare refusal (`serve-real-textfit`, NEW gate, 2026-09-16).
    **RECORDED:** a BEFORE INSERT trigger assigning an over-long value to `NEW` (`NEW.S = 'abcdefg'` into a `VARCHAR(5)`) raises on NEITHER side and stores NULL on both - measured, so the trigger write path was left exactly as it was.
  - **A NON-ASCII LITERAL WAS ENCODED TWICE - in an unqualified CAST, and in EVERY MERGE.** Two independent double-encodes, both silent wrong ANSWERS, found by measuring a backlog item that turned out to be stale.
    **(1) An unqualified `CAST` of a LITERAL.** [cast_source_charset] consulted [text_form] only when the cast NAMED a character set; a bare `CAST(x AS VARCHAR(n))` fell through to [err_spell_charset], which answers `CS_UTF8` for a literal - and the evaluator then moved that value into a byte-carrier attachment a SECOND time, the literal already holding one char per byte there ([stmt_text_decode]). Measured under a NONE attachment: `CAST('éé' AS VARCHAR(20))` was 8 octets for the engine's 4 and read back as the mojibake `Ã©Ã©`, a 3-byte `'€'` went 3 -> 6, an OCTETS destination shipped `C383C2A9C383C2A9` for the engine's `C3A9C3A9`, and a plain `SELECT` of the cast showed it too - so this was never a store bug. A COLUMN source, a named `CHARACTER SET`, an ASCII literal and a real attachment all already agreed, which is why the gate that owns this law passed: every one of its cells was column-sourced. The guard now covers `CastTarget::Text { .. }`, which moves ONLY the `TfCs::Att` case - for an unqualified target the `Ttype` branch returns exactly what `err_spell_charset` already returned.
    **(2) Every literal in a MERGE.** [mark_merge_params] renumbers a statement's `?` before the MERGE is split, and it walked BYTES - `b[i] as char` made each byte of a multi-byte character its own char, which pushing re-encoded as UTF-8. So a MERGE doubled EVERY non-ASCII literal it carried, under EVERY attachment charset: a `SET` value (`SET V = 'éé'` stored 8 octets for the engine's 4), a literal in the SOURCE select, and - the sharpest - one in the `ON` or `WHEN .. AND` condition, where the doubled text simply stopped COMPARING EQUAL, so the row was silently not matched and the branch never fired at all. A hex literal and a column value were never affected, and the plain `UPDATE` twin of the same clause was always right: it never comes through here. It iterates characters now (`'` and `?` are ASCII, so the marker scan is unchanged).
    **What made it hard, and what fixed that:** nothing printed the desugared per-row statement, so four successive hypotheses (the constant fold, [stmt_text_decode], a carrier-specific path, [merge_subst]'s assembly) were each disproved only by measurement. `FC_SRV_TRACE` now prints each desugared branch statement WITH ITS BYTE LENGTH - a literal encoded once too often reads identically on a UTF-8 console and differs only in the count - and that one line located it immediately. The switch is read once per statement, not once per source row ([timed]'s lesson).
    Gates: `serve-real-castnone` 9 -> 18 (its literal half, the part it never had), `serve-real-merge` 30 -> 36; on `cc80ce2` the first fails 2 and the second 5. 345 wire unit tests; 42 adjacent gates over both code paths (`textcolcmp`'s failure in the batch run was collateral damage from the sweep's own cleanup killing a live server - re-run alone it is 359 / 0) (2026-09-16).
    **RECORDED, NOT FIXED - a THIRD double-encode, in the CONCAT itself:** a non-ASCII literal concatenated with a REAL-charset column under a byte-carrier attachment doubles the literal, with or without a cast around it. Measured: `SELECT 'é' || u` (u UTF8) under a NONE attachment is `OL 7 / CL 5` on the engine and `OL 9 / CL 6` here; the cast forms follow 7 -> 9. A NONE column agrees, an ASCII literal agrees, and every shape agrees under a UTF8 attachment. It fails on the previous binary too, so it predates this slice; `serve-real-castnone` carries the agreeing NONE-column cell and records this one beside it.
    **THE CONTRACT, MEASURED IN FULL (2026-09-16) - and an attempt that was WITHDRAWN.** A 42-cell matrix (both operand orders x NONE / UTF8 / WIN1252 attachments x 2- and 3-byte characters x ASCII and non-ASCII column values x the store paths) plus `SET SQLDA_DISPLAY ON` for every shape pins the engine's rule:
    * the RESULT SET follows [cs_join] - OCTETS absorbs, a real set beats the attachment sentinel, NONE is weakest - and the describe then RE-ANNOUNCES it in the attachment's charset, except OCTETS which keeps its own (measured: under NONE `'é' || <utf8>` is charset 4, `'é' || <win1252>` 53, `'é' || <octets>` 1; under UTF8 everything is 4 but OCTETS stays 1; under WIN1252 everything is 53 but OCTETS stays 1);
    * the VALUE is each operand MOVED INTO THE JOIN SET (CVT_move), carrier <-> real being a raw byte copy: under a NONE attachment `'é' || <utf8 'ab'>` is 3 characters / 4 octets, the literal's two CARRIER chars read as the one UTF-8 character they spell;
    * and under a REAL attachment THE FIRST OPERAND DECIDES, which the asymmetry proves: under WIN1252 `'é' || <utf8>` is 4 octets / 4 chars while `<utf8> || 'é'` is 6 / 4, and under UTF8 `'é' || <win1252>` is 4 / 3 against `<win1252> || 'é'`'s 3 / 3 - same characters, different octets, because OCTET_LENGTH reads the JOIN set while both orders announce the same charset.
    fire-crab gets the value wrong wherever a LITERAL meets an operand of another set: the eval arm joins two `String`s with a bare `format!`, and a `String` means different things per charset here (a byte carrier's octets ride one char per byte, a real set's are characters).
    **The attempt - a width-transparent `Expr::CsMove` wrapper inserted per operand by a shared `concat_expr`, with the sentinel resolved before the join - fixed all 42 cells and was reverted anyway**, because it broke three things a narrower view had hidden. Any future fix has to satisfy ALL of these at once: (1) a BYTE-CARRIER result counts each operand's BYTES, so `<UTF8 VARCHAR(32)> || <OCTETS VARCHAR(32)>` must announce 160 and not 64 - measuring the operands in the JOINED set instead of their own is what produced 64 (`serve-real-concatcs`, 18 cells); (2) one BINARY operand makes the whole result binary, subtype 0, not subtype 1 UTF8 (`serve-real-blobexpr`); (3) a LITERAL must keep carrying BOTH of its widths until emission - resolving the sentinel inside [text_form] collapses a decision [resolve_text_cs] makes later, and the unit test `a_literal_carries_both_of_its_widths` pins exactly that; (4) a COLUMN || COLUMN pair must NOT be re-decoded - applying CVT_move to every operand changed bytes that were already right, which is what took `serve-real-concatcs` from 0 to 18 DIFFs while the literal-bearing probes stayed green. The lesson for the next attempt: the value probes only covered literal-bearing shapes, so they could not see a column-only regression; run `concatcs` and `blobexpr` on EVERY iteration, not at the end.
  - **THE UPSTREAM `firebird-qa` SUITE NOW RUNS AGAINST BOTH SERVERS**, and its first finding is already fixed. FirebirdSQL/firebird-qa is the engine's own pytest suite - 1110 test modules driving real statements through `fbclient` - and it is a submodule of the paper repo now (`extern/firebird-qa`). One driver config names two servers, so the same tests run against the engine on 3050 and against fire-crab on its own port.
    **What it found immediately:** every test that creates a database failed in SETUP with *feature is not supported*. The plugin's `database_fixture` calls `set_async_write()` after each `create`, and its `drop()` calls `no_linger()` - both are `isc_action_svc_properties`, an action fire-crab's service manager had no framing for at all (`action 8 has no framing for tag 106`). The SPB table now carries PROPERTIES: `isc_spb_dbname` as a counted string, `isc_spb_prp_write_mode` as a BARE BYTE, and `isc_spb_options` as an int. The write mode drives [Database::apply_header_dpb] - the very function the DPB spelling of `gfix -write` uses, so both routes write the same header bit and refuse the same way (`isc_obj_in_use`, `isc_read_only_database`); measured against the engine, `-write async` clears the header's *force write* attribute and `-write sync` restores it. `isc_spb_prp_nolinger` answers success with no output: fire-crab keeps no linger state at all, and the engine's own `no_linger` on a database whose `RDB$LINGER` is already null is likewise a no-op.
    **Where that leaves the two servers** on the groups measured so far: `tests/functional/ltt` - engine 9 passed / 1 error, fire-crab 6 passed / 3 failed / 1 error, where the single error is the SAME upstream one on both sides (a test using a `ConfigManager` API newer than the released plugin) and the three failures are honest: that directory tests LOCAL TEMPORARY TABLEs. `tests/functional/basic` - engine 2 failed / 9 passed / 32 skipped / 27 deselected, fire-crab identical but for ONE extra error; both servers' two failures are a pytest-internal crash in `_pytest/assertion/truncate.py`, not a server difference.
    **RECORDED, from the suite:** `CREATE LOCAL TEMPORARY TABLE` is unimplemented - [plan_create_table] reads only `GLOBAL TEMPORARY` between CREATE and TABLE (relation types 4 and 5, `serve-real-gtt`), and LOCAL has no spelling there. And `serve-real-services` had been SELF-SKIPPING for want of a python driver (`SKIP ... set FCPY to a venv python`), which is why it reported nothing while it owned this very code path; run with `FCPY` pointing at a driver-bearing interpreter it is 15 OK and one DIFF - the `MON$` architecture query answering one all-NULL row - which is the same probe the QA plugin runs at startup to classify the server. **And that DIFF was the gate demanding a WRONG ANSWER.** Its line asserted `row == (None, None, None)` - a divergence recorded back when any `MON$` query answered one all-NULL row. The chunk that sent `MON$` down the ordinary planner path fixed it and nobody unrecorded the assertion, so a green-looking gate had been requiring the broken behaviour ever since. Re-measured with two driver connections on each server: the engine answers `(1, 'TCPv4', 0)` and so does fire-crab, and that is what the gate asserts now, with three new cells behind it (one `MON$ATTACHMENTS` row per live connection, each on TCPv4 under one server pid, ids matching what the connections report) - 19 OK. The ID FILTER is what makes the comparison legitimate at all: the engine's own server also holds INTERNAL attachments (measured: ids 5 and 6, same pid, NULL protocol) that no other server has, so an unfiltered `count(*)` answers 3 there against 1 here - a difference in what the two servers attach FOR, not in what they report.
  - **A GLOBAL TEMPORARY TABLE WAS A PERMANENT TABLE WEARING A TYPE CODE.** Everything fire-crab wrote about a GTT was right - the catalog row, the format, `RDB$RELATION_TYPE` 4 and 5, byte for byte against the engine - and `serve-real-gtt` checked all of it. What nothing checked was the ROWS, and the gate's own header said why: it recorded that they "live in per-connection temporary space at runtime, which is not part of what fire-crab writes". They were part of what it wrote. Measured against the live engine: `ON COMMIT DELETE ROWS` kept its rows through the commit that should have emptied them (2 where the engine says 0) and through a ROLLBACK (2 vs 0), and a FRESH ATTACHMENT read the previous one's rows in both kinds (2 and 2 vs 0 and 0) - a silent wrong answer on a shape this server already accepted, which outranks the LOCAL TEMPORARY TABLE gap beside it, since that one at least refuses honestly.
    **The engine's contract, measured in full** (one attachment unless said): in-transaction 2; after COMMIT, DELETE ROWS 0 and PRESERVE ROWS 2; after ROLLBACK 0; **after COMMIT RETAIN 2, for BOTH kinds**; a fresh attachment 0 and 0. GTT rows also honour SAVEPOINTS (`rollback to savepoint` takes 3 rows to 2) and a rollback undoes a DELETE as well as an INSERT. Concurrently (two driver connections): each attachment sees only its own rows, A's commit leaves B's untouched, and **A's DETACH does not remove B's PRESERVE rows**.
    **THE FIRST DESIGN WAS THE WRONG ONE AND THE MEASUREMENT KILLED IT.** A per-connection in-memory row store looked obvious - `Database` is per connection, and `RowSource::TableScan` already has the [mon_rows] seam for rows that come from somewhere other than pages. But GTT rows honour rollback and savepoints, so that store would have had to re-implement the undo windows and MVCC the pages already provide correctly. **What was missing was never the storage; it was the two moments the rows are supposed to die.** So the rows stay on the pages and the fix is two purges: at every non-retaining COMMIT for type 5, riding the committing transaction so its delete stubs become real with it, and at DETACH for both types, under a committed id of its own (the attachment's transaction has just been killed, and nothing after it would flush).
    **The two guards are not decoration.** The commit purge runs UNGUARDED because `collect_dml_targets` sees only what this attachment sees - another attachment's uncommitted rows are invisible to it, so the MVCC that already hides them is what makes it safe. The DETACH purge runs ONLY WHEN THIS IS THE LAST ATTACHMENT, because the engine keeps a second attachment's PRESERVE rows across the first's detach and fire-crab holds every attachment's rows in one shared file where the purge cannot tell whose are whose: with somebody else attached it does nothing, leaving the old wrong answer (rows outstaying their attachment) rather than creating a new one (deleting a live session's data).
    **RECORDED, NOT FIXED, and NOT this slice's doing:** through ISQL a `COMMIT RETAIN` over a DELETE ROWS GTT leaves the engine 2 rows and fire-crab 0. The wire contract is not what differs - driven through firebird-driver every form agrees (`commit(retaining=True)` 2, a following plain commit 0, a plain commit after an insert 0, the DSQL text `COMMIT RETAIN` 2). What differs is that isql sends fire-crab an EXTRA plain `op_commit` (op 30) straight after the retaining statement, and that commit legitimately empties the table. The op sequence is byte-for-byte identical on the previous binary, so the purge only made it visible; what isql is answered about a DSQL COMMIT RETAIN is its own chunk. **CORRECTION, measured after that entry was written:** the trigger is NOT the ROLLBACK that happened to precede the retain in the first probe - it is ANY EARLIER TRANSACTION END IN THE SAME isql SESSION. Bisected, each a fresh session ending in `COMMIT RETAIN` then a count, engine 2 throughout: `INSERT; INSERT; COMMIT RETAIN` **agrees** (fc 2); `INSERT; COMMIT; INSERT; INSERT; COMMIT RETAIN` diverges (fc 0); the same with `ROLLBACK` in place of that COMMIT diverges; the full original diverges. So one prior commit is enough and the rollback was incidental. NOT the cause either: the statement TYPE, which is identical on both servers through the driver - `COMMIT` 10, `COMMIT RETAIN` 10, `ROLLBACK RETAIN` 11 - so isql is not dispatching on it. What IS measurably different is the size of the `op_response` chunks each server writes (the engine's 32 bytes against fire-crab's 24, seen through a relay), but that holds for every response in the session including the agreeing one, so it is a lead and not the cause. Settling it needs a relay that FRAMES the protocol rather than sampling chunk boundaries - the first step of that chunk. **AND THE SERVER'S OWN HANDLING IS NOT AT FAULT**, which narrows that chunk to one question. Replayed through firebird-driver with no isql anywhere, the diverging sequence AGREES: engine 2, fire-crab 2, and the ops fire-crab receives after the retaining statement are `63, 67, 62, 68` - **no op 30 among them**. isql sends that extra commit; the driver never does, and fire-crab answers every op it is actually sent correctly. What remains to find is what fire-crab REPLIES to the executed COMMIT RETAIN that makes isql follow up with a commit the engine is not sent.
  - **A GATE HAD BEEN DEMANDING A REFUSAL THE ENGINE DOES NOT MAKE - on every run, on every binary.** `serve-real-nofallback` reported 63 OK / 1 DIFF for `SELECT COUNT(*) FROM T UNION ALL SELECT ID FROM U2` "answered [3 1] instead of raising". Measured against the live engine: the ENGINE answers `[3 1]` too, and so do the neighbouring shapes (UNION distinct, `SUM`, a grouped `COUNT`, a literal branch) - fire-crab has been right about all of them for a long time. The line dated from when unions were a fixed-answer-fallback hazard, and when fire-crab learned to answer them nobody unrecorded it.
    **WHY IT COULD GO STALE AT ALL, which is the part worth fixing:** the gate's `refuses` helper is ONE-SIDED. It asks fire-crab alone whether a statement raises and never checks that the engine raises too, so a shape this server later learns to answer CORRECTLY reads as a failure for ever. The gate already had the right helper beside it - `both_refuse`, which asserts the ENGINE's refusal first and fails the case if the engine accepts - and eleven cases were simply not using it. All THIRTEEN old `refuses` lines were measured against the engine on the gate's own fixture: eleven raise on both (converted to `both_refuse`, so they can never go stale silently again), one answers on both (converted to `answers`), one is a real gap. 63 OK / 1 DIFF -> **64 OK / 0 DIFF, on BOTH binaries** - this is a gate-only correction and proves no behaviour change, which is exactly why the old binary must be clean here rather than red.
    **THE REAL GAP IT LEFT BEHIND, kept as a refusal with the engine's own answers recorded so it carries an expiry date: A SELECTABLE PROCEDURE IS NOT A COMPOSABLE ROW SOURCE.** fire-crab answers the bare call and a `FIRST` over it and raises on everything else the engine answers - `WHERE` (engine [3 4 5]), `ORDER BY` ([5 4 3 2 1]), `COUNT(*)` ([5]), `GROUP BY` (5 rows), a JOIN against a table (3 rows), and the same call inside a derived table. Every one is a law-safe refusal rather than a wrong answer, which is why they stay refused; the shape of the fix is that `RowSource` has no procedure leaf at all, so `Plan::ProcSelect` (deferred, replaced by `Plan::ProcRows` at op_execute) can only ever be the whole statement instead of a leaf the ordinary Filter/Sort/Aggregate/Join nodes compose over.
  - **AN ALIAS ON A SELECTABLE PROCEDURE CALL WAS REFUSED OUTRIGHT** - and the describe gate that owns this surface never asked. `SELECT K FROM GEN(5) G`, `SELECT G.K FROM GEN(5) G`, `SELECT K FROM GEN(5) AS G` and `SELECT K FROM ONEROW G` all answer on the engine and every one raised here, while the UNALIASED spellings had always agreed - which is why nothing noticed.
    **THE CAUSE IS ONE LINE:** [split_proc_call] took the call's arguments with `rest.strip_suffix(')')`, so the moment an alias followed the closing paren the whole item stopped being a procedure call at all. It then fell through to relation handling, which went looking for a table literally named `GEN(5) G`; the paren-less spelling failed the same way through [proc_call_name], which refuses any trailing token. Now the ARGUMENTS end at the MATCHING paren ([matching_paren], which also skips string literals - `PSEL('a(b') G` is unit-tested) and the alias is whatever follows it, with `AS` optional and the name canonicalised, so an unquoted alias folds and `"g"` keeps its case.
    **THE CONTRACT, measured rather than assumed - and item 25 is the point.** The engine's describe for an aliased call is `[R, R, PR, P, SYSDBA, PUBLIC]` and `[K, K, PGEN, G, SYSDBA, PUBLIC]`: the RELATION stays the procedure, and the BINDING ALIAS (item 25) becomes the user's alias, empty when there is none - the table rule exactly. `proc_out_col` hardcoded that field to `None`, and rather than read the number once and bake it in, the cells were added to `serve-real-describe`, whose `both` helper takes the engine's answer as the expectation verbatim and will keep doing so.
    **QUALIFIED COLUMNS follow the ordinary rule too: AN ALIAS HIDES THE NAME.** `SELECT GEN.K FROM GEN(5) G` refuses exactly as `SELECT T.ID FROM T TT` does, while the unaliased `SELECT GEN.K FROM GEN(5)` answers. The engine spells the refusal -206 `Column unknown "GEN"."K"`; **this server has no column-unknown vector for ANY relation** - measured, the aliased-TABLE case answers a generic `42000 Dynamic SQL Error` here too - so the generic refusal is the bar, and minting a -206 for procedures alone would have been a fabricated vector rather than a fix.
    `serve-real-describe` 110 -> 116 checks (a parameterised procedure joins the fixture so the parenthesised spelling is covered at all); on the previous binary it is 111 OK / 5 DIFF. 161 ods + 346 wire unit tests (a new one pins every alias spelling, including the derived table that must still NOT be a call). Adjacent by code path, all green: qualname 118, modifiers 66, nofallback 64, subqalias 32, callproc 15, exeproc 14, cursors 14, createproc 10, callpkg 9, ctxdefault 9, fncall 7, execblock 5, crosstype 3, procfmt 1, altfmt 1.
    **A THIRD STALE ASSERTION FELL OUT OF THE SWEEP.** `serve-real-nosuspend`'s E7 pinned "the engine's EXECUTE PROCEDURE on an empty body succeeds silently, fire-crab refuses" and had been reporting a DIFF on every run, on every binary, because **both servers now answer nothing at all** - the boundary closed and nobody unrecorded it. Its own comment had promised exactly this ("asserted so that implementing it says so"); it said so. Converted to a strict agreement cell - BOTH sides must be silent, not merely equal, since equal-and-both-broken is the vacuity that keeps biting here - and it now reads 82 OK / 0 DIFF on BOTH binaries, which is what a gate-only correction should look like.
    **STILL REFUSED, and now recorded with the engine's own answers:** a WHERE, ORDER BY, GROUP BY, COUNT(*), a JOIN or a derived table over a procedure call. The seam is known - [plan_over_source] already builds exactly that outer query over a bound row source for a VIEW, so `BoundSrc::Inner(Plan::ProcSelect)` would give the clauses for free - but `RowSource::PlanRows` resolves at FETCH with only a `&Database`, where running a body needs `&mut Option<Database>` and a `SessionCtx`, so the deferred leaf must be substituted at op_execute first, generalising the two bespoke arms that already do it for the bare call and for a modifier over one.
  - **A SELECTABLE PROCEDURE IS A ROW SOURCE NOW, not a whole statement.** `WHERE`, `ORDER BY`, `GROUP BY`, `HAVING`, an aggregate and a derived table over a procedure call all refused here and all answer on the engine; every one of them agrees now. Measured: `SELECT K FROM GEN(5) WHERE K > 2` [3 4 5], `ORDER BY K DESC` [5 4 3 2 1], `COUNT(*)` [5], `MIN/MAX` [1 5], `SUM` through an alias [15], `GROUP BY` 5 rows, `HAVING K > 3` 2 rows, `FIRST 2 ... WHERE K > 1` [2 3], and the same call inside a derived table [3 4 5].
    **NEITHER HALF INVENTED ANY MACHINERY.** The planner hands the statement to [plan_over_source] with the call as `BoundSrc::Inner(Plan::ProcSelect)` - the same call a VIEW makes two branches below - so the projection, the filter, the grouping, the HAVING and the ordering are the ones every bound source already gets. The BINDING KEY is the alias when the call carries one, else the procedure's bare name, and [ColBinding]'s rule that *an alias is exclusive* then delivers the engine's own behaviour for free: `GEN.K FROM GEN(5) G` refuses exactly as `T.ID FROM T TT` does, with no procedure-specific code.
    **The second half is why the first could not stand alone.** `Plan::ProcSelect` is DEFERRED - the body must run before anything above it has rows - and the fetch path holds only a `&Database` where running a body needs `&mut Option<Database>` and a [SessionCtx], because the body may WRITE. That is the constraint [materialise_laterals]' own doc states for LATERAL, one step further out. So [materialise_procedures] walks the plan at op_execute and substitutes every `ProcSelect` leaf with its `ProcRows`, IN PLACE - a returning walk would rebuild `JoinGroup`'s thirteen fields and `Join`'s seven by hand at every level - reaching inner plans (`Derived`, `Modified`, `Returning`, `Union`) and the `RowSource` bases of `Join`/`JoinGroup`/`Lateral` alike, since a grouped read over a procedure hides one below the plan tree entirely. A cheap [plan_has_procselect] guards it, so an ordinary statement never pays for the clone.
    **TWO CORRECTIONS DURING THE WORK, both caught by measurement rather than review.** The routing condition first copied only HALF of `plan_over_source`'s own rule - it keyed on the clauses and forgot that `grouped` is "a GROUP BY **or** an aggregate item" - so `SELECT COUNT(*) FROM GEN(5)` still refused while the grouped form already agreed; the trace showed `where=None group=None having=None order=None` followed by `prepare refused`, which named the cause in one run. And the gate comment I wrote claiming "every shape listed as refusing then agrees now" was FALSE for the JOIN, which I had dropped from the converted cells without re-measuring - corrected, and the join re-recorded with its measured engine answers rather than quietly removed.
    **RECORDED, NOT FIXED - and verified IDENTICAL on the previous binary, so none of it is this slice's doing:** (1) a procedure JOINED to a table still refuses in all three spellings (`GEN(3) G JOIN T`, `T JOIN GEN(3) G`, and with a WHERE) where the engine answers - the bound-source route hands a join to `plan_join_bound`, which this slice did not follow through; (2) **THE BODY IS STILL RUN EAGERLY**, which costs two answers the engine gives: a raising body loses the rows BEFORE the raise (engine `1 2` then the exception, fire-crab the exception alone), and `FIRST n` does not STOP the body, so `SELECT FIRST 2 K FROM PRAISE` answers `1 2` with no error there and raises here. That is the laziness `RowSource::for_each`'s doc already names - "a raiser three rows in delivers the two rows before it, and `FIRST 1` stops the scan" - and it is now a measured backlog item rather than a suspicion.
    `serve-real-nofallback` 64 -> 71 checks, six of them failing on the previous binary (65 OK / 6 DIFF there) and one a fresh recorded refusal for the join; the stale `refuses` line this chunk's own predecessor left behind is converted, since a `refuses` cell only ever asked THIS server while `answers` compares the values against the engine. 161 ods + 346 wire unit tests. Adjacent by code path, twenty gates, all green: describe 116, qualname 118, nosuspend 82, modifiers 66, viewjoin 36, cte 35, subqalias 32, corrsub 27, joinderived 25, callproc 15, exeproc 14, cursors 14, createproc 10, callpkg 9, ctxdefault 9, fncall 7, execblock 5, crosstype 3, altfmt 1, procfmt 1.
  - **`EXECUTE PROCEDURE` ON A BODY THAT SUSPENDS THREW AWAY EVERYTHING THE BODY HAD WRITTEN.** One INSERT before a SUSPEND: the engine keeps the row, this server kept nothing. The same body under `SELECT` kept it on BOTH - which is exactly why it hid for so long.
    **THE CAUSE IS ONE CONDITION.** [run_body_source] wraps a body in a nested undo window - "a statement that fails partway undoes everything the body wrote before it" - and decided with `outcome.is_err()`. But `PsqlStop::Exit` rides the ERROR CHANNEL because it STOPS the body, not because anything failed; the match two lines below says so itself, treating `Ok(()) | Err(PsqlStop::Exit)` alike as "what the body left behind IS the answer". `EXECUTE PROCEDURE` is the one path that sets `stop_at_suspend`, SUSPEND returns exactly `Err(PsqlStop::Exit)` there, and so every such call undid the body's work. The fix unwinds on `Ok` or `Exit` and undoes on everything else.
    **A WRITE AFTER THE FIRST SUSPEND IS A DIFFERENT RULE AND IS UNTOUCHED:** the engine drops it under `EXECUTE PROCEDURE` and performs it under `SELECT` (already recorded on [run_procedure]), and both servers agreed before and after. Measured across all six shapes - write before / after / both x EXECUTE PROCEDURE / SELECT - every one agrees now, and the three SELECT forms are bit-for-bit what they were.
    **WHY NO GATE SAW IT, which is the part worth keeping:** the only cells that assert a LANDED WRITE are `serve-real-nosuspend`'s E5/E6, and their procedure does not suspend; `serve-real-exeproc`'s suspending procedures are all read-only `FOR SELECT ... DO SUSPEND`. The defect needed a SUSPENDING WRITER and nothing in the suite had one. E8 does now, with a positive control on the engine's side so "neither wrote anything" cannot read as agreement. The gate's own check-counter then caught the addition (82 -> 83) - working exactly as designed.
    **HOW IT WAS FOUND: by widening a probe for a different chunk.** Scoping the eager-body work, a side-effect probe (a procedure that logs one row per iteration, so the log count IS the number of iterations) showed `EXECUTE PROCEDURE` running the body ZERO times where the engine runs it once. A lost write outranks the missed optimisation that probe was written for, so it went first.
    **AND A FOURTH STALE ASSERTION, this one MY OWN doing:** `serve-real-psql` demanded that a WHERE over a procedure call REFUSE - written when the call could not be a row source, to stop the filter being silently ignored. The previous chunk made it answer, and that gate was NOT in the adjacent set I picked for it, so the DIFF shipped uncaught for one commit. It is converted and now TWO-SIDED (`same`, which compares the rows against the engine) rather than asking this server alone whether it refused - the one-sidedness being what let it rot. Measured: [3 4], [], [4 3 2], [3 9 4 16] on both.
    **STILL RECORDED, NOT FIXED - the chunk this one interrupted, now with its full contract measured by side effect** (log rows = iterations the body actually ran): bare 5, `FIRST 1` -> 1, `FIRST 2` -> 2, `FIRST 99` -> 5, `WHERE` -> 5 (a filter does NOT stop the body), `FIRST 1 + WHERE` -> 4 (it stops after the nth SURVIVING row), `ORDER BY` / `COUNT(*)` / `DISTINCT` -> 5 (blocking), a derived table + `FIRST 1` -> 1 (the limit propagates through it), `EXECUTE PROCEDURE` -> 1. fire-crab runs to completion in every case, so `SELECT FIRST 2 K FROM PRAISE` raises where the engine answers two rows and never raises at all.
    `serve-real-nosuspend` 82 -> 83 checks (previous binary 82 OK / 1 DIFF), `serve-real-psql` 65 -> 68 (previous binary 64 OK / 4 DIFF). 161 ods + 346 wire unit tests. Adjacent by code path, all green: describe 116, modifiers 66, rollback 31, savepointtx 30, autonomous 29, trigdb 24, exceptions 19, undo 16, callproc 15, exeproc 14, cursors 14, createproc 10, ctxdefault 9, events 7, execblock 5, execstmt 4, trigreraise 4.
  - **A SELECTABLE PROCEDURE'S BODY IS PULLED, NOT RUN AND THEN SLICED.** The engine stops a body as soon as the consumer stops asking; fire-crab ran every body to completion and sliced afterwards. That is invisible for a pure body and TWO WRONG ANSWERS for one that is not: a body with a SIDE EFFECT performed it too many times, and **a body that RAISES past the limit raised where the engine never reaches the raise at all** - `SELECT FIRST 2 K FROM PRAISE` answers two rows and no error there, and raised here.
    **ROWS CANNOT SEE THIS, so the contract was measured by SIDE EFFECT:** `FIRST 2` over a run-out body and over a stopped one return the same two rows. The probe procedure logs one row per iteration, so the log count IS the number of times the body ran. Measured against the live engine: bare 5, `FIRST 1` 1, `FIRST 2` 2, `FIRST 99` 5, `FIRST 1 SKIP 1` 2, `ROWS 2` 2, `ROWS 2 TO 3` 3, `SKIP 2` 5, a limit through a DERIVED TABLE 1, `WHERE` 5, `ORDER BY` 5, `FIRST 2 ... ORDER BY` 5, `COUNT(*)` 5, `DISTINCT` 5, `EXECUTE PROCEDURE` 1.
    **THE BLOCKING HALF MATTERS AS MUCH AS THE STOPPING HALF** and is asserted with it: a sort, an aggregate or a DISTINCT consumes the WHOLE body on the engine, so a cap applied there would be a WRONG ANSWER rather than a saving. `FIRST 2 ... ORDER BY` is five iterations on both.
    **HOW IT IS EXPRESSED, and EXECUTE PROCEDURE turned out to be the same rule:** `PsqlFrame::stop_at_suspend: bool` becomes `stop_after: Option<usize>`, checked where SUSPEND already pushed its row - EXECUTE PROCEDURE is simply `Some(1)`, which is what it always meant. The cap is EARNED ON THE WAY DOWN the plan walk rather than assumed: a bounded non-DISTINCT `Modified` contributes `skip + take` (the same sum [stream_first_n]'s caller computes), a `Derived` passes it through ONLY when it is transparent - no filter, no ORDER BY, no window - and a union, a join, a grouped fold or a `Returning` passes none. The BLR path is SKIPPED when a cap is set: `try_procedure_blr` takes only `first_only` and `bind_and_execute_mode` only `halt_at_stall`, so running it with a cap would silently ignore the limit - the very thing being fixed.
    **RECORDED, NOT FIXED, both measured in the new gate:** (1) a limit WITH a filter counts SURVIVING rows, which this cap cannot express - `FIRST 1 ... WHERE K > 3` is four iterations on the engine and five here, same rows, different side effect; it needs the filter evaluated inside the body's pull. (2) ROWS BEFORE A RAISE: when the raise IS reached the engine delivers what it already produced and THEN raises (`SELECT K FROM PRAISE` is `1 2` then the exception, and `FIRST 3` likewise) where fire-crab raises with no rows - that needs a "these rows, then this error" carrier no plan shape has yet.
    `qa/serve-real-proclazy.sh` is NEW, 19 checks, with the protections this suite keeps earning the hard way: a POSITIVE CONTROL that the ENGINE itself runs an unbounded body five times (or every cell would compare zero with zero), a per-cell empty guard, the value printed on success, and **the exec bit set deliberately** - a gate at 0664 gives a sweep `rc=126` and measures nothing. Against the immediately preceding state the six stopping cells and the three raising-body cells fail and the rest pass; against `fcwire-pre` (2bd44e3) fourteen fail, the extra five belonging to the row-source and EXECUTE-PROCEDURE chunks before it.
    **AND THE FIXTURE TAUGHT ITS OWN LESSON:** the first cut created the scratch databases OVER TCP, so they belonged to the ENGINE's user and the gate died at `chmod: Operation not permitted` - a database this gate cannot chmod is one fire-crab cannot write. Created EMBEDDED and chmodded after, as every other gate here does. The transport rule is about where the DIFFERENTIAL'S QUERIES go, not where the fixture is built.
    161 ods + 346 wire unit tests. Adjacent by code path - everything that RUNS A BODY, not merely everything named "procedure" - nineteen gates, all green: describe 116, nosuspend 83, nofallback 71, psql 68, modifiers 66, autonomous 29, trigdb 24, exceptions 19, callproc 15, exeproc 14, cursors 14, createproc 10, callpkg 9, ctxdefault 9, events 7, execblock 5, execstmt 4, trigreraise 4, procfmt 1.
  - **A BODY THAT RAISES AFTER SUSPENDING ROWS NOW DELIVERS THEM FIRST.** The engine gives the client what the body already produced and raises AFTER it - `SELECT K FROM PR_PURE` is `1 2` followed by the exception - where this server raised with NO ROWS AT ALL. Lost rows, which is why it outranked the two other items the pull chunk recorded.
    **MEASURED FIRST, AND IT NARROWED THE WORK:** the body's WRITES are rolled back on a raise on BOTH servers (a writing, suspending, raising body leaves an empty log either side), and a limit that stops the body before the raise answers cleanly on both. So the undo window - changed two chunks earlier - must NOT be touched here; only the already-suspended rows had to survive.
    **NO NEW PLAN SHAPE WAS NEEDED, because [EmitErr]'s own doc already described the answer:** "the good rows already streamed are followed by the engine's error op_response, in place of the terminator". So `Plan::ProcRows` gained a trailing `Option<EvalErr>`, its [emit_rows_inner] arm encodes the rows and then returns `Err(EmitErr::Eval(e))`, and [emit_rows_uncounted] writes the error after them - the same path a per-row arithmetic exception mid-cursor has always taken. `ProcErr` gained the rows themselves: [run_body_source]'s raise arm had `frame.suspended` in scope and was discarding it.
    **TWO PLACES SWALLOWED THE RAISE, and only one was obvious.** [branch_rows_res] answers `Ok(rows)` for a `ProcRows` and has nowhere to put an error, so a `Modified` over one - `FIRST 3` past a two-row raising body - delivered both rows and no error; its arm now consults the inner for a trailing raise. The second was invisible from reading: the fetch FLATTENS any non-`Plan::Rows` plan through `branch_rows` into `Plan::Rows`, which dropped the trailing error and never reached [emit_rows] at all. Guarded by [plan_has_trailing_raise]. `FIRST 3` had worked all along only because it takes the `small_first` route and skips that flattening.
    **THE TRACE IS WHAT FOUND IT, after reading lost three times in a row** (I blamed the statement cache, then the scrollable cursor, then the arm order - each wrong): `materialise_procedures: substituted=true -> ProcRows[2,raise]`, `fetch plan = ProcRows[2,raise]`, and then NO `emit ProcRows` line at all - which said in one run that emit_rows was never reached. All three traces are kept, `plan_kind` included: it answered `other` for the one shape this whole chunk revolves around, and that is exactly what made me mis-read the run before.
    `serve-real-proclazy` 19 -> 22 checks (the recorded "rows before a raise" note is now three cells, one of which pins the rows AND the rolled-back writes together); 22 OK / 0 DIFF here, 5 OK / 17 DIFF on `fcwire-pre` with all three new cells failing. 161 ods + 346 wire unit tests. Adjacent by code path - everything that EMITS ROWS and everything that RUNS A BODY - thirty gates, all green: qualname 118, describe 116, nosuspend 83, nofallback 71, psql 68, modifiers 66, viewjoin 36, cte 35, subqalias 32, rollback 31, savepointtx 30, autonomous 29, corrsub 27, joinderived 25, trigdb 24, exceptions 19, gtt 17, undo 16, callproc 15, exeproc 14, cursors 14, createproc 10, callpkg 9, ctxdefault 9, retain 8, events 7, execblock 5, execstmt 4, trigreraise 4, procfmt 1.
    **STILL RECORDED:** a limit WITH a filter counts SURVIVING rows (`FIRST 1 ... WHERE K > 3` is four iterations on the engine, five here; identical rows, different side-effect count) - it needs the filter evaluated inside the body's pull.
  - **A PROCEDURE CALL IN A JOIN OR A SUBQUERY: ONE REFUSAL IS THE ROOT OF ALL FOUR SHAPES - measured, NOT implemented.** `FROM GEN(3) G JOIN T ...`, `T JOIN GEN(3) G ...`, a comma join, a LEFT JOIN onto a call, `EXISTS (SELECT 1 FROM GEN(3))`, `IN (SELECT K FROM GEN(3))` and `(SELECT MAX(K) FROM GEN(3))` all refuse here and all answer on the engine ([1 x 2 y 3 <null>], [x 1 y 2 <null> 3], [1], [1 2 3], [3]).
    **THE ROOT IS ONE DELIBERATE `None`.** [parse_table_ref] refuses a procedure call on purpose - its own comment says `FROM PUBLIC.PW6(5)` must answer None "so `split_proc_call` stays the only handler ... fire-crab must not assume a dotted FROM item is a table", and two unit tests pin it. [parse_from] therefore answers None for any FROM holding a call, so a join never reaches [plan_join_bound] at all (the trace shows no `bound row source` line for any join spelling), and [corr_scan_span] - which does `parse_from(table_s)?` then `look(tr)?` per side - declines every subquery shape for the same reason. The scalar subquery is the one that ROUTES and then refuses, in the select-list fold's `corr_scan`/`corr_register`.
    **THE EXECUTION HALF IS ALREADY THERE, which is what makes this tempting:** [plan_join_bound] builds a side from a PLAN for both derived tables and views (`src: RowSource::PlanRows`, columns from [derived_view], per-column `rels`/`fnames`), [scope_rel_of] has the mirror branch for a subquery scope, and [materialise_procedures] already walks a `Join`/`JoinGroup` base through `RowSource::PlanRows`. A procedure side needs `probe_src: None` and `flatten: None`, which the view side and the `join_sides()` test helper both already use.
    **AND THE OBVIOUS FIX IS WRONG - recorded so the next attempt does not spend the time twice.** Rewriting the item into a derived table (`FROM (SELECT * FROM GEN(3)) G`, the [splice_ctes] trick) would satisfy every path, because a derived side is supported everywhere. But a derived table MUST bind an alias, and the engine's describe for an UNALIASED call in a join is relation `GEN` with binding alias `""` - measured, beside `GEN`/`G` for the aliased form and `GEN`/`DT` for a real derived table. The rewrite would answer `GEN` where the engine answers `""`: a silent describe corruption of exactly the kind `TableRef`'s doc warns about, which no row-comparison gate would catch. A procedure side must be a FIRST-CLASS side, whose `rel_alias` is `None` when unaliased.
    **WHY THE PARSER WAS NOT SIMPLY WIDENED:** `parse_from` has **57 call sites** and `parse_table_ref` **21**. Admitting calls there hands procedure items to every one - the qualifier guard, `splice_ctes`, the view DML path, `corr_scan_span`, the describe paths - each of which reads `None` as "not a table" today. That is the shape of change this file has already withdrawn once (the concat `CsMove` wrapper: 42 cells green, four families broken). The entry point has to be a narrow, procedure-aware pre-pass, and that is a slice of its own rather than a tail-end addition.
    Recorded in `serve-real-nofallback` as four refusals carrying the engine's own answers, so they keep an expiry date.
    `serve-real-gtt` 11 -> 17 checks; on the previous commit 13 OK / 4 DIFF. 161 ods + 345 wire unit tests; adjacent by code path all green - retain 8, rollback 31, savepointtx 30, undo 16, autonomous 29, ddltx 32, gendurable 263, trigreraise 4, gencomp 22, trigdb 24, concurrency 20, insert 11, query 10 (`nofallback` 63 OK / 1 DIFF on a UNION ALL shape, IDENTICAL on both binaries - pre-existing).
  - **A LIMIT *WITH* A FILTER STOPS AT THE nTH SURVIVING ROW** - the last item the pull chunk recorded and did not fix. `SELECT FIRST 1 K FROM PLOG(5) WHERE K > 3` runs the body FOUR times on the engine and ran it five here; `WHERE K > 0` runs it ONCE and ran it five. The ROWS were always right - only the side-effect count was wrong, which is precisely what this fixture exists to see and why it could sit recorded for a commit rather than be a lost answer.
    **A SURVIVOR LIMIT IS A THIRD ANSWER, not either half of the two already implemented.** A filter alone does not stop a body (five iterations) and a limit alone stops it at the nth SUSPENDED row; together they stop it at the nth row that PASSES. Measured against the live engine: `FIRST 1 WHERE K > 3` 4, `FIRST 1 WHERE K > 0` 1, `FIRST 2 WHERE K > 2` 4, `FIRST 3 WHERE K > 1` 4, `ROWS 2 WHERE K > 2` 4, `FIRST 1 SKIP 1 WHERE K > 1` 3 (so the bound is `skip + take` SURVIVORS, the same sum the unfiltered cap uses), and a filtered limit through a DERIVED TABLE 3.
    **THE CAP GREW A PREDICATE RATHER THAN THE WALKER GROWING A SPECIAL CASE.** `PsqlFrame::stop_after` becomes a `BodyCap { take, keep: Option<Predicate> }`, and the SUSPEND site asks *does the consumer keep this row?* BEFORE the push, counting the cap down only when it does - `keep: None` is the unfiltered limit and `EXECUTE PROCEDURE`'s own rule, both unchanged. The `Derived` arm of [materialise_procedures] therefore stops SWALLOWING a cap when a filter is present and instead binds that filter ([bind_filter_eval], with the statement's arguments now threaded into the walk because a consumer's WHERE may carry `?`) and sends it down with the cap.
    **WHAT STILL BLOCKS, and it is the same list:** an ORDER BY or a window above the filter cancels the cap outright, because the engine runs the whole body under `FIRST 1 ... WHERE K > 1 ORDER BY K` (five iterations, measured, and a cell) - there a cap would change the ANSWER rather than the cost. And a filter NOTHING passes must run the body out, five iterations, which is the cap simply never being reached; both are cells that pass on BOTH binaries, which is what makes them controls rather than passengers.
    **TWO PLACES WHERE A SHORTCUT WOULD HAVE BEEN A WRONG ANSWER.** (1) A filter that RAISES is the engine's own raise and must travel: the SUSPEND site maps it to `PsqlStop::Raise(Thrown::Runtime{..})` rather than treating the row as not-matching, since swallowing it would silently skip exactly the row the engine raises on - [Predicate::matches]' own contract. (2) The predicate speaks the procedure's OWN OUTPUT POSITIONS, which is what a suspended row carries only when the call's `picks` is the identity; under a narrowed `picks` the cap is dropped WHOLE rather than kept without its predicate, because counting every row under a survivor limit would stop the body EARLY - a wrong answer - where running it out is merely the cost this chunk saves.
    **LANDED AS TWO VERIFIED HALVES, as the pull chunk was.** First the type change alone, every caller passing today's value, with the whole iteration table re-run to prove it inert; then the filter wired on top. `serve-real-proclazy` 22 -> 31 checks and its "RECORDED, NOT FIXED" note is now nine cells; on the previous commit's binary SEVEN of them fail (each with the right rows and the iteration count stuck at 5) and the two controls pass, which is the shape a real measurement leaves. 161 ods + 346 wire unit tests. Adjacent by code path - everything that RUNS A BODY THAT SUSPENDS and everything that PLANS OVER ONE - nineteen gates, all green: describe 116, nosuspend 83, nofallback 74, psql 68, derived 61, returning 44, autonomous 29, callproc 15, psqlerrors 15, exeproc 14, cursors 14, procdescribe 13, ovlazy 20, procdefault 9, selectinto 8, reraise 7, execblock 5, psqlbody 5, procfmt 1.
  - **AN INPUT PARAMETER IS DESCRIBED IN THE ATTACHMENT'S CHARACTER SET** - the output half of this law has been implemented since 2026-08-21 ([resolve_text_cs]), and the input half was never wired at all. Every `?` was announced in its DESTINATION's charset whatever the client had attached as, which is a describe a client sizes its buffers from.
    **RE-MEASURED RATHER THAN TRUSTED, and the stored contract was wrong in one cell.** The backlog carried this with a note that fire-crab "agrees on all eight cells under a NONE attachment"; probing eleven input slots under three attachments found a NONE-attachment divergence the record never mentioned (below). Measured: a `?` into `VARCHAR(5) WIN1252` under a UTF8 attachment is len 20 charset 4, into `VARCHAR(5) UTF8` under WIN1252 len 5 charset 53, a `CHAR(5) UTF8` the same; the CHARACTER count is what survives, not the byte width. The BYTE CARRIERS are the exception - a NONE or OCTETS destination is never transliterated on any attachment - **but ASCII IS**, which is why this could not be written with `intl::byte_carrier`, whose own definition counts ASCII as a carrier; `resolve_text_cs`'s rule is `col_cs > 1`, and reusing that function rather than re-deriving its rule is the whole implementation.
    **A CAST SLOT CANNOT CARRY A BARE ZERO, which is what made this more than threading a parameter.** `CAST(? AS VARCHAR(5))` described as CHARACTER SET NONE where the engine announces the ATTACHMENT's set - and a flat `0` is indistinguishable from a genuine NONE COLUMN, which the engine does NOT transliterate. So [cast_text_bytes] now emits the SENTINELS with a CHARACTER width (`ATT_SUBTYPE` for "whatever the attachment is", `enc_real_cs(c)` for "this set, unless an attachment overrides it") and `resolve_text_cs` resolves them - which also reproduces the engine overriding an EXPLICIT `CAST(? AS VARCHAR(5) CHARACTER SET UTF8)` to 5 bytes charset 53 under WIN1252.
    **THERE ARE TWO BIND-VAR EMITTERS AND FIXING ONE WOULD HAVE LEFT THE OTHER WRONG:** [append_bind_section] builds the info-items describe and [answer_prepare] builds op_prepare's own bind vars, both straight from [wire_for]. The roadmap already named them as a pair; both now resolve through the same [bind_text_cs], the mirror of the `out_vars` resolution sitting three lines above it in `answer_prepare`.
    **THE WIDTH CHANGE WAS ONLY SAFE BECAUSE NOTHING ELSE READS IT,** and that was checked before it was written rather than after: a prepared parameter's descriptor reaches the describe and three arity comparisons and nothing else - the value-fitting trio ([encode_wire_value], [wire_value_fit_error], [text_bytes_for]) all take COLUMN descriptors from table formats, and a bound value for a CAST is fitted from the CAST target itself. A character count standing where a byte count stood would otherwise have moved a truncation boundary silently, which is the class of regression this file has withdrawn a chunk over.
    **RECORDED, NOT FIXED, measured in the new gate:** a `?` TYPED BY A LITERAL is typed from the literal - `WHERE ? = 'x'` is 452 TEXT at the literal's own character count in the attachment's charset (len 4 charset 4 under UTF8, len 1 charset 53 under WIN1252) where fire-crab announces 448 VARYING len 32763 charset 0. It diverges under a NONE attachment too, so it is a SLOT-TYPING root of its own rather than part of this one, and it is the cell the stored contract had missed.
    `qa/serve-real-bindcs.sh` is NEW, 34 checks over three attachments, with a positive control that asserts the ENGINE ITSELF moves a slot when the attachment changes (every cell compares two servers, so a probe describing nothing would make both agree on empty). On the previous commit's binary it is 23 OK / 11 DIFF - and the 23 that pass there are exactly the NONE-attachment cells and the byte carriers, which is what a control is supposed to look like. 161 ods + 346 wire unit tests. Adjacent by code path - the describe and prepare surface, which this touches in its entirety - twenty-six gates, all green: bindconv 198, describe 116, paramshapes 114, litcs 78, slottype 73, subqdesc 70, viewdml 61, concatcs 44, charset 43, cscast 42, octets 36, outblr 32, notnulldesc 27, mergeparam 26, quotedname 26, dmlparamexpr 25, textfit 24, blobcast 21, params 19, casttext 18, castnone 18, castparam 16, castparamtext 9, charform 8, charsetddl 7, coltypes 7.
  - **A `?` TAKES THE OTHER SIDE'S OWN DESCRIPTOR** - the slot-typing root recorded by the chunk above, measured out in full and closed. A parameter compared against an EXPRESSION is announced as that expression: its type, its CHARACTER width in the charset the expression resolves to, and - when the expression reads no column - NOT NULL. fire-crab announced `448 VARYING len 32763 charset 0` for every text case on every attachment, and `580 INT64` for every integer one.
    **THREE AXES, EACH ITS OWN DEFECT.** TYPE: `? = 1` is a **LONG**, not a BIGINT, and only past INTEGER range (`? = 5000000000`) a BIGINT - which [result_width_bytes]' own doc already recorded ("never SMALLINT"); a text LITERAL is 452 TEXT while a computed text expression (`'x' || 'yy'`, `UPPER(U)`) is 448. WIDTH: the literal's own character count scaled to the announced charset - `? > 'abc'` is 3 bytes NONE, 12 UTF8, 3 WIN1252. NULL: a slot typed by an expression that READS NO COLUMN is NOT NULL (`? = 1 + 1`, `? = CAST('x' AS CHAR(3))`, `? = UPPER('abc')`), while one over a column keeps that column's nullability (`? = I + 1` stays nullable) - exactly [expr_has_col], and the axis on which `? = 1.5` and `? = DATE '...'` differed by nothing else at all.
    **NONE OF IT NEEDED NEW MACHINERY:** the projection describe has typed expression columns this way for months, so the comparison arm now calls the same [result_width_bytes] and [text_form] that [build_expr_col_from] does, with the same charset sentinels. A `?` LIKE/STARTING pattern slot likewise: its literal pattern's charset IS the attachment's, so it carries `ATT_SUBTYPE` (measured 2 bytes NONE, 8 UTF8, 2 WIN1252) and NOT NULL, where it had a flat `0` and a nullable flag.
    **BETWEEN AND IN OBEY OPPOSITE RULES, AND THE OBVIOUS GENERALISATION IS WRONG IN BOTH DIRECTIONS.** `? BETWEEN <lo> AND <hi>` takes the LOW bound **positionally**: `1 AND 5000000000` is a LONG (the NARROWER bound) and `5000000000 AND 1` a BIGINT, `1 AND 2.5` drops the scale and `2.5 AND 1` keeps it. `? IN (...)` takes the WIDEST element wherever it sits: `('bb','a')` is 2, `('a','bbb','cc')` 3, `(5000000000, 1)` a BIGINT, `(1, 2.5)` a scaled INT64. Both desugar into leaves that each claim the one slot as they resolve, so the fix is which leaf goes LAST - BETWEEN emits its high leaf first, IN stable-sorts its items by width - and an AND of two comparisons or an OR of equalities is order-insensitive, so nothing but the surviving descriptor moves. Reordering predicate leaves is why the INDEX and RANGE-SELECTION gates are in this chunk's adjacent set rather than only the describe ones.
    **MY OWN NEW GATE CONTAINED A PASSENGER, AND RE-MEASURING FOUND IT.** The cell `? IN ('a','bb')` passed while IN was still wrong: its widest element is also its LAST, which is precisely what a last-write-wins server produces, so the cell could not fail. `('bb','a')` and `('a','bbb','cc')` joined it and both failed - a cell that cannot distinguish the defect from the fix is not a test. The numeric orderings were measured BEFORE implementing for the same reason: had "widest" been generalised from the text cells to BETWEEN, four freshly written cells would have been confidently wrong.
    **RECORDED, NOT FIXED:** `? CONTAINING 'abc'` - the engine describes 448 VARYING at the pattern's width, NOT NULL, and fire-crab REFUSES the statement outright, describing nothing. That is a missing predicate shape rather than a describe defect (`? LIKE` and `? STARTING WITH` both resolve), and it is left as a slice of its own. `? LIKE ?`'s two slots keep their flat descriptor deliberately: both servers say 30, and their charset and nullability were never probed - stamping them would be a guess.
    `serve-real-bindcs` 34 -> 123 checks; on the previous commit's binary 49 OK / 74 DIFF, the 49 being the column-side and byte-carrier controls. One unit test moved with the change and it was mine: it pinned BETWEEN's old leaf order, which is now LOAD-BEARING, and says so. 161 ods + 346 wire unit tests. Adjacent by code path - the predicate and describe surface, plus the index and range-selection gates the leaf reordering could reach - thirty-eight gates, all green: index 403, bindconv 198, leftjoinindex 169, textnumwhere 152, numericwhere 149, describe 116, paramshapes 114, subqdesc 70, viewdml 61, wherefn 51, wherexpr 45, intlike 40, joinchain 39, btree 37, starting 36, uniqueorder 34, containing 33, similar 33, corrsub 27, mergeparam 26, dmlparamexpr 25, predfull 20, params 19, joingroup 18, having 16, offsetfetch 16, project 14, outerjoin 13, syscat 12, havingparam 11, insert 11, join 11, groupby 10, query 10, types 10, where 9, orderagg 8, idxcost 5.
  - **A PARAMETER PATTERN SCALES LIKE A LITERAL ONE - and the note saying otherwise was MINE, written one commit earlier.** The chunk above recorded `? LIKE ?`'s two slots as "flat on both servers, charset and nullability never probed, so stamping them would be a guess". They are not flat: the engine announces both slots as the fixed THIRTY CHARACTERS **in the attachment's charset** - 30 bytes under NONE, **120 under UTF8**, 30 under WIN1252 - and **NOT NULL** on both.
    **THE MISTAKE IS THE INSTRUCTIVE PART, so it is corrected rather than deleted:** that pair had been measured under a NONE attachment ALONE, where a character width and a byte width coincide and the scaling is invisible. The nullability was a divergence even there, on the very attachment it was measured on - it simply was not looked at, because the width agreed and the cell was written off. **A cell measured under one attachment is not a measurement**, which is the rule `serve-real-litcs` has recorded since August ("three attachments, always") and which this file then broke while quoting it.
    The fix is the treatment the literal pattern already had - the character width behind `ATT_SUBTYPE`, plus `PARAM_NOT_NULL` - applied to both slots of `? LIKE ?` and `? STARTING WITH ?`. **The boundary it must not cross is now pinned by two cells rather than by a comment:** a NULL pattern stays a bare `VARYING(1) CHARACTER SET NONE`, nullable, on every attachment (measured), and those two cells pass on BOTH binaries - which is exactly what separates a control from a passenger, the distinction the chunk above had to learn about its own `IN` cell.
    `serve-real-bindcs` 123 -> 135 checks; on the previous commit's binary 129 OK / 6 DIFF, the six being precisely the two teeth cells across three attachments while both NULL-pattern controls pass there too. 161 ods + 346 wire unit tests. Adjacent by code path - `resolve_param_lhs`'s pattern arms and the describe readers beside them - twelve gates, all green: bindconv 198, textnumwhere 152, describe 116, paramshapes 114, litcs 78, wherefn 51, intlike 40, starting 36, containing 33, similar 33, params 19, charform 8.
    **STILL RECORDED, NOT FIXED:** `? CONTAINING <pattern>` in every spelling - `? CONTAINING 'abc'` (448 VARYING at the pattern's character width in the attachment's charset, NOT NULL), `? CONTAINING ?` (both slots the scaled 30), `? CONTAINING NULL` (the flat `VARYING(1)`) and `<column> CONTAINING ?` - all refused here, all answered there. The ANSWER contract is measured too, so the next slice starts from it rather than from a probe: the match is case-insensitive both ways (`'xAByz' CONTAINING 'ab'` is true, as is `V CONTAINING ?` bound `'AN'` over `BANANA`), a NULL bind is UNKNOWN under BOTH polarities, an EMPTY pattern matches anything non-null, and an integer bind renders to its decimal text (`10 CONTAINING '1'` is true). [containing_term] already builds exactly the right Term from a pattern string, and [Term::ParamLike]'s bind-time `fetch` is the twin to reuse - so this is a missing SHAPE, not missing machinery.
  - **`<text column> CONTAINING ?` IS ANSWERED NOW** - a bound PATTERN, where only a literal one resolved before. CONTAINING is the one predicate that folds case on EVERY character set, so the shape needed the fold to arrive with the value: the new [Term::ExprContainingParam] carries the COLUMN's ttype, and [containing_term] - which already upper-cases by that charset, canonicalises under an ICU collation, escapes the pattern's own `%`/`_` and wraps it - builds the real term at bind. The construction mirrors the `SIMILAR TO ?` and `STARTING WITH ?` arms beside it, claims the column's own descriptor through the same `claim` closure (which is what the engine announces for that slot, measured), and reuses the literal arm's `canonical_known` gate verbatim. One subtlety: the blanket `RawKind::Containing(..) => return None` sat ABOVE the match and would have swallowed a `?` before any arm saw it, so it is narrowed to the binary pattern it was actually written for.
    **MEASURED AND AGREEING:** the fold both ways (`NAME CONTAINING ?` bound `'o'` and `'O'` both take 'ok' and 'open'), a mid-string match, no match, an EMPTY pattern taking every non-NULL row, a NULL bind that is UNKNOWN under BOTH polarities, and - the rule this predicate exists to hold still - **NO WILDCARDS**: a bound `%` or `_` matches only a literal one. Over a UTF8 column the accent fold works (`U CONTAINING ?` bound the accented letter takes both the upper- and lower-case rows), which is the column's charset deciding, not the attachment's.
    **THE SCOPE IS DRAWN BY A MEASUREMENT I COULD NOT EXPLAIN, and the two shapes outside it keep refusing** - recorded in the gate with the engine's own answers. `<INTEGER column> CONTAINING ?` was never probed (LIKE and STARTING render such a column to decimal text; whether CONTAINING folds that rendering is a guess). And a bound TESTED side (`? CONTAINING <literal>`) hits a genuine engine anomaly: under a **UTF8 attachment only**, it is FALSE for a non-ASCII pattern **even same-case**, while bound-vs-bound, literal-vs-bound and literal-vs-literal are all TRUE there, and the ASCII form of the same cell is TRUE. No theory survives those numbers, and answering it would make this server say TRUE where the engine says false - a wrong answer replacing a refusal, which is the trade this file never takes.
    **FOUR HARNESS LIES IN ONE CHUNK, and they cost more than the code did - so the rules they taught are recorded beside them.** (1) node-firebird has NO attachment-charset knob: `attach({charset})` is inert, nothing in its lib consumes `lc_ctype`, so every "NONE vs UTF8" row taken through it was the SAME attachment. (2) **isql does not transcode its statement text** - `-ch WIN1252` still ships the file's raw UTF-8 bytes, so a non-ASCII literal arrives as two WIN1252 characters and every comparison quietly answers the wrong question; `-ch` changes how the engine READS the bytes, not what isql sends. (3) `SELECT OCTET_LENGTH(?)` is *Data type unknown* on the engine too - a malformed probe, not a finding; it needs a CAST to give the slot a type. (4) A fixture built on ONE side: three "divergences" in the final probe were rows that existed only in the reference database. **Every charset-sensitive probe now carries a live control** (`CHAR_LENGTH` of an accented letter is 2 under NONE, 1 under UTF8 and WIN1252) and the only client here that can vary the attachment charset for a BOUND parameter is `firebird-driver` in the harness venv, which transcodes both statement text and parameters.
    `serve-real-paramshapes` 114 -> 126 checks; on the previous commit's binary 116 OK / 10 DIFF - exactly the ten new cells, while the two recorded refusals read "refusal kept" on BOTH binaries, which is what a boundary record should do. 161 ods + 346 wire unit tests. Adjacent by code path - the predicate and pattern family, the COLLATION family `containing_term` leans on, and the INDEX family a new `Term` variant reaches through [filter_has_params] - fifteen gates, all green: index 403, textcolcmp 359, bindconv 198, leftjoinindex 169, describe 116, icucoll 89, intlike 40, starting 36, containing 33, similar 33, collclause 30, params 19, collate 16, utf8case 11, idxcost 5.
  - **A NUMERIC COLUMN'S SYNTHESIZED PATTERN SLOT IS THIRTY *CHARACTERS*, NOT THIRTY BYTES** - and asking the same question one column type further along found it. `N LIKE ?`, `N STARTING WITH ?` and `N92 LIKE ?` claim a fixed-width TEXT slot rather than the column's own descriptor, and **four resolvers built that descriptor inline, all four flat `CHARACTER SET NONE`**. The engine announces it in the ATTACHMENT's charset: 30 bytes NONE, **120 UTF8**, 30 WIN1252. So every one of those shapes was wrong on any non-NONE attachment, and had been since the slot was introduced.
    **THE DOC ITSELF CARRIED THE MISTAKE:** `NUM_LIKE_PATTERN_LEN` said "the engine describes it as a FIXED `VARYING(30)` ... probed: N92, I, BIGINT, INT128, DECFLOAT(34) and (16) all len 30". Six column types - and one attachment, where a character width and a byte width coincide. The four copies are now one [num_pat_desc], carrying the `ATT_SUBTYPE` sentinel, and the doc says which half of it was measured.
    **THE PROOF SHOWS THE COINCIDENCE THAT HID IT.** Against the previous binary the new cells fail under UTF8 and WIN1252 and PASS UNDER NONE - on both binaries - because that is exactly where the two widths agree. A gate that ran isql's default attachment alone would have scored this class green forever, which is the rule `serve-real-litcs` records and this file has now been bitten by twice in one day.
    **AND THE OTHER HALF OF THE FAMILY: `<numeric column> CONTAINING ?` is answered now.** The engine RENDERS the column to its decimal text and matches the pattern against that, so the ttype is 0 - a rendered number has no case to fold. Measured: `N CONTAINING ?` bound `'1'` takes N=1,10,100 and `'0'` takes 10,100; an INTEGER bind renders; an EMPTY pattern takes every non-NULL row; a NULL bind is UNKNOWN under both polarities; `N92 CONTAINING ?` bound `'1.5'` takes 1.50 and -1.50 while `'.'` takes every non-NULL, the rendering carrying its decimal point.
    **THE SAME TRAP, WALKED INTO TWICE: a blanket refusal ABOVE the match.** An INTEGER column reaches [param_or_typed_term] and fell into my own new arm's `_ => None`; a SCALED numeric column never reaches that arm at all, because [col_kind] names no kind for it and [resolve_predicate] routes it straight to [numeric_term], whose `RawKind::Containing(..) => return None` sat above its match - the identical shape recorded one commit earlier. **When adding an arm to a resolver, look for the early blanket refusal first: the build will not tell you, and the gate will fail in a way that looks like the arm is wrong rather than unreached.**
    **THE GATE'S REFUSAL LIST EXPIRED ITSELF.** `N CONTAINING ?` was recorded one chunk ago as a deliberate refusal carrying the engine's answer; this chunk implemented it, and `serve-real-paramshapes` printed *"now agrees (update the refusal list)"* on the fresh binary. That is the whole argument for recording a refusal WITH the engine's own answer rather than as a comment: it has an expiry date and announces it.
    `serve-real-bindcs` 135 -> 147 checks (previous binary 138 OK / 9 DIFF), `serve-real-paramshapes` 126 -> 134 (previous 125 OK / 8 DIFF). 161 ods + 346 wire unit tests. Adjacent by code path - every gate driving a pattern predicate over a numeric column, since `numeric_term`'s blanket refusal was narrowed and it is shared by all of them - twelve gates, all green: index 403, dfparambind 260, bindconv 198, textnumwhere 152, numericwhere 149, dfparam16 121, describe 116, wherefn 51, intlike 40, starting 36, containing 33, similar 33.
    **STILL REFUSING, recorded:** `? CONTAINING <literal>` (the bound TESTED side, blocked by the measured UTF8 anomaly) and CONTAINING over a DECFLOAT column (its rendering under a fold is unprobed).
  - **A TEXT BLOB'S CHARACTER SET FOLLOWS THE ATTACHMENT TOO - in BOTH directions, and neither was right.** Found by sweeping rather than guessing: two chunks had each turned up a slot that was only correct under a NONE attachment, so instead of hunting a third instance I inventoried every remaining flat text descriptor in the parameter paths (none left outside unit-test fixtures - that class is closed) and then described the parameter shapes no attachment-aware cell had ever covered. A text BLOB was one of them.
    **THE LAW, measured across five blob kinds and three attachments, through INSERT, UPDATE and SELECT alike:** a text blob (sub_type 1) of a REAL charset is announced in the ATTACHMENT's charset when the attachment is real - a `CHARACTER SET UTF8` blob is charset 53 under WIN1252, a WIN1252 one is charset 4 under UTF8; a NONE blob keeps 0 on every attachment; under a NONE attachment every blob keeps its own; and a BINARY blob (sub_type 0) has no charset at all. Exactly the law text COLUMNS already obeyed.
    **TWO DEFECTS, ONE CAUSE EACH.** The INPUT side had no rule whatsoever: a blob's charset rides in `scale`, not `sub_type`, so [bind_text_cs] - which resolves a sub_type - never saw it, and the slot was announced in the STORAGE charset on every attachment. And the OUTPUT side, which did have [blob_out_charset], asked `intl::byte_carrier`, **which counts ASCII as a carrier**: an `ASCII` text blob was announced charset 2 under a UTF8 attachment where the engine says 4. That is the third time today that helper's definition has been the trap - the text-column path draws the line at `cs > 1` instead, and its ASCII control has always agreed. Both sides now draw it the same way, and the input twin is [bind_blob_cs].
    `serve-real-bindcs` 147 -> 180 checks, and it grew an OUTPUT-message helper so that ONE gate holds this law in both directions rather than splitting it across two. On the previous commit's binary 173 OK / 7 DIFF - five input cells and two output ones - while every NONE-blob and BINARY-blob cell passes on BOTH binaries in BOTH directions, which is what separates a control from a passenger. 161 ods + 346 wire unit tests. Adjacent by code path - every gate that describes a blob - fifteen gates, all green: describe 116, blobinfo 103, blobfilter 95, litcs 78, subqdesc 70, blobupdate 64, blobexpr 53, charset 43, outblr 32, blobcast 21, comment 20, noneblob 15, commentdb 11, blobwrite 8, blob 4.
    **AND THE SWEEP LEFT TWO BACKLOG ITEMS with their describes already measured:** `? SIMILAR TO <literal>` (the tested side; the engine announces the pattern's character width in the attachment's charset, NOT NULL - the same shape `? LIKE` and `? STARTING WITH` take, and [resolve_param_lhs] simply has no arm for it), and **a procedure call's ARGUMENT slots** - `SELECT K FROM P1(?, ?)` describes nothing here and the engine announces each argument's own declared type. Both are missing shapes rather than wrong describes.
  - **`? SIMILAR TO <pattern>` COMPLETES THE TESTED-SIDE PATTERN FAMILY - and uncovered three older defects on the way in.** The slot takes the shape LIKE and STARTING WITH already take (a literal pattern's CHARACTER count in the attachment's charset and NOT NULL - measured 2/8/2 and 5/20/5 under NONE/UTF8/WIN1252; both slots of `? SIMILAR TO ?` the fixed thirty scaled; a NULL pattern the flat nullable `VARYING(1)`), and the regex is compiled at BIND through the existing [sim_compile]/[sim_match].
    **THE TRAP WAS ONE LAYER EARLIER THAN LAST TIME.** The resolver arm and the bind-time term were both written and both unreachable: the TESTED-SIDE PARSER had arms for LIKE, STARTING, BETWEEN and IN and none for SIMILAR, so `? SIMILAR TO 'a%'` never became a param-lhs term at all. Last chunk it was a blanket refusal above a resolver's match; this time nothing produced the RawKind. **When a new arm has no effect, check what BUILDS its input before re-reading the arm.**
    **AND MY OWN NEW CELLS WENT VACUOUS IN A WAY THE HOUSE IDIOM INVITES.** The four raise cells tested `ERR*:ERR*` - the established shape for "both refuse" - which **cannot tell a prepare-time REFUSAL from the engine's execute-time RAISE**, so they reported OK while the whole shape was missing. Tightened to require the engine's own message on both sides; `ERR*:ERR*` now reports a DIFF explaining itself.
    **THREE PRE-EXISTING DEFECTS, ALL ONE CAUSE, none of which the `similar` gate could see because it drove only VALID patterns.** A malformed pattern reached the client as a generic *Dynamic SQL Error* instead of the engine's *Invalid SIMILAR TO pattern*; it escaped [Predicate::bind] as an `Err`, so it fired OUTSIDE the invariant pass and a FALSE conjunct written before it could not suppress it; and a LITERAL malformed pattern REFUSED AT PREPARE where the engine raises only when a row reaches it. All three are fixed by making the failure a TERM - [Term::BadSimilar] - with [EvalErr::InvalidSimilar] carrying gds 335544884, which was DERIVED rather than guessed (`335544320 + <jrd message number>`, cross-checked on arith_except, escape_invalid, string_truncation and trunc_limits).
    **ONE LINE IN [term_row_independent] GIVES BOTH MEASURED BEHAVIOURS,** which is the sign the shape was right: `Term::BadSimilar(e) => !expr_has_col(e)`. Over a BOUND value the expression is a literal, so the conjunct joins the invariant pass and raises in WRITTEN ORDER - only a FALSE before it suppresses the raise. Over a COLUMN it reads a row, so it stays in the per-row walk and raises only when a row reaches it - which is why EITHER FALSE order answers no rows there. Both halves measured on the engine, and without that arm the term fell to the catch-all, its conjunct was skipped by the pass entirely, and a FALSE written AFTER the bad pattern silently won.
    `serve-real-bindcs` 180 -> 192 (previous binary 180 OK / 12 DIFF), `serve-real-paramshapes` 137 -> 151 (previous 133 OK / 18 DIFF), `serve-real-similar` 33 -> 40 (previous 34 OK / 6 DIFF - the seventh new cell, a NULL bind, passes on BOTH, which is what makes it a control). 161 ods + 346 wire unit tests. Adjacent by code path - the pattern-predicate family, the index family the invariant pass feeds, and the REFUSAL LEDGER since two shapes that refused now answer - fourteen gates, all green: index 403, bindconv 198, numericwhere 149, describe 116, litcs 78, nofallback 74, subqdesc 70, psql 68, exprshape 54, wherefn 51, intlike 40, starting 36, containing 33, predfull 20.
  - **`EXECUTE PROCEDURE P(?, ?)` BINDS ITS ARGUMENTS - and the charset defect it uncovered was older and wider than the shape itself.** A call's `?` argument claims its DECLARED parameter's descriptor, which is what the engine announces: measured across three attachments, a `VARCHAR(5) CHARACTER SET UTF8` argument is 20/UTF8 under NONE and UTF8 and 5/WIN1252 under WIN1252, a NONE argument never moves, a declared `NOT NULL` parameter marks its slot NOT NULL, and a LITERAL argument claims no slot at all. Slots are numbered by TEXT POSITION, which for a whole-statement EXECUTE PROCEDURE makes its arguments slots 0..n-1. The values are written into the plan's argument seats at op_execute - on BOTH execute paths, since op_execute2 is the one the OO clients drive and substituting on one alone would have run the body with a NULL in every seat.
    **THE OLDER DEFECT: every text procedure parameter was announced CHARACTER SET NONE, in BOTH directions.** A table column's descriptor comes from the STORED RECORD FORMAT, whose sub_type already carries the ttype; a procedure parameter is rebuilt from `RDB$FIELDS`, where the charset lives in `RDB$CHARACTER_SET_ID` and `RDB$FIELD_SUB_TYPE` is 0 - so [domain_type_info] never saw it and `domain_desc` faithfully stored the zero. Outputs had it too: a `VARCHAR(5) CHARACTER SET UTF8` OUTPUT read 20/NONE where the engine says 20/UTF8. One fix in the two `domain_desc` closures (the procedure loader and the function loader) repairs arguments and outputs together. The ttype's high byte is the COLLATION and is deliberately left 0: measured, a `COLLATE UNICODE` parameter stores collation 2 in the catalogue and the engine still announces `charset: 4` - the describe carries the charset alone.
    **THE FROM-CLAUSE FORM IS RECORDED, NOT FIXED, AND AN ATTEMPT WAS REVERTED.** `SELECT K FROM PU(?, ?)` takes the bound-row-source route, where [sql_over_from] rebuilds the statement by splicing the FROM item OUT - which deletes the call's `?` from the text - and the outer re-plan then numbers the surviving placeholders from zero, colliding with the argument slots. Claiming the slots anyway made `SELECT FIRST 1 K FROM PU(?, ?)` **ANSWER `K = NULL`** where it had refused, and it was reverted for exactly that: a wrong answer replacing a refusal is the trade this file never takes. Doing it properly needs [plan_over_source] to accept a parameter BASE, which is a slice of its own; the contract is measured and waiting (`PU(?, ?) WHERE K = ?` numbers three slots in text order, and `WHERE ID = ? AND ID IN (SELECT K FROM PX(?))` numbers the WHERE's first because it is written first).
    **TWO PROCESS COSTS, BOTH CAUGHT BY MACHINERY RATHER THAN BY READING.** A python edit whose anchor matched TWICE exits before its single write, so the tree is unchanged - and the build then "succeeds" on the OLD binary. That happened twice here, and the only reason it was not mistaken for a result is that the script prints an `APPLIED:` line it never reached. And a gate comment reading "the charset comes from RDB-dollar-CHARACTER_SET_ID" sat inside an UNQUOTED heredoc while the gate runs under `set -u`: the shell expanded `$CHARACTER_SET_ID`, the fixture died with *unbound variable*, `make_db` hid the error, and `serve-real-bindcs` reported `FAIL scratch WORK` twice while EVERY by-hand reproduction passed - because each probe substituted the variables away before isql saw them. **A gate that builds its fixture through an unquoted heredoc must not contain a literal RDB-dollar name, even in a comment.**
    `serve-real-bindcs` 192 -> 204 checks (previous binary 192 OK / 12 DIFF - nine argument-slot cells and three output-charset ones), `serve-real-paramshapes` 151 -> 152 (previous 151 OK / 1 DIFF). 161 ods + 346 wire unit tests. Adjacent by code path - the two loaders feed every procedure and function describe, and the PSQL interpreter's parameter typing sits beside them - sixteen gates, all green: describe 116, nofallback 74, psql 68, funcwidth 48, proclazy 31, callproc 15, exeproc 14, procdescribe 13, procsample 11, numproc 10, procdefault 9, createfunction 7, fncall 7, pkgfunc 6, funcbody 5, procfmt 1.
  - **A `?` ARGUMENT IN A FROM-CLAUSE PROCEDURE CALL - and the whole of it was ONE STRAY `params.clear()`.** `SELECT K FROM PU(?, ?)` now describes its argument slots and binds them: the declared parameter's descriptor under the ordinary attachment law (20/UTF8 under NONE and UTF8, 5/WIN1252 under WIN1252), a literal argument claiming no slot, and the bound value reaching the BODY in the right seat - measured with a procedure that returns its SECOND argument, so a swapped pair or a dropped slot is a wrong ANSWER and not merely a wrong describe.
    **THE BUG.** The procedure branch cleared the parameter sink immediately before returning its plan. Every OTHER `params.clear()` in that function precedes `return plan_query_inner(&rewritten, ..)` - a RE-PLAN of rewritten text that re-claims every slot from scratch - so clearing is right there. This one returned a plan DIRECTLY, with nothing to refill the sink, and it discarded every argument slot the branch had just claimed. It was harmless for as long as a call's arguments were always literals, which is exactly why it survived.
    **AND THE "REFUSAL" WAS NEVER A REFUSAL.** With the slots wiped, the statement prepared fine and announced `INPUT message field count: 0` - which a slot-extracting probe reads as an empty result, indistinguishable from a refused prepare. That misreading cost several turns and one reverted attempt; **an empty INPUT message and a refused prepare look identical unless the field count is printed.**
    **HOW IT WAS FOUND, after two wrong guesses:** trace the planner's ENTER, the claim site and `plan_query`'s exit, printing `params.as_ptr()` AND `len` at each. The same allocation read `len=2` at the claim and `len=0` at the exit - and a `Vec` cannot shrink itself, so something cleared it. The pointer-and-length pair settled in one run what reading had not.
    **THREE ROUTES, THREE SUBSTITUTION POINTS** (traced with literal arguments, all three answering today): a bare call takes the plain leaf and is filled in [materialise_procedures]; a MODIFIER over the call runs the body on its OWN arm, so the seat must be filled there too; and a CLAUSE over the call takes the bound-row-source route. Filling only some of them is what made `SELECT FIRST 1 K FROM PU(?, ?)` answer `K = NULL` in the reverted attempt - the modifier arm ran the body with an unfilled seat.
    **ROUTE 2 KEEPS REFUSING, deliberately and by an explicit guard:** [sql_over_from] rebuilds the statement by splicing the FROM item OUT, which deletes the call's `?` from the text, and the re-plan then renumbers from zero and collides with the argument slots. It needs [plan_over_source] to accept a parameter BASE - its ungrouped branch starts at `proj_params`, its grouped one at `dgroup_params` after [renumber_proj_params] rewrites indices in place - and that is a slice of its own. Its contract is measured and the cells are written as recorded refusals, ready to go live: `PU(?, ?) WHERE K = ?` is three slots in text order, and a nested call numbers AFTER an outer WHERE's `?`.
    [plan_kind] also learned `ProcSelect` and `ProcInvoke`: every trace of the shape this work revolves around said `other`, which is the same diagnostic gap that cost a chunk earlier when it said `other` for `ProcRows`.
    `serve-real-bindcs` 204 -> 207 checks (previous binary 204 OK / 3 DIFF), `serve-real-paramshapes` 152 -> 158 (previous 152 OK / 6 DIFF). 161 ods + 346 wire unit tests. Adjacent by code path - the planner's procedure branch, `materialise_procedures`, the modifier arm, and every gate driving a procedure as a row source - sixteen gates, all green: describe 116, nofallback 74, psql 68, derived 61, funcwidth 48, cte 35, proclazy 31, corrsub 27, joinderived 25, subqdesc 70, callproc 15, exeproc 14, procdescribe 13, procsample 11, numproc 10, procdefault 9.
  - **A WRONG ANSWER SHIPPED IN THE CHUNK ABOVE, AND THIS IS THE FIX.** `SELECT MAX(K) FROM PU(?, ?)` bound `('ab', 7)` answered **NULL** where the engine answers 7; `SUM` and an aggregate over a text argument likewise. An AGGREGATE over the call takes a THIRD route - neither the leaf nor the modifier arm, but the BOUND-ROW-SOURCE one, where the call becomes an inner plan under a fold - and that inner `ProcSelect` was built with EMPTY argument seats. So the statement described its slots correctly (they are claimed in the OUTER parameter sink), accepted the client's values, and then ran the body with `Value::Null` in every seat.
    **THE FIX IS THAT THE SEATS TRAVEL WITH THE INNER PLAN.** [materialise_procedures_src] already carries the bound values down to that leaf, so filling them there is all it takes - the shape ANSWERS rather than refusing. It is safe for exactly the shapes that reach it: anything carrying an OUTER `?` (a WHERE, GROUP BY, HAVING or ORDER BY) is refused by the guard above, because that route's re-plan renumbers from zero, and a projection `?` over a call describes nothing on BOTH servers (measured).
    **THE CELL THAT AGREED THROUGHOUT IS THE LESSON.** `SELECT COUNT(*) FROM PU(?, ?)` passed on the broken binary and on the fixed one - counting a row never reads its arguments, so it cannot see a NULL seat. It was in the gate before this defect and it did not catch it. The teeth are the VALUE-CARRYING folds: the fixture's procedures return their second argument and the character length of their first, so a lost seat becomes a NULL ANSWER. **The third time this session a cell agreed for a reason unrelated to what it claimed to test** - after the `IN` list whose widest element was also its last, and the raise cells that compared one error against another.
    `serve-real-paramshapes` 158 -> 162 checks; on the previous commit 159 OK / 3 DIFF, exactly those three value-carrying folds, with the `COUNT(*)` control passing on BOTH. 161 ods + 346 wire unit tests. Adjacent by code path - the bound-row-source route every derived table, CTE, view and fold travels - fourteen gates, all green: bindcs 207, describe 116, nofallback 74, subqdesc 70, psql 68, derived 61, viewjoin 36, cte 35, proclazy 31, corrsub 27, joinderived 25, callproc 15, exeproc 14, procdescribe 13.
  - **A WHOLE CLASS OF STATEMENT WAS BROKEN FOR ONE KIND OF CLIENT ONLY - and every gate was green through it.** `SELECT K FROM PU(?, ?)` through the python firebird-driver died with a bare *Dynamic SQL Error*, while **isql and node-firebird ran the identical SQL correctly**. The literal-argument control failed the same way, so it was never about parameters or argument seats - the variable was the CLIENT. The cause is `isc_info_sql_stmt_flags`, info item 27: the word the OO API dispatches on to choose `openCursor()` over the singleton `execute()`. Announced without FLAG_HAS_CURSOR, the driver sent **op_execute2** (op 76) for a cursor statement, and the server answered the honest SQL error that path owes a statement it cannot serve. The statement TYPE was right (1) on both servers, which is exactly why the two clients that dispatch on the TYPE never saw it.
    **THE LAW, measured prepare-only over 38 shapes against the live engine:** FLAG_HAS_CURSOR (1) exactly when the statement is a cursor one - stmt_type 1 (select) or 12 (select_for_update); FLAG_REPEAT_EXECUTE (2) for everything EXCEPT real DDL, which answers a bare 0. The engine therefore answers only three values, 3 / 2 / 0. Two rows discriminate the rule from every plausible approximation: `INSERT .. RETURNING` is type 8 and answers **2** (a singleton) while `UPDATE .. RETURNING` and `DELETE .. RETURNING` are type 1 and answer **3** (cursors), and `EXECUTE BLOCK` splits the same way on whether its body SUSPENDs. A third says why the second bit cannot be read off the type code: **`SET TIME ZONE` reports stmt_type 5, the DDL type, like every session-management statement - and still answers 2, not 0.** "Is DDL" is a plan-shape test.
    fire-crab set NEITHER bit correctly. `has_cursor` was a hand-listed set of plan shapes that omitted `ProcSelect`, `Union`, `Derived`, `Modified`, `ProcRows`, `Returning` and `ExecBlockSelect`, and FLAG_REPEAT_EXECUTE was never set on any statement at all - 25 of the 28 gate cells were wrong, the only agreements being real DDL, where 0 matched 0 because both bits were equally unimplemented. **The list was duplicated in TWO emitters and had drifted** - [build_describe]'s prepare path and [answer_info_sql] - the same two-emitters hazard the describe pair has produced before. The fix is one [stmt_flags_of] DERIVED from [stmt_type_of] and called by both, so the two cannot disagree again; the subtle RETURNING and EXECUTE BLOCK rows then fall out of the existing type law for free rather than being re-listed.
    **THE HARNESS LAW THIS BUYS - three clients disagree BY DESIGN, so a feature can be entirely green and still dead:** node-firebird and isql dispatch on the statement TYPE, the python OO driver on the FLAG WORD. Every gate in the procedure family (bindcs 207, paramshapes 162, callproc, exeproc, procdescribe) was green while a bare `SELECT ... FROM <selectable procedure>` was unusable from every fbclient OO client - which is what the firebird-qa pytest suite uses for every database operation. **A client-visible shape must be probed through the python driver too.** It was found by asking whether a FOURTH route existed after two wrong answers came from filling some routes and not all; `FC_SRV_TRACE` then named the op in a single run.
    **RECORDED, NOT FIXED:** `SELECT ... FOR UPDATE` is stmt_type **12** on the engine and 1 on fire-crab - the flags now agree at 3 because [stmt_flags_of] accepts both, but the type itself needs the plan to carry a for-update marker.
    `serve-real-pydriver` 12 -> 41 checks: 28 cells measured DIFFERENTIALLY against the live engine on the same fixture (the gate gained a selectable procedure and a sequence for it), plus a guard that fails if the engine did not answer all three of 0/2/3 - a battery that saw one value would agree with almost any implementation. On the previous commit's binary **25 DIFF**, with the three real-DDL cells OK on both binaries as the positive controls. 704 unit tests. Adjacent by code path - the prepare-info emitters and everything that dispatches on what they say - eighteen gates, all green: bindcs 207, paramshapes 162, describe 116, icucoll 89, nofallback 74, subqdesc 70, psql 68, attimezone 49, returning 44, returningexpr 33, rollback 31, proclazy 31, services 19, gtt 17, callproc 15, exeproc 14, procdescribe 13, multistmt (skips).
  - **ROUTE 2 - A CLAUSE OVER A FROM-CLAUSE PROCEDURE CALL WHOSE ARGUMENTS ARE BOUND.** `SELECT K FROM PU(?, ?) WHERE K > ?` refused; it now answers, and so do a WHERE with no `?` of its own, an ORDER BY, and a GROUP BY over the call. The refusal was deliberate and recorded: [sql_over_from] rebuilds the statement with the FROM item spliced OUT, so the call's `?` vanish from the text while their slots stay claimed in the sink - and the re-plan then numbered the rebuilt statement from zero, colliding with the arguments.
    **THE FIX IS ONE LINE OF NUMBERING, NOT A MECHANISM.** [plan_over_source] captures `param_base = params.len()` at entry and both its branches number from there - [renumber_proj_params_from] for the projection, and the WHERE's `np` after it - so a clause's `?` lands AFTER the arguments, which is the engine's textual order. [plan_join_bound] already floored its own numbering at `params.len()` for exactly this reason, so this is that existing rule one level up rather than a new parameter threaded through five call sites. A CTE, a derived table and a view claim nothing before they are planned, so their base is 0 and their numbering is untouched - `derived` 61, `cte` 35, `view` 34, `recursive` 43 and `lateral` 36 all unchanged.
    **MEASURED FIRST, AND THE MODEL WAS WRONG TWICE.** The recorded contract said the blocker was "an outer `?`", but `... WHERE K > 0` - which has none - refused too, while `SELECT MAX(K) FROM PU(?, ?)` answered: the guard was keyed on a clause AND bound arguments, and the aggregate escaped it only by having no clause. Re-measuring also showed the slot counts unchanged from the stored contract (3 for a WHERE with its own `?`, 2 otherwise) and, with `FC_SRV_TRACE`, that with LITERAL arguments every one of these clauses ALREADY worked - so what landed is strictly the bound half.
    **STILL REFUSED, AND EACH FOR ITS OWN REASON** - recorded as self-expiring cells that carry the engine's answer: `HAVING ... > ?` over the grouped form (the grouped branch refuses a WHERE/HAVING `?` outright - and it refuses with LITERAL arguments too, so it is a PRE-EXISTING gap rather than route 2's), and a derived table or CTE over a bound call (refused by a different guard entirely - that inner statement carries no clause at all, so route 2's guard was never what stopped it). Both stay refusals, never wrong answers: a shape [plan_over_source] cannot number returns None and the call site turns that into `Plan::Refused`.
    `serve-real-paramshapes` 162 -> 175 checks and `serve-real-bindcs` 207 -> 219. On the previous commit's binary **paramshapes 166 OK / 9 DIFF** - exactly the nine value-carrying cells - and **bindcs 207 OK / 12 DIFF**, the four describe cells across three attachments, each reading "described NO input slot" where the engine announces the full three- or two-slot message. The controls pass on BOTH binaries: `COUNT(*)`, the LITERAL-argument clause, the three recorded refusals and the engine-moves-a-slot control. Every predicate cell is paired with one that must answer NOTHING - a filter that never ran would return the row anyway. 704 unit tests. Eighteen adjacent gates green: bindcs 219, paramshapes 175, describe 116, nofallback 74, subqdesc 70, psql 68, derived 61, similar 40, recursive 43, viewjoin 36, lateral 36, cte 35, view 34, proclazy 31, corrsub 27, joinderived 25, callproc 15, exeproc 14, procdescribe 13, pydriver 41.
    **AND A HARNESS COST WORTH MORE THAN THE CHUNK.** Mid-verification every gate began reporting `OK=0` with DIFFs on cells the change cannot touch, then hanging to its timeout. It was not a regression: the REFERENCE ENGINE was wedged. Two instances of one gate had been running at once (these gates use FIXED fixture paths and ports, so they destroy each other), and `kill -9` on the clients left the server on `pthread_mutex_trylock failed. Error code 22` after `INET/inet_error: read errno = 104`. `systemctl is-active` still said `active` and LOCAL attaches still read the files perfectly while every TCP attach failed 08006 or blocked - that asymmetry is the diagnostic, and `systemctl restart firebird` fixed it. It stayed invisible for several turns because the runs printed only `grep '^DIFF'` of their output: the head said `engine: CONN_ERR` on every line. **Print the HEAD of a gate's output, never only its DIFFs; never run two instances of one gate; and when many unrelated cells go DIFF at once, suspect the harness before the change.**
  - **A `?` IN THE WHERE OR HAVING OF A GROUPED BOUND SOURCE - one removed fence, four shapes.** `SELECT V, COUNT(*) FROM (SELECT V FROM T) X GROUP BY V HAVING COUNT(*) > ?` refused; so did the same over a CTE, over a view, and `SELECT K, COUNT(*) FROM PU(?, ?) GROUP BY K HAVING COUNT(*) > ?` - the last recorded only one chunk earlier as route 2's leftover. All four answer now, because all four reach the ONE grouped branch of [plan_over_source]. This is ordinary SQL any client can write, not a procedure corner: the measurement that set the scope was deliberately run over a derived table and a CTE as well as the call, and it was the derived/CTE rows that showed the refusal was general.
    **THE GUARD WAS A SCOPE FENCE, NOT PROTECTION, AND `git log -S` SAID SO IN ITS OWN WORDS.** The slice that brought the grouped projection param wrote `if np != dgroup_params { return None; }` and recorded why: *"numbers a WHERE/HAVING `?` past it - which, being still unsupported there, keeps that path's refusal exact"*, with *"SCOPE: ... A `?` in a grouped WHERE/HAVING ... remain separate slices"*. Everything that slice was waiting for already existed by the time this chunk opened: [resolve_predicate] and [resolve_having] both take the `?` sink (the latter's doc says a HAVING `?` is described as the compared value's type), and BOTH predicates are already bound at execute - [validate_select_bind] binds `filter` and `having` for `Group`/`JoinGroup`, and so does the fetch. The one missing piece was the NUMBERING, which the previous chunk's parameter base supplied. Checking the execute-side binding for `having` BEFORE removing the guard is what kept this from being the session's third "filled some paths, not all" wrong answer.
    **MEASURED, and the slot order is the part with teeth:** a grouped derived `HAVING ?` is one slot; `WHERE ? ... HAVING ?` is two; `CAST(? AS INTEGER) ... HAVING COUNT(*) > ?` is two with the PROJECTION's first (its two cells would swap if it were not); the procedure form is three, and answers with LITERAL arguments too. Every predicate cell is paired with a twin that must answer NOTHING - a filter that never ran would return the rows anyway, and a dropped HAVING would keep every group.
    **THE CONTROL THAT NAMES THE BRANCH:** `grouped join: HAVING ?` passes on BOTH binaries. A grouped JOIN goes through [plan_join_bound], which already floored its numbering at `params.len()`, so it was never fenced - the cell says precisely which builder moved and which did not. `plain table: HAVING ?` does the same for the non-bound path.
    `serve-real-castparamgroup` 14 -> 21 checks (its header's own SCOPE note retired with it) and `serve-real-paramshapes` 175 -> 177, where the HAVING refusal EXPIRED ITSELF exactly as a self-expiring cell is meant to. On the previous commit's binary **castparamgroup 10 OK / 11 DIFF** - the eight derived and three CTE cells - and **paramshapes 174 OK / 3 DIFF**, the three flipped cells alone, with route 2's own cells passing on both. 704 unit tests. Nineteen adjacent gates green: bindcs 219, paramshapes 177, describe 116, nofallback 74, subqdesc 70, psql 68, derived 61, recursive 43, pydriver 41, similar 40, viewjoin 36, lateral 36, cte 35, view 34, proclazy 31, corrsub 27, joinderived 25, castparamgroup 21, callproc 15, exeproc 14, procdescribe 13.
    **STILL REFUSED:** a derived table or CTE over a bound CALL (a different guard - that inner statement carries no clause at all), and an expression over a grouping column (`SELECT V*2 ... GROUP BY V`), which was the other half of the original slice's scope note.
  - **A `?` INSIDE A DERIVED TABLE OR CTE BODY.** `SELECT ID FROM (SELECT ID FROM EMP WHERE ID > ?) X` refused - and there is no procedure anywhere in it. The inner query was planned into a FRESH sink and any slot it claimed killed the statement outright (`if !inner_params.is_empty() { return Some(Plan::Refused) }`, commented "a `?` inside a derived table"). `git log -S` puts it in the ORIGINAL derived-table slice, whose own "Refused, with reasons" list does not mention it: a conservative limitation from before parameters reached that path, not protection against anything measured.
    **THE FIX IS TEXT-POSITION NUMBERING.** Slots are numbered left to right across the whole statement, so the inner body's `?` start after everything written before the derived span: the branch counts them with the same `mask_literals(&sql[..at]).matches('?')` trick a procedure call's `arg_base` uses, and [plan_query_inner_at] - [plan_query_inner_ctx] with a base - plans the inner into THIS statement's sink from there. The outer WHERE then numbers from `params.len().max(proj_params)`, so it lands after whatever the FROM claimed. `plan_query_inner` keeps its 35 callers on a zero-base wrapper.
    **MEASURED ACROSS THE FAMILY, each predicate paired with a twin that must answer NOTHING:** an inner WHERE `?`, an inner `?` with an outer `?` (either half excluding), a NESTED derived table, a CTE body, a CTE body plus an outer `?`, and an inner `?` over a TEXT column. Engine slot counts 1 and 2, matched. The literal-argument forms already worked, which is what says only the BOUND half was missing.
    **STILL REFUSED, one root cause between them:** an inner PROJECTION `?` (nothing binds it - [bind_plan_params] reaches the top-level plan's columns only, so it would evaluate unbound, and a refusal is the honest answer); an OUTER projection `?` above an inner one, and an AGGREGATE, GROUP BY or JOIN above an inner `?` - these take routes whose base is INFERRED from `params.len()` ([plan_over_source]) or keep their own copy of the old guard ([plan_join_bound]). One EXPLICIT base would retire all of them; that is the next slice, and each is recorded as a cell carrying the engine's answer so it expires itself.
    `serve-real-derived` 61 -> 80 checks. On the previous commit's binary **71 OK / 9 DIFF** - exactly the nine value-carrying cells - with all six recorded refusals and all four controls (the two literal-argument twins included) passing on BOTH. 704 unit tests. Twenty adjacent gates green: bindcs 219, paramshapes 177, describe 116, nofallback 74, subqdesc 70, psql 68, recursive 43, pydriver 41, similar 40, viewjoin 36, lateral 36, cte 35, view 34, proclazy 31, corrsub 27, joinderived 25, castparamgroup 21, callproc 15, exeproc 14, procdescribe 13.
    **THE GATE CAUGHT TWO THINGS THE HAND-PROBE DID NOT, and one of them was the gate itself.** Three shapes went in as live cells on the strength of a probe that never covered them - a JOIN, an aggregate and a GROUP BY above an inner `?` - and all three refuse; they are recorded instead. Worse, the cells were first appended at the END of the file, BELOW the sections that deliberately BLOCK: fire-crab serves connections SERIALLY, so nothing can attach after a blocking cell, every new cell answered `CONN_ERR` on both sides, and `CONN_ERR == CONN_ERR` scored a vacuous OK - twelve cells and a previous-binary "proof" that could not tell the binaries apart. It was visible only because this suite prints the VALUE on OK. The cells moved above the blocking sections and `bothq`/`refusesq` now FAIL when either side does not answer, so the trap is disarmed rather than merely avoided.
  - **AN OUTER PROJECTION `?` ABOVE A DERIVED BODY THAT HAS ONE - and the correction of a prediction I had already committed.** `SELECT CAST(? AS INTEGER) AS C, ID FROM (SELECT ID FROM EMP WHERE ID > ?) X` is two slots with the PROJECTION's first (measured); fire-crab refused it. The cause was the shortcut taken two slices earlier: [plan_over_source] inferred `param_base = params.len()`, which is right only while the projection has no `?` of its own - once the FROM had claimed slots (a call's arguments, or a derived body's own `?`), the projection was numbered AFTER them.
    **THE FIX IS SMALLER THAN THE PLAN THAT WAS RECORDED FOR IT.** The previous chunk's note said this needed an EXPLICIT base threaded through five call sites. Reading those five showed why it does not: [plan_over_source] re-plans a WHOLE STATEMENT at every one of them - a derived table, a CTE, a view, a FROM-clause procedure call - and a statement's own SELECT list always precedes its FROM. So the projection numbers from ZERO, and what the FROM claimed is a FLOOR under the WHERE (`np = proj_params.max(params.len())`), not a base under the projection. No new parameter, no call site touched, and route 2's own shapes keep their numbering because the floor does the work the inferred base was doing.
    **THE PREDICTION THAT WAS WRONG, AND THE MEASUREMENT THAT CORRECTED IT.** That same note claimed one explicit base would retire the aggregate, GROUP BY and JOIN shapes too. It retires NONE of them: measured after the change, they refuse exactly as before. They are not a numbering problem at all - an aggregate or GROUP BY above an inner `?` PREPARES (stmt type 1, its slots described), its `op_execute` is ACCEPTED, and it fails at the FETCH, `fetch plan = JoinGroup`. Confirmed in the code rather than inferred: `RowSource::rows(&self, db)` and [join_rows] take no `args`, so the fold binds its own filter, having and parts with the arguments and then materialises its BASE row source without them - the derived inner's `?` is still unbound when the fold pulls rows. Binding the base row source is what would retire these; an explicit base never would. The gate's recorded note now says so, in place of the claim it replaced.
    `serve-real-derived` 80 -> 81 checks (one recorded refusal became two live cells, whose values would SWAP if the projection were numbered after the derived body). On the previous commit's binary **79 OK / 2 DIFF** - exactly those two - with every other cell and all five remaining recorded refusals passing on BOTH. 704 unit tests. Ten adjacent gates green across every [plan_over_source] caller: paramshapes 177, cte 35, view 34, viewjoin 36, recursive 43, lateral 36, joinderived 25, corrsub 27, castparamgroup 21.
  - **A FOLD ABOVE A DERIVED BODY WITH A BOUND `?` - and a stale comment that my own earlier slices turned into a landmine.** `SELECT COUNT(*) FROM (SELECT ID FROM EMP WHERE ID > ?) X` PREPARED correctly (its slot described, stmt type 1), its `op_execute` was ACCEPTED, and it died at the FETCH. The cause is one line: `RowSource::rows_materialised`'s `PlanRows` arm called `branch_rows_res(plan, db, &[])` - EMPTY arguments - under a comment that explained why that was safe: *"a derived side refuses `?` at plan time, so there are no bound arguments to thread through here"*. That was true by construction until the slices above taught a derived body to TAKE a `?`, and nothing pointed back at the shortcut the old invariant had justified. [Predicate::bind] against an empty slice answers "missing parameter value" for every slot, so the fold bound its OWN filter, having and parts correctly and then read its base without the values.
    **THE FIX MIRRORS THE WALK BESIDE IT.** [materialise_bound_bases] and [materialise_bound_src] are [materialise_laterals] and [materialise_laterals_src] one variant over: at the same fetch hook, where `bound_args` is already in hand, a bound `RowSource::PlanRows` base is materialised into `RowSource::Rows` before the fold pulls from it, walking through the Filter/Sort wrapping exactly as [materialise_procedures_src] does. **Only [Plan::JoinGroup] is walked**: a fold consumes its whole input anyway, so nothing lazy is lost, while a plain `Plan::Join` and a `Plan::Lateral` are deliberately left alone BECAUSE they can stream under `FIRST n`. The comment at the choke point now says what is true instead of what used to be.
    **MEASURED, and the empty-inner twins are a type check as well as teeth:** COUNT, SUM, MAX, AVG, GROUP BY, HAVING and the CTE spelling, each paired with a twin over an inner that selects NOTHING - where SUM answers NULL and COUNT answers 0, so a fold that silently skipped its base would fail one of them whatever it returned. A grouped JOIN over real tables and `FIRST n` over a bound derived table are the controls, and both pass on BOTH binaries: the walk touches a bound DERIVED base and nothing else. The procedure-over-fold shapes were re-measured for a different hazard - [materialise_procedures_src] already substitutes a `ProcSelect` base at op_execute, so the new walk could have run a body TWICE - and `MAX`, `COUNT`, `SUM` and `HAVING` over a call all answer exactly once.
    `serve-real-derived` 81 -> 92 checks; two recorded refusals became eleven live cells. On the previous commit's binary **81 OK / 11 DIFF** - exactly those - with the controls and the three remaining refusals passing on both. 704 unit tests. Ten adjacent gates green, `proclazy` 31 among them because it is what pins a procedure body's LAZY execution against an eager materialisation: castparamgroup 21, paramshapes 177, cte 35, view 34, viewjoin 36, recursive 43, lateral 36, joinderived 25, corrsub 27.
    **STILL REFUSED, and each for its own stated reason:** a JOIN above an inner `?` (a PLAN-time refusal in [plan_join_bound]'s own copy of the old derived-side guard - not this failure at all), a `?` in the INNER projection (nothing binds it - [bind_plan_params] reaches the top-level plan's columns only), and a derived SIDE of a join.
  - **A `?` INSIDE A DERIVED SIDE OF A JOIN - and the recorded plan for it dissolved on contact with the code.** `SELECT X.ID FROM (SELECT ID, DEPT_ID FROM EMP WHERE ID > ?) X JOIN DEPT D ON D.ID = X.DEPT_ID` refused: [plan_join_bound] kept its OWN copy of the old derived guard, planning each side into a FRESH sink and refusing if it claimed anything. The plan recorded across three slices said this needed the statement TEXT threaded in so each side's `?` could be counted by span offset.
    **IT NEEDED NO TEXT AT ALL.** Sides are built in FROM order, and every ON is numbered only AFTER the last of them (`on_np` already floors at `params.len()`), so what a side must number after is exactly what has been claimed so far: `params.len().max(proj_params)` at the moment that side plans. The running count IS the text position. That also disposes of a trap the span plan would have walked into - a CROSS or NATURAL step carries `CROSS_ON`/`NATURAL_ON`, the sentinels `\0cross` and `\0natural`, which are STATIC CONSTANTS and not slices of the statement at all, so pointer arithmetic over ON texts would have computed a garbage offset for exactly the shapes that look most ordinary.
    **THE EXECUTE HALF WAS MANDATORY, and the slice above had just proved it:** a derived side keeps its rows in its own `JoinPart::src`, which reads through the same argument-less choke point as a fold's base, so [materialise_bound_bases] now walks a `Plan::Join`'s base AND every stepped side. It also recurses through `Plan::Modified`, `Plan::Derived` and `Plan::Union`, because a `FIRST`/`SKIP`/`DISTINCT` hid the join from a top-level match: measured, `SELECT FIRST 2 ...` over a join with a bound side failed while the identical statement without `FIRST` answered. [materialise_laterals] recurses through the same three, for the same reason.
    **MEASURED, every predicate paired with a twin that must answer nothing:** a derived side on the left and on the right, two derived sides with a `?` each, a side `?` followed by an ON `?`, a side `?` followed by an outer WHERE `?`, a LEFT JOIN whose right side is derived (its NULL padding intact), a fold over the whole join, and FIRST / SKIP / DISTINCT / a derived table above it. A join of plain tables and an ON `?` with no derived side are the controls and pass on BOTH binaries.
    `serve-real-derived` 92 -> 108 checks; two recorded refusals became fourteen live cells. On the previous commit's binary **94 OK / 14 DIFF** - exactly those - with the controls and all three remaining refusals passing on both. 704 unit tests. Ten adjacent gates green: paramshapes 177, derived 92, recursive 43, viewjoin 36, lateral 36, cte 35, view 34, corrsub 27, joinderived 25, castparamgroup 21.
    **STILL REFUSED, one deliberately and one recorded as older than this work:** a chain of MORE THAN ONE join whose ON carries a `?` - an ON is then written BETWEEN two sides, and this planner numbers every side before any ON, so it would MIS-NUMBER rather than refuse (`multi_on_param` refuses it instead of guessing); and a `?` ANYWHERE IN A UNION BRANCH, which was measured identical on this binary and the one before it and has nothing to do with derived tables - `SELECT ID FROM EMP WHERE ID > ? UNION ALL SELECT 99 FROM RDB$DATABASE` refuses where the same union without a `?` answers.
  - **A `?` IN A UNION BRANCH - two causes in sequence, the second one stale housekeeping the first had justified.** `SELECT ID FROM EMP WHERE ID > ? UNION ALL SELECT 99 FROM RDB$DATABASE` refused: ordinary SQL, no derived table and no procedure in it. [plan_union] planned each branch into a FRESH sink and refused if the branch claimed anything - "a branch claims no parameter slots in this slice", a scope fence like the three before it. Removing it was not enough: a **`params.clear()` after the branch loop** then emptied the sink, so the statement PREPARED, described `INPUT message field count: 0` for a union that has one, and the execute failed on a value for a slot the server had just denied having. That clear was harmless while a branch could not claim - housekeeping over something already empty - and destructive the moment one could. The five other `params.clear()` in the file are correct because each precedes `return plan_query_inner(&rewritten, ..)`, a RE-PLAN that re-claims every slot; this one cleared and carried on building.
    **THE FIX IS THE SAME RUNNING COUNTER AS THE THREE SLICES BEFORE IT.** `parts` is in SOURCE order (the trailing ORDER BY belongs to the whole union and is lifted off the last branch first), so branch k numbers from `params.len()` when it plans - measured 1 slot for one branch, 2 for two, 3 for three, left to right. **The execute side needed nothing, and that was CHECKED rather than assumed** after the two slices where it was mandatory: [validate_select_bind], [branch_rows_res] and the fetch arm's `branch_rows_each`/`branch_rows_res` all already thread the arguments into each branch.
    **MEASURED across eleven cells, each predicate paired with a twin that must answer nothing from its branch:** a `?` in the first branch, in the second, in both, one in each of three branches, a de-duplicating UNION, the union's own ORDER BY, and a DERIVED TABLE, a FOLD and a JOIN inside a branch. The same union with no `?` is the control and passes on both binaries.
    `serve-real-union` 28 -> 39 checks and `serve-real-derived` 108 -> 110. On the previous commit's binary **union 28 OK / 11 DIFF** and **derived 107 OK / 3 DIFF** - exactly the new cells - with every control and the remaining recorded refusals passing on both. 704 unit tests. Ten adjacent gates green: paramshapes 177, derived 108, branchtype 69, cte 35, view 34, uniondecfloat 31, union 28, joinderived 25, castparamgroup 21, unionrep 11.
    **AND THE GATE HAD TO LEARN TO BIND.** `serve-real-union` was isql-only, so it could not express a `?` at all; it gained a node-based parameterized twin carrying the vacuity guard the derived gate earned - a side that does not ANSWER is not agreement. That guard paid for itself on its first run: all eleven new cells came back `engine=[CONN_ERR]`, because the gate's fixture is created by local isql and the engine, which runs as its OWN user, cannot open it over TCP - the isql cells reach it by a local path and never needed the mode. `chmod 666` on the fixture fixed it. Without the guard those eleven would have scored OK by comparing two non-answers, exactly as twelve cells silently did once before.
  - **A PROJECTION `?` UNDER A WRAPPER, AND ONE IN A DERIVED BODY'S SELECT LIST - two families, one blind spot.** `SELECT FIRST 2 CAST(? AS INTEGER) ...` refused, and so did `SKIP`, `DISTINCT` and a `?` projected inside a UNION branch - ordinary statements with no derived table in them - alongside `SELECT C FROM (SELECT CAST(? AS INTEGER) AS C FROM T) X`. [plan_has_proj_param] and [bind_plan_params] both inspected the TOP-LEVEL plan's `cols` and nothing else, so the derived-body case was refused DELIBERATELY (the guards this suite added when the derived body learned to take a `?`) and the wrapper cases simply evaluated nothing.
    **THE OBVIOUS FIX WOULD HAVE BEEN INERT, and the code said so before it was written.** Adding `Plan::Modified` and `Plan::Union` arms to those two functions changes nothing: a modifier's own columns are built `ProjCol { field_id: i, expr: None, .. }`, POSITIONAL over the rows the plan below has ALREADY projected - the fetch path spells that out where it refuses to apply them to base records. The `?` lives in the INNER `Project`'s columns, so the fix had to be RECURSION through `Derived.inner`, `Modified.inner` and `Union.branches`. Checking where the expression actually lives, rather than which variant the plan is, is the whole difference between a fix and a no-op.
    **MEASURED, with the teeth chosen to catch a projection that was never bound:** FIRST, SKIP, DISTINCT and a UNION branch at top level; an inner projection `?`, one beside a real column under an inner `WHERE ?`, its excluding twin, a CTE body's, a nested derived body's, and `SELECT CAST(? AS INTEGER) AS O, C FROM (SELECT CAST(? AS INTEGER) AS C ...)` answering `(7, 9)` - two cells that would SWAP if the inner body's slot were numbered before the outer projection's. One cell is honestly a CONTROL rather than a flip: `FIRST` with a `WHERE` that excludes every row passes on BOTH binaries, because a projection over no rows never evaluates.
    **STILL REFUSED, and one cause between them:** a FOLD over an inner projection `?`, and a JOIN SIDE carrying one. [bind_plan_params] walks a plan's FIELDS and never a `RowSource::PlanRows` - which is exactly where a fold keeps its base and a join keeps a derived side. The join-side guard was left in place deliberately and MEASUREMENT decided it stayed, rather than a guess either way.
    `serve-real-castparam` 18 -> 23 checks (it binds through node now: its C rig ships one scalar and cannot express these), `serve-real-castparamderived` 11 -> 18, `serve-real-derived` 110 -> 113. On the previous commit's binary **castparam 18 OK / 5 DIFF**, **castparamderived 11 OK / 7 DIFF** and **derived 110 OK / 3 DIFF** - exactly the new cells - with every control and both recorded refusals passing on both. 704 unit tests. Ten adjacent gates green across every other projection-parameter path: paramshapes 177, union 39, lateral 36, cte 35, view 34, joinderived 25, castparamgroup 21, castparamt 16, castparamjoin 9, castparamtext 9.
  - **A FOLD'S BASE AND A JOIN'S DERIVED SIDE - the third and last place a plan can hide.** With a projection `?` bound through plan fields and through the wrappers, two shapes still failed: `SELECT SUM(C) FROM (SELECT CAST(? AS INTEGER) AS C FROM T) X` and the same derived body used as a JOIN side. They fail DIFFERENTLY, which is what named the cause: the fold PREPARED, had its op_execute ACCEPTED and died at the FETCH (`fetch plan = JoinGroup`, no refusal line anywhere), while the join side refused at PLAN time - through the guard left in place last slice precisely so measurement, not a guess, would decide whether it retired.
    **ONE CAUSE, ONE FIX.** A fold keeps its base and a join keeps a derived side in a `RowSource::PlanRows`, which is not one of the plan FIELDS [bind_plan_params] walks - so the inner `Project`'s columns still held a `Param` when [branch_rows_res] evaluated them with `value_of`. [materialise_bound_src] now binds that plan's projection before materialising it, and the join-side guard is gone. An unbindable projection leaves the source alone rather than inventing rows, exactly as its neighbouring error arm does.
    **THE BLIND SPOT HAD THREE LEVELS, and this finishes them:** a plan's own fields, a wrapper's `inner` / a union's branches, and a row source. Each was invisible from the one before, and each needed the same question asked again - WHERE can the plan hide? - rather than a new mechanism.
    **MEASURED:** SUM, COUNT, MAX, AVG, GROUP BY and HAVING over an inner projection `?`; a derived side carrying one on the left, on the right, under `FIRST`, and with a fold above the join; and a side whose WHERE excludes every row answering NOTHING. Two cells are honestly CONTROLS rather than flips - a fold over an EMPTY inner and a literal inner projection both pass on either binary, because a projection over no rows never evaluates.
    `serve-real-castparamderived` 18 -> 26 checks and `serve-real-derived` 113 -> 116. On the previous commit's binary **castparamderived 17 OK / 9 DIFF** and **derived 112 OK / 4 DIFF** - exactly the flipped cells - with every control and the remaining multi-ON refusal passing on both. 704 unit tests. Twelve adjacent gates green: paramshapes 177, union 39, lateral 36, cte 35, view 34, proclazy 31, joinderived 25, castparam 23, castparamgroup 21, castparamt 16, castparamjoin 9, castparamtext 9.
  - **THE CONCAT DOUBLE-ENCODE, RE-MEASURED AND RECORDED - NOT ATTEMPTED.** It is the only known open WRONG ANSWER, so by the ranking it outranks every refusal; it is also the one item carrying a fix that was ATTEMPTED AND WITHDRAWN, and re-measuring changed what the defect even is. **The first probe of it this session was VACUOUS and nearly banked as a pass:** driven with ASCII data (`'e' || U`) it went 15/15 SAME, because ASCII transliterates identically in every charset and cannot express a double-encode at all.
    **RE-MEASURED with a non-ASCII literal, in CHAR_LENGTH/OCTET_LENGTH because rendered text hides bytes**, across three attachments: under NONE, `'é' || U` and `U || 'é'` are engine 3/4 and fire-crab **4/6** - the literal doubled in BOTH orders; under WIN1252 only the literal-FIRST order diverges (engine 4/4, fc 4/6) while `U || 'é'` agrees, which is the "first operand decides" law showing through; and under UTF8 the error runs the OTHER WAY - `'é' || W` is engine 3/4 against fc **3/3**, and both OCTETS orders are engine 4/4 against fc 3/3.
    **THE DECISIVE NEW FACT: THE ANNOUNCED DESCRIPTOR AGREES ON ALL EIGHTEEN CELLS** - sqltype, VARYING and charset identical on both servers under every attachment - and so does every one of the twelve column||column pairs. So this is a VALUE defect in how operands are transcoded and joined, not the width algebra. That INVERTS what the withdrawn attempt steered by: it broke on ANNOUNCED widths (`UTF8(32) || OCTETS(32)` announcing 64 where the engine says 160) while the values stayed byte-identical.
    **WHY IT IS RECORDED RATHER THAN SWUNG AT A FOURTH TIME.** A literal carries TWO widths - [text_form] counts characters, [text_form_oct] counts the literal's own bytes through `carrier_encode` - and [resolve_text_cs] chooses between them BY THE ATTACHMENT rather than by the charset the expression resolves to, with [cs_join]'s first-operand rule on top. Three widths, one join rule, and the divergences pull in OPPOSITE directions (doubling under NONE/WIN1252, under-widening under UTF8/OCTETS). No rule predicts all eighteen cells yet, and the withdrawal record is explicit that three successive `text_form` designs each merely moved the failures between families - which it names as the signal that the rule is not pinned. **The ranking rule does not oblige attempting a defect that cannot yet be STATED.**
    `serve-real-concatcs` 44 -> 52 checks: five self-tripping `known_len_diff` cells - they FAIL if the divergence silently starts agreeing - beside three ordinary `both` cells pinning the column||column pairs the withdrawn attempt broke, so a future fix cannot trade one family for the other unnoticed. Watch-set baseline recorded with it: concatcs 44, blobexpr 53, castnone 18, litcs 78, xlit 58, cscast 42, all 0 DIFF.
  - **A MULTI-ON CHAIN WITH A BOUND ON - narrowing a guard I had written too wide myself.** The derived-side slice refused every chain of more than one join in which ANY `ON` carried a `?`, on the reasoning that an ON is written BETWEEN sides while this planner numbers all sides before any ON. That reasoning is right; the test for it was not. Slots number by TEXT POSITION - `s0, s1, on0, s2, on1, ...` - so the two orders agree UNLESS a side written after an ON claims a slot while that ON claims one too. Only a DERIVED side can claim one (a plain relation has no `?`), and the sides after the first ON are those at index >= 2, with side `i` introduced by `joins[i - 1]`. The narrowed test is exactly that: a derived side at index >= 2 carrying a `?`, with some `ON` in `joins[0 ..= i - 2]` carrying one.
    **THE PREDICTION COULD ONLY BE TESTED BY NARROWING THE GUARD,** because while it refused every shape alike, fire-crab's numbering for the shapes it refused was unobservable - a refusal hides the very order under test. That is a legitimate move only with the fallback stated in advance: if a newly admitted shape ANSWERED but disagreed with the engine, revert rather than ship, since a wrong answer replacing a refusal is never the trade. It answered correctly, on every shape and both teeth.
    **MEASURED, before and after:** a derived side ahead of two bound ONs (three slots), a `?` in the first ON only and in the second ON only (two slots each), and two derived sides ahead of two ONs (four slots) all now answer and match the engine's slot counts; the one arrangement that would SWAP two slots - a derived side written AFTER an ON that carries its own `?` - still refuses, and is recorded with the engine's answer beside a twin whose values discriminate the order. Each admitted cell is paired with a twin that must answer NOTHING, one excluding through the last ON and one through the derived side itself.
    **THE RULE IS STRUCTURAL, NOT POSITIONAL,** which is what makes it safe: it reads join INDICES rather than comparing span pointers, so the `CROSS_ON` / `NATURAL_ON` sentinels - `\0cross` and `\0natural`, static constants rather than slices of the statement - cannot contribute a garbage offset the way a span walk would have.
    `serve-real-derived` 116 -> 122 checks. On the previous commit's binary **116 OK / 6 DIFF** - exactly the six newly admitted cells - with the narrowed refusal kept on both. 704 unit tests. Twelve adjacent gates green: paramshapes 177, union 39, lateral 36, viewjoin 36, cte 35, view 34, proclazy 31, corrsub 27, castparamderived 26, joinderived 25, castparam 23, castparamgroup 21.
  - **A `WITH LOCK` OVER A TARGET THE ENGINE WILL NOT LOCK - one wrong answer, one over-refusal, and the four vectors neither of them carried.** `SELECT DISTINCT <col> FROM T WITH LOCK` ANSWERED ROWS where the engine refuses outright - the failure direction this suite ranks above every refusal - while an all-`UNION ALL` chain of plain tables REFUSED where the engine locks and answers. [with_lock_target_ok] tested a GROUP BY, an aggregate in the projection and a single bare base-relation FROM; it had no DISTINCT test at all, and `split_query` answers None for a union, so a chain never reached its rule.
    **THE CONTRACT, measured cell by cell off the live engine.** It locks a single PHYSICAL base table - not a view, join, derived table or CTE - with no aggregate, no GROUP BY and no DISTINCT, or an all-`UNION ALL` chain of exactly those (8 rows over two branches, 13 over three). Everything else is one of three -104 DSQL errors: `isc_dsql_wlock_simple` (336397326) "can be used only with a single physical table", `isc_dsql_wlock_aggregates` (336397328) "cannot be used with aggregates", and `isc_dsql_wlock_conflict` (336397329) "cannot be used with @1", whose `@1` is `DISTINCT` or `UNION`. **All four render as exactly FOUR isql lines with NO trailing line/column item** - unlike every other -104 this server answers ([respond_name_too_long] ships one), which is why they got their own arm instead of reusing a neighbour's shape.
    **WHEN A STATEMENT BREAKS SEVERAL RULES THE ENGINE REPORTS ONE, and the order is not the obvious one:** UNION > single-physical-table > aggregates > DISTINCT. `SELECT DISTINCT ... JOIN` and `SELECT COUNT(*) ... JOIN` both report the TABLE rule rather than their own; `SELECT DISTINCT COUNT(*)` reports aggregates; a bare UNION reports UNION even when one of its branches is a join, and a chain MIXING `UNION ALL` with a bare `UNION` reports UNION in EITHER order. Inside an all-ALL chain the FIRST bad branch left to right supplies the message - `COUNT(*) UNION ALL DISTINCT` is aggregates, `DISTINCT UNION ALL COUNT(*)` is DISTINCT, `DISTINCT UNION ALL <join>` is DISTINCT.
    **THE DISTINCT TEST IS NEITHER OF THE TWO OBVIOUS ONES.** Not a leading-word test - the idiom used elsewhere in this file - because `FIRST` and `SKIP` precede the keyword: `SELECT FIRST 2 SKIP 1 DISTINCT ID ... WITH LOCK` refuses with the DISTINCT message, `FIRST (1 + 1) DISTINCT` does too, and `DISTINCT FIRST` is a syntax error. A leading-word test MISSES all three and lets fire-crab answer rows the engine refuses, which is the very defect under repair. Not a bare word search either: `x IS [NOT] DISTINCT FROM y` writes the same keyword at depth 0 of a select list and ANSWERS, parenthesised or not, so the preceding word vetoes the match; `COUNT(DISTINCT x)` sits at depth 1 where the aggregate test has already claimed it (and the engine agrees - it reports aggregates for that one); and a `'DISTINCT'` LITERAL answers, so the scan runs on masked text.
    **ADMITTING THE UNION CHAIN WHOLESALE WOULD HAVE TRADED THE OVER-REFUSAL FOR A WRONG ANSWER.** An all-ALL chain under a depth-0 ORDER BY makes the ENGINE ANSWER ZERO ROWS - measured on two separate databases against three controls (8 rows without the ORDER BY, 8 without the lock, 0 with both), and 0 for DESC and for three branches too, while the single-table `ORDER BY ... WITH LOCK` answers its 5. Nothing here reproduces that, so that one shape REFUSES, and refuses WITHOUT a vector, because the engine posts none to copy. It is a self-expiring cell carrying both sides' behaviour rather than a silent omission.
    **A SECOND, WIDER DEFECT SURFACED WHILE GATING THE FIRST, AND IT WAS OLDER THAN THIS CHUNK.** With the rule in place the vectors reached the client for a plain aggregate, a view and a join - and NOT for DISTINCT, a CTE, a UNION or any branch cell, which still printed a bare `Dynamic SQL Error`. The decisive pair is `SELECT COUNT(*) FROM T WITH LOCK` against `SELECT DISTINCT COUNT(*) FROM T WITH LOCK`: the same `EvalErr`, built by the same line, one typed and one bare. **A sub-planner may answer `Some(Plan::Refused)` rather than `None`** - a wrapper refusing by returning a generic refusal PLAN - and that never reaches the arm where a posted [PREPARE_REFUSAL] is consumed, so ANY typed prepare refusal under a wrapper arrived stripped of its vector. It is taken now over a BARE `Plan::Refused` as well, beside the [first_unknown_relation] upgrade that already solves the same problem for -204, and only over a bare one, so a typed refusal is never overridden.
    **THREE HYPOTHESES DIED BY READING; TWO `eprintln!`s SETTLED IT IN ONE RUN.** A nested `plan_query` clearing the cell (its three re-entrant call sites are two MERGE sources and `op_exec_immediate` - none on a plain SELECT's path), the take inside the `SET TIME ZONE` planner, and the DML branch's clear were each eliminated by reading, which cost several turns. Instrumenting BOTH ENDS - where the refusal is set and where it is consumed - printed "set" for both statements and the consumption probe for NEITHER, which says the take arm was never reached at all. **When a value is SET and later MISSING, probe the SET and the TAKE, not the paths between them.**
    `qa/serve-real-rowlock.sh` 5 -> 55 checks, and its refusal cells became FULL-TEXT comparisons: they counted `grep -ci error` before, which cannot tell a bare `Dynamic SQL Error` from the engine's named vector and would have passed whatever this chunk emitted. On the previous commit's binary **18 OK / 37 DIFF**. 161 ods + 346 wire unit tests. Ten adjacent gates green, chosen by CODE PATH - the wrappers that answer `Some(Plan::Refused)` and the gates asserting typed vectors: derived 122, describe 116, nofallback 74, slottype 73, modifiers 66, union 39, cte 35, readonly 34, unionrep 11, tzdml 8.
    **AND THE ADJACENT SWEEP LIED THE FIRST TIME IT WAS RUN.** Five of those ten measured NOTHING: without `NODE_PATH` their node twins cannot connect, print `CONN_ERR` on both sides, and `CONN_ERR == CONN_ERR` scores a vacuous OK - `cte` reported 35 green with ZERO of its OK lines free of `CONN_ERR`, `describe` 115 the same way, and `slottype` exited 0 having skipped every cell. **A clean DIFF count is not proof a gate ran; count the OK lines that are NOT `CONN_ERR`.** Two background waiters then burned 30 and 40 minutes because `pgrep -f 'serve-real-'` MATCHES THE WAITER'S OWN COMMAND LINE, and a `pkill -f` written to clean up killed the tool call itself - the self-match pitfall already recorded for `fcwire`, which applies to any pattern that appears in your own command.
  - **`SELECT ... FOR UPDATE` IS STATEMENT TYPE 12, NOT 1 - and re-measuring the stored contract caught two errors in it before a line was written.** The flags chunk recorded this one as "not fixed: the type itself needs the plan to carry a for-update marker". Measured now, 15 of 22 shapes diverged: fire-crab announced plain select (1) for EVERY `FOR UPDATE` statement, where the engine announces `isc_info_sql_stmt_select_for_update` (12). A client dispatches on this word, which is what made it worth a chunk rather than a footnote.
    **THE CONTRACT, re-measured rather than trusted - and the stored one was WRONG TWICE.** A depth-0 `FOR UPDATE` types the statement 12: `FOR UPDATE OF <col>`, lower case, after an `ORDER BY` or a `ROWS`, beside a `WITH LOCK`, and over a star projection, an aggregate, a GROUP BY, a join, a view, a derived table or a CTE alike - and unlike `WITH LOCK` it NEVER refuses. Two things cancel it back to 1, and only at the TOP LEVEL: a `DISTINCT` on the statement's own projection, and a depth-0 BARE `UNION` (a `UNION ALL` stays 12, and 12 for three branches, while a chain MIXING the two cancels in EITHER order). The stored contract said a top-level DISTINCT and a bare UNION cancel, and both survived; what it did not say, and what the draft rule got wrong, is that **a `DISTINCT` in EITHER branch of a `UNION ALL` chain does NOT cancel** - the draft read [split_query]'s projection, which for a chain is the FIRST BRANCH'S, and would have cancelled `SELECT DISTINCT ... UNION ALL ...` to 1 - and that **a `WITH` statement is typed by its MAIN select**: a plain CTE is 12, `WITH C AS (...) SELECT DISTINCT ... FOR UPDATE` is 1, and a `DISTINCT` inside the CTE BODY leaves it at 12. Neither branch had ever been probed; the draft would have shipped a guess on both.
    **THE DISTINCT READER CAME FREE FROM THE PREVIOUS CHUNK, and it is the reason a leading-word test will not do.** `SELECT FIRST 2 DISTINCT ID ... FOR UPDATE` and `SELECT SKIP 1 DISTINCT ID ... FOR UPDATE` both cancel to 1, while `DISTINCT FIRST` is a syntax error - exactly the shapes [proj_distinct] was written for one chunk earlier, where the same keyword decided a `WITH LOCK` refusal. `COUNT(DISTINCT x)`, a `'DISTINCT'` LITERAL and `IS [NOT] DISTINCT FROM` are all still 12, and `FIRST`/`SKIP` alone are too.
    **THE MARKER NEEDED NO NEW STATE - and the first design conclusion was wrong.** `strip_row_locking` CUTS the clause off before planning (deliberately: the rows are the plain query's), so the plan cannot carry it, and both type emitters take only a `&Plan`. Reading the info arm suggested the statement text was out of reach there, which pointed at a new per-handle map; `use_stmt!` then showed that [switch_stmt] already saves and restores `stmt_sql` in its `StmtSlot` ALONGSIDE the plan. So the text has exactly the plan's per-handle fidelity, and the fix is one bool derived from it at each emitter - no `Plan` variant, no walker touched, and nothing that can go stale between statements. [stmt_type_of] keeps its signature and its six unit tests; the promotion lives in [stmt_type_announced]. The FLAG word needs no promotion at all: measured, a `FOR UPDATE` select answers 3 exactly as a plain one does, and [stmt_flags_of] already counts 12 as a cursor type.
    The depth-0 `FOR UPDATE` scan is FACTORED OUT of [strip_row_locking] into [find_depth0_for_update], so the clause has ONE reader for the two things that need it - the strip that removes it and the rule that types by it. That is the two-emitter drift the flags chunk had to repair once already.
    **A TYPE BATTERY NEEDS ITS OWN VACUITY GUARD, because the flag word cannot tell these cells apart.** Every cell compares the `(type, flags)` PAIR and the flags are 3 for a plain select and a `FOR UPDATE` one alike, so a battery that never saw a 12 would agree with a server that has never heard of the clause. The gate now fails unless the ENGINE answered a 12 - beside the existing guard that the flags spanned 0/2/3.
    `qa/serve-real-pydriver.sh` 41 -> 65 checks (its type/flags battery 28 -> 50 cells). On the previous commit's binary **48 OK / 17 DIFF**, and the 17 are EXACTLY the seventeen cells the engine types 12. The five CANCEL cells - a top-level DISTINCT, DISTINCT after FIRST, after SKIP, a bare UNION, and a CTE's top-level DISTINCT - pass on BOTH binaries, because both servers answer 1 there: they are controls, not passengers, and counting them as fixes would overstate the chunk. 161 ods + 347 wire unit tests, the new one asserting all 28 measured cells of the rule. Twelve adjacent gates green, chosen by code path - the prepare/info describe surface this threads a parameter through: bindcs 219, paramshapes 177, describe 116, icucoll 89, nofallback 74, slottype 73, subqdesc 70, psql 68, rowlock 55, attimezone 49, returning 44, rollback 31.
    **RECORDED, NOT FIXED, and measured to be none of this law's business:** a chain that MIXES `UNION ALL` with a bare `UNION` is refused by fire-crab OUTRIGHT - [split_union] answers None for a mixed chain, so the planner never reaches a shape it can plan and the statement cannot be typed at all - while the engine prepares it as a plain select. It behaves identically WITH and WITHOUT the `FOR UPDATE`, and on the binary before this slice, with an all-ALL chain and a bare-UNION-only chain agreeing on both servers as the controls. The type rule would answer 1 there; it is simply never asked. A self-expiring cell carries it.
  - **A BYTE-CARRIER `LIKE` PATTERN AGAINST A REAL-CHARSET OPERAND - the last predicate left out of the byte-space law, and the one the code itself had deferred.** `f692502` taught equality, `STARTING WITH`, `CONTAINING` and `SIMILAR TO` to reinterpret a NONE-attachment literal into the other side's charset, and recorded in [adopt_carrier_literal]'s own doc why LIKE was excluded: *"the engine SPLITS it - `<utf8> LIKE '<accented>%'` RAISES 22000 Malformed string while `LIKE '%<accented>%'` and an exact `LIKE '<accented>'` ANSWER - a raise/answer predicate that also turns on the column's byte width, not cheaply reproduced; recorded for its own measured chunk"*. This is that chunk. fire-crab found NOTHING for every non-ASCII pattern (`u LIKE '%é%'`, `'%é'`, an exact `'café'`, `'%café%'` and `'c%é%'` all 0 rows where the engine matches), and answered 0 where the engine RAISES - a silent wrong answer in both directions.
    **THE PREDICATE THE DEFERRAL WAS WAITING FOR, and it is pattern shape alone:** the engine raises 22000 exactly when the pattern's LEADING LITERAL SEGMENT - the run before the first UNESCAPED wildcard - both EXISTS and ENDS IN A MULTI-BYTE CHARACTER. Raise: `'é%'`, `'café%'`, `'é_'`, `'é_%'`, `'漢%'`, `'漢字%'`, `'漢_%'`. Answer: `'éx%'` and `'漢字t%'` (the segment ends ASCII), `'caf%'` and `'c%é%'` (likewise), `'%é%'`/`'%é'`/`'_é'`/`'%café%'`/`'%%é%'`/`'%é%é%'` (a wildcard comes FIRST, so the segment is empty), and `'café'`/`'é'`/`'éclair'`/`'漢字table'` (NO wildcard at all - the whole pattern converts as one piece and validates). Thirty-three cells, all predicted, over 2-byte and 3-byte characters with the accented char first and mid-value.
    **AND THE DEFERRAL'S OTHER HALF WAS WRONG.** The byte width it named does NOT enter into it: `'é%'` raises identically over VARCHAR(20), VARCHAR(5), VARCHAR(4) and CHAR(5). Nor does the change land in the transliteration core the earlier entry predicted - [coll_value_cmp] keys only PXW_INTL and the ICU family and a byte-carrier operand never reaches it. What it needed was one arm in [adopt_carrier_literal] beside the four predicates already there, one guard at the term site, and an expression-path twin.
    **THREE BEHAVIOURS, all measured, and the raise is the narrow one:** the pattern reinterprets for LIKE and NOT LIKE alike; only the POSITIVE, LITERAL form raises (`NOT LIKE 'é%'` answers `[1,3]` and a BOUND `? = 'é%'` answers `[2]`, both where the literal raises); and the raise is PER ROW, not at prepare - `1 = 0 AND u LIKE 'é%'`, the reverse order, an empty table, an OR-guard and a NULL-only row all answer with no raise, so it is a TERM ([Term::MalformedLike], shaped on [Term::BadSimilar] because the semantics are identical - value-independent raise, NULL as UNKNOWN, suppressed by a FALSE written before it). Mirroring that variant collapsed six walker touch points to three.
    The EXPRESSION path needed its own half: `u || '' LIKE '%é%'` and `UPPER(u) LIKE '%É%'` matched on the engine and found nothing here, and `u || '' LIKE 'é%'` raised - so [carrier_expr_like] reads the operand's charset through [cmp_text_charset] where there is no column descriptor. A `CAST(u AS VARCHAR(n))` operand already agreed (its own descriptor makes both sides real) and is the control.
    `qa/serve-real-nonecmp.sh` 17 -> 41 checks - the gate `f692502` shipped for this law. Its raise cells needed a STRICTER helper than the gate had: `sig()` maps every failure to the single word `REFUSE`, so two DIFFERENT errors would have scored as agreement; `malformed()` matches the message. On the previous commit's binary **25 OK / 17 FAIL** - exactly the seventeen new cells, twelve matches and five raises. 161 ods + 348 wire unit tests, the new one asserting all thirty-three probed cells of the predicate. Fifteen adjacent gates re-run against recorded baselines and unmoved, the ones the earlier entry named as this area's guard among them: textcolcmp 359, leftjoinindex 169, subqindex 123, litcs 78, xlit 58, blobexpr 53, concatcs 52, joinexpr 46, charset 43, cscast 42, octets 36, textfit 24, blobcast 21, castnone 18, noneblob 15.
    **STILL OPEN, measured and recorded rather than swept in:** `POSITION`, `REPLACE` and `TRIM` with a non-ASCII needle, and `CONTAINING` over a BLOB - the same root on the SysFn/blob paths (`POSITION('é' IN u)` is engine 4 / fc 0, `REPLACE(u,'é','e')` 4 / 5, `TRIM(TRAILING 'é')` 3 / 5, and the mirror direction under a UTF8 attachment where the NONE COLUMN is the carrier); and a BOUND LIKE pattern, which cannot reinterpret at bind because `Predicate::bind` takes no descriptors - its charset would have to travel in the term from resolve time. Separately noted: fc refuses `OVERLAY(... PLACING ... FROM ...)` outright (42S02) on this binary and the one before it, unrelated to charsets.
  - **THE STRING FUNCTIONS AND A TEXT BLOB TAKE THE SAME BYTE-SPACE LAW - `POSITION`, `REPLACE`, `TRIM` and a blob `CONTAINING`.** With equality, `STARTING WITH`, `CONTAINING`, `SIMILAR TO` (`f692502`) and `LIKE` (the entry above) reinterpreting a byte-carrier literal into the other side's charset, the search FUNCTIONS still compared carrier chars against real ones and found nothing: under the default NONE attachment against a UTF8 column, `POSITION('é' IN V)` answered 0 where the engine says 4, `POSITION('café' IN V)` 0 against 1, `POSITION(V IN 'xcafé')` 0 against 2, and `REPLACE(V,'é','e')` and `TRIM(TRAILING 'é' FROM V)` returned the string UNCHANGED (5 octets) where the engine returns 4 and 3 - the needle never matched. `B CONTAINING 'é'` over a text BLOB was 0 against 1.
    **ANY ARGUMENT MAY BE THE ONE THAT NEEDS IT, so the rule is per-argument rather than per-position:** the literal is the NEEDLE in `POSITION('é' IN V)` and the CONTAINER in `POSITION(V IN 'xcafé')`, and `REPLACE` carries a third text operand. The argument ORDER was read rather than assumed - `position_impl(sub, s, start)`, `REPLACE(s, find, repl)`, and `TRIM`'s pad is `vs[0]` in the two-argument form while `vs[0]` is the SOURCE in the one-argument form (`TRIM(TRAILING FROM s)`, whose pad resolution fills in the charset's own space). A blind "rewrite argument 0" would have corrupted the source string in exactly the shape under repair.
    **THE BLOB REACHES IT BY A DIFFERENT ROUTE, which is why it was still failing after the LIKE slice:** `col_kind` answers `None` for a blob, so [adopt_carrier_literal] - the literal fast path keyed on a column descriptor - never sees one. A blob column lands on the EXPRESSION arm instead, where `text_form` already yields its charset (a text blob carries that in the descriptor's SCALE, not `sub_type`, which [blob_charset] reads); the arm now takes the same single transcode before [containing_term].
    So one uniform step at resolve covers all of it: [carrier_fn_args] rewrites every `Expr::Str` argument of a string function when exactly one REAL charset is in play among its siblings, and leaves a call with two different real sets alone rather than guessing between them.
    `qa/serve-real-nonecmp.sh` 41 -> 62 checks, and its fixture gained a text BLOB through `ALTER TABLE ADD` + `UPDATE` so the blob route is gated too - which also showed fire-crab reads a table carrying a SECOND RECORD FORMAT without trouble. On the previous commit's binary **52 OK / 10 cell failures** (the raw `FAIL` count is 11: the gate's own summary line is one of them). 161 ods + 348 wire unit tests - no unit test for [carrier_fn_args] itself, which is descriptor- and attachment-dependent, so the differential gate is its test rather than a mocked-up descriptor. Eighteen adjacent gates re-run against recorded baselines and unmoved: textcolcmp 359, leftjoinindex 169, subqindex 123, functions 80, litcs 78, xlit 58, blobexpr 53, concatcs 52, wherefn 51, funcwidth 48, joinexpr 46, charset 43, cscast 42, octets 36, textfit 24, blobcast 21, castnone 18, noneblob 15.
    **THE CONTROLS ARE LABELLED HONESTLY, including one I predicted wrong.** `POSITION('f' IN V)`, `TRIM(LEADING 'c' FROM V)`, `b LIKE '%é%'` and `b CONTAINING 'é'` under a UTF8 attachment all pass on BOTH binaries, and so does `POSITION('é' IN w)` over the WIN1252 column - I had predicted that last one would flip, and it does not, because `C3 A9` read as WIN1252 is not `E9`, so both servers correctly find nothing. It is a control, not a fix.
    **STILL OPEN, measured after this change rather than assumed:** the MIRROR direction, where the CARRIER is the COLUMN and the literal is real - under a UTF8 attachment `POSITION('é' IN N)` is 4/0, `POSITION(N IN 'xcafé')` 2/0, `REPLACE(N,'é','e')` 4/7, `TRIM(TRAILING 'é' FROM N)` 3/5 and `N CONTAINING 'é'` 1/0 ([carrier_fn_args] guards on the ATTACHMENT being a carrier, so it cannot reach that direction; `N LIKE '%é%'` agrees there, observed and not yet explained); and COLUMN-vs-COLUMN, where no literal exists to rewrite at all - `POSITION(N IN V)` and `POSITION(V IN N)` 1/0, `TRIM(TRAILING N FROM V)` 0/5, `REPLACE(V,N,'x')` 1/5 under BOTH attachments, plus three shapes fire-crab REFUSES rather than answers wrongly (`V CONTAINING N`, `N CONTAINING V`, `V LIKE N`). Those values must meet in byte space at EVAL, which is the comparison-core work the 2026-09-08 entry imagined and a slice of its own.
  - **THE MIRROR DIRECTION - ATTEMPTED TWICE, WITHDRAWN BOTH TIMES, and RECORDED with its full contract.** The byte-space law has one direction left: a byte-carrier COLUMN meeting a REAL literal, under a REAL attachment. Measured, nine cells, every one a wrong answer: over a NONE VARCHAR column `POSITION('é' IN nn)` is engine 4 / fc 0, `POSITION(nn IN 'xcafé')` 2/0, `REPLACE(nn,'é','e')` 4/7, `TRIM(TRAILING 'é' FROM nn)` 3/5 and `nn CONTAINING 'é'` 1/0; over an OCTETS CAST `POSITION('é' IN octets)` 4/0, `octets CONTAINING 'é'` 1/0, `REPLACE(octets,'é','e')` 4/5; and over a PADDED OCTETS cast `POSITION('é' IN padded)` 4/0. All nine AGREE under a NONE attachment - two byte carriers need no reconciliation - which is what makes them controls rather than passengers.
    **ATTEMPT ONE WAS INERT, and the cause is a primitive I reimplemented instead of reusing.** [carrier_fn_args] was re-keyed on the OPERANDS (`byte_carrier(att) != byte_carrier(sibling)`, which is [cmp_sides]' own test) and the literal re-spelled as the carrier of its own bytes. It changed nothing: `intl::encode_text` answers `Ok(None)` for any set that is not TABLED - UTF8 among them - and my hand-rolled conversion took only the `Some(bytes)` branch, so the literal passed through untouched. [Expr::CarrierEnc]'s own evaluator has exactly the missing `unwrap_or_else(as_bytes)` fallback, three lines away. **The gate caught it; reading had not.**
    **ATTEMPT TWO FIXED THE PRIMITIVE AND REGRESSED A CORRECT ANSWER.** With the fallback restored and [adopt_carrier_literal]'s guard re-keyed the same way, EIGHT of the nine cells flipped to agreement - and `nn LIKE '%é%'` under a UTF8 attachment, which had AGREED on both servers, began to differ. A correct answer turned wrong, which this suite ranks below every refusal and below doing nothing at all.
    **WHY IT IS RECORDED RATHER THAN PATCHED.** The LIKE regression says the rule is not pinned: LIKE evidently reaches the mirror correctly by some other route, and forcing its literal into carrier space breaks that route. A one-line `RawKind::Like` exclusion would probably have closed it - and declining that is deliberate. The revert-if-not-all-nine condition was written down BEFORE the run, precisely so the decision would not be made by the result; two attempts, inert and then regressing, is the same signal this file already records for the CONCAT double-encode, where three designs each moved failures between families. Also still unexplained after attempt two: `octets CONTAINING 'é'` never flipped at all - an OCTETS CAST is an expression, so it takes the expression `CONTAINING` arm rather than the literal fast path.
    `qa/serve-real-nonecmp.sh` 62 -> 77 checks, and the fixture gained a NONE column (by `ALTER TABLE ADD`, since the `insert ... values` list carries no column list) plus the OCTETS casts. The nine cells are SELF-EXPIRING through a new `differs` helper: each passes only while the two servers still disagree, FAILS the moment they agree - the signal to promote it back to `agree` - and FAILS if either side does not answer, because `sig()` collapses every error to the single word REFUSE and two REFUSEs must never score as a difference. `nn LIKE '%é%'` is kept as an EQUALITY cell for exactly the regression above, so a future attempt cannot break it silently. **No server change landed, so there is no teeth run and no baseline sweep to report: the code is byte-identical to the commit before it, whose eighteen adjacent gates were verified clean.** 161 ods + 348 wire unit tests.
  - **A WINDOW INSIDE A ROW SOURCE - the fold existed, the SPINE refused it.** `SELECT W FROM (SELECT SUM(V) OVER (PARTITION BY G) AS W FROM T) Z` answered a bare `42000 / Dynamic SQL Error` where the engine answers, and the refusal was in none of the places a refusal is normally looked for. The TRACE is what settled it, after three fruitless reads of the planner: the statement PREPARED, BOTH levels PLANNED (the outer projection, then the inner `SUM(V) OVER (...)`), the fetch was ENTERED (`fetch plan = Derived(Project)`), and **no `prepare refused` line was emitted at all**. The site is `branch_rows_res` - the row-source spine that every nested shape draws from - which refused a windowed `Plan::Project` with `EvalErr::Unsupported` (hence the contentless `42000`), under a comment naming "a derived table or a CTE level" as a deliberately later slice; a twin refusal sat in the streaming variant `branch_rows_each`. THE FIX ADDS NO SECOND FOLD: the spine calls the SAME `fold_project_windows` the batch fetch uses, whose order is load-bearing (scan with an empty sort key, fold each window over its partition, and only THEN sort), and projects through `cols` exactly as the Join arm beside it does. The streaming twin DELEGATES to the collecting path instead of growing its own, because a fold must see the whole partition before it can answer any row - a windowed projection is a BLOCKING shape, which is what that path is for. `Plan::Derived` already carried `windows`/`win_base` and already folded them; only the inner Project had no source, so the plan representation needed no change (46 lines added, 13 removed). MEASURED AGAINST THE ENGINE, 16 shapes agreeing and 30 gate cells flipping on the previous binary: a derived table (running SUM, PARTITION BY, ROW_NUMBER, LAG, RANK ties, two windows, a ROWS frame), outer WHERE / ORDER BY / DISTINCT / FIRST / aggregate / GROUP BY over one, derived-over-derived, a CTE level, a CTE joined to itself, a union BRANCH and both branches, **a VIEW carrying a window** (not predicted - the probe found it), a filtered VIEW, scalar / IN / EXISTS subqueries, a JOIN's derived side and a LEFT JOIN's, a `FOR SELECT` loop, and `INSERT ... SELECT` read back from the table. **STILL REFUSED, characterised exactly rather than left vague: a window whose OWN row source is not a single relation** - over a JOIN directly, over a derived table WRAPPING a join, or over a RECURSIVE CTE's rows - because the fold hangs off `Plan::Project` and `Plan::Join` carries no `windows` field; plus a window mixed with GROUP BY. **AND ONE NEW SLICE, found by a cell I added and separated by three controls:** `FOR SELECT` + derived table + window never reaches the spine at all - it dies in the TEXTUAL splitter (`split_query failed`, then `prepare refused`, reported at `server.rs:49140`), while `FOR SELECT` over a derived table with NO window, `FOR SELECT` with a window DIRECTLY, and a plain `SELECT` of that same derived table all work. All five are law-safe refusals, recorded by a new self-expiring `known_diff` helper that FAILS if they ever start agreeing. Four shapes are kept as named CONTROLS because they already worked on the previous binary and this change must not be credited with them: a selectable procedure, a window over a plain CTE, a window over a derived UNION, and a recursive CTE with the window outside. `qa/serve-real-window.sh` 139 -> 178 checks (fixture gained `T2` and a windowed VIEW); the spine's callers swept by CODE PATH, all clean: derived 122, cte 35, union 39, insertselect 22, subquery 70, lateral 36, view 34, psql 68, execblock 5, modifiers 66, joinderived 25, viewjoin 36, winbatch 15.
  - **ARITHMETIC over literals into a DECFLOAT column is DECIMAL arithmetic on their decimal forms** - never through a double. Measured: `1E+3 * 2` is 2E+3, `1E+3 / 4` is 2.5E+2, `1E+3 * 1.5` is 1.5E+3, `-1E+3 * 2` is -2E+3, `1.5E+2 + 1` is 151, `0.1E0 * 3` is 0.3 (NOT the double's 0.30000000000000004), `1E+200 + 0` the 34-digit form, `1E+3 * 2 + 1` is 2001, and `1E+200 * 1E+200` is 1E+400 - past DOUBLE's range entirely, the cell that proves no f64 is involved. The DECFLOAT(16) twin and both UPDATE forms follow; a DOUBLE destination keeps double arithmetic (2000.000000000000), a NUMERIC one 2000.0000, and an all-exact tree (`2 * 3`, `0.1 + 0.2`) is unchanged.
    The fix RE-READS the item's text with its exponent literals lexed as DECIMALS ([decfloat_const_expr], a thread-local parse mode) and lets the existing evaluator do the rest - any decfloat operand already promotes both sides to a `Dec` and computes to 34 digits HALF-UP. Re-lexing is what carries each LEAF's spelling: the parsed tree keeps only an f64 per literal, and widening `RawExpr::Double` to hold its text would have made two literals of equal value and different spelling compare unequal, since it derives `PartialEq`. A constant whose DOUBLE fold OVERFLOWS now survives the fold with its text and no wire value, so a DECFLOAT destination can still re-read it (that is how 1E+400 stores) while every other destination refuses it (`serve-real-dfparambind`, 2026-09-16).
    **RECORDED:** `1E+200 * 1E+200` into a DOUBLE column refuses on both sides, but the engine raises 22003 where fire-crab answers 42000 - law-safe, vector differs.
  - **An exponent LITERAL stored into a DECFLOAT column is read as DECIMAL FROM ITS OWN SPELLING** - `1E+200` stores 1E+200 where the folded double stored 9.9999999999999997E+199, and the refusal that stood in for it is gone. The rule is NOT the text verbatim but decNumber's form of the value the literal denotes, coefficient and exponent AS WRITTEN (measured cell by cell): `1e+200`, `1E200` and `+1E200` all store 1E+200, `1.5E+2` stays 1.5E+2 and `-2.5E+3` keeps its sign, while `1.50E+2` is 150, `0.1E+1` is 1 and `1.0E+0` is 1.0 - which is exactly what [text_to_dec128_clamped] plus the decfloat render already produce, so the fix CARRIES THE SPELLING rather than inventing a form. INSERT keeps it on [InsVal::ApproxConst] and hands it to the encoder as a text (whose DEC128 / DEC64 arms parse by the decNumber grammar); UPDATE swaps the literal for an `Expr::DecFloat34` at plan, because a SET value stays an expression evaluated per row with no later place to spell it. A NEGATED literal counts (the minus parses as a negation OVER the literal, and the captured text carries it), and a value the fold substituted from a scalar subquery works the same way (`VALUES((SELECT 1E+200))`, `UPDATE .. = (SELECT 1E+200)`, a MERGE source literal column - all measured storing 1E+200). A DOUBLE destination is untouched (`1.000000000000000e+200`) (`serve-real-dfparambind`, 2026-09-16).
    **STILL REFUSED:** anything built OVER a literal, which has no single spelling to carry - `1E+3 * 2` (the engine's decimal 2E+3), `1E+200 + 0` (its 34-digit decimal sum) - plus `INSERT .. SELECT 1E+200` and a MERGE branch's `INSERT (.., 1E+200)`, two paths that spell their value elsewhere.
  - **A MERGE's own `?` converts at its slot too, in both branches** - `WHEN MATCHED THEN UPDATE SET` and `WHEN NOT MATCHED THEN INSERT` now take the same law as a plain UPDATE / INSERT, the multiply asymmetry included (measured inside a MERGE, one statement per FRESH row: `SET N = ? * 2` bound 1.115 stores 2.00 where `SET N = 2 * ?` stores 2.24, `? / 2` 0.56, `? + 0` 1.12, `SET I = ? * 2` bound 1.6 is 4, `SET DF = ? / 3` bound 10 the decimal 3.333333333333333333333333333333333, and the INSERT branch cell for cell the same). A MERGE desugars per source row into statements built as TEXT, so a marker cannot carry a slot through: [Plan::Merge] now keeps each slot's descriptor and a scale-0 flag, and [merge_subst] spells the bound value CONVERTED THROUGH IT - the per-row text is planned after the substitution, so there is nowhere later for the conversion to happen. A marker in the ON clause or a WHEN .. AND condition is a COMPARISON, not an assignment, and keeps the value as it arrived (both regression-gated). Also here: a DECFLOAT value gained its literal spelling in [dml_subq_literal] (a CAST over its exact decimal text), and the UPDATE planner's prepare-time TYPE GATE learned the decfloat question - a desugared `UPDATE .. SET DF = CAST('10' AS DECFLOAT(34)) / 3` typed None and refused at re-plan (`serve-real-slottype`, 2026-09-16).
    **STILL REFUSED, and the refusal is the point:** a marker inside a CALL (`SET DF = COALESCE(?, 0)`). COALESCE types its parameter from its OTHER arguments - the engine stores 1 for a bound 1.1 - and MERGE's text-based typing reproduces the destination rule only; admitting the shape stored 1.1000000000000001, a WRONG VALUE where the old code refused, so [merge_marker_shape_ok] keeps a parenthesised marker refusing until the overrides are read off the text the way [resolve_dest_param_expr] reads them off the tree.
  - **A `?` INSIDE a DML value expression converts AT ITS SLOT, before the arithmetic** - `(I) VALUES (? * 2)` bound 1.6 is 2 * 2 = 4 where fire-crab spliced the raw double, computed 3.2 and stored 3 (SILENTLY wrong, for every exact destination; `(BI) VALUES (? * 2)` the same, and `(N) VALUES (? / 3)` stored 3.00 for 3.33). [resolve_dest_param_expr] already pushed the DESTINATION descriptor down the tree the way the engine's PASS1_set_parameter_type does - CAST typing its own operand, COALESCE typing from its other arguments - but emitted a BARE `Expr::Param`, so the conversion happened once, at the store, after the arithmetic. It now wraps each placeholder in a CAST to its own slot type, which is where the engine converts.
    **THE MULTIPLY ASYMMETRY (measured cell by cell, and the DESCRIBE does not show it):** the LEFT operand of a multiply converts at SCALE 0 while every other position takes the destination's scale. Into NUMERIC(9,2) with 1.115 bound: `? * 2` and `? * 2.0` store 2.00 (the value became 1), `2 * ?` stores 2.24, `? / 2` 0.56, `? + 0` and `? - 0` 1.12; into NUMERIC(9,4) `? * 2` bound 1.11115 is 2.0000; `? * ?`, `? * <column>`, `(? * 2) + 0` and `? * 2 * 2` follow the same left-operand rule, and a DOUBLE destination keeps its double (2.23). SQLDA_DISPLAY announces the destination's own scale in every one of them, so the slot PUBLISHED and the conversion APPLIED differ - which is why this is a value-path law, not a describe one.
    A DECFLOAT destination now converts through its slot too (`(DF) VALUES (? / 3)` bound 10 is 3.333333333333333333333333333333333, the decimal division), which retired the explicit DECFLOAT refusals on both the INSERT and UPDATE arms: a decfloat VALUE travels to the store as its exact decimal text, since `WireParam` has no decfloat carrier and the destination's encoder already reads one by the decNumber grammar (`serve-real-slottype`, 2026-09-16).
  - **A `?` INSIDE an IN / NOT IN / EXISTS / scalar SUBQUERY, answered instead of refused** - the last of the bind-error leftovers, and a MECHANISM change rather than a vector one. An uncorrelated subquery is evaluated AT PREPARE and folded to literal tokens, which a placeholder cannot survive; a parameter-bearing one now takes the PER-ROW route instead ([resolve_subqueries] declines the fold on `text_has_param`), where its `?` is rewritten to an `FC$PRM<slot>` marker - the same mechanism the outer references already use - and the bound value is spelled in as a SQL literal at bind ([subst_prm_markers] in `subst_params_expr`, with a fresh cache per bind). The slot NUMBERS in text order: `corr_register` takes the clause's base plus the placeholders written before the subquery plus the slots earlier subqueries claimed, the inner DESCRIPTORS come from the inner plan's own input sink (published through `CORR_PRM` so the statement's describe announces them), and the outer parser skips those slots when it meets the subquery's token - so `WHERE I = ? AND ID IN (SELECT .. WHERE I = ?)` binds outer-then-inner and the reverse spelling binds inner-then-outer. The raise waits for the inner stream exactly as the engine's does: an empty inner side answers, and so does an empty OUTER one (`serve-real-bindconv`, 2026-09-16).
...
- **Parameter slot typing inside an expression: DONE** (`serve-real-slottype`, 2026-09-16) - see the entry in the answered list.
...
- **ROUND leftovers** (the exact value is DONE, `serve-real-roundexact`): an UPDATE through a CORRELATED `(SELECT ROUND(..))` into a DECFLOAT column still refuses; a positive-scale ROUND stored into a VARCHAR renders the flattened digits; aggregates over ROUND (`SUM(ROUND(dp, 2))` in a projection with CAST) refuse.
...
- **Bind-error leftovers** (a plain column, an expression left side, the wide magnitudes, `CAST(? AS <type>)`, a JOIN's ON and a SUBQUERY's own `?` are all DONE - `serve-real-bindconv`): what still refuses with the generic *Dynamic SQL Error*. A subquery `?` reached through a caller that cannot say where the statement's numbering starts - the DML WHERE paths and the view-body resolver pass `param_base: None` to [resolve_subqueries], which keeps the old refusal rather than take a slot number it cannot prove (binding the wrong value to it would be a silent wrong answer); only the single-table SELECT path proves its base today. A **TEXT cast target** (`S = CAST(? AS VARCHAR(5))`, refused for a FITTING value too): the slot's charset is the ATTACHMENT's, which [cast_target_descriptor] leaves flat, while the projection resolver types the cast source UTF8 - and [cmp_sides]' text path reconciles byte-carrier against real charsets, so admitting it would GUESS a charset for a NONE column. The engine raises *string right truncation, expected length 5, actual 10* for an over-long value, and the projection `SELECT CAST(? AS VARCHAR(5))` already answers it - only the comparison side is left. Also still refusing: a text whose exponent underflows (`I < ?` ['1e-400'], where the engine compares against a tiny positive - a zero column value sorts BELOW it, so sign alone cannot decide it), and capital-hex '0X10' (the engine reads uninitialized memory). Law-safe, vector and timing differ.
...
- **A NONE-charset text value compared to (or cast at) a REAL charset is a SILENT WRONG ANSWER - the highest-severity find of the third hunt, and BIGGER than the hunt agent's "one-spot blob fix" claim (verified by direct measurement).** Under the default isql attachment, which is effectively NONE (`CHAR_LENGTH('café')` is 5 on both servers), the engine treats a NONE operand's BYTES as already being in the other side's charset: `WHERE v = 'café'` over a UTF8 column MATCHES (the NONE literal's `c3 a9` bytes read as UTF8 `café`), and `CAST(<utf8 text blob> AS VARCHAR(n))` - which describes as charset NONE on BOTH servers - carries the blob's RAW STORED BYTES (`octet_length` 5), untransliterated. fire-crab instead carrier-decodes the NONE bytes into carrier chars and re-encodes/compares them as real codepoints, so the SAME equality returns 0 (a qualifying row silently vanishes), `POSITION('é' IN b)` is 0, and the NONE cast decodes-then-`carrier_encode`s to 4 bytes (`café` -> `63 61 66 e9`). The `octet_length`/`char_length` of the blob and of an EXPLICIT-charset cast are already right; only the NONE-context byte identity and the NONE-vs-real comparison are wrong. The correct model is the engine's: **a byte-carrier (NONE/OCTETS) operand is bytes - a comparison against a real charset compares the real side's ENCODED bytes to the carrier's bytes, and a blob-to-NONE cast passes the stored bytes through.** **RE-MEASURED 2026-09-17 AND MOSTLY CLOSED - read this before planning from the paragraph above.** The EQUALITY half and the blob-to-NONE CAST half both AGREE now (`WHERE u = 'café'` matches a UTF8 column and `CAST(<utf8 blob> AS VARCHAR(n))` is 5 octets on both servers), closed by `f692502`'s byte-space reinterpretation; LIKE - both the match and the engine's 22000 raise - is closed by the entry above. What REMAINS, measured cell by cell: `POSITION`, `REPLACE` and `TRIM` with a non-ASCII needle and `CONTAINING` over a BLOB, which sit on the SysFn/blob paths rather than the predicate builders (`POSITION('é' IN u)` engine 4 / fc 0, `REPLACE` 4 / 5, `TRIM(TRAILING 'é')` 3 / 5, and the MIRROR direction under a UTF8 attachment where the NONE column is the carrier); and a BOUND LIKE pattern, unreachable at bind because `Predicate::bind` takes no descriptors. Its framing was ALSO wrong twice - see "Stale claims retired": the fix did not touch `coll_value_cmp`, and the raise does not turn on the column's byte width. Measured 2026-09-08; re-measured and narrowed 2026-09-17, and narrowed AGAIN the same day - the LITERAL side of the function family closed with the string-function entry above, so what is left here is exactly TWO shapes, both needing the VALUES reconciled in byte space at EVAL rather than a literal rewritten at prepare. (1) THE MIRROR - a carrier COLUMN meeting a REAL literal under a real attachment: `POSITION('é' IN N)` 4/0, `POSITION(N IN 'xcafé')` 2/0, `REPLACE(N,'é','e')` 4/7, `TRIM(TRAILING 'é' FROM N)` 3/5, `N CONTAINING 'é'` 1/0, while `N LIKE '%é%'` agrees (observed, unexplained). (2) COLUMN-vs-COLUMN, where there is no literal to rewrite at all: `POSITION(N IN V)` and `POSITION(V IN N)` 1/0, `TRIM(TRAILING N FROM V)` 0/5, `REPLACE(V,N,'x')` 1/5 under BOTH attachments, plus three shapes fire-crab REFUSES rather than answering wrongly - `V CONTAINING N`, `N CONTAINING V`, `V LIKE N`. **BOTH SHAPES RESOLVED 2026-09-18 as far as the FUNCTION path reaches.** Shape (2)'s four wrong answers are FIXED, and shape (1)'s are fixed for `POSITION`/`REPLACE`/`TRIM`; the fix is operand-keyed ([carrier_fn_operands]) rather than a literal rewritten at prepare, which is exactly what this entry predicted was needed and is why it reaches column-vs-column at all. The three REFUSALS turned out NOT to belong to this family: measured, fire-crab refuses a non-literal LIKE/CONTAINING pattern even when both sides carry the SAME charset (`U CONTAINING V`, `U LIKE V`, and an expression pattern `U CONTAINING V||''`), so they are a missing capability, recorded as `refuses` cells. `CONTAINING` over a carrier with a LITERAL needle remains divergent on purpose - it is the predicate path, not the function path. **CLOSED 2026-09-18 as well, in its own slice** (`serve-real-nonecmp` 96, 2 flip on `302c4d1`): both of CONTAINING's arms - the expression arm and the plain-COLUMN fast path - now take the symmetric condition, while the shared literal path they sit beside is untouched. **So shape (1) of this entry is fully closed and shape (2)'s wrong answers were closed by the slice before it**; what remains from this paragraph is only the non-literal-pattern capability gap, which measurement moved out of this family entirely.


- **A SELECT OF AN UNALIASED NON-ASCII STRING LITERAL NO LONGER DROPS THE CONNECTION DONE (2026-09-08, `serve-real-litname`):** `SELECT '<non-ascii>' FROM RDB$DATABASE` with no column alias PANICKED and dropped the connection (SQLSTATE 08006). The implicit-column-name derivation split a select item on the LAST whitespace to spot a trailing alias, matching UNICODE whitespace and then advancing ONE BYTE past it; a multibyte literal under a NONE attachment reads its UTF-8 high bytes one char per byte, so a `0xA0` byte decoded to U+00A0 NBSP (a two-byte whitespace char) and the one-byte advance landed inside it -> `char boundary` panic. Restricted the split to ASCII whitespace (SQL separates `expr alias` on real whitespace, and the one-byte advance is then always valid). The column is named `CONSTANT` and the value returned, as the engine does. Found by a second eight-surface differential hunt; it was the only HIGH (a crash) among that hunt's finds.
...
### F2 - DONE, LAZILY (2026-09-08, `serve-real-ovlazy`). The rescale overflow raises exactly where the engine raises, and nowhere else

**FIXED, and the earlier eager fix CORRECTED.** The engine raises 22003 for an old row an ALTER TYPE pushed out of range only when it must PRESENT that row's value - an output column, a filter that references the column, an aggregate, a sort/group/distinct key, an expression that reads it - and never for a row a filter excludes on some OTHER column (measured: `SELECT N FROM OV WHERE ID=2` returns the other row, it does not raise). The first fix (below, 2026-09-07) raised EAGERLY at the read, which over-raised every filtered query over the table - a regression invisible to gates that only tested unfiltered shapes. Replaced by a POISON value: `present_field` yields `Value::OutOfRange` (infallible) on overflow, it rides through unread slots, and it raises `EvalErr::NumericOutOfRange` at each point of CONSUMPTION - `ProjCol::value_of` (output and sort/group/distinct keys), `Term::matches` (a filter that touches the column), `Expr::Col` (an expression), `AggSrc` `src_value` and the fast `COUNT(col)` path (aggregates). The raise lands at a row boundary, before any wire byte, so a mid-row raise never drops the connection. Measured against the engine over byte-identical files, natural and indexed, 20 query shapes agree (`serve-real-ovlazy`): every presentation raises, every filtered-out or untouched row does not, no wrong number, no over-raise, no 08006. Value::OutOfRange round-trips through the external-sort spill format. The FK and UNIQUE walks correctly do NOT raise (the engine works on index keys there and does not present the value; measured - and ALTER TYPE on a key column is refused outright), so they need no poison consumer.

### F2 (first, eager - superseded) - DONE for the PSQL path (2026-09-07, `serve-real-altfmt`). The rescale overflow raises where the engine raises

**FIXED on the procedure/function path.** `fire_crab_ods::format::present_field` now returns `Err(PresentErr::OutOfRange)` when an exact rescale overflows (the 128-bit accumulator, or the target's i64/38-digit backing), and `present_record`/`present_through` propagate it; the executor turns it into `numeric value is out of range`, so a selectable procedure over a `BIGINT` holding 9e17 after `ALTER ... TYPE NUMERIC(18,4)` raises `22003` as the engine does instead of answering `90000000000000.0000`. SUPERSEDED by the lazy poison model above (2026-09-08) - this eager client raise over-raised on filtered queries. Kept for history: the streaming/materialising walk raised. `ReadView::values_checked` (a raising sibling of `values`) and `for_each_record_while_checked` (used by `RowSource::for_each`) carry `EvalErr::NumericOutOfRange` up, so a plain client `SELECT`, a `WHERE` and a `SUM` over the out-of-range old row all raise `22003` as the engine does. Still on the leave-unchanged path (an even narrower remainder, recorded): the index-driven by-recno retrieval (`records_at_in_with`), the hash-join build (`frozen_side_rows`) and the FK-partner probe (`fk_partner_could_carry`) - each answers through a `bool`- or `Vec`-returning walk with no error channel, and an overflow reached only by an index or join ON the altered column. The floor unit test now asserts the engine's behaviour (an overflow declines to `None` on the client path via `present_field(...).ok().flatten()`, and the wire narrowing/rounding cases are unchanged).
...
- ~~**A DUPLICATE UNDER A `COLLATE UNICODE_CI` PRIMARY KEY IS ACCEPTED.**
  fire-crab writes `'ABC'` next to `'abc'`... **WRONG WRITE (silent),
  PRE-EXISTING** ... It silently writes a duplicate into a unique
  index and deserves its own chunk.~~ **RE-MEASURED 2026-09-18 AND THE
  SHAPE IS WRONG: THIS IS AN OVER-REFUSAL, NOT A WRONG WRITE.** Nothing
  is written. This server refuses an insert into a table whose
  unique/primary key column carries an ICU collation **on the FIRST
  ROW, with no duplicate involved at all** - so it never reaches the
  question the entry describes. Measured with controls that isolate it:
  a plain table, a plain-PK table, a `UNICODE_CI` column with NO key and
  a `UNICODE` column with no key all insert fine here; the engine
  accepts the first row, refuses the case-variant duplicate with
  `23000 ... Problematic key value is ("C" = 'ABC')`, and accepts it
  under a case-SENSITIVE `UNICODE` key. **Cause: [coll::keyable_ttype]
  admits only `collation_id == 0` or `PXW_INTL`, while its own doc
  comment three lines above says the three root UTF8 collations are
  answered from the UCA via [coll::icu_key] - the comparison world
  learned ICU and the key path never did.** Ranked accordingly: a
  refusal, below any wrong answer.
  **AND WHAT IS NOT BROKEN, checked before it could be "fixed":**
  `ORDER BY`, `=`, `>` and `MAX` all AGREE with the engine under
  `UNICODE`, `UNICODE_CI` and `UNICODE_CI_AI` (`apple < Apple < Ápple <
  banana`, the UCA order, not the bytes), and unwrapped `DISTINCT`,
  `GROUP BY`, `UNION` and `UNION ALL` agree under no collation, under
  `UNICODE` and under `PXW_INTL`. The one dedup gap is CI/AI folding,
  which [coll_groupable_ttype] excludes DELIBERATELY, with the reason
  measured three ways: which SPELLING survives a fold has no rule.
  **A first probe appeared to show the whole dedup family refusing under
  every ICU collation - because every cell of mine was wrapped in
  `SELECT COUNT(*) FROM (...)` and the wrapper was what refused. The
  no-COLLATE control refused too and I nearly read past it. One cell,
  one variable.**
...
- **DONE 2026-09-07 - fire-crab's SERVICE-SIDE RESTORE APPLIES THE PRIVILEGES IT ONCE SET ASIDE, AND ITS RESTORE OF THE ENGINE'S EMPLOYEE BACKUP IS THE ENGINE'S OWN RESTORE.** The gate (`qa/serve-real-emprestore.sh`): the engine backs up the employee database, the engine restores that backup, fire-crab's `gbak -c -se` restores the same backup, and the engine opens both - one database. Equal: the extracted DDL, the constraint and index names (INTEG_<n>, RDB$PRIMARY<n>, RDB$FOREIGN<n> from the file, not freshly drawn), relation ids with the view at its stream position, security and default classes numbered as the engine numbers them (every global field first, then relations, generators, exceptions, procedures; a table's default class drawn per its first appearance among the privilege records, an empty table's after, a view's last), formats (a computed-column table at 2), dependencies, every user-object RDB$USER_PRIVILEGES row, every data row read by explicit column, blobs and arrays. What the restore now does that it did not: (1) it STORES the file's RDB$USER_PRIVILEGES rows verbatim (grantor present) and recomputes each granted object's ACL through `grant_privileges_deferred` - a restore that stored the rows without the classes answered a non-owner USAGE the engine refused; before this, privileges were parsed and dropped; (2) the widened restore surface - SMALLINT/INTEGER/BIGINT/INT128, CHAR/VARCHAR, FLOAT/DOUBLE, DATE/TIME/TIMESTAMP, BOOLEAN, BLOB and ARRAY columns, COMPUTED BY columns (expression stored, RDB$FORMAT bumped to 2), column DEFAULTs, named and RDB$n domains created first so every table and procedure binds to them, rec_array slices decoded (the canonical XDR slice, its raw length a big-endian long) and rewritten as stream blobs with every blob and array of a table created before its rows so the ids fall as the engine's do; (3) the file's constraint and index names threaded through the DDL writers (a name book), the reader's att 9/11 sub_type/scale un-swapped, RDB$INDEX_TYPE and global-field RDB$FIELD_SUB_TYPE stamped from the file, view fields' RDB$COLLATION_ID set for text. RECORDED, NOT DONE: the engine numbers RDB$FIELD_ID by the field records' arrival order and lays the record format out to match; this restore numbers by field-id rank, so on a table whose position order differs from its field-id order (employee JOB, COUNTRY, CUSTOMER) the RDB$FIELD_ID column differs and a bare SELECT * lists the columns field-id-first - matching the engine means rebuilding the index-segment and field-id bindings around an arrival-order format, and every position, source, class, dependency, privilege and DATA row (read by explicit column) already matches. Also not compared: system-domain USAGE grants and the security-class count, seeded by the database shell. INVESTIGATED 2026-09-07, NOT CLOSED: the engine assigns RDB$FIELD_ID by field-record arrival order and lays the record format's descriptors in that order; the backup's row XDR is in the SOURCE format order (ascending source field id, rec_field att 22), which gbak remaps field-by-field. Creating the columns in arrival order (so RDB$FIELD_ID = creation rank = arrival, matching the engine) needs the row XDR remapped from source-id order to arrival order at decode. Two decode models were tried against all three employee backups (the shipped sample, fire-crab's own, and the engine's backup of fire-crab's build): sequential-in-arrival is data-correct with a field-id-order format (the committed state) but leaves field ids at field-id rank; remap-by-att22 with an arrival-order format decoded the shipped sample but refused the engine-of-fire-crab backup with "a varchar longer than its column", so the att22 field number is NOT a reliable stand-in for the source field-id order across files. The XDR byte order needs to be established from the burp.h format record directly (RDB$FORMAT descriptor order per source), not inferred from att 22, before this can be closed. The committed restore is data-correct and catalog-correct in every column but this id.
- **DONE 2026-09-07 - fire-crab's `gbak -b` OF THE EMPLOYEE DATABASE IS THE ENGINE'S, JUDGED BY WHAT THE REAL gbak RESTORES FROM EACH.** The criterion, and the gate (`qa/serve-real-empbackup.sh`): no check compares `.fbk` bytes - relation order in a backup is the file's physical RDB$RELATIONS order and the header carries timestamps - instead the engine and fire-crab each back up THE SAME FILE, the real `gbak -c` restores both, and the two restores must be one database: `isql -x`, generated names, ids, security classes, owners, dependencies, every RDB$USER_PRIVILEGES row, formats, every data row, every array. Measured on the script-built employee database and on the restored sample: equal, save the CREATE DATABASE path comment. What the stream had wrong, in the order the engine's own stream exposed it: (1) attributes 9 and 11 SWAPPED for years (burp.h: type 8, sub_type 9, length 10, scale 11) - every NUMERIC had restored as a plain BIGINT; (2) columns paired with their descriptors by POSITION, not field id - a gbak-restored table's field ids are not positional; (3) records relaid into the newest format in EMITTED column order (blobs first) instead of field-id order - SALES refused on the restored sample; (4) a VARCHAR's RDB$FIELD_LENGTH written with its 2-byte count; (5) arrays: rec_array (23) is att 3 field number, 14 dimensions, 15/16 ranges, att 7 + u32 raw length, att 18 + u32 XDR length + the canonical slice - which OPENS with its raw length as a big-endian long ("expected length 15, actual 16" was that missing word); rec_field_dimensions (24) is `48 PUBLIC, 1 name, 29 dim (0-based), 32 low, 33 high`, written after the global fields; (6) owners and classes the engine writes and fire-crab did not: every global field ends `45 SYSDBA`, a named domain carries its class as att 25, an exception its class 6 and owner 7, a relation its class 8 and owner 12 (a view's order is 21 1 2 16 8 14 12 18) - without them the engine's restore INVENTS: its "adding missing privileges" phase grants USAGE to PUBLIC on every ownerless domain and exception (twenty grants in one restore, none in the other), and a relation without a class draws a fresh SQL$DEFAULTn in arrival order; (7) a privilege on a schema itself (object type 38) carries NO att 10 object-schema - stamping one made both USAGE ON SCHEMA grants vanish from the restore; (8) invented domains ride in RDB$FIELDS physical row order, not table-then-column order, because the restore hands classes out in arrival order; (9) a parameter domain's 41/42/43/44 come from ITS catalog row (looked up by RDB$FIELD_SOURCE, since the written name is renumbered), each only where the row has it; (10) rec_user_privilege (22) rides after the trigger messages, `WITH RDB$GRANTOR NOT MISSING`, and fire-crab's service-side restore now narrates each one ("restoring privilege for user") so the verbose gate's strict comparison holds. Also folded in: `catalog_field_list` sorts by field id (a permuted re-format had been produced), views interleave with tables in catalog order, the schema record carries its class and owner. Open: fire-crab's own restore still sets privileges aside (narrated, not applied); the RDB$FORMATS descriptor blob's encoding differs in size from the engine's (both readable).
- **DONE 2026-09-07 - THE EMPLOYEE DATABASE BUILDS THROUGH fire-crab FROM ITS OWN SCRIPTS, AND WHAT IT BUILDS IS WHAT THE ENGINE BUILDS.** Firebird's `examples/empbuild/empddl.sql` (941 lines: 15 domains, 2 generators, 10 tables, a view, 4 triggers, 5 exceptions, 10 procedures, 22 grants) and `empdml.sql` (440 statements) now run through fire-crab into an engine-created empty database with ZERO failures, where the previous binary refused 51 of the DDL statements. The engine then reads both files: `isql -x` extracts the same DDL from both (the only differing line is the `CREATE DATABASE` path comment); every generated name comes out in the engine's order - `INTEG_1..80`, the shared index counter `RDB$PRIMARY1, RDB$PRIMARY2, RDB$FOREIGN3, RDB$4, ... RDB$FOREIGN26`, `CHECK_1..28`, `SQL$470..512`, `SQL$DEFAULT61..71`, relation ids 128..138, procedure ids 13..22, exceptions 1..5; RDB$FORMAT 2 for the three tables with a column-level key; all 137 privilege rows; all 163 + 6 dependency rows; and every row of every table, the view and the two generators. Gate: **`qa/serve-real-empbuild.sh`** (the two scripts kept under `qa/fixtures/empbuild/`, 7 checks). What it took, in the order the failures fell: **the comment stripper was fine but the diagnosis was not** - isql's `After line N` numbers do not name the failing statement, and `-e` echo buffers mislead too; the reliable attribution is the server trace, which the DDL chain and the execute-immediate path now write (`[srv] ddl prepare refused` / `immediate refused`), together with a raw-text trace and traces in `respond_error` / `respond_eval_error`. **Domain CHECKs** beyond the pinned int/NONE-text compiler (BETWEEN, IN, STARTING WITH, UPPER, a scaled VALUE, OR-chains) compile through the DSQL validation compiler; a text `IN` list stores each member `blr_cast blr_varying2 0,0 len,0 <literal>` - cast to the tested operand's declared type (measured on PRODTYPE), so the compiler now takes a `TypeSpec` for the VALUE / the table's columns. **Table CHECKs** likewise fall back to `compile_check_for(sql, table, cols)`, whose NEW stream carries the table's name so `employee.job_code` resolves to context 1; the two refusals the pinned path made stay (an inline check naming a later column; an integer column against a non-numeric text literal). **`BLOB(400,1)`** is `BLOB SUB_TYPE 1 SEGMENT SIZE 400`; a bare **`CHAR`** is CHAR(1). **The CREATE TABLE body splitter was blind to literals**: `COMPUTED BY (last_name || ', ' || first_name)` split at the literal's comma; both scans skip quotes now. **Computed columns** type beyond the integer surface: a concatenation is VARCHAR(sum of the operands' characters) in the operands' charset with NO precision (RDB$9: 37/37/37/0), a DOUBLE operand makes the result DOUBLE with no precision, a timestamp difference is NUMERIC(18, 9) precision 18 - and the operands may be DOMAIN-typed, so `plan_create_table` now takes the database and resolves the domain's RDB$FIELDS row; `ComputedCol.precision` is an Option because the engine stores NULL for the first two. The parenthesised atom of the expression parser re-enters at the concatenation level (it used to stop at addition, refusing every `(a || b)`). **Trigger bodies** may name NUMERIC, DOUBLE and TIMESTAMP columns (`body_col_class`), and `USER` / `CURRENT_USER` / `CURRENT_ROLE` are expression leaves (`blr_user_name` 44, `blr_current_role` 174) in the ods `Expr`. **The DSQL body compiler** learned the sample's four shapes, each measured against the engine's stored BLR: the paren-less `EXECUTE PROCEDURE p :a RETURNING_VALUES :b`; a comma-separated FROM (`FROM sales s, customer c`) as ONE flat `blr_rse 2` with the WHERE as its boolean (an explicit JOIN chain stays nested); an array element `col[:i]` as `blr_index <field> <u8 count> <subscripts>`; a selectable procedure with arguments as a stream, `blr_procedure <name> <ctx> <u16 argc> <args>`; and a bare column across two streams resolves through a CATALOG the server hands the compiler (`set_catalog`: every table and procedure the statement names, with its columns or outputs), in the join's merge scope too. **Catalog fidelity, five laws:** an unnamed FOREIGN KEY's index is `RDB$FOREIGN<n>` from the shared index counter, drawn right after its `INTEG_<n>` (a named constraint names its index after itself); a NUMERIC/DECIMAL parameter keeps its declared sub_type (1|2) and precision p (`RETURNS (TOT_BUDGET DECIMAL(12, 2))` extracts as written, not `NUMERIC(18, 2)`); a stored body keeps its COMMENTS - the client's raw text rides beside the comment-blanked copy the planners read, and the same offsets recover the source (`RAW_STMT` / `raw_source`); RDB$FORMAT 2 with a second identical RDB$FORMATS row for a table with a column-level PRIMARY KEY / UNIQUE; a view's owner holds the five table privileges. **Dependencies come from the BLR walker everywhere now** (`store_dependencies_deferred`, proven on the restore): CHECK triggers, user triggers (the declared charset row aside), procedures (kind 5), views (kind 1) and - new kind 3 - computed columns; the walker learned that `blr_modify`'s new-record context is the modified relation (the engine records `LOG.Y` for `UPDATE LOG SET Y = :v`, which the old field-list path never saw). **A view's format descriptor is laid out like a table's** (the ini.epp walk: a VARCHAR carries its 2-byte count, fields align) - the raw-length layout made the ENGINE refuse fire-crab's PHONE_LIST with a string truncation. **Open, recorded:** RDB$FORMATS descriptor BLOBS differ in size from the engine's (JOB 100 vs 144 bytes; the engine reads both, and its own format-1 row for a re-formatted table is 28 bytes while format 2 is 88 - an encoding this server has not decoded); the DML fidelity items the research found (a selectable-procedure source with WHERE/ORDER BY refuses; `ASCII_CHAR` refuses; `98000 * 0.10` describes INT64 where the engine says INT128; assigning a computed column answers the bare error, not `attempted update of read-only column`); the arrays and blobs `empbuild.epp` loads through `op_put_slice` and blob cursors are outside the SQL scripts on both servers. Gates: the trigger, check, domain, view, procedure, PSQL, package, aggregate, describe, union, gbak, attach, restored, grant families and the 13 database-argument gates at 0 DIFF; 78 + 4 + 161 + 345 unit tests.
...
- ~~**HIGH (silent wrong write, plain table)** - a correlated scalar
  ...
  **STRUCK 2026-09-18: IT DOES NOT REPRODUCE - FIXED, AND THE BULLET WAS
  NEVER STRUCK.** Re-measured in its own two shapes and values before
  any chunk was spent on it: the bullet's own statement answers `3|20`
  on BOTH and leaves the same stored row on both, and shape (b)
  (`UPDATE T t SET S = (SELECT NM FROM D WHERE D.ID = t.ID)`) writes
  `3|7.50` on both with all three rows identical - the unaliased
  control too. `qa/serve-real-corrsub.sh` carries those exact statements
  (27 checks, green), and [strip_dml_alias]'s own doc comment now
  states the whole law with "measured:" beside it. **FOURTH stored claim
  to fail re-measurement in one day** (the `UNICODE_CI` key's SHAPE was
  wrong; the `?`-in-SET-arithmetic entry was likewise already fixed by
  `e81c64f`; and one of the four was MY OWN probe's confound). **A
  bullet is a hunt's output, not a defect - and an entry nobody struck
  reads exactly like one nobody fixed.** The original text follows.
...
- ~~**MEDIUM (wrong value, bound params)** - a `?` inside SET arithmetic
  ...
  `SET N = ? * 2` -> 2.5 vs 2.~~ **STRUCK 2026-09-18: the PLAIN-TABLE
  halves were fixed by `e81c64f` (2026-09-16) and the bullet was never
  struck - re-measured in its OWN shape and value, all eight plain-table
  shapes agree (`? * 2`, `2 * ?`, `? * 2.0`, `? * 3`, `? * ?`, a DECIMAL
  destination, an INTEGER destination, `? - 1`), and `serve-real-slottype`
  carries the cell. THIRD stored claim in one day to fail
  re-measurement.** **BUT ITS LAST CLAUSE - "Identical through a trigger
  view" - WAS THE ONE TRUE PART LEFT, and only a correctly built fixture
  showed it:** my first attempt fed `CREATE TRIGGER ... BEGIN ... END;`
  to isql with no `SET TERM`, so the body's own semicolons cut the
  statement at column 99 and **the trigger was never created** - the
  cells that "agreed" ran over a plain updatable view. The `-104` was in
  my probe, not in either server. Fixed and gated in the entry below.
...
- **DONE 2026-09-20 (`qa/serve-real-nanrow.sh` extended to 44 cells, floor 44) — A NaN INDEX KEY NEVER CONFLICTS WITH ANOTHER.** Key equality asks the same question predicate truth does and gets the same answer: **a NaN is not equal to itself, so a PRIMARY KEY on a DOUBLE accepts a SECOND NaN** — measured on the live engine, which then holds two NaN keys, while a duplicate `1.5` and a duplicate `inf` STILL raise a proper violation, both of which this server already got right. **THE MECHANISM IS NOT THE COMPARATOR**, which is why the previous chunk's fix did not reach it: `double_key` is an ORDER-PRESERVING TRANSFORM, so two NaN rows encode to **identical key bytes** and the B-tree's own entry-level check calls the second a duplicate before any comparison happens. A NaN key is therefore EXEMPT from uniqueness, for a DIFFERENT reason from an all-NULL key (btr.cpp:5629 `key_all_nulls`), so it travels as its own flag; only a PLAIN COLUMN segment is read, an expression index keeps enforcing. **THE INDEX ORDER IS UNTOUCHED and that is what kept this to twenty lines** — `double_key` already puts +NaN ABOVE +Infinity, the engine's own totalOrder placement, so only the UNIQUENESS TEST was wrong, and the sort/dedup surfaces stay recorded rather than changed. **TEETH: 1 red of 44 on `/tmp/fcwire-prev-f70890a`, 0 red CONTROL cells** — low by construction, since this chunk changes exactly one behaviour and the other 43 cells are the previous chunk's. `cargo test` wire **369** / ods **162**. **Sixteen gates swept, ALL CLEAN**: `index` 403, `fk` 18, `dblparam` 422, `nanparam` 174, `nonfinitetext` 45, `merge` 36, `batch` 22, `trigdb` 24, `ddlsequence` 25, `growth` 32, `undo` 16 among them.
  **TWO THINGS MEASURED AND DELIBERATELY NOT FIXED.** (a) The round's probe reported this server answering a duplicate `inf` with a bare error instead of a PK violation; **re-measured, that is already right** on `VALUES (?)` and `INSERT .. SELECT` alike — the only differing cell was the NaN. *Re-measure a stored claim before building on it.* (b) **`INSERT .. SELECT` OF A NON-FINITE REFUSES, and it is not a key matter**: it refuses into a table with NO INDEX AT ALL and for ±Infinity equally, while the `UPDATE .. = (subquery)` twin and `INSERT VALUES (?)` both work. The cause is structural and already written down in `dml_subq_literal` — that path spells its source value as a SQL LITERAL and **a non-finite has no literal spelling**, the engine raising 22018 for every one. Unpicking it means not routing the value through text, which is its own chunk.
- **DONE 2026-09-20 (`qa/serve-real-nanrow.sh` NEW at 40 cells, floor 40) — A NaN THAT IS A ROW VALUE IS NEVER EQUAL, AND TYPE RANK PICKS THE LESSER.** The NaN chunk two entries up taught this server the engine's order for a NaN arriving as a BOUND PARAMETER. A NaN arriving as a ROW VALUE still went through `value_cmp`, whose approximate arm ends in `partial_cmp().unwrap_or(Equal)` and so made **a NaN EQUAL TO EVERYTHING**. **THE RULE, which ~200 measured predicate cells fit** over stored NaN / ±Infinity / NULL rows on a heap twin, an indexed twin and an expression-index twin: **(1) THE RESULT IS NEVER "EQUAL"** — `=` FALSE, `<>` TRUE, and **TWO-VALUED, not UNKNOWN** (`NOT (D >= 0.0e0)` is TRUE); `D = D` is FALSE on a NaN row, `D IS NOT DISTINCT FROM D` is FALSE, and a self-join on `A.D = B.D` pairs only the rows that are not NaN. **(2) ONE OPERAND IS THE PRIMARY AND IT IS THE LESSER** — the primary is the operand of **HIGHER TYPE RANK, DOUBLE > FLOAT > exact**, and on a TIE the **FIRST-WRITTEN** one. So BOTH spellings of `<` are TRUE between two DOUBLEs (`D < D2` and `D2 < D` on the same row) and a NaN in a FLOAT column meeting a DOUBLE is the GREATER operand. **THE RANK CLAUSE IS WHAT A FIRST CUT GOT WRONG** by carrying the parameter law over unchanged: "against a FLOAT side the NaN is always the lesser" predicts `FL > 0.0e0` FALSE and the engine says TRUE. **And the UNIT TEST caught the tie clause the same way** — I asserted `(double, nan)` compares Greater and it is Less, because a tie goes to the first-written operand *whichever side holds the NaN*, which is exactly why both spellings of `<` are true. **A NaN HAS FOUR SURFACES AND THIS CHUNK FIXES ONE** — the equality-based one (predicate truth, join ON). The rest are measured and RECORDED with both answers pinned: **SORT PLACEMENT is IEEE-754 totalOrder INCLUDING THE SIGN BIT** (`-NaN < -Inf < finite < +Inf < +NaN`, proved by `-NaN` sorting below `-Inf` and by `ORDER BY -D` — no "greatest" or "least" rule can produce it); **DISTINCT dedups BY THAT SORT KEY** so two +NaN rows COLLAPSE; **GROUP BY uses EQUALITY instead** so the same two rows do NOT — the engine answering **7 for DISTINCT and 8 for GROUP BY over one column on one set of rows** is the sharpest fact in the round and the reason dedup and grouping cannot share a comparator; and **a UNIQUE/PRIMARY KEY follows the EQUALITY rule**, accepting a SECOND NaN while still raising for a duplicate 1.5. **AND THE PRICE, STATED RATHER THAN HIDDEN: the access-path split is on the UPPER bound, not the lower.** The NaN's index key sorts above +Infinity, so a range with a STOP bound drops it; before this chunk the HEAP answer was wrong and the INDEXED answer right by coincidence, and now that is inverted, because this server scans where the engine ranges and nothing at evaluation time can see which. Everything else — the whole equality family and both bounds written the other way round — is now right on BOTH paths. Two cells pin the trade. **THE ROWS CANNOT BE WRITTEN IN SQL AT ALL**, which decides the fixture: `CAST('nan' AS DOUBLE PRECISION)` is 22018, `1e308*10` and `EXP(1000)` raise 22003, `0e0/0e0` raises 22012 — a BOUND PARAMETER is the only way in, so the gate puts its schema in through isql and its ROWS through node against the live engine, then copies the engine's own file across, behind a sentinel that stops the run unless eight rows landed and two really render `nan`. **TEETH: 22 red of 40 on `/tmp/fcwire-prev-3912303` with 0 red CONTROL cells.** `cargo test -p fire-crab-wire` **369** / `-p fire-crab-ods` **162**. **Nineteen gates swept, sixteen clean** — `index` 403, `dblparam` 422, `cmpparam` 1894, `nanparam` 174, `nonfinitetext` 45, `groupby`, `having`, `orderagg`, `joinorder`, `bigsort`, `fk` among them, so the sort, group and key paths are demonstrably untouched; only `numericwhere`'s 14 DECFLOAT reds remain, unchanged.
  **NEXT, ALL MEASURED BY THIS ROUND'S PROBE:** **(a) THE SORT SURFACE** — totalOrder for ORDER BY and the index key, and the sort-key dedup for DISTINCT/UNION, which together are the other half of the NaN story and touch the 52 `value_cmp` call sites this chunk deliberately left alone. **(b) MIN/MAX IS ROW-ORDER-DEPENDENT IN THE ENGINE** and that is not a bug to smooth over: it is the predicate rule applied to a running accumulator, so `MIN(D)` over the same two rows read in the other order gives a different answer (`NaN,1.5` against `1.5,NaN`), and `MAXVALUE(D, 0.0e0)` is `nan` while `MINVALUE(D, 0.0e0)` is `0` on the same row. **(c) A UNIQUE/PK ON A DOUBLE** must accept duplicate NaNs and must raise the proper violation for a duplicate `inf`, where this server currently gives a bare Dynamic SQL Error for both. **(d) THE DECFLOAT TEXT PATH**, still the only red left in the sweep.
- **DONE 2026-09-20 (`qa/serve-real-nonfinitetext.sh` NEW at 45 cells, floor 45) — THE ENGINE'S TEXT FORM OF A NON-FINITE DOUBLE, IN BOTH DIRECTIONS.** **OUTPUT:** a non-finite double renders LOWER CASE and SHORT — `nan`, `inf`, `-inf`. This server rendered `NaN`, `Infinity`, `-Infinity`, and **it is not cosmetic**: `CAST(? AS VARCHAR(30)) = 'inf'` is TRUE on the engine and was FALSE here, `CHAR_LENGTH` of it is 3 and not 8, and a CHAR(10) pads `inf       ` rather than `Infinity  `. The round's refuter counted **ELEVEN routers the text reaches, TWO OF THEM ROW-MUTATING** — a stored VARCHAR column and a trigger's computed text. **INPUT:** the engine accepts NO non-finite spelling as a number and raises 22018 for every one (`D = 'Infinity'`, `D = 'inf'`, `D = 'nan'`, `D < 'Infinity'`, any case, any sign); this server accepted them all, because **Rust's own f64 parser reads `nan` / `inf` / `infinity`**, and `D = 'nan'` then answered EVERY ROW since the NaN it had parsed compared equal to everything. **THE SIBLING PATHS WERE ALREADY RIGHT** — `text_to_approx` ends with `d.is_finite()` and a TEXT *bind* refuses — so this was ONE LITERAL ARM OUT OF STEP WITH ITS OWN NEIGHBOURS, which is exactly why nothing had caught it: every adjacent spelling behaved. **AND A WRITTEN CAST TO TEXT ABSORBS THE NaN, by rendering it**: past the cast there is no double left, only the string `nan`, and the comparison above it is an ordinary text comparison — measured over all six operators against `'inf'` and with a text column on either side, the engine's answers are exactly what comparing the literal string `nan` gives (`'nan' < 'inf'` FALSE, `'nan' > 'inf'` TRUE). Only a WRITTEN cast; the implicit slot cast is excluded as it already was for the numeric targets. **That third rule only became available once the renderer was right** — the ±Infinity twins of those same twelve cells started matching the moment `inf` was spelled the engine's way, which is what showed the cast was already absorbing correctly and only the spelling had been wrong. **TEETH: 28 red of 45 on `/tmp/fcwire-prev-5960ec5` with 0 red CONTROL cells.** `cargo test -p fire-crab-wire` 368 / `-p fire-crab-ods` **162**. **EIGHTEEN gates picked by "does this render an approximate value", not by topic word:** `dblparam` 422, `cmpparam` 1894, `nanparam` 174, `approx` 47, `approxmath` 39, `slottype` 73, `selparam` 44, `castparam` 23, `aggexpr` 38, `aggexpr2` 45 all clean.
  **AND THE REMAINING REDS ARE ONE FAMILY, WHICH IS THE NEXT ITEM: THE SAME LAW FOR DECFLOAT.** `numericwhere` (14), `absround` (4) and `bindconv` (1) are red, all of them on a DECFLOAT column's text conversion, **all four red on the previous binary too and unchanged by this chunk** — `D34 = 'inf'` ANSWERS where the engine raises 22018, and `D16 = 'inf'` raises with the right text but prints describe lines the engine does not. DECFLOAT is NOT the same law by assumption: IEEE decimal genuinely HAS infinities and NaNs as values, so whether the engine's refusal there is the same rule or a different one is **unmeasured**, and this chunk deliberately does not extend to it. Measure before extending.
- **DONE 2026-09-20 (`qa/serve-real-nanparam.sh` NEW at 174 cells, floor 174) — A NaN COMPARISON IS DECIDED BY THE WRITTEN ORDER AND THE OTHER SIDE'S CLASS, and the refusal that stood in for it was costing the floor.** The previous chunk recorded "a NaN has no single engine order" as a boundary. It was half true: the order is not a TOTAL order, but it is DETERMINED, and the access path matters for exactly ONE shape. **THE LAW, measured over 72 cells** (six operators x both WRITTEN ORDERS x DOUBLE / FLOAT / INTEGER / SMALLINT / BIGINT / INT128 / NUMERIC(9,2) / NUMERIC(18,1), each on a HEAP twin, an INDEXED twin and an EXPRESSION-INDEX twin), **39 more over a table carrying NULL rows, and ~1400 in the round's four-family boundary probe**: (1) **WHICH OPERAND IS THE LESSER** — against a DOUBLE PRECISION side THE FIRST-WRITTEN OPERAND IS THE LESSER, so the verdict FLIPS when the same term is written the other way round (`D < ?` is every row, `? < D` is none through an index); against a FLOAT (single) or an EXACT side the NaN is ALWAYS the lesser. [parse_leaf] rewrites `? op X` into `X mirror(op) ?`, so ONE term carries both spellings and `RawTerm::mirrored` is the only thing that tells them apart — **the flag the previous chunk added and never read; this is what it was for.** (2) **WHETHER EVERY ACCESS PATH AGREES** — a FALSE verdict always does, since an emptied index range cannot turn a non-matching row into a matching one; a TRUE verdict does too, EXCEPT ON A LOWER BOUND, where the engine's index range over a NaN is empty or partial and its scan is not: `ID > ?` [NaN] is every row on a heap and NONE indexed, `BI > ?` is every row against `1;2;3;9`. **Those, and only those, refuse** — and it is not theoretical: `DELETE FROM MI WHERE BI > ?` [NaN] removes SIX rows on a heap and FOUR through the index. **THE VERDICT IS SPELLED AS A COMPARISON AGAINST -Infinity, not as a constant**, so it stays UNKNOWN on a NULL row exactly as the engine's does — 39 cells over a NULL-carrying table say the NULL row is never taken and never mutated, which a fixture without NULLs cannot tell apart from a constant. **WHAT IT RECOVERS:** the previous binary refused 64 of the 72 matrix cells; the DML refuter measured 60 UPDATE/DELETE shapes, 20 of 21 MERGE shapes, UPDATE OR INSERT, triggers, INSERT..SELECT, every clause and every router refused alike, and **the law predicted EVERY ONE of the ~330 engine answers that family measured, with no exception**. **AND ONE CAP, measured rather than assumed: the EQUALITY OPERATORS ARE NOT ANSWERED, for any class.** An `IN` list arrives as ordinary `Eq` / `Ne` terms, indistinguishable from the written `OR` and `AND` it desugars to, and the engine does NOT read them alike for a NaN — on a DOUBLE column the list becomes a RANGE whose open end the NaN sets and WHICH END DEPENDS ON WHERE IN THE LIST IT SITS (`D IN (?, 1)` is `D <= 1`, `D IN (1, ?)` is `D >= 1`, so `D NOT IN (?, 1)` is `D > 1`, three rows where the desugared OR gives five); on a SMALLINT or NUMERIC column the NaN is ABSORBED AS ZERO (`SM IN (?, 1)` picks up the SM = 0 row); an INTEGER column does neither, and the boundary between the three is UNCHARACTERISED. **The narrower cap — refusing the equality only on approximate sides — was implemented, measured and REJECTED: it turned `NOT IN`'s recorded refusal into a wrong row set.** **TEETH: 63 red of 174 on `/tmp/fcwire-prev-0306bba` with 0 red CONTROL cells.** `cargo test -p fire-crab-wire` **368** / `-p fire-crab-ods` **161**. **`qa/serve-real-dblparam.sh` 422 green with SIXTEEN of its `eng_only` NaN cells PROMOTED** — they recorded a refusal and the server now answers each with the engine's own answer on every path, which is exactly what a self-expiring cell is for.
  **STILL DIVERGENT, MEASURED BY THIS ROUND'S PROBE AND RANKED — none of it this chunk's, all of it live on HEAD.**
  **(a) A NaN THAT IS A ROW VALUE, not a bound parameter — THE TOP CANDIDATE, and it is the same law one site deeper.** `value_cmp`'s approximate arm still does `partial_cmp().unwrap_or(Equal)`, so a NaN arriving as a COLUMN compares EQUAL to everything: `WITH C AS (SELECT CAST(? AS DOUBLE PRECISION) AS P ...) SELECT m.ID FROM M m, C WHERE m.D = C.P` answers EVERY ROW where the engine answers none, `m.D < C.P` answers none where the engine answers every row, and `m.ID >= C.P` is answered at all where the engine SPLITS — a guess, not merely a wrong answer. The engine's answers there match the law above exactly, so the rule is already written; what is missing is routing the comparator through it.
  **(b) NULLIF WITH A NaN — A DELETE THAT EMPTIES A TABLE.** `DELETE FROM M WHERE NULLIF(D, ?) IS NULL` [NaN] removes ALL SIX ROWS where the engine touches NOTHING, in both spellings, re-measured three times; the probe also pinned the right answer including three-valued logic (over a table with a NULL row the engine deletes exactly the genuine NULL). Same cause as (a): `=` against a NaN must be FALSE, and inside NULLIF it is TRUE here.
  **(c) A CHECK CONSTRAINT READS IT THE OTHER WAY.** `INSERT INTO CKD (ID, D) VALUES (1, ?)` [NaN] against `CHECK (D < 100)`: the engine STORES the row (a NaN is the lesser, so the check passes) and this server REJECTS it with a constraint violation — row-mutating in the inverse direction, a row refused that the engine keeps.
  **(d) THE CANONICAL TEXT OF A NON-FINITE DOUBLE, and it is NOT cosmetic — it CHANGES A VERDICT.** The engine writes `nan`, `inf`, `-inf` in LOWER CASE and the short spelling; this server writes `NaN`, `Infinity`, `-Infinity`. `SELECT CASE WHEN CAST(? AS VARCHAR(30)) = 'inf' THEN 1 ELSE 0 END` is **1 on the engine and 0 here**, and the `-inf` twin likewise. **COUNT THE ROUTERS: eleven places the wrong text reaches, two of them row-mutating** (a stored VARCHAR column and a trigger's computed text), measured three times and confirmed byte for byte.
  **(d2) AN AGGREGATE OVER A STORED NON-FINITE.** With an infinite row in the table the engine REFUSES `SUM(D)` / `AVG(D)` and this server returns `Infinity`; with a NaN row `MIN(D)` is `NaN` on the engine and `0` (or the smallest finite value) here. ENG-ONE on every path, measured four times.
  **(d3) AND A CELL THAT MEASURED NOTHING, found by the probe in its own harness:** `CAST(? AS VARCHAR(3))` with a non-finite raises on BOTH servers, so an ERR-vs-ERR comparison scored it `ok` — while the engine raises *Arithmetic exception, numeric overflow, or string truncation* and this server raises a *Conversion error from string*. **Two refusals are not agreement until their messages are compared**, which is the same law the gates learned for values.
  **(d4) +-INFINITY ITSELF IS CLEAN** and needs nothing: a 216-cell plain-comparison matrix over ten column types is ENG-ONE and matching on every cell, with no operand-order dependence and no access-path split, and BETWEEN / IN / NOT IN / IS DISTINCT FROM with an infinity are equally clean. It is worth recording as a negative result, because the NaN law's shape invites the assumption that the infinities share it and they do not.
  **(e) THE `IN`-LIST LAW ITSELF** — the range-on-DOUBLE / zero-on-SMALLINT / neither-on-INTEGER readings above, each ENG-ONE and so answerable in principle, and each reaching DML.
  **(f) A NUMERIC BIND AGAINST A VARCHAR COLUMN SPLITS** heap vs indexed and this server answers anyway — but the ORDINARY-bind controls (`S = ?` [2.5], `S > ?` [-4]) split identically, so it is the text-column conversion family and not a NaN matter at all.
  **(g) AN EXISTS SUBQUERY'S LOWER BOUND DOES NOT SPLIT** (`EXISTS (SELECT 1 FROM MI i WHERE i.ID = o.ID AND i.ID > ?)` is ENG-ONE) where the law's access-path clause predicts it would, so the refusal there is an OVER-REFUSAL the next round can narrow: the clause bites only when the NaN term drives the retrieval it sits in.
- **DONE 2026-09-20 (`qa/serve-real-dblparam.sh` NEW at 422 cells, floor 422) — A DOUBLE MESSAGE BOUND TO AN EXACT SLOT IS COMPARED AS A DOUBLE, AND FOUR ROUTERS THAT NEVER SAW THE LAW.** Every JS, Python and Delphi client sends a non-integral number as a `blr_double` message whatever the describe says, so `WHERE PRICE > ?` bound 2.5 against a NUMERIC column failed on EVERY predicate path and every procedure argument: the previous binary described the slot exactly, as the engine does, and then raised a generic *Dynamic SQL Error* AT EXECUTE — the worst category the house rule names. **THE LAW:** the engine ANNOUNCES the slot EXACT (`ID > ?` sqltype 496 LONG, `BI > ?` 580 INT64, `H2 > ?` 32752 INT128) and then COMPARES THE DOUBLE UNROUNDED — `ID = ?` [2.5] takes NO row where a rounded bound takes row 2 or 3, `ID > ?` [2.9999999] takes 3;9 where a rounded one takes 9 alone, `SM >= ?` [1e-300] skips the SM = 0 row, `ID < ?` [3000000000.5] never raises. **THE STORE LAW IS A DIFFERENT LAW:** scale to the target, add (0.5 + 1e-14) away from zero, truncate, range-check — 2.5 → 3, -2.5 → -3, 7.245 → 7.25 into NUMERIC(9,2) and 7.2 into NUMERIC(18,1), NaN → 0, ±Inf and overflow → 22003; 2147483647.4 fits an INTEGER and does NOT fit a NUMERIC(9,2). **THE ACCESS PATH IS PART OF THE ANSWER:** an index on an INT64 / INT128 / NUMERIC(18,x) / NUMERIC(38,x) column ROUNDS the double to the key's scale to build its bound and re-checks each row as a double, so the SCANNED and INDEXED answers DIFFER (`BI > ?` [2.5] is 3;9 on a heap and 9 on its indexed twin); SHORT- and LONG-backed keys are DOUBLE keys and do not round. Every section-1 cell is emitted TWICE, once per table, and where the engine's two answers differ this server REFUSES rather than picking one. **NaN HAS NO SINGLE ENGINE ORDER** and is refused: against a DOUBLE column cmp(D, NaN) = cmp(NaN, D) = -1 in BOTH operand orders while `D IN (?, 99)` takes every row; the previous binary read a NaN as EQUAL to everything (`partial_cmp().unwrap_or(Equal)`) — wrong answers, including a wrong DELETE. **MEASURED SURPRISES, pinned:** UPDATE OR INSERT ... MATCHING compares the RAW double against the key and then INSERTS the ROUNDED value, so [0.6] and [2.5] raise a PRIMARY KEY violation; NULLIF compares the raw client double before its slot converts it (`NULLIF(?, 3)` [3.4] is 3, not NULL — the previous binary answers NULL and WRITES NULL).
  **ROUND 13 (this session) FOUND FOUR THINGS THE FIRST TWELVE DID NOT, three of them by COUNTING ROUTERS and one by READING THE CODE.**
  **(1) `Predicate::keys_known` WAS WRITTEN, CARRIED AND NEVER READ.** `grep -nE "(if|&&|\|\||!)\s*[a-z_.]*keys_known"` returned NOTHING, while its doc claimed the router paths refuse. Measured, they did not: a JOIN's combined row, a derived table, a CTE, a UNION leg and a view build their filter without reading a catalog, so `Predicate::keys` is EMPTY there, and the `(?)` rung read that empty list as "no index leads with this column" and answered the SCAN reading — **SEVEN WRONG ANSWERS** over an indexed relation (`BI > (?)` [2.5] through a derived table, a CTE, a view, a JOIN and a UNION leg; `N18 > (?)` [2.45] the same at scale 1), one of them reached through a DML IN-list, every one also wrong on the previous binary. The rung now refuses wherever the path is UNKNOWN, which is exactly the policy the BARE `?` spelling already had (`ColKind::WideExact` is marked from the column's WIDTH alone). **THE COST IS GATED, NOT HIDDEN** (`11E COST` cells): a JOIN, a view, a derived table, a CTE and a selectable procedure's BIGINT output over a HEAP relation, which the previous binary answered CORRECTLY, now refuse. And an EMPTY key list is not an UNKNOWN one — `resolve_having` now SAYS its fold row has no keyable column, because an aggregate is never an index key; without that the fix regressed `HAVING SUM(ID) > (?)` and **the gate's floor cell caught it on the first run**.
  **(2) MERGE'S CONDITION IS A DIFFERENT ROUTER — four ROW-MUTATING wrong answers.** MERGE DESUGARS its ON predicate and its WHEN MATCHED AND condition into the TEXT of a per-row UPDATE / DELETE, so the `?` is gone before the predicate is planned and the double's reading goes with it — the value is spelled as a decimal literal, which an INT64-backed key compares EXACTLY. `MERGE INTO MBI .. ON T.BI > ?` [2.5] UPDATED and DELETED row 3 where the engine's indexed bound rounds to 3 and matches nothing; so did the `WHEN MATCHED AND` arm, the multi-row `USING SRC .. ON T.ID = S.ID AND T.BI > ?` form and the scaled INT128 key `H2 > ?` [2.495] — while the plain `UPDATE .. WHERE BI > ?` twin already refused. The condition now refuses a double the key's scale cannot hold, and **that costs nothing against its own sibling**: the plain DML twin refuses the same bind on the HEAP as well (measured).
  **(3) A NUMERIC FUNCTION'S `?` IS A DOUBLE SLOT, NOT THE DESTINATION'S — and it BYPASSED CONSTRAINTS.** Measured on the engine's own input SQLDA: the `?` argument of ABS, SIGN, ROUND, ROUND(?,n), TRUNC, TRUNC(?,n), FLOOR, CEIL and CEILING is announced `sqltype 480 DOUBLE len 8` — the destination plays no part, exactly as it plays none in a CONDITION. This server passed the destination down, so the value CONVERTED BEFORE THE FUNCTION READ IT and the fraction the function exists to inspect was gone: `SET N = FLOOR(?)` [2.5] stored 3 against the engine's 2, `CEIL(?)` [2.1] stored 2 against 3, `SIGN(?)` [0.4] stored 0 against 1 — **a sign flipped to zero** — and `ROUND(?, 1)` [2.45] stored 2 against 3. **`INSERT INTO CKD (ID, N) VALUES (5, FLOOR(?))` [3.5] COMMITTED A ROW WHERE THE ENGINE HAS NONE** (its FLOOR(3.5) = 3 fires `CHECK (N <> 3)`), and `INSERT INTO U (ID, N) VALUES (FLOOR(?), 5)` [3.5] inserted a new PRIMARY KEY where the engine collides on key 3 and raises; the wrong value also travelled through a BEFORE trigger and a COMPUTED column, three columns wrong from one bind. All pre-existing. **CAPPED AT AN INT128-BACKED DESTINATION**, and the cap is measured rather than cautious: this server's store path refuses an APPROXIMATE source for an INT128 / NUMERIC(38,x) column at prepare, so typing the slot DOUBLE there turned three cells the previous binary answered EXACTLY like the engine into refusals — three floor cells for one wrong answer is the wrong trade, so those widths keep the destination's slot and their one remaining divergence (`SET H = CEIL(?)` [2.1] stores 2 where the engine stores 3) is RECORDED on both sides.
  **(4) A MERGE CONDITION THAT COMPUTES WITH ITS PARAMETER — two more row-mutating answers, and one statement that diverged TWICE.** The previous chunk's law says a `?` that is an OPERAND OF ARITHMETIC is CONVERTED AT ITS SLOT before the arithmetic runs, while a whole side is read as it arrived; MERGE's text desugar spells the operand as its raw value and computes from the wrong number. `ON T.ID > ? + 0` [1.5] matched row 2 as well as row 3 where the engine rounds the operand to 2 and matches row 3 alone; and `ON T.ID = ? + 0` [2.5] matched NOTHING where the engine rounds to 3 and DELETES row 3 — **and then ran the NOT MATCHED branch and INSERTED a row the engine never inserts**. It now refuses a double the slot's scale cannot hold, and the controls that say the refusal is about the CONVERSION and not about arithmetic, a condition or MERGE are gated beside it (an integer message, an integral double, a scale the slot holds, a whole side, a value position, and no parameter at all — all six green).
  **(5) A PANIC WEARING A REFUSAL'S CLOTHES.** `SELECT ID, SUM(CAST(? AS INTEGER)) OVER () FROM T` PANICKED the connection thread on both binaries — an unnumbered slot is `usize::MAX`, `i + 1` wrapped to 0 in a release build, `resize(0)` then `sink[usize::MAX]` — and the crash reaches a client as *Connection to Firebird server was lost*, i.e. as an ordinary query error, **the same word as a refusal**. Both copies of the sink helper now refuse an unnumbered slot at prepare; the gate's `panic_free` cell is what keeps the four window cells beside it meaning anything.
  **(6) AND ONE FIX THAT WAS INERT, CAUGHT BY MEASURING IT RATHER THAN BY REASONING ABOUT IT.** A first cut split `resolve_having`'s key declaration on "every term is an aggregate", to stop a grouped KEY (`GROUP BY ID, BI HAVING BI > (?)`, which IS index-matchable) from answering the scan reading. It changed NO cell — the grouped rung never reaches `Predicate::bind`'s ExprCond arm at all — so it was reverted rather than left in: **unexercised code carrying a claim is the exact defect this round was opened by.** The wrong answer it was aimed at is gated in section 14 with both answers pinned.
  **TEETH: 241 red of 422 on `/tmp/fcwire-prev-0e5a8f4` (191 of 343 on the inherited gate, 203 of 361 and 223 of 405 at the intermediate ones), with **0 of the gate's 44 FLOOR cells red there and 0 red section-9 CONTROL cells in every run**. Seven numbered CONTROLs are red on that binary as `CONN_ERR - the cell never ran`, which is the measurement rather than a flaw: **the previous binary PANICKED 24 TIMES during its run and `panic_free` failed there**, where this one logs none.** `cargo test -p fire-crab-wire` **367** / `-p fire-crab-ods` **161**. **EIGHTEEN CODE-PATH-ADJACENT GATES RUN AGAINST THE FINAL BINARY:** cmpparam 1894, routerparam 66, selparam 44, slottype 73, paramshapes 177, dmlparamexpr 25, corrsub 27, coalescesib 42, trigviewparam 35, havingexpr 28, havingparam 11, castparam 23, aggexpr 38, aggexpr2 45, groupexpr 30, groupexprkey 9 all clean; `bindconv` (1) and `numericwhere` (14) red IDENTICALLY on the previous binary, so pre-existing and recorded, not this chunk's.

  **STILL DIVERGENT AFTER ROUND 13, MEASURED AND RANKED — every one of these was found by the round's own eight-family refuter and every one is shared byte for byte with the previous binary unless said otherwise.**
  **(a) THE NaN REFUSAL IS WIDER THAN THE SPLIT, and it costs the floor — THE TOP CANDIDATE.** 674 three-way cells: for `D <= ?`, `D > ?`, `? > D` and a NaN-bound comparison in a PURE PROJECTION (`CASE WHEN D > ? THEN 1 ELSE 0 END`, where no index can be involved at all) the engine gives the SAME answer on the heap, on the indexed twin, on the expression-index twin and through both views — **no split, and the previous binary answered every one of them correctly while this one refuses**, including the ROW-MUTATING `UPDATE M SET NN = 77 WHERE D <= ?` [NaN] (the engine updates all six rows) and its `INSERT .. SELECT` twin. The machinery to fix it is already in the tree and simply not consulted for the bare spelling: `Rhs::Param` and `Term::ExprParam` carry a `mirrored` flag saying WHICH ORDER WAS WRITTEN, and [nan_whole_side_keeps] already uses it for the `(?)` rung. The rule to route through it: the FIRST-WRITTEN operand is the lesser against a DOUBLE side, and only the FALSE verdicts and the UPPER-bound TRUE verdict survive an emptied index range.
  **(b) A NaN INSIDE NULLIF IS STILL READ AS EQUAL — a table-emptying DELETE.** `DELETE FROM M WHERE NULLIF(D, ?) IS NULL` [NaN] deletes EVERY row where the engine deletes none, and the parameter-first spelling `UPDATE M SET NN = 77 WHERE NULLIF(?, D) IS NULL` updates all six where the engine updates none. No split on M, MI or ME.
  **(c) A NaN AGAINST A VARCHAR COLUMN:** the engine converts the text to a number and sorts the NaN BELOW everything, so `S > ?` [NaN] is true for every row; this server answers false for every row (`UPDATE`/`DELETE` therefore mutate nothing where the engine mutates all).
  **(d) THE CANONICAL TEXT OF A NON-FINITE DOUBLE IS WRONG, in a STORED column and not only in a projection:** `UPDATE M SET S = CAST(? AS VARCHAR(20))` [NaN] writes `NaN` where the engine writes `nan`, and the infinities write `Infinity` / `-Infinity` against the engine's `inf` / `-inf`.
  **(e) THE WIDE-KEY REFUSAL IS ALSO WIDER THAN ITS SPLIT.** Over the keyed twin the engine's two paths differ in only THREE of 18 measured operator x value cells (`> (?)` at 2.5 and 2.6, `< (?)` at 2.4); the other 15 are over-refusals the previous binary answered. So are `ABS(BI) > (?)` and `CAST(ID AS BIGINT) > (?)`, which can match NO index in any fixture, and `ID + 0` / `NM + 0` / `BI + 0` on relations carrying NO computed index. And an `ORDER BY` on the bound column makes the ENGINE SCAN (`MI.BI > (?) ORDER BY BI` is 3;9, not 9), as does a second conjunct with a better index, and as does a LEFT JOIN with the predicate in the ON clause. **BUT DO NOT NARROW THIS FROM THE SELECT SIDE:** on `UB` (PK on a BIGINT) the engine SCANS for the SELECT and INDEXES for the DML — `SELECT .. WHERE BI > (?)` [2.5] is row 3 while `UPDATE UB SET V = 0 WHERE BI > (?)` changes NOTHING — so a rule read off the SELECT would put back exactly the wrong mutation the refusal prevents (the previous binary updated row 3 there, and deleted it in the DELETE twin).
  **(f) A GROUPED KEY'S HAVING** (`GROUP BY ID, BI HAVING BI > (?)`), gated in section 14 with both answers pinned: index-matchable, unlike an aggregate's, and [resolve_having]'s key declaration is too coarse for it. The grouped rung never reaches [Predicate::bind]'s ExprCond arm, so narrowing it means routing the grouped key's own retrieval.
  **(g) THE INT128 STORE PATH REFUSES AN APPROXIMATE SOURCE AT PREPARE**, which is the cap on (3) above and the reason `SET H = CEIL(?)` is recorded rather than fixed.
  **(h) STRUCTURAL REFUSALS THAT ARE NOT THIS LAW AT ALL, each proven by its INTEGER-bind control refusing identically on both binaries:** a scalar PSQL FUNCTION called with a `?` (`SELECT F1(?)`), an EXECUTE BLOCK with ANY input parameter, a procedure with an INT128 ARGUMENT (refused even with a literal argument), a `?` in an INSERT..SELECT select list, and a selectable procedure reached through UNION ALL / a scalar subquery / a JOIN / an IN or EXISTS subquery / a lateral argument. The one refusal that IS squarely inside this law and worth its own slice: a procedure's OUTPUT stream compared with a double (`SELECT .. FROM P2(3,3) WHERE RA > ?`), where the stream has no index and nothing can split.
- **DONE 2026-09-19 (`qa/serve-real-cmpparam.sh` NEW at 1894 cells, floor 1894; TWELVE ROUNDS, 2026-09-18 into 2026-09-19) — A `?` INSIDE ARITHMETIC ON ONE SIDE OF A COMPARISON, AND A `?` IN A CASE/IIF CONDITION, IS TYPED FROM THE OTHER SIDE'S COMPLETE DESCRIPTOR.** The previous two entries recorded both as `eng_only`; this is that chunk. A `?` that is an operand of `+ - * /`, of unary minus or of `||`, at any depth of bare arithmetic, on one side of a comparison whose other side carries no `?`, takes the other side's **dtype, scale, subtype, length, charset AND nullability** — the engine's MAKE_desc of that side, the arithmetic **sibling ignored**: `ID * ? = 2` → LONG NOT NULL (from the literal), `? + 1 = ID` → LONG Nullable, `ID * ? = NM` → LONG scale -2 subtype 1, `ID * ? = 2.5` → INT64 scale -1, `S || ? = 'abx'` → TEXT len 3 NONE, `SUM(ID) > ? + 1` → INT64 Nullable. **NULLABILITY (L2) comes from the other side too**: a literal NOT NULL, a nullable column Nullable, COUNT NOT NULL, and SUM/AVG/MIN/MAX Nullable **even over a NOT NULL column**. The same law inside a CASE/IIF CONDITION in every clause, with a bare `?` on either side of that condition (`IIF(ID = ?, 1, 0)` → LONG Nullable). **THE SPELLINGS DIED IN FOUR DIFFERENT PLACES, which is why one fix was never going to do it:** the unparenthesised forms died in the TOKENIZER ([texpr_atom_bare] had no `Tok::Param` arm at all), the parenthesised twins in [resolve_proj_expr]'s catch-all, a condition's `?` in [resolve_raw_cond] (no sink — the new [resolve_raw_cond_sink] routes `Cmp` to [resolve_cmp_pair] and delegates everything else, so patterns and `? IS NULL` keep refusing), and [raw_has_param] never descended a conditional's CONDITION, so a conditional with a `?` only in its condition took the param-free router and left its slot at `usize::MAX`. With them: [resolve_expr_term]'s bare-lhs branch and its rhs `CmpExpr` branch, [resolve_cmp_param_side] / [param_desc_from_expr] / [claim_param_slot] / [cmp_param_rung] / [arith_operand_rung], [resolve_having]'s fold block plus a new [agg_param_flags], and at execute time [expr_has_param] / [subst_params_expr] descending conditions and every `cmp_sides` wrapper. **THE VALUE LAW (L6) IS SIDED, AND THE FIRST CUT HAD THE SIDE WRONG IN 22 OF 96 MEASURED CELLS — including a DELETE that took the wrong row:** under `*` **exactly one operand is rounded to an integer (half away from zero) and the other keeps its fraction**, and which one is measured, not reasoned — the **LEFT** operand when the slot AND the sibling are both SHORT/LONG (`NM = ? * ID` ['7.25'] → none while `NM = ID * ?` → 1 keeps), the **RIGHT** otherwise (an INT64 slot, or an INT64 sibling under a SHORT/LONG slot). A `/` divisor rounds for every exact slot and a dividend keeps; `+`/`-` convert at the NODE's scale `min(slot.scale, sibling.scale)` (`ID = 2.5 + ?` ['-0.45'] → 2 and `NM = 2 + ?` ['0.5'] → 3 pin both directions); an APPROXIMATE sibling makes the node double and the value is read whole whatever the slot. **THE SIBLING WIDTH IS A WHITELIST, NOT A TABLE** ([mul_sibling_width], round 4): three refuter rounds each found a sibling shape whose BLR width an open-ended table got wrong — a scaled literal, ABS over a SHORT, a derived / CTE / view column carrying a literal — and **every one was a WRONG ANSWER, rows mutated by UPDATE / DELETE / INSERT .. SELECT**, so the rule now sizes only a base-table column, an integer or scaled literal under minuses, an explicit exact CAST, a direct `?`, COUNT/SUM/AVG and MIN/MAX over a base column in HAVING, and parentheses around those; everything else REFUSES AT PREPARE and is gated `eng_only` with the engine's measured value and width (62 boundary cells in section 4l alone). A scaled decimal literal is NARROW when its **unscaled integer fits 32 bits** — the BLR width, not the INT64 the projection announces. **A BARE `?` THAT IS A WHOLE SIDE is NEVER converted into its slot**: the described slot only tells the client what to send and the compare reads the client's value whole — `WHERE IIF(ID = ?, 1, 0) = 1` ['2.4'] → none, `SELECT IIF(? = 2.5, 1, 0)` ['2.45'] → 0;0;0, `-? = ID` ['-2.4'] → none (the first cut cast it in and rounded: 2 for ['2.4']). Its TEXT GRAMMAR is a step-for-step port of the engine's own reader ([engine_cmp_class] ← cvt.cpp `CVT_get_numeric`, [engine_decompose_i128] ← `cvt_decompose`, [cmp_align_check] ← `adjustForScale`/`Int128::setScale`): the text classes as int64, int128 or neither, **both operands align to the finer scale in that width, and the column aligns ROW BY ROW — so the raise depends on the row** (`IIF(N = ?, 1, 0)` bound '1.' + 38 zeros answers row 1 and raises on row 2). **TEETH, EVERY ROUND ON `/tmp/fcwire-prev-c34c1c8`, CONTROLS GREEN IN ALL TWELVE:** 176 red of 243 (R1), 359/429, 437/535, 669/891, 816/1142, 937/1385 (R6), 812/1394 (R7), 886/1527 (R8), then per-section teeth as the gate outgrew a whole-gate diff — 76 of 159 on section 4p (R9), 6 of 24 on 4q (R10), 32 of 51 on 4r against the round-10 binary (R11), and **213 red of 271 on the round-12 cells with the ROUND-11 binary also 212 red**, which is the measurement that the caps CHANGED behaviour rather than described it. The 60 designated controls (17 `both_refuse`, 19 already-worked, 24 recorded `eng_only`) are 0 FAIL on the previous binary in every round, and no `floor:` cell — a cell the previous binary answers exactly like the engine — was ever red there. `cargo test -p fire-crab-wire` **355 passed** (349 at round 1; the new [round12_caps_mark_and_refuse_the_chunk_new_surface] asserts the caps directly, and **two pre-existing asserts were re-measured rather than kept**, one of them my own grammar assert that miscounted a 19-digit text as canonical — the test caught it before any gate did). Nine neighbouring gates green on the final binary (routerparam 66, selparam 44, trigviewparam 35, dmlparamexpr 25, slottype 73 PASS, corrsub 27, paramshapes 177, coalescesib 42, bindconv 198 PASS), and the FINAL sweep ran **every port-argument gate plus the 13 database-path gates against this binary: 436 green, 15 not**, every one of the 15 classified against the previous binary as pre-existing or self-expiring and none of them this chunk's. **THE SCOPE CUT (round 7) IS PART OF THE RESULT, NOT AN APOLOGY FOR IT.** Five families of rounds 5-6 went BACK to the previous committed binary's tree because each sat OUTSIDE this chunk's law and each had produced a regression: the DML destination router's `? * k` / `k / ?` side rule at the destination's width (R-K), the simple-CASE THEN/ELSE bare `?` bound whole (R-J), the refinement of a conditional's branch from the comparison's other side (R-I), the zoned CAST evaluation (R-H), and the compare-side negation chain under an operator (R-S7). **A law that is right and out of scope is still a regression**, and the only part of round 5 kept is the one the engine forced: the negate's overflow check looks THROUGH the implicit slot cast (`CS_SLOT_CAST`) to the MESSAGE's width. **THE CAPS (round 12) ARE THE HONEST END OF ELEVEN ROUNDS THAT DID NOT CONVERGE.** Every refuter found new wrong answers, almost all from exotic TEXT spellings or from one chunk-new `?` sharing a statement with an older reading, so round 12 added NO new law and capped what the chunk-new code accepts, driven by ONE mark on the slot's descriptor (`PARAM_CHUNK_NEW`, set where the descriptor is REGISTERED — **not in a thread-local index set, which cannot survive the renumberings a nested plan does**: a body's slots return from a private sink through `CORR_PRM`, an INSERT..SELECT source re-plans into a second sink, two rewrite paths clear the sink and plan again). **WHAT THIS CHUNK REFUSES BY DESIGN, each measured and gated:** (K1) a statement carrying a chunk-new slot AND a classic one — **MIXED STATEMENTS refuse at prepare**, exactly as the previous binary refused them; (K3) a chunk-new comparison whose `?` is read in DOUBLE — an operand rung beside a DOUBLE slot or sibling, an approximate CAST or COALESCE in a condition — while the bare `?` as the WHOLE side stays (measured right in every round); (K4) a TEXT slot beside a conditional's bare-`?` value arm, and a one-operand `TRIM` over a `?` in any conditional arm (**the previous binary PANICKED the server thread on every non-null bind — the final verification lost the connection 52 times over its 216 such cells and found 54 `panicked` lines in that binary's log, while this binary logged none**; the two-operand `TRIM(' ' FROM ?)` keeps answering); a NEGATED whole side over a SHORT/LONG/NUMERIC slot (round 11's W1 — the engine's negated read of a 16-plus-digit fraction is unmeasured, so it refuses rather than guesses); an IN list inside a condition mixing a `?` item with a `?`-free one; a chunk-new statement calling an alias-qualified built-in (`b.IIF(..)`), as the engine does. **AND (K2) THE CANONICAL-TEXT BOUNDARY, stated as a boundary and not as agreement:** a TEXT bound into a chunk-new NUMERIC slot must match `^[+-]?[0-9]+(\.[0-9]*)?$` or `^[+-]?\.[0-9]+$` with **at most 18 significant digits** (trailing fraction zeros counted), or the statement raises the engine's *conversion error from string* **at EXECUTE, before any row is read or written** — where the ENGINE ANSWERS IT. That is a recorded divergence, held by a new `boundary_err` helper that FAILS LOUDLY if this server ever answers such a cell, and 4,567 of round 12's 49,532 re-measured cells land in it. Integer and double messages, and chunk-new TEXT slots, are untouched. **THE CLOSING MEASUREMENTS: round 12's own 49,532 cells three-way — 0 shapes where the previous binary was right on every measured bind and this one differs, 0 new wrong answers inside the grammar, 0 prepare-then-fail on a canonical bind, 0 panics**, plus an INDEPENDENT commit-gate run of 3,644 three-way cells after it — 2,904 agree, 469 refused at prepare and shared with the previous binary, 242 both raise, and all 29 divergences shared byte for byte with the previous binary (0 introduced), with 642 cells the previous binary answered right all identical. **STILL DIVERGENT, RECORDED, NOT PINNED GREEN** (a wrong answer cannot be a green cell; each shared byte for byte with the previous binary unless said otherwise): the classic plain-column `N = ?` and `N = '<lit>'`, and `WHERE SM IN (?, 7)` which the engine rounds; the parameter-free `SELECT - -2147483648` → 2147483648 where the engine raises Integer overflow, and SIGN/ABS/MOD over `-SM` at the SHORT minimum; the classic `NM = ?` with a 38-digit text; the DML destination side rule at the destination's width (`SET N18 = ? * 2` ['2.5'] stores 6, the engine 5.0; `SET NM = 2 / ?` 0.8 against 0.66; `NM = NM / ?` 2.9 against 2.42); a conditional's arithmetic branch typed from its sibling (`IIF(ID = 2, ? * 2, 1.5) = 5.0` ['2.5'] none, the engine 2); the simple-CASE bare `?` cast into its slot (`CASE ID WHEN 2 THEN ? ELSE 0 END = 2` ['2.4'] answers row 2, the engine none); `CAST(<WITH TIME ZONE> AS TIMESTAMP / DATE / TIME)` raising where the engine converts to the session zone; and the announcement gaps — a `?` in a CASE subject / IIF condition / HAVING slot announced Nullable where the engine carries NOT NULL, a FLOAT column announced DOUBLE. **INSTRUMENT, not server (each cost a run before it was written down):** node-firebird sends a JS fraction as a **blr_double** message whatever the describe says, a string into a temporal slot as **blr_timestamp**, cannot fetch DECFLOAT, and **cannot bind NULL into an INT128 or DECFLOAT slot at all** (TypeError in its own `connection.js`) — 36 cells were excluded for that, not explained away. Every fraction in the gate is therefore bound as a STRING. **GATE HYGIENE FOUND ALONG THE WAY, NOT THIS CHUNK'S AND NOT SWEPT UNDER IT:** self-expiring cells have fired and are waiting to be promoted in `simexpr` (1), `exprpattern` (4), `exprpattern2` (5), `exprvalue` (5), `blobexpr` (1), `list` (1) and `recorder` (4) — all identical on the previous binary, so all pre-existing — `bindconv`'s was promoted in round 11 (198 OK), and `qa/serve-real-where.sh` carries **two cells that measure nothing** under the `mkjoindb` fixture (lines 115-117 and 127-129: both sides return 0 rows and `compare()` passes on empty-equals-empty). **NEXT, RANKED AND MEASURED:** a **double message bound to an exact slot** (a JS, Python or Delphi client sends any non-integral number as `blr_double`, so `WHERE PRICE > ?` bound 2.5 against a NUMERIC column fails today on every predicate path and every procedure argument — low overlap with this chunk, two arms in `Predicate::bind` plus one in `bind_proc_args`), then the **grouped ORDER BY / HAVING key expressions**, whose COLLATION SORT-ORDER fix (13 wrong answers, grouped and `||` sorts coming back in byte order under UNICODE / PXW_INTL) **must be split off and landed first, or that chunk adds new wrong answers of its own**.
- **DONE 2026-09-18 (`qa/serve-real-routerparam.sh` NEW at 66 cells, floor 66) — A TYPED OR SIBLING-TYPED PARAMETER IN THE FOUR REMAINING ROUTERS: the AGGREGATE ARGUMENT, the GROUP BY KEY, an ORDER BY EXPRESSION and a HAVING SIDE.** The previous entry recorded them as different routers, each resolving through [resolve_expr], which refuses every `?`. Re-probed first (the house rule): every one still refused, and the engine answers all of them — `SUM(CAST(? AS INTEGER))`→6, `SUM(ID * CAST(? AS INTEGER))` (INT128 out), `SUM(COALESCE(?, N))`, `COUNT(CAST(? AS INTEGER))` bound NULL→0, `COUNT(DISTINCT ..)`, `AVG(CAST(? AS NUMERIC(9,2)))`, `MIN(CAST(? AS VARCHAR(3)))`, `SUM(ID) + CAST(? AS INTEGER)` (an expression OVER the fold), the same over a derived table and a join; `ORDER BY CAST(? AS INTEGER)` (a constant key), `ORDER BY COALESCE(?, ID) [DESC]`, `ORDER BY ID * CAST(? AS INTEGER)` bound -1, a join's, a derived table's, under DISTINCT and FIRST, and a grouped query's `ORDER BY SUM(ID * CAST(? AS INTEGER)) DESC`; `HAVING SUM(CAST(? AS INTEGER)) > 3`, `HAVING COUNT(CAST(? AS INTEGER)) > 1`, `HAVING COALESCE(?, 0) > 0` both ways, `HAVING SUM(ID) > COALESCE(?, 0) + 3`; `GROUP BY CAST(? AS INTEGER)`, `GROUP BY COALESCE(?, N)`, `GROUP BY N + CAST(? AS INTEGER)`, `GROUP BY 1` naming `COALESCE(?, N)`, and the key beside a typed constant item (two slots). **THE TYPING LAW WAS ALREADY WRITTEN** (chunks 46/47: a CAST target, or the reconciled siblings of a conditional) — one helper, [resolve_expr_sink], routes a shape with a `?` through [resolve_proj_expr] and everything else through [resolve_expr], and the four routers ([resolve_agg_src], [parse_group_by], the ORDER BY resolver closures of the single-table, join, derived and grouped planners, and [resolve_having]'s aggregate arms) take the statement's sink. **WHAT EACH ROUTER ACTUALLY LACKED WAS THREE THINGS, not one:** (1) the SINK; (2) the NUMBERING of its `?` — measured on the engine's input SQLDA, parameters number in TEXT ORDER: the select list with an aggregate's argument IN PLACE (`CAST(? AS SMALLINT), SUM(CAST(? AS INTEGER)), CAST(? AS BIGINT)` describes SHORT, LONG, INT64), then WHERE, GROUP BY, HAVING, ORDER BY — and [renumber_raw_params] never descended into an aggregate, [renumber_proj_params_from] skipped `SelItem::Agg`, the HAVING term parser cloned a `Tok::Agg` target unnumbered (twice: the lone form and the arithmetic leaf), and the clause-level routers parse from TEXT at plan time with nothing to number from; (3) the execute-time BINDING — [bind_plan_params] substituted the projection's `?` only, so an aggregate source, a group key or an order key reached eval as a bare `Expr::Param`. It now binds `AggSrc::{Expr,Pair,Percentile,List}`, `key_exprs` and every `OrderKey.expr`, and [plan_has_proj_param] reports them. **THE NUMBERING TRAP I WALKED INTO, caught by the gate's two-slot cell:** I numbered a GROUP BY key from the sink's LENGTH, and `SELECT CAST(? AS INTEGER) K, COUNT(*) FROM T GROUP BY CAST(? AS INTEGER)` refused — the projection's slots are NUMBERED before the keys parse but REGISTERED after ([build_group_items] runs on the keys), so the sink was short by the projection's count, the key took slot 0 twice, matched the item STRUCTURALLY, and slot 1 was never claimed. [parse_group_by] now takes the caller's COUNTER (projection + WHERE) as its first slot. **AND THE GROUPED DERIVED PLANNER parses its WHERE AFTER its GROUP BY**, so a `?` in both cannot number in text order there: that one pairing REFUSES rather than binding each other's value (gated), while either alone answers. **THE NUMBERING CELLS BIND DIFFERENT VALUES, so a swapped slot is a different answer:** `HAVING SUM(CAST(? AS SMALLINT)) > 3 ORDER BY SUM(ID * CAST(? AS INTEGER)) DESC` bound [2, -1] answers 4 and answers NOTHING swapped; the three-clause `WHERE .. CAST(? AS SMALLINT) GROUP BY CAST(? AS VARCHAR(5)) HAVING .. CAST(? AS BIGINT)` binds [1, 'x', 1]. **EVERY `both` CELL COMPARES THE WHOLE DESCRIBE** (every input and output slot), not the first line. **TEETH: 42 of 66 red on `/tmp/fcwire-prev-3132acd` with ALL 24 CONTROLS GREEN** (seven the engine itself refuses — `SUM(?)`, `SUM(ID * ?)`, `ORDER BY ?`, `GROUP BY ?`, `GROUP BY N + ?`, a keyed item whose `?` is not the key's; seven that already worked; the recorded cells). Twenty-three neighbouring gates clean by code path (selparam, coalescesib, havingexpr, havingparam, groupalias, params, paramshapes, the five castparam gates, slottype, dmlparamexpr, aggexpr, aggexpr2, groupexpr, groupexprkey, joinorder, derivnoalias, trigviewparam, and the fixture-fed groupby/having/orderagg); `cargo test -p fire-crab-wire` 348/0. **TWO SELF-EXPIRING CELLS FIRED AND WERE PROMOTED:** selparam's seven `eng_only` cells for exactly these routers now answer (promoted to `both`, values AND describes agree), and havingexpr's `HAVING u SIMILAR TO v` gap — closed by the SIMILAR TO expression-pattern chunk earlier today and never promoted, proven PRE-EXISTING on the previous binary before touching it. **FOUND ALONG THE WAY, PRE-EXISTING, RECORDED AND GATED (`desc_differs`):** the engine types a HAVING `?` from `COUNT(*)` INCLUDING its NOT NULL (`HAVING COUNT(*) > ?` describes the input INT64 NOT NULL) where this server announces every input slot Nullable — value right, announcement differs, on the previous binary too. The same policy shows on `SUM(IIF(ID=1, ?, 0))`'s input slot (chunk 47's recorded CASE/IIF announcement). **STILL REFUSED, RECORDED AND GATED (`eng_only`):** (a) THE COMPARISON-TYPING LAW — a `?` inside ARITHMETIC under a comparison is typed from the OTHER SIDE of the compare (measured: `HAVING SUM(ID) > ? + 1` describes INT64 Nullable from the SUM, `HAVING N + ? > 4` and `WHERE ID * ? = 2` LONG NOT NULL from the literal — the engine's setParameterType walks the arithmetic down), which [resolve_expr_term] refuses; and the same law inside a CASE/IIF CONDITION (`ORDER BY IIF(ID = CAST(? AS INTEGER), 0, 1)`, [resolve_raw_cond]) — the next candidate, its own chunk; (b) a GROUPED query's NON-AGGREGATE order expression (`GROUP BY N ORDER BY COALESCE(?, N)`) — the grouped ORDER BY closure admits aggregates only, and the param-free `ORDER BY N + 1 DESC` refuses identically, which is what proves it is not a parameter matter; the four-clause statement refuses on exactly that; (c) the grouped-derived WHERE + GROUP BY `?` pairing above.
- **DONE 2026-09-18 (`qa/serve-real-selparam.sh` NEW at 44 cells, floor 44) — A CONDITIONAL TYPES ITS PARAMETER FROM ITS SIBLINGS IN THE PROJECTION ROUTER TOO**, which is chunk 46's law in the SECOND router that needed it. Every shape below REFUSED here: `COALESCE(?, 0)`→1, `(?, 0.5)`→1.3, `(?, N)`, `(?, NM)`→1.25, `COALESCE(0, ?)` (parameter second), `(?, 0, 0.5)` (widest scale), `(?, N, BI)` (widest rank), two parameters in one node, a nested COALESCE (node-local), an all-text sibling, both NULLIF shapes, BOTH WHERE COMPARISON SIDES, a derived table's select list and a GROUPED query's select list. **AND THE RULE IS NOT THE DML ONE — measured, not copied: a SELECT LIST HAS NO DESTINATION to push down, so the engine types all four nodes from their siblings, where on the DML path only COALESCE reads siblings and NULLIF/CASE/IIF push the destination.** `coalesce_sibling_desc` is REUSED rather than written twice; it only had to take borrowed siblings so NULLIF's two boxes and CASE's branch results pass without cloning. **CASE AND IIF HAD NO ARM AT ALL** — they fell to the catch-all, which is why `CASE WHEN ID=1 THEN ? ELSE 0 END` refused while the engine answered. **TEETH: 23 of 44 red on `/tmp/fcwire-prev-cd65a97`, with every one of the eleven controls green** (six the engine itself refuses, five that already worked). Six sweep gates clean by code path, `cargo test` 706/0. **THE COMPILER CAUGHT THE ONE REAL SLIP**, not a gate: `coalesce_arg_raises` was a NESTED `fn` inside [resolve_dest_param_expr], invisible to the projection router (E0425) — it and `untyped_param` are module-level now, capturing nothing. **AND I MIS-SCOPED GROUP BY:** I read 53601 as the group-KEY path when it is the grouped planner's SELECT-ITEM branch. `GROUP BY COALESCE(?,0)` still refuses — and so does `GROUP BY CAST(? AS INTEGER)`, a fully typed parameter, **which is what proves it was never a sibling-typing matter**; [parse_group_by] resolves a key through [resolve_expr]. Moved from "must now answer" to recorded, where it belongs. **RECORDED, NOT FIXED, each gated:** (a) ORDER BY, `SUM(...)`, HAVING and the GROUP BY KEY are DIFFERENT ROUTERS, each resolving through [resolve_expr] — which has NO SINK to register a parameter slot into and 110 call sites against this router's 16, so widening it is its own chunk; (b) a `?` in a CASE/IIF CONDITION is typed from the COMPARISON, [resolve_raw_cond]'s rule, and that resolver refuses parameters; (c) CASE and IIF announce NOT NULL on the engine while every expression column here is announced NULLABLE by deliberate policy ([nullable]: "libfbclient renders the raw buffer instead of `<null>` for a NOT NULL announcement") — the VALUES are right and only the announcement differs, on shapes that refused outright before. **AND A PROCESS LESSON THAT COST A WHOLE PROBE RUN:** my build script captured `tail -50` of cargo's output, which kept "could not compile … due to 1 previous error" and HID the `error[E0425]` block that named the cause; the after-probe then measured a STALE BINARY and every "must now answer" cell read as still-refusing. The binary-versus-source timestamp printed beside each run is what caught it. **Capture the error, not the tail — and print what you are actually measuring.**
- **DONE 2026-09-18 (`qa/serve-real-coalescesib.sh` NEW at 42 cells, floor 42) — A COALESCE TYPES ITS PARAMETER ARGUMENT FROM THE RECONCILED TYPE OF ITS NON-PARAMETER SIBLINGS.** [resolve_dest_param_expr]'s Coalesce arm took `v.iter().find(|a| !raw_has_param(a))` — the FIRST sibling — and so agreed only when that sibling happened to BE the reconciled type. **That coincidence is what hid it: `COALESCE(?, 0.5, 0)` agreed while `COALESCE(?, 0, 0.5)` stored 1 against the engine's 1.3.** Measured on the engine's own input SQLDA and order-independent: **TEXT > DECFLOAT(34>16) > DOUBLE > FLOAT > exact**, and within the exact family the WIDEST rank, the MOST NEGATIVE scale and the HIGHEST `sub_type` (0 plain < 1 NUMERIC < 2 DECIMAL) — **each reconciled INDEPENDENTLY, which a first-sibling rule cannot express: `COALESCE(?, DE1, NU4)` takes INT64 from the DECIMAL(18,1) and scale -4 from the NUMERIC(9,4), two different siblings.** A NULL literal is not a sibling (`COALESCE(?, NULL, 0.5)` types from the 0.5; `COALESCE(?, NULL)` is the engine's -804, which this server ANSWERED as 1.25). **A VALUE-ONLY GATE WOULD HAVE CALLED THREE OF THESE GREEN:** `COALESCE(?, N, BI)`, `(?, 0, H)` and `(?, N, H)` store the SAME value on both servers and describe LONG len 4 here against INT64 len 8 / INT128 len 16 — so every cell compares the VALUE *and* the announced descriptor. **AND EVERY CELL WRITES A SENTINEL FIRST:** my own after-probe reported five DECFLOAT cells green because a refused UPDATE left the row holding the previous cell's 1.25, which is exactly what the engine then wrote — **agreement manufactured by cell order, and only the gate's sentinel caught it.** **TEETH: 20 of 42 red on `/tmp/fcwire-prev-da9e3cf` with ALL EIGHT "first sibling already was the reconciled type" CONTROLS GREEN.** Six sweep gates clean by code path (slottype, dmlparamexpr, dfparambind, trigviewparam, merge, viewdml). **RECORDED, NOT FIXED, each held by a self-expiring cell:** (a) a TEXT sibling beside a numeric one — the engine reconciles to VARYING at the NUMBER'S RENDERED width (11 beside INTEGER, 23 beside DECFLOAT(16), 24 beside DOUBLE, measured), which needs the value side to render exactly as the engine does, so this REFUSES at the same boundary the UNION reconciliation draws — **a refusal replacing a wrong value, confirmed by the teeth run: the old binary ANSWERED `COALESCE(?, 0, S3)` as 1**; (b) a DECFLOAT sibling, which refuses because a `CastTarget::DecFloat` cast has no `ExprType` ([Expr::type_of] declines it deliberately, to fail-close nesting it "where a type is needed — a conditional branch") — **measured against the previous binary WITH A SENTINEL: these refused before this change too, so they are neither fixed nor broken here**; (c) PRE-EXISTING and unrelated, proven pre-existing the same way — NULLIF, CASE and IIF push the DESTINATION's scale down but the engine announces a WIDENED RANK (INT64 len 8 for a NUMERIC(9,2) destination, LONG len 4 here), the value agreeing and only the announcement differing.
- **DONE 2026-09-18 (`qa/serve-real-trigviewparam.sh` NEW at 35 cells, floor 35) — A MULTIPLY CONVERTS ITS LEFT PARAMETER OPERAND AT SCALE 0, and that ONE rule was missing in TWO routers. Both were SILENT WRONG WRITES into a scaled exact column.** Into a `NUMERIC(9,2)`, `? * 2` bound 1.25 stores **2**, not 2.50. **The DESCRIBE is not the divergence: `SET SQLDA_DISPLAY ON` prints `LONG scale -2 subtype 1` on BOTH servers for every statement in the gate, including through the view — the slot PUBLISHED and the conversion APPLIED differ, which is why a gate comparing only descriptors would have called this green.** **(A) THE TRIGGER-VIEW PATH never applied it:** [plan_view_trig] hands the SET/VALUES clause on as TEXT and plans a generated `SELECT`, so [view_trig_type_params] wrapped every `?` as `CAST(? AS NUMERIC(9,2))` and 1.25 * 2 stored 2.50 — **and this server was answering CORRECTLY for the statement it had rewritten to: the engine gives 2.50 for that generated text too (measured).** The table planner has carried the rule since `e81c64f`; this path never reached it. **(B) THE TABLE PATH missed it through a UNARY MINUS:** the guard matched a bare `Param` on the left, so `Neg(Param)` slipped past and `-? * 2` stored -2.50 where the engine stores -2 — the same wrong value the rule exists to prevent, one node deeper, on a PLAIN TABLE and in both DML verbs. **THE PROPOSED FIX WAS VALIDATED BY MEASUREMENT BEFORE IT WAS WRITTEN, and the FIRST candidate was killed that way:** "leave the `?` unwrapped and let the planner type it" produces text the ENGINE ITSELF rejects (`SELECT ? * 2` is *"Invalid data type for multiplication in dialect 3"*, bare `SELECT ?` is -804), which would have traded a wrong value for a needless refusal; the twin text that shipped was measured first (`CAST(? AS INTEGER) * 2` → 2, `-CAST(? AS INTEGER) * 2` → -2, `2 * CAST(? AS NUMERIC(9,2))` → 2.5). **CONTROLS BOUND THE RULE, each measured:** `-(? * 2)` was already right (the negation sits ABOVE the multiply), `2 * -?` and `? * -2` keep the destination's scale, `+` `-` `/` never convert at scale 0, and INTEGER, BIGINT and DOUBLE destinations are untouched — a rule one shape too wide is a new wrong answer pointing the other way. **TEETH: 16 of 35 red on `/tmp/fcwire-prev-72dcdc6` while ALL EIGHTEEN CONTROLS stayed green.** Six sweep gates clean by code path (slottype, dmlparamexpr, viewdml, bindconv, merge, params). **RECORDED, NOT HALF-FIXED, and held by a self-expiring `differs` cell:** `COALESCE(?, 0) * 2` through the trigger view (engine 2, here 2.5) — the engine types that `?` from COALESCE's SIBLING, and reproducing it in the generated SELECT means typing a bare `?` from a sibling, which this server cannot do (it errors on that text). The table path already answers that shape correctly. **A unit test asserted the defect** (`"? * 2 + ?"` → `CAST(? AS NUMERIC(9,2)) * 2 + …`) and was changed deliberately, with the twin's width mapping pinned for SMALLINT/INTEGER/BIGINT/NUMERIC(38).
- **DONE 2026-09-18 (`qa/serve-real-derivnoalias.sh` NEW at 31 cells, floor 31) — AN UNNAMED DERIVED TABLE was refused in EVERY row-source position, and the engine answers all of them.** `SELECT ID FROM (SELECT ID FROM T)` - no alias after the closing paren - plus both sides of a comma join, the right of a `JOIN ... ON`, two of them together, over a `UNION`, with `ORDER BY` inside, nested, and the column-list form that names no table (`(SELECT ID FROM T) (A)`, which the engine also takes). **WHY 445 GATES NEVER CAUGHT IT: `qa/serve-real-derived.sh` aliases ALL FOURTEEN of its derived tables. A suite can cover a feature thoroughly and still share one habit with the code it tests** - which is an argument for probing the shape a fixture never writes, not for another gate over the shape it does. **FIVE SITES, and my first count of two was wrong:** two parsers ([parse_table_ref], [parse_derived_table], both `canon_ident(rest)?`) and THREE that refused on a missing alias by themselves - `tr.alias.clone()?` twice and `tr.alias.as_deref()?` once - so widening a parser alone would have built clean and changed nothing, the LikeExpr routing shape again. **THE FIX LANDED IN TWO STEPS AND THE PARTIAL RESULT WAS THE DIAGNOSIS:** the JOIN half worked immediately (a join BINDS a side and needs no text), while the single-side path REBUILDS the statement ([sql_over_from]) and cannot spell the empty name - `render_canon_ref("")` is `""`, which [canon_ident] rejects, so the rebuilt SQL does not re-parse (traced: the inner plan succeeds, then `plan_over_source` returns None). It now binds under a reserved `RDB$FC_UNNAMED_DERIVED` **and refuses any statement that mentions that name** - which is NOT decoration: **measured, a bare `CREATE TABLE RDB$FC_UNNAMED_DERIVED (ID INT)` SUCCEEDS on the engine** (the `RDB$` namespace is not protected against it), the word is a legal alias and qualifier, and `SELECT RDB$FC_UNNAMED_DERIVED.ID FROM (SELECT ID FROM T)` is -206 there - so binding under it unguarded would ANSWER a statement the engine refuses. A refusal instead of a wrong answer, which is the only direction that trade goes. **THE NAME IS ABSENT, NOT INVENTED**, and section 4 holds that line: `X.ID` and the inner table's own `T.ID` stay -206, and two unnamed derived tables raise -204 on an ambiguous column while answering `COUNT(*)` - the engine renders the name as NOTHING ("between derived table  and derived table"), which is why the empty name is the faithful one and a synthesised word would shadow a real table. A second collision surfaced from the same choice: both unnamed sides keyed `""` and the join's "every side must be distinguishable by qualifier" check called them duplicates, where the engine answers 4; an empty key names nothing and is now exempt. **AND WIDENING THE PARSER LIT UP PATHS THAT WERE DEAD CODE for this shape** - the correlated-subquery planner among them - so all seven were measured before landing rather than after: every one agrees, and none answers where the engine refuses. **AND THE OLD `derived` GATE ANNOUNCED ITS OWN OBSOLESCENCE**, going red in the sweep with *"a derived table with NO alias answered"*: it carried a `refuses` cell for this exact shape, under a comment reading "SQL requires a derived table to be named; without one there is nothing to qualify its columns with" - reasoned, never measured, and wrong on both halves. **That cell asserted the defect as a rule for as long as it stood, and it was the ONLY place the unnamed shape appeared in 445 gates.** Promoted to `both`, the comment replaced with what was measured. A gate that tells you it has been overtaken is worth more than one that merely passes - the third time this suite has done that (the `nonecmp` cells on the carrier chunk were the first two).
- **DONE 2026-09-18 (`qa/serve-real-groupalias.sh` NEW at 20 cells, floor 20) — an AGGREGATE select item ALIASED TO THE NAME OF A GROUPED COLUMN makes the statement invalid, and this server ANSWERED it. It now refuses.** The fix is ONE guard in [parse_group_by]'s bare-name branch, ahead of `find_col`: it matched `SelItem::Col` and `SelItem::Expr` aliases but NEVER `SelItem::Agg`, and its own comment called a shadowing alias "harmless" — which is exactly what kept it silent. **TEETH: ten of the twenty cells are red on `/tmp/fcwire-prev-5e3dae2` — these six, printing `2|1` and `30|20`, plus the four of the ORDER BY entry below — while all ten CONTROLS stay green, which is what distinguishes a gate that discriminates from one that is merely red against an old binary.** ~~a SELECT-list alias that SHADOWS A GROUPED COLUMN is accepted in `HAVING` and `ORDER BY`, where the engine REFUSES~~ **— THE FIRST FILING OF THIS WAS WRONG ABOUT ITS SHAPE, corrected within the hour by a boundary probe: `HAVING` and `ORDER BY` are NOT required. The BARE statement `SELECT COUNT(*) AS N FROM T GROUP BY N` already refuses on the engine with SQLSTATE 42000, -104, "Cannot use an aggregate or window function in a GROUP BY clause" **(the full text MEASURED on the engine at fix time; what was recorded here at filing, "Cannot use an aggregate function…", was my PARAPHRASE — and I nearly typed that paraphrase into the gate's own assertion. A gate asserting a remembered message is not a measurement.)** (and so does `SELECT MAX(K) AS N FROM T GROUP BY N`), while this server answers `2|1` and `30|20`. `SELECT COUNT(*) AS C FROM T GROUP BY N` answers on both. The alias makes `GROUP BY N` appear to reference the AGGREGATE, which is what the engine rejects - no reference in HAVING or ORDER BY is needed, and `ORDER BY 1` refuses too. I filed it from the first surprising cell instead of from a boundary probe; the lesson is the entry's, not just the defect's.** Measured, engine first: `SELECT COUNT(*) AS N FROM T GROUP BY N HAVING N = 'plain'` refuses 42000 on the engine and answers `1` here; `SELECT MAX(K) AS N FROM T GROUP BY N HAVING N = 'plain'` refuses there and answers `10` here; `SELECT COUNT(*) AS N FROM T GROUP BY N ORDER BY N` refuses there and answers `1,1` here. **BOTH CONDITIONS ARE REQUIRED, and the controls pin that:** with a NON-aggregate select item (`SELECT N AS N ... HAVING N = 'plain'`) both servers answer, and with the alias shadowing a NON-grouped column (`... AS V ... GROUP BY N HAVING N = 'plain'`) both answer. `WHERE` with the same alias agrees on both (WHERE cannot see a select alias), and a plain aliased `SELECT` agrees. ~~So the shape is: an AGGREGATE select item, aliased to the name of a GROUPED column, that name then used in HAVING or ORDER BY~~ **— STRUCK AT FIX TIME. That sentence is a survivor of the first filing: the correction was written at the TOP of this bullet and this tail, at the BOTTOM, went on asserting that HAVING or ORDER BY was part of the shape. It is not. The shape is an aggregate select item aliased to a grouped column's name, full stop — the BARE statement refuses. A correction that does not sweep the whole bullet leaves the wrong claim still readable, and the reader has no way to know which half is current.** The engine rejects the ambiguity and this server silently resolved it. Nothing to do with charsets or patterns; found while diagnosing a failing cell in `qa/serve-real-carriermix.sh` whose own alias collided - **which is the argument for a diagnostic probe carrying controls it does not need.**

> **EVERY `**HIGH**` BULLET IN THIS LIST WAS RETIRED - the second time this list had failed a re-measurement - AND THEN MEASUREMENT ADDED A NEW ONE (2026-09-17): a generator read through a derived table answers ZERO where the engine counts, found in passing while probing an unrelated column class. "No HIGH bullets left" is a statement about what has been LOOKED AT, never about what is there.** **AND THAT NEW ONE WAS FIXED THE SAME DAY** (the generator draws once per DELIVERED REFERENCE at every level - see the DONE entry), so the tier is empty AGAIN - worth exactly what it was worth the previous two times it was empty. Five were struck on 2026-09-16 and the last four on 2026-09-17: the scalar-subquery charset claim, the `UTF8 -> OCTETS` claim (BOTH halves - the value AND the announced 64-vs-160 width) and the `NONE || WIN1252` claim do not reproduce at all, and the NONE-widening claim is half fixed with its surviving half already recorded, in full, elsewhere on this page. A bullet here is a HUNT'S OUTPUT, not a defect. Re-probe the EXACT shape before spending a chunk on it, print the VALUES (an all-agree run that prints nothing is indistinguishable from an `error == error` sweep), and make the positive control touch the FIXTURE rather than just the connection - `SELECT 1 FROM RDB$DATABASE` passes happily over a database whose tables were never created. What survives below is the MEDIUM/LOW tier, which is overwhelmingly DESCRIBE-level (announced width, sub_type, nullability, field name) rather than wrong VALUES.
>
> **THE MEDIUM TIER HAS NOW BEEN RE-MEASURED TOO, and it went the same way.** Of its THIRTEEN bullets: ONE was already struck on 2026-09-16; EIGHT are struck stale here by measurement (the four scalar-subquery describes and the four temporal ones - every announce read with `SET SQLDA_DISPLAY ON` against the live engine, over a verified fixture); ONE was real and is FIXED (ASCII_VAL, whose probe also turned up a second wrong answer and a refusal the bullet never mentioned); ONE reproduces exactly and stays live (LPAD/RPAD's non-literal length, 65532 against the engine's 32764); and TWO - both common-table-expression entries - are UNMEASURED, so nothing here should be read as a claim about them either way.
>
> **THE ARITHMETIC, RE-COUNTED WITHIN THE SECTION'S OWN BOUNDS (2026-09-17, after the scope-nullability landing): 34 bullets, 30 struck, 4 live** - HIGH 9 of 9 struck, MEDIUM 13 of 15, LOW 8 of 10. (Re-counted within the section's bounds again on 2026-09-18, after the tabled-attachment landing struck the HIGH entry that had been FILED, WIDENED and STRUCK on the same day: 9+15+10 = 34, struck 9+13+8 = 30, live 0+2+2 = 4. Every live bullet left is MEDIUM or LOW - the describe-level pad width, the NONE-widening remnant, the recursive-CTE join order and the LATERAL nullability.) **(Re-counted 2026-09-18 after the right-join driver landing: 35 bullets, 31 struck, 4 live - HIGH 10 of 10 struck, MEDIUM 13 of 15, LOW 8 of 10. The total GREW again, and again because MEASUREMENT ADDED AN ENTRY: a `RIGHT JOIN` returning the WRONG ROWS under `FIRST`/`SKIP`, found while re-probing the recursive-CTE join-driver bullet below and recorded NOWHERE until that probe ran - on plain tables, with no CTE and no charset in sight, pre-existing on three binaries. It was filed as HIGH and struck the same night, so the tier is empty for the THIRD time and is worth exactly what it was worth the previous two. Note also what the probe was AIMED at: the LOW recursive-CTE bullet, which is still live and still unfixed - the chunk's value came from the shape it walked past, not the one it went looking for.)** (Re-counted 2026-09-18: the total GREW again, as it did before, because MEASUREMENT ADDED an entry - a fourth-attachment probe run while verifying the `CONTAINING` mirror found `LIKE` and `STARTING WITH` dropping rows over a byte carrier under a TABLED single-byte attachment, a shape no gate in the suite exercised. Nothing was miscounted into existence; 9+15+10 = 34, struck 8+13+8 = 29, live 1+2+2 = 5.) (Re-counted again 2026-09-17 after the wrapped-literal charset landing, which struck one MEDIUM - the `CS_dynamic` entry - and, like several before it, found the bullet had UNDERSTATED the class: it was filed as a describe divergence and was a wrong ANSWER.) (That landing struck two at once: the CTE-expression entry it fixed, and the GROUP BY entry beside it whose headline no longer reproduces at all - a bullet naming two defects where measurement finds one. The LATERAL entry they were grouped with stayed LIVE, because it is a different cause.) (The describe-name work struck three at once: the EXISTS entry, the recursive-CTE one, and the scalar-subquery-in-a-derived-table entry whose two halves were closed by two different chunks - it had been left LIVE with both halves already noted as fixed, which is the under-counting twin of the over-counting this note warns about. Count, then read what the count means.) **The previous figures here - "32 bullets, 21 struck, 11 live, MEDIUM 11 of 15" - were counted over the WRONG RANGE**, which is the same failure as counting from memory and is why they disagreed with themselves: the total was section-scoped while the struck count swept the whole FILE, pulling in the `CONNECTION KILL` bullet that belongs to another section, and the live figure matched neither range. A tally here must bound the section (from this heading to the next `###`) before it counts, not just run a `grep -c` over the page. The total GREW because measurement added three entries it had not held before (the folded-subquery pad width, the join-driver order, and the recursive-walk follow-on); nothing was miscounted into existence.
>
> **THE LOW TIER HAS NOW BEEN PROBED TOO** - an earlier draft of this note said it never had been, which was true when written and is not now. It went the same way as the others: ONE bullet was a genuine wrong ANSWER and is fixed (the recursive-CTE walk order); ONE is a wrong-answer-class finding its own bullet understated (an unnamed derived-table column, which the engine REFUSES and this server answers); TWO are stale; and THREE are a single root cause wearing three faces (an expression over a wrapped column losing NOT NULL), with three more being one describe name-leak seen three ways. Re-probe the EXACT shape, print values on OK, make the positive control touch the FIXTURE - and read the owning gate's FIXTURE rather than its result: three times in these sittings a green gate turned out to be green because its fixture could not reach the shape its own header described.
>
> (Two figures in an earlier draft - "twelve" MEDIUM bullets and "thirty" in total - were carried from memory and were both wrong. Count the section with a script before writing any tally into this file.)

- ~~**HIGH** (scalar subqueries) — Scalar subquery over a CHAR/VARCHAR column drops the column's CHARACTER SET: a WIN1252 value comes back double-encoded (Ã©Ã  instead of éà) and CHAR pads to the wrong width~~ **STALE, measured 2026-09-17** with `isql -ch` under three attachments (default NONE, UTF8, WIN1252): 34 of 34 cells agree, and the run is PROVED NON-VACUOUS rather than assumed - the fixture holds `W` as the six bytes `737472619F65`, and every cell prints a real value. The scalar subquery returns `stra<9F>e` exactly as the DIRECT read beside it does, on both servers; the `CHAR(8)` column comes back `ab<9F>` padded to 8 through the subquery just as it does directly; `CHAR_LENGTH` is 8 and `OCTET_LENGTH` 6 on both. Neither half of the claim reproduces - the charset is not dropped and CHAR does not mis-pad. (A first pass reported the same 34/34 while printing values only on disagreement, which would have been indistinguishable from an `error == error` sweep; the values above are the re-run that settles it.)
- ~~**HIGH** (window functions) — RANGE BETWEEN UNBOUNDED PRECEDING AND <offset> FOLLOWING~~ **STALE, measured 2026-09-17: agrees row for row (6,6,10,10) — see "Stale claims retired"**. Original report: RANGE BETWEEN UNBOUNDED PRECEDING AND <offset> FOLLOWING excludes the NULL ordering-key peer group from every later row's frame
- ~~**HIGH** (date/time arithmetic) — DATEADD on any TIMESTAMP WITH TIME ZONE returns a fixed zeroed buffer~~ **STALE, measured 2026-09-17: all seven parts agree, offset preserved — see "Stale claims retired"**. Original report: DATEADD on any TIMESTAMP WITH TIME ZONE returns a fixed zeroed buffer (1858-11-16 00:01:00.0000 -23:59) for every part
- ~~**HIGH** (date/time arithmetic) — DATEDIFF between two TIMESTAMP WITH TIME ZONE values always returns 0~~ **STALE, measured 2026-09-17: all seven parts agree — see "Stale claims retired"**. Original report: DATEDIFF between two TIMESTAMP WITH TIME ZONE values always returns 0
- ~~**HIGH** (the CAST matrix and string functions a) — CAST to a narrower NUMERIC silently wraps~~ **STALE, measured 2026-09-17: every narrowing cast raises 22003 on BOTH servers — see "Stale claims retired"**. Original report: CAST to a narrower NUMERIC silently wraps modulo 2^32 / 2^16 instead of raising 22003 "numeric value is out of range"
- ~~**HIGH** (the CAST matrix and string functions a) — **A TABLED SINGLE-BYTE ATTACHMENT IS NOT UTF8: every predicate over a BYTE-CARRIER column with a NON-ASCII literal silently drops the row.**~~ **DONE 2026-09-18, the same day it was filed - `qa/serve-real-nonecmp.sh` 96 -> 102 (6 flip on `9bf97ca`).** THE CAUSE WAS ONE WORD IN AN EXISTING STEP: [param_or_typed_term] already lifts a REAL literal into the carrier spelling before building the term, and did it with [intl::to_carrier] = `carrier_decode(s.as_bytes())` - the UTF-8 bytes, one char per byte. That is right only when the literal's characters came from UTF-8 statement text; under a TABLED attachment [stmt_text_decode] has ALREADY decoded one char per byte via `decode_text`, so the lift ran TWICE and `'café'` (5 carrier chars) became 7 against the column's 5. The guard beside it excludes a byte-CARRIER attachment for exactly that reason - its own comment records the double-encode - and simply did not cover the tabled case. Fixed by lifting through [transcode_text] from the ATTACHMENT's charset: identity under a tabled attachment, byte-for-byte what `to_carrier` did under UTF8 (`encode_text` answers `Ok(None)` for a non-tabled set, so its `as_bytes` fallback IS the UTF-8 spelling). The shared [adopt_carrier_literal] was not touched. **FIVE HYPOTHESES DIED FIRST and the TRACE ended it**: `FC_SRV_TRACE` showed the prepares under NONE and WIN1252 BYTE-IDENTICAL (`N = 'cafÃ©'`) answering 1 and 0 - same text, same stored value, different attachment - which left only a step that reads the attachment. The fork that localised it was two queries: the same comparison as a PROJECTION answers correctly where the WHERE answers 0, and an expression right-hand side (`N = 'caf' || x'C3A9'`) works because it routes through [cmp_sides]. Surviving as its own section-H item, measured unchanged across this fix: an OCTETS CAST under `STARTING WITH`, which fails under a NONE attachment too - where this lift never runs. (Widened 2026-09-18, the day it was filed: it was first written as "`LIKE` and `STARTING WITH`" from the two cells a four-attachment probe happened to cover. A finer measurement adds **EQUALITY** - `N = 'café'` is engine 1 / here 0 under WIN1252 and ISO8859_1 - so the family is every predicate over a carrier column *except* `CONTAINING`, which its own slice fixed and which now agrees on all four attachments. The exemption is the useful part: it says the predicates do not share a route, and that the fixed one is the model.) Found 2026-09-18 by probing a FOURTH attachment while verifying the `CONTAINING` mirror fix. Over a NONE column holding `63 61 66 C3 A9`, under a **WIN1252** or **ISO8859_1** attachment: `N LIKE '%é%'` and `N STARTING WITH 'café'` are **1 on the engine and 0 here** - a qualifying row silently vanishes, this page's highest class. Both AGREE under NONE and under UTF8, which is exactly why every existing instrument missed them: of `serve-real-nonecmp`'s 24 attachment-qualified cells all but two were `@UTF8`, and those two test a WIN1252 COLUMN rather than a carrier column under a tabled attachment - **no gate in the suite ran this shape**. Measured IDENTICAL on `302c4d1` and on the binary that fixed the `CONTAINING` mirror, so it is pre-existing and not that slice's doing. Note the asymmetry that makes it interesting: `CONTAINING` was ALREADY right under these two attachments on the old binary and wrong only under UTF8, so the three predicates do NOT share a route here. Held by four self-retiring `differs` cells; its own chunk.
- ~~**HIGH** (joins) — **A `RIGHT JOIN` RETURNS ITS ROWS IN THE WRONG ORDER, AND `FIRST`/`SKIP` THEN RETURN THE WRONG ROWS.** Found 2026-09-18 while measuring the recursive-CTE join-driver bullet below, which turns out to be a special case of the same question. THE ENGINE DRIVES A RIGHT JOIN FROM THE PRESERVED (RIGHT) SIDE, mirroring a LEFT join driving from the left - its plan names that side first (`PLAN JOIN ("B" NATURAL, "A" NATURAL)`, and `JOIN ("C", JOIN("B","A"))` for a chain) - while this server streams the SYNTACTIC LEFT and appends the preserved side's unmatched rows as a mirror at the end. Over `TA(1,2,3,4)` joined to `TB` stored `4,2,1,3`, `SELECT A.ID FROM TA A RIGHT JOIN TB B ON B.ID = A.ID` answers `4,2,1,3` on the engine and `1,2,3,4` here. **ORDER-ONLY IS NOT WHERE IT STOPS**: every top-level slice takes a DIFFERENT SET OF ROWS - `FIRST 1` is the engine's `[4]` against this server's `[1]`, `FIRST 2` `[4,2]` against `[1,2]`, `SKIP 2` `[1,3]` against `[3,4]`, `ROWS 2 TO 3` `[2,1]` against `[2,3]` - six of six slice shapes, silently and with no error, which is the same escalation the recursive-walk entry hit and the reason that one was ranked a wrong answer rather than a describe difference. MEASURED BREADTH: 9 of 13 shapes diverge - plain tables, with a `WHERE`, projecting either side's column, all four three-way chains (`RIGHT..RIGHT`, `LEFT..RIGHT`, `RIGHT..LEFT`, `JOIN..RIGHT`), and a derived or CTE right side; `LEFT`, `INNER` and `FULL` agree, and a top-level `ORDER BY` collapses it (`PLAN SORT (JOIN (...))`). The engine's order is STABLE (three repeats identical) and INVARIANT to an index on the preserved side. **PRE-EXISTING, measured on three binaries** - `bd5e39d`, `302c4d1` and `1762f79` all answer alike - so no recent slice caused it. **A GATE ALREADY AIMS AT THIS SHAPE AND CANNOT SEE IT**: `serve-real-jointypes` has a `FIRST 1 over a RIGHT join` cell, but its fixture inserts only TWO rows into the preserved side, in ASCENDING order, so driving from either side yields the same first row - a fixture whose preserved side is sorted and tiny is structurally blind to a driver-order defect, and new cells need at least three rows stored out of order. THE FIX IS EMISSION ORDER, NOT SIDE ORDER: `JoinSide::offset` is a cumulative column offset and every reference is `side.offset + field_id`, so swapping the sides would renumber the projection, the ON, the WHERE and the ORDER BY - an order fix that becomes wrong values. The contained change is to iterate the preserved rows in scan order inside the `JoinKind::Right` arms alone (the streaming path at `server.rs` ~8457 and `join_step` ~56121), emitting each right row's matches and its padded row when it has none - same layout, same semantics, different order - leaving `LEFT`, `INNER` and the two-phase `FULL` (measured as already agreeing) untouched.~~ **DONE 2026-09-18** (`qa/serve-real-jointypes.sh` 64 -> 78, 9 flip on `bd5e39d`) — **and it took THREE sites, where the plan above named two.** [join_step] now tags every emitted row with the right-row index that produced it and, for a `Right` part alone, stable-sorts on that tag: the pairing, the ON evaluation and the partnerless raise are untouched, and only the emission order changes. The streaming arm's guard narrowed from `Right | Full` to `Full`, so a right join falls past both streaming arms to the materialising path. **That fixed every SLICE and left every unsliced listing still wrong** - `FIRST 1` came right while `SELECT ... RIGHT JOIN` did not - which is what exposed the third site: a bare right join is delivered by [JoinCursor], whose `driver_output` expands ONE base row at a time and whose `mirror_output` runs only after the driver phase exhausts, so its unmatched preserved rows can only ever trail. No buffering inside either phase could fix that, so `JoinCursor::open_inner` now DECLINES a bare `Right` last part (the mechanism its own doc already describes for every other shape it cannot deliver) and the materialising path takes it. TWO deliberate exclusions, both measured: `FULL` keeps the cursor, and the `raw` (ORDER BY) path keeps it too, because a sort decides the final order there and `RIGHT JOIN ... ORDER BY` already agreed. THE MODEL CAME FROM MEASUREMENT, NOT FROM THE SHAPE OF THE CODE: over `JR` stored 40,10,30,20 against `JL` stored 3,1,4,2 the engine answers `40/1 40/2 10/3 30/-1 20/1 20/2` - preserved rows in storage order, each followed by its matches in the ACCUMULATED side's order (which this server already produced, so that half was left alone and is pinned as a control), and **an unmatched preserved row padded IN ITS STORAGE POSITION** - 30 lands THIRD, not last. That last fact is the whole licence for "group by the right-row index"; had the mirror grouped at the end, the cheap fix would have been wrong. THE COST IS REAL AND RECORDED: right joins no longer stream, so `FIRST n` over one materialises where it used to stop early - paid because the alternative was answering the wrong ROWS. Controls measured unmoved: `LEFT`, `INNER`, `FULL`, `ORDER BY`, and the recursive-CTE driver probes (6/4, 9/4) - the LOW bullet below is a separate INNER-join question and stays live. Sweep, 12 gates, 0 DIFF: joinchain 39, leftjoinindex 169, derived 172, recursive 58, cte 56, joinexpr 46, viewjoin 36, joinderived 25, joingroup 18, joinorder 15, outerjoin 13, join 11. **THE GATE THAT SHOULD HAVE CAUGHT THIS HAD NO COUNTER AT ALL** (now `ran`/floor 78) and its own `FIRST 1 over a RIGHT join` cell was blind to it: the fixture's preserved side was TWO rows in ASCENDING order, so driving from either side gave the same first row. The new cells use a preserved side stored out of order with duplicate keys and an unmatched row in the MIDDLE. **And three of the sweep's gates first reported 69 fabricated divergences** - `joinchain` alone showing exactly the 39 that this page's own QA note predicts - because they are `<clean-db-path>` gates and I handed them a port number; every cell answered `CONN_ERR` against a real engine answer. Re-run with a per-gate `mkjoindb.sh` fixture, all four are clean. A sweep that greps the wrong token, or invokes a gate the wrong way, measures nothing while looking exactly like a regression.
- **MEDIUM** (the CAST matrix and string functions a) — NARROWED, not struck: ~~Implicit CHARACTER SET NONE widening decodes NONE bytes as Latin-1 instead of treating them transparently, corrupting concatenation, REPLACE, POSITION and equality~~ **HALF STALE AND HALF ALREADY RECORDED ELSEWHERE, re-measured 2026-09-17.** The direction where the COLUMN is the byte carrier is fixed and gated: `serve-real-nonecmp.sh` is green at 77 checks carrying real values (`u = 'café'`, `w = 'café' (win1252)`, `u = x'636166FF'`), covering equality, REPLACE, POSITION, CONTAINING and TRIM. What is left is the MIRROR direction alone - a real-charset literal meeting a byte-carrier column - and that is NOT an unrecorded defect: it is held by nine self-expiring `differs` cells in that same gate and by this page's own "THE MIRROR DIRECTION - ATTEMPTED TWICE, WITHDRAWN BOTH TIMES" entry, which carries the full contract. Plan from that entry, never from this bullet. **NARROWED AGAIN 2026-09-18, on the THIRD attempt: SEVEN of those nine cells are now FIXED and promoted to `agree`, and so are all four COLUMN-vs-COLUMN wrong answers** (`qa/serve-real-nonecmp.sh` 77 -> 86, 13 flip on `c49807a`). The two attempts before it re-keyed the LITERAL paths - [carrier_fn_args] and [adopt_carrier_literal] - which is why one was inert and the other turned the CORRECT `nn LIKE '%é%'` wrong; this one is OPERAND-keyed and lives on the FUNCTION path ([carrier_fn_operands]), wrapping the REAL operands in the existing [Expr::CarrierEnc] exactly as [cmp_sides] already does for comparisons, so the shared predicate route is untouched by construction. WHAT SURVIVES, both deliberate: `CONTAINING` over a carrier (engine 1, here 0) is built by [containing_term] into a `Term::ExprLike` - the predicate path whose re-keying broke LIKE last time - and is still held by two `differs` cells; and a NON-LITERAL LIKE/CONTAINING pattern is a MISSING CAPABILITY rather than this law, measured: fire-crab refuses `U CONTAINING V` and `U LIKE V` with both sides the SAME charset, so it ranks below every wrong answer (three new `refuses` cells record it). **RE-MEASURED AND MUCH WIDER THAN THIS SAYS (2026-09-18, on `aedbacd`): 11 of 13 shapes refuse here where the engine answers, and NONE is a wrong answer.** It is not two predicates but ALL FOUR pattern families - `LIKE`, `CONTAINING`, `STARTING WITH`, `SIMILAR TO` - plus the negation (`U NOT LIKE V`), the pattern on the LEFT (`V LIKE U`), the `ESCAPE` form, and EVERY non-literal SOURCE: a sibling column, a concatenation (`V || '%'`), a function (`TRIM(V)`), a `CASE`, and a scalar subquery. Only the two literal controls answer. **BUT A PARAMETER PATTERN ALREADY WORKS** - measured 7 of 7 on `b48d84d`: `U LIKE ?`, `U CONTAINING ?`, `U STARTING WITH ?`, `U SIMILAR TO ?`, `U NOT LIKE ?` and `U LIKE ? ESCAPE` all answer and all agree with the engine. So the gap is not "non-literal" either: a `?` is not a literal and is fully served, through `Rhs::Param` and a `Term::ParamSimilar` that compiles its regex at [Predicate::bind]. The gap is an EXPRESSION pattern, and it is refused in the PARSER rather than downstream: [parse_pattern] is [parse_value] plus `x'…'`, and `parse_value` accepts only literal tokens and `Tok::Param` - a column, a concatenation or a function hits `_ => return None`, so the predicate never becomes a `RawTerm` and the whole statement declines. A fix therefore starts by giving a pattern an expression form, not by relaxing a guard; `Term::Like` already holds an `Rhs` rather than a compiled matcher, while `Term::Similar` holds a `SimRe` compiled once, so LIKE/CONTAINING/STARTING and SIMILAR are not equally placed. **THE DESIGN IS SCOUTED (2026-09-18) - written down here so the next attempt starts from it rather than re-deriving it.** (a) THE PARSER HALF IS FREE: [parse_side] already implements "a literal when it ends at a boundary, else a full expression" (`Side::Val`/`Side::Expr`), it is what BETWEEN, IN and every comparison side use, and [texpr] stops at the first token it cannot consume - so `ESCAPE` still parses after a pattern. [parse_pattern] becomes `parse_side`-shaped. (b) THE MULTI-COLUMN HAZARD IS ALREADY SOLVED: a pattern that is an expression makes the term read MORE than one column, and the four fid-attribution sites ([term_side_only], [collect_term_fids], `matches`' out-of-range check, [null_rejecting]) take a single `fid` - but the Expr-carrying variants beside them already answer through `expr_reads`, so a new variant reports BOTH expressions the same way. Getting this wrong would mis-gate a WHERE push-down or a partnerless raise, i.e. a WRONG ANSWER, so it is the part to write first and gate hardest. **AND THE COMPILER-FORCED LIST IS EXACTLY ONE SITE**, measured by adding the variant and building: `negate_term` alone (`E0004`, `&RawKind::LikeExpr(_, _, _)` not covered). Greping for functions that mention `RawKind::` without a wildcard arm predicted FIVE - `side_kind` matches on `Side` and merely constructs a `RawKind`, while `resolve_predicate`, `numeric_term` and `decfloat_term` mention the type dozens of times without ever matching exhaustively on one. Add the variant and let the compiler name the sites; a grep answers about a pattern, not about the code. **AMENDED after reading the consumers: the sharp hazard is NOT the negation flip.** [negate_term] matches every `RawKind` variant with NO catch-all, so a new form FAILS TO COMPILE until negation is handled - the compiler enforces it and `NOT <x> LIKE V` cannot silently lose its `NOT`. The silent hazards are the two pattern REWRITES, both of which end in `other => other` and would therefore pass an expression pattern through UNTRANSFORMED: the carrier reinterpretation at ~93263 (`RawKind::Like(Rhs::Str(v), e, n) => ... redo(v)`) and the tabled-attachment lift at ~93520 (`RawKind::Like(r, e, n) => RawKind::Like(lift(r), e, n)`). Both are the charset laws chunks 30-33 established. **MEASURED, AND THE HAZARD IS NOT REAL FOR THIS SHAPE (2026-09-18):** every expression-pattern row answers IDENTICALLY under NONE, WIN1252 and UTF8 - `U LIKE PU` (both UTF8), `W LIKE PW` (both WIN1252), both cross-charset pairs, and `U LIKE U || '%'` all give the same rows under all three attachments. Only the LITERAL pattern moves with the attachment (`<utf8 col> LIKE 'café%'` is 22000 under NONE, empty under WIN1252, a match under UTF8) - which is the carrier law itself, and it applies to a literal precisely BECAUSE a literal arrives as statement text that the attachment decodes. An expression pattern is a VALUE: it carries its own column's charset and never passes through that decode, so `other => other` in [redo] and the lift is CORRECT for it, and adding a per-row transcode would be the wrong thing. The gate cells should still pin the attachment-independence (the same statement under three attachments). **AND THE NONE PAIRINGS ARE THEIR OWN CONTRACT, measured rather than assumed - all three attachment-INDEPENDENT:** `<none> LIKE <none>` matches both rows; `<none value> LIKE <utf8 pattern>` matches only the ASCII row - the non-ASCII one silently does NOT match, and it does NOT raise (`COUNT(*)` answers 1); and `<utf8 value> LIKE <none pattern>` **RAISES 22000 Malformed string**. So the NONE direction is not symmetric: a carrier PATTERN against a real value raises, while a carrier VALUE against a real pattern quietly misses. Any slice must reproduce both, and both want cells. (c) IT IS TWO PARALLEL FAMILIES, not one: the predicate world (`Term::ExprLike/ExprStarting/ExprSimilar`, evaluated per row) and the VALUE world (`RawCond::Like` -> `Cond2::Like`, ten sites, evaluated at `Cond2::Like(a, pat, ..) => a.eval(values)`), both holding the left side as an expression and the pattern as a `String`. Doing only the first leaves `SELECT <x> LIKE V` refusing while `WHERE <x> LIKE V` answers - an asymmetry to record, not to ship silently. (d) SIMILAR IS THE EXPENSIVE CORNER: its pattern is a compiled `SimRe` in both worlds, so a per-row pattern means `sim_compile` per row - a cost to MEASURE before shipping, not to assume. (e) Index banding needs nothing: [choose_index] bands only `Term::Cmp(fid, Cmp::Eq, Rhs::Str)` for text and `_ => continue`s everything else, so a row-dependent pattern declines for free. So the gap is "the pattern must be a literal or a `?`", not "LIKE and CONTAINING want one" - and the fix direction is a capability (a pattern evaluated per row rather than compiled once at prepare), which can only turn refusals into answers, never a correct answer into a wrong one. **THE `CONTAINING` HALF IS NOW CLOSED TOO (2026-09-18, `serve-real-nonecmp` 86 -> 96, 2 flip on `302c4d1`), so what is left of this bullet is the capability gap alone.** It was the SAME one-directional guard a third time - `byte_carrier(att) && !byte_carrier(cs)`, a carrier ATTACHMENT meeting a real operand - and CONTAINING reaches [containing_term] by TWO routes, an EXPRESSION (an OCTETS cast, a blob) and a plain COLUMN through [param_or_typed_term]'s fast path. Fixing only the first left the headline cell unmoved, which the probe showed immediately. The conversion never changed: `transcode_text(att, cs, p)` already runs whichever way round the two sit, so the expression arm became [cmp_sides]' own test (`byte_carrier(a) != byte_carrier(b)`) and the column arm took the mirror branch explicitly, since [adopt_carrier_literal] above it handles the other direction and would otherwise transcode twice. **The shared [adopt_carrier_literal] was deliberately NOT re-keyed** - it also feeds Cmp, STARTING WITH, SIMILAR TO and LIKE, and re-keying it is what turned a correct `nn LIKE '%é%'` wrong in a withdrawn attempt; all four neighbours are measured agreeing. **THE EXPRESSION PATTERN IS NOW SERVED - FOR `LIKE` ALONE (2026-09-18, new gate `qa/serve-real-exprpattern.sh`, 32 cells, counted floor 32, exit 1 and 19 cells red on `4af9add`).** `RawKind::LikeExpr(RawExpr, Option<char>, bool)` and `Term::ExprLikeExpr(Box<Expr>, Box<Expr>, Option<char>, bool)`, the pattern resolved through [resolve_expr] - the same call the LEFT side uses, so there is no second resolver to get wrong - and evaluated per row beside the existing `ExprLike`. The scouted design above held on every point it was checked on: [parse_side]'s "literal at a boundary, else an expression" shape transferred directly (the arm saves `(*pos, *np)`, tries [parse_pattern], and rewinds BOTH before falling back to [texpr], which still stops before `ESCAPE`); the four fid-attribution sites took the new variant by reporting BOTH expressions through `expr_reads`; and `other => other` in [redo] and the lift is correct for it, confirmed by six gate cells running the same statement under NONE, WIN1252 and UTF8. **AND THEN IT COMPILED CLEAN AND ANSWERED NOTHING.** `cargo` was green, `E0004` zero, all six edits verified in place - and every expression-pattern shape still refused, because [resolve_predicate] routes to [resolve_expr_term] only when `matches!(rt.lhs, RawLhs::Expr(_)) || matches!(rt.kind, RawKind::CmpExpr(..))`. `<col> LIKE <expr>` has an ORDINARY COLUMN on the left, so it fell through to the typed path and died on [typed_term]'s own decline arm. `CmpExpr` is named in that guard for exactly this reason and `LikeExpr` was not; the same guard exists a second time for the JOIN predicate (~40051) and needed the same word. THE LESSON, and it is the compiler's blind spot: an exhaustive `match` forces you to HANDLE a new variant, and never once asks whether anything CONSTRUCTS the path that reaches it. A clean build proves a capability compiles, not that it runs - only a measured row does. **THE GATE THEN CAUGHT THREE WRONG ANSWERS, which is the whole reason it exists.** Resolving both sides and comparing them ignores the NONE asymmetry this bullet records two sentences up: `N LIKE PU` listed the non-ASCII row the engine MISSES (1,2 against the engine's 2) and `U LIKE PN` answered rows where the engine RAISES 22000. That is the deferred slice [carrier_fn_operands]' own doc already names - "where the CARRIER side is a COLUMN there is nothing to reinterpret at prepare" - and [carrier_expr_like] only ever answered the ATTACHMENT carrier. So the arm now REFUSES a carrier/real mix between the two sides (`byte_carrier(a) != byte_carrier(b)`, both sides' contributed charsets), the matched pair `N LIKE PN` still answers, and the two mixes are recorded as gaps rather than guessed at: three refusals replaced three wrong answers, which is the only direction that trade may go. **WHAT IS STILL REFUSED, all gated so they go red when closed:** `CONTAINING`, `STARTING WITH` and `SIMILAR TO` with an expression pattern (this slice is LIKE only - and ~~`SIMILAR` remains the expensive corner (d) warns about~~ - **AND IT IS NOT, measured 2026-09-18 by READING [sim_compile]: an eight-line recursive-descent parse into a small `SimRe` enum tree, no regex engine, and `SimRe` derives Clone, so a per-row compile is a cheap tree build. The deferral was an ASSUMPTION, repeated in three chunk records before anyone opened the function, and the expression pattern for SIMILAR landed the same day the claim was finally checked); a `?` INSIDE an expression pattern; a COLLATE-canonical left side (the literal arm rewrites the pattern text with `icu_canonical` at PREPARE and there is no text to rewrite until the row arrives - the same `!is_cmp` guard this page records at the collation entry); the carrier/real mix; and the VALUE world (c), untouched, so `SELECT <x> LIKE V` still refuses while `WHERE <x> LIKE V` answers. **Two defects in my own gate, both found by running it rather than reading it:** it ended on an `echo` and so exited 0 while 24 cells were FAILING - three runs reported success - and four cells compared an empty listing against an empty listing, which is exactly what a match-nothing bug also produces (counted now). A separate PRE-EXISTING refusal surfaced and is filed here rather than fixed: `IS UNKNOWN` over any predicate refuses on this server, `(U IS NULL) IS UNKNOWN` included, measured on `4af9add`. **AND NOW `CONTAINING` AND `STARTING WITH` TOO (2026-09-18, new gate `qa/serve-real-exprpattern2.sh`, 35 cells, counted floor 35, exit 1 and 22 cells red on `f47820e`).** `RawKind::StartingExpr`/`ContainingExpr` and `Term::ExprStartingExpr`/`ExprContainingExpr`, the same literal-first-with-rewind parser shape (both families are `Tok::Ident` arms in the very `parse_leaf` the LIKE arm lives in, and both called `parse_pattern` exactly as it did), and both routing guards widened IN THE SAME EDIT as the variants - the one lesson the previous slice paid for, since no compiler error names a path nothing constructs. The compiler then named exactly three sites, all declines: `typed_term`, `numeric_term`, `decfloat_term`. **THE CARRIER CONTRACT IS PER-FAMILY, WHICH IS WHY IT WAS MEASURED RATHER THAN COPIED FROM THE LIKE SLICE:** ~~against a byte-carrier PATTERN the engine gives three different answers - `LIKE` **raises 22000**, `STARTING WITH` **raises 22000**, and `CONTAINING` **ANSWERS 2 with no raise at all**~~ **— WRONG, AND CORRECTED 2026-09-18 (chunk 42's contract probe). The rule is NOT per-family: it is per-ROW, on the PATTERN's BYTES. All four families behave IDENTICALLY — an ASCII carrier pattern decodes and ANSWERS, a NON-ASCII one RAISES 22000, and in the same statement the row whose pattern is ASCII answers while the row whose pattern is not raises. This entry's `CONTAINING` measurement was real but its fixture's carrier pattern was `'af'`/`'pp'`, PURE ASCII, so it exercised the answering half only. ONE FIXTURE WAS MISTAKEN FOR A LAW, and the mistake survived into two later records.** Had the LIKE rule been assumed to transfer, the `CONTAINING` direction would have been wrong in a way no compiler and no existing gate would have caught. This server REFUSES every carrier/real mix in both families - a deliberate gap, gated four ways (three `gap`, one `gap_raise`): the literal arms reach their answers by transcoding a LITERAL at prepare, which a per-row pattern cannot do, and this server has no STARTING raise mechanism at all (`Term::MalformedLike` is built only from [carrier_expr_like] and [param_or_typed_term], both LIKE's). **WHAT THE PER-ROW FORMS ACTUALLY DEFER:** `STARTING` canonicalises its prefix under the written collation, which the literal arm does at prepare; `CONTAINING` rebuilds the whole of [containing_term]'s pattern half per row - upper-case in the operand's charset, canonicalise at the collation's strength, escape `%`/`_`/the escape itself, wrap in `%…%` - and that is not a new idea, because `ExprContainingParam` ALREADY defers exactly that work to BIND for a `?` pattern. The VALUE side keeps its prepare-time `CollCanon` wrapper, which does not depend on the pattern. **STILL REFUSED, all gated:** the carrier/real mix; a COLLATE-canonical left side (the `!is_cmp` guard this page records at the collation entry - the engine answers `CI CONTAINING <expr>` 2 and this server declines); a `?` inside an expression pattern; `SIMILAR TO`, whose pattern is a compiled `SimRe` so a per-row pattern means `sim_compile` per row, a cost to measure before shipping rather than assume; and the VALUE world, measured 9 of 9 refusing (`SELECT <x> LIKE V`, `CASE`, `IIF`, `HAVING`, `ORDER BY`, `(U LIKE V) = TRUE`) while the WHERE forms answer. **THE LIKE GATE'S THREE `gap` CELLS DID THEIR JOB:** two went red the moment this landed and were PROMOTED to `agree` rather than deleted, and they fail on `f47820e` - so the promotion detects the change instead of merely relabelling it. Twelve sweep gates clean across three batches; wire 348 / ods 161. **AND THE VALUE WORLD IS NOW CLOSED TOO, for the same three families (2026-09-18, new gate `qa/serve-real-exprvalue.sh`, 38 cells, counted floor 38, exit 1 and 22 cells red on `a84b09f`).** `SELECT <x> LIKE V`, `CASE WHEN <x> CONTAINING SUB`, `IIF(<x> STARTING WITH PRE, ..)`, `COALESCE`, `(<x> LIKE V) = TRUE` and `ORDER BY (<x> LIKE V)` all answer, where 9 of 9 shapes refused before and none was a wrong answer. This closes the asymmetry the two predicate-world chunks created - `WHERE <x> LIKE V` answering while `SELECT <x> LIKE V` refused - and it is (c) of the design scouted above, the VALUE world, done. **THE TWO WORLDS PARSE DIFFERENTLY, so nothing transferred by copying:** the predicate world runs on TOKENS ([parse_leaf], `Tok::`), while this one is a CHARACTER-level recursive-descent parser ([parse_raw_cond] over `&[char]`) whose pattern positions read a quoted literal only ([read_quoted], whose own doc says it is "how the pattern predicates decline a `?` or an expression pattern"). The fallback here is [expr_add] - the same entry point the LEFT side uses - and the chain `expr_add -> expr_mul -> expr_concat -> expr_at_tz -> expr_atom` was READ before relying on it, which is what proves `V || '%'` parses in the pattern position. **THE SILENT/FORCED SPLIT WAS MEASURED BEFORE WRITING ANY CODE, and it is the reason this chunk was safe:** of twelve mirror sites, NINE are exhaustive (the compiler named all nine, exactly as predicted - [cond_has_param], [subst_params_cond], [cond_reads], [walk_cond_aggs], [substitute_cond_aggs], [normalize_cond], [renumber_cond_params], [raw_cond_bad_substring_len], [cond_no_raise]) and THREE carry catch-alls and are therefore SILENT: [expr_contains_genval], [expr_has_corr] and `cond_types`. Those three were written in by hand; a pattern expression reading a correlated column or a `GEN_ID` would otherwise have been missed there, and `expr_has_corr` answering `false` would let a correlated condition be evaluated ONCE instead of per outer row - a wrong answer. [renumber_cond_params] carries the same weight: renumbering one side and not the other binds the wrong argument. **AN AGGREGATE MAY NOW LIVE IN THE PATTERN, and it is gated:** `SELECT CASE WHEN MAX(U) LIKE MAX(V) .. GROUP BY U` and the bare `SELECT MAX(U) LIKE MAX(V)` answer, which is what [walk_cond_aggs]/[substitute_cond_aggs] walking BOTH sides buys; three cells hold it, so the both-sides walk cannot be reverted with every cell still green. **AND A THIRD ROUTING SITE WAS FOUND, RECORDED AND NOT FIXED:** `HAVING` has its own resolver ([resolve_having], over `Vec<Vec<RawTerm>>` and a synthetic group-row view) which shares neither routing guard, so `HAVING U LIKE V` refuses with no aggregate in it whatever while `WHERE U LIKE V` answers and `HAVING MAX(U) LIKE 'caf%'` answers. Deliberately not half-fixed from here - a mistake against a synthetic slot is a wrong answer - and filed with its own entry under "Refusal boundaries recorded", together with the PRE-EXISTING `HAVING MAX(U) = MAX(V)` refusal the narrowing probe turned up as a control. **ONE GATE CELL OF MINE WAS WRONG AND ONE COMMENT WAS FALSE**, both caught by running rather than reading: the HAVING cell was written as `agree` on the assumption the aggregate work reached it (it is now two `gap` cells plus a literal-form control), and a comment on [walk_cond_aggs] cited `HAVING MAX(U) LIKE MAX(V)` as the shape justifying it - a shape that never passes through that function. Ten sweep gates clean; wire 348 / ods 161. **AND THE THIRD ROUTING SITE IS NOW CLOSED TOO, the same day it was recorded (2026-09-18, new gate `qa/serve-real-havingexpr.sh`, 28 cells, counted floor 28, exit 1 and 16 cells red on `c0fe691`).** The gap was WIDER than the row that filed it: measured, the boundary was **"in HAVING the right side must be a LITERAL"** - not only the four families' expression patterns and `NOT (U LIKE V)`, but comparisons too (`MAX(N) = MAX(ID)`, `MAX(N) > MAX(ID) - 1`, `MAX(U) = V`, `U = V`), sixteen shapes in all, engine answering every one, NO wrong answers. Every working control had a literal on the right (`COUNT(*) >= 1`, `MAX(U) LIKE 'caf%'`, `MAX(U) = 'cafe'`, `MAX(N) * 2 > 2`, `MAX(U) IS NULL`), which is what made the boundary describable in one sentence. **ONE ROOT CAUSE:** [resolve_having] folded aggregates from `rt.lhs` ONLY, so an aggregate or column on the RIGHT was never lifted into a group slot - which is why its own `(RawLhs::Agg, RawKind::CmpExpr)` special case still refused `MAX(N) = MAX(ID)`. It now lifts a plain group-key column as well as an aggregate into a `RawExpr` when the kind carries a right-side expression, folds and substitutes aggregates from BOTH sides, and resolves against [synth_group_view]. **AND IT LANDED IN TWO STEPS, which is the part worth keeping.** The first cut fixed exactly the shapes where BOTH sides were aggregates (`MAX(U) LIKE MAX(V)`, `MAX(N) = MAX(ID)`) and left every GROUP-KEY shape still refusing (`U LIKE V`, `MAX(U) LIKE V`, `U = V`) - because [synth_group_view] exposes a key only once it is a `GItem::Key`, and the keyed column path adds those LAZILY, BELOW the branch I had routed into. The measured split said exactly that, and the fix was to seed a Key slot for every grouped column first, using the keyed path's own idiom. **THE SAFE FAILURE IS WHY THIS WAS ATTEMPTABLE AT ALL:** a column that is not a group key is simply ABSENT from the synthetic view, so such a term REFUSES rather than binding to a wrong slot - `HAVING ID LIKE V` with `ID` ungrouped refuses on BOTH servers. **FOUR OF MY OWN GATE CELLS WERE VACUOUS** and my own vacuity guard caught every one: `MAX(u) = MAX(v)`, `MAX(u) = v`, `u = v` and `MAX(n) <> MAX(id)` match nothing in that fixture, so both sides answered NOTHING - the agreement a match-nothing bug also produces. Replaced with forms that return rows and keep the same paths. `SIMILAR TO` stays out, as in both previous chunks. Eight sweep gates clean - and NONE of them carries a counted floor, checked rather than assumed: every `-lt` in them is a retry loop. **"THE SAME GUARD EXISTS TWICE" IS NOW THREE ROUTERS, ALL CLOSED:** [resolve_predicate] (two guards), [resolve_raw_cond], and [resolve_having]. When a capability must be routed, COUNT THE ROUTERS before believing a count. wire 348 / ods 161. **AND THE FOURTH FAMILY CLOSES THE ARC: `SIMILAR TO` TAKES AN EXPRESSION PATTERN, IN ALL THREE WORLDS (2026-09-18, new gate `qa/serve-real-simexpr.sh`, 29 cells, counted floor 29, exit 1 and 19 cells red on `979c144`).** `RawKind::SimilarExpr` / `RawCond::SimilarExpr` -> `Term::ExprSimilarExpr` / `Cond2::SimilarExpr`, and HAVING came along for free because chunk 40's [kind_rhs_expr] routes any kind carrying a right-side expression - once the new kind was NAMED there, which no compiler error would have demanded. **THE DEFERRAL WAS AN ASSUMPTION, AND READING THE FUNCTION REFUTED IT.** Three chunk records called SIMILAR "the expensive corner" because its pattern is a compiled `SimRe`; [sim_compile] is an EIGHT-LINE recursive-descent parse into a small enum tree (`Lit`/`Any`/`AnySeq`/`Class`/`Concat`/`Alt`/`Repeat`), no regex engine, and `SimRe` derives Clone - a per-row compile is a cheap tree build, no dearer than the rebuild [containing_term]'s per-row form had already been doing for two chunks. The scouted design said "MEASURE it" and was right to; what went wrong is that later records repeated the guess as though it had been measured. All four copies of the claim are now struck or annotated. **THE ONE GENUINELY NEW MECHANISM IS THE PER-ROW RAISE.** A malformed pattern cannot be caught at prepare when it arrives per row, and the engine's rule is VALUE-GATED, measured: a row whose pattern is `[` raises 42000, and the same statement with that row excluded by a prior conjunct ANSWERS 2. So this raises [EvalErr::InvalidSimilar] at EVALUATION - the error [Term::BadSimilar] already used for the prepare-time case - and never refuses at prepare, which would have been a wrong answer for the filtered query. `cond_no_raise` answers FALSE for the variant for the same reason. Three `both_raise` cells pin that both servers raise AND say the same thing, and the decisive cell pins that the excluded-row form answers. **A GATE CELL OF MINE ASSERTED A FIXTURE THAT DID NOT EXIST:** the carrier/real mix cell called `v` a "real pattern", but this gate's database has NO default charset, so `u`, `v`, `sub` and `n` are ALL byte carriers - the cell was carrier-vs-carrier, which legitimately answers, and it went red reporting its own gap as closed. Fixed by adding an explicitly-UTF8 `vu` column, splitting the cell into the matched pair (answers) and the genuine mix (refuses, gated). Twelve sweep gates clean across all three worlds; wire 348 / ods 161. **AND THE CARRIER/REAL MIX IS ANSWERED NOW, IN ALL FOUR FAMILIES AND ALL THREE WORLDS (2026-09-18, new gate `qa/serve-real-carriermix.sh`, 27 cells, counted floor 27, exit 1 and 18 cells red on `d7b5a77`).** Eight refusal guards - four in the value world, four in the predicate world, HAVING inheriting the latter - become one call to [carrier_pattern]. **THE RULE IS NOT PER-FAMILY, and the correction above this line is the point:** measured across all four families, carrier VALUE / real PATTERN answers in BYTE SPACE (the real side re-spelled as the carrier of its own octets via [Expr::CarrierEnc], exactly what [cmp_sides] already does for a comparison, so the non-ASCII row silently MISSES and nothing raises), while real VALUE / carrier PATTERN DECODES the pattern into the real set and raises 22000 when its octets do not spell it - **PER ROW**, so one statement answers for a row whose pattern is ASCII and raises for a row whose pattern is not. Both mechanisms already existed: the new `Expr::CarrierDec` is the decode counterpart of `CarrierEnc` and simply calls [transcode_text], whose carrier-source vector is already `MalformedString`. **THE SILENT/FORCED SPLIT AGAIN, and `expr_no_raise` is why it was checked rather than copied:** of the six prepare-time readers `text_form_m`'s own comment enumerates, three are exhaustive (the compiler named `type_of` and `rank_of`) and three carry catch-alls - and `CarrierDec` had to be listed with the RAISING coercions there, not with `CarrierEnc`, because decoding can fail where re-spelling cannot. **THREE RECORDED GAPS CLOSED AS A SIDE EFFECT:** `serve-real-nonecmp`'s three `refuses` cells for a COLUMN pattern in a mix now answer, and were promoted to `agree` - confirmed the way this page demands, by running `d7b5a77`, which passes all 138. Seventeen sweep gates clean. **AND THE DIAGNOSTIC FOUND TWO DIVERGENCES OF ITS OWN**, both filed above: a select-list alias shadowing a GROUPED column is accepted in HAVING/ORDER BY where the engine refuses (a WRONG ANSWER), and `ORDER BY` a grouped column beside an aggregate under a different alias is refused where the engine answers. Neither has anything to do with charsets; both surfaced because one failing cell was diagnosed with controls instead of patched. wire 348 / ods 161.
- ~~**HIGH** (the CAST matrix and string functions a) — UTF8 -> OCTETS implicit conversion transliterates to Latin-1 instead of taking raw storage bytes, and announces len 64 where the engine announces 160~~ **STALE IN BOTH HALVES, measured 2026-09-17.** `CAST(<UTF8> AS VARCHAR(32) CHARACTER SET OCTETS)` yields the RAW STORAGE BYTES `73747261C39F65` (`OCTET_LENGTH` 7) identically on both servers - no transliteration - and the ANNOUNCE for `<UTF8 VARCHAR(32)> || <OCTETS VARCHAR(32)>`, read with `SET SQLDA_DISPLAY ON`, is `len: 160 charset: 1 SYSTEM.OCTETS` on BOTH: the very number this bullet says fire-crab gets wrong as 64. The carrier byte-width rule is gated by `serve-real-concatcs.sh` (green at 52) and the cast forms by `serve-real-castnone.sh` (18) and `serve-real-octets.sh` (36).
- ~~**HIGH** (the CAST matrix and string functions a) — NONE || WIN1252 resolves the NONE bytes through Latin-1 instead of WIN1252 (0x9F becomes U+009F, not U+0178)~~ **STALE, measured 2026-09-17 under all three attachments.** `<NONE> || <WIN1252>` comes back as the two operands' stored octets IN ORDER - `73747261C39F65` then `737472619F65` - byte-identical on both servers under a default NONE, a UTF8 and a WIN1252 attachment, announced `len: 64 charset: 53 SYSTEM.WIN1252` on both. There is no Latin-1 resolution left to diverge over: the law became a BYTE COPY (`transcode_text`), and `serve-real-concatcs.sh` gates it across the same three attachments - its own header documents this exact 0x9F/U+0178 defect in the past tense.
- ~~**MEDIUM** (scalar subqueries) — Scalar subquery describes from the executed VALUE, not the inner column: SMALLINT and small BIGINT both come back LONG, a large BIGINT comes back INT64 — the same statement describes differently depending on the data~~ **STALE, measured 2026-09-17** (`SET SQLDA_DISPLAY ON`, both servers, fixture verified): a subquery over SMALLINT announces `SHORT scale 0 subtype 0 len 2` and one over a small BIGINT announces `INT64 scale 0 subtype 0 len 8` — the inner COLUMN's type, distinct from each other, identical on both servers. The describe does not follow the executed value.
- ~~**MEDIUM** (scalar subqueries) — Scalar subquery over NUMERIC/DECIMAL announces sub_type 0, erasing the NUMERIC(1) vs DECIMAL(2) discriminator, and widens the storage type~~ **STALE, measured 2026-09-17**: a subquery over `NUMERIC(1)` announces `SHORT scale 0 subtype 1 len 2` and one over `NUMERIC(9,2)` announces `LONG scale -2 subtype 1 len 4` — the discriminator is carried, the width is not widened, and both servers agree cell for cell.
- ~~**MEDIUM** (scalar subqueries) — Scalar subquery that matches no rows describes as TEXT(1) CHARACTER SET NONE instead of the inner column's SMALLINT~~ **STALE, measured 2026-09-17**: `SELECT (SELECT SM FROM TX WHERE ID=999) FROM RDB$DATABASE` — a subquery matching NO rows — announces `SHORT scale 0 subtype 0 len 2` on BOTH servers, the inner column's own type. No TEXT(1) anywhere.
- ~~**MEDIUM** (scalar subqueries) — Subquery-bearing select-list expressions lose the Nullable flag and propagate the non-nullable claim through arithmetic, CHAR_LENGTH and CASE~~ **STALE, measured 2026-09-17**: `(SELECT SM ...) + 1` announces `INT64 Nullable` and `CHAR_LENGTH((SELECT NM ...))` announces `LONG Nullable`, identically on both servers. The flag survives both arithmetic and a consuming function.
- ~~**MEDIUM** (window functions) — RANGE BETWEEN <offset> PRECEDING AND UNBOUNDED FOLLOWING~~ **STALE, measured 2026-09-17: agrees row for row (10,10,10,7) — see "Stale claims retired"**. Original report: RANGE BETWEEN <offset> PRECEDING AND UNBOUNDED FOLLOWING gives a NULL ordering-key row only its own peer group instead of the tail of the partition
- ~~**MEDIUM** (common table expressions) — A CTE whose name shadows a real table and self-references is answered with PostgreSQL-style scoping; the engine refuses it as cyclic~~ **FIXED 2026-09-17, and the shape is WIDER than the bullet: the CYCLE is the error, with or without a table of that name.** Inside a `WITH`, an unqualified name matching a CTE of the same `WITH` binds to THAT CTE; if that closes a cycle the engine raises `-104 CTE '"<N>"' has cyclic dependencies`. This server resolved such a name to the TABLE wherever one existed and ANSWERED (the bullet's PostgreSQL-style scoping), and gave a contentless 42000 where none did - so the same root cause produced a wrong answer in one spelling and a message gap in another. It reaches a self-reference in a SUBQUERY, in a JOIN, inside a nested DERIVED table, a MUTUAL pair, a three-CTE ring, a non-recursive UNION body, and a cycle the main query never even reaches (still an error - only a CYCLE-FREE unused CTE is a warning). **The name in the message is the CTE the resolution ENTERED at** - one three-CTE ring answers A1, A2 or A3 depending only on which the main query reads - falling back to the LAST-DECLARED member when no part of it is reachable. Two more diagnoses share the fix: `Recursive CTE (@1) must be an UNION` and `Non-recursive member is missing in CTE '@1'`. Still legal and still answering: a CTE that shadows without naming itself, a body reading another table, a QUALIFIED self-reference, and a forward reference with no cycle.
- ~~**MEDIUM** (common table expressions) — String expression over a CTE/derived literal column is described with charset 255 CS_dynamic where the engine says charset 0 SYSTEM.NONE~~ **DONE 2026-09-17 - `qa/serve-real-litcs.sh` 78 -> 134 (17 flip on `b1df783`), and the bullet UNDERSTATED it: this was a WRONG ANSWER, not a describe entry.** The 255 is not a `CS_dynamic` leak - `ATT_SUBTYPE` is `-1` - it is `charset_id(-1)`, which is `(sub_type as u16 & 0xFF)` = 255. [desc_of_projcol] resolves the real-charset sentinel ([enc_real_cs]) into a ttype but passes `ATT_SUBTYPE` through untouched, and [text_form_m]'s `Expr::Col` arm read that descriptor's `sub_type` straight into `TfCs::Ttype` with no negative guard - so a LITERAL wrapped in a derived table, a CTE or a view became charset 255 in every consumer: [cs_join] masks it, [build_expr_col] announced `enc_real_cs(255)`, and [expr_value_charset] found 255 neither a byte carrier nor tabled and fell through to UTF-8 semantics. Under a NONE attachment `OCTET_LENGTH` over a wrapped `'a<C3A9>b'` answered **6 where the engine answers 4**, and `Z.C || 'x'` raised `string right truncation, expected 3, actual 5` where the engine answers the row. [corr_text_form] is this arm's twin and had guarded the sentinel all along; the fix mirrors it, and moves no width (`bytes_per_char(255)` is already 1 and the sentinel's length is already a character count).
- ~~**MEDIUM** (date/time arithmetic) — DATE - DATE describes as INT64 len 8 where the engine says LONG len 4, and the wrong width cascades to INT128 in further arithmetic~~ **STALE, measured 2026-09-17**: `DATE - DATE` announces `LONG Nullable scale 0 subtype 0 len 4` on BOTH servers — the width the bullet says only the engine gives — and the value agrees at 250. Gated by `serve-real-tempdiff.sh` (19) and `serve-real-datemath.sh` (41), both green.
- ~~**MEDIUM** (date/time arithmetic) — TIME - TIME describes as INT64 subtype 0 len 8 where the engine says LONG subtype 1 (NUMERIC) len 4~~ **STALE, measured 2026-09-17**: `TIME - TIME` announces `LONG Nullable scale -4 subtype 1 len 4` on BOTH servers — exactly the NUMERIC form the bullet attributes to the engine alone.
- ~~**MEDIUM** (date/time arithmetic) — TIMESTAMP - TIMESTAMP describes sub_type 0 where the engine says 1 (NUMERIC), and marks the all-literal form Nullable where the engine does not~~ **STALE IN BOTH HALVES, measured 2026-09-17**: the column form announces `INT64 Nullable scale -9 subtype 1 len 8` on both, and the ALL-LITERAL form announces `INT64 scale -9 subtype 1 len 8` — NOT Nullable — on both. Subtype 1 and the nullability both match.
- ~~**MEDIUM** (date/time arithmetic) — Implicit temporal-to-string conversion in concatenation describes VARCHAR(32765) instead of the natural width (DATE 10, TIME 13, TIMESTAMP 25)~~ **STALE, measured 2026-09-17**: `<DATE> || 'x'` announces len 44, `<TIME> || 'x'` len 56 and `<TIMESTAMP> || 'x'` len 104 — identically on both servers. Those are the natural character widths at UTF8's four bytes per character (11, 14 and 26 characters), not 32765.
- ~~**MEDIUM** (the CAST matrix and string functions a) — ASCII_VAL over a non-ASCII character returns the code point where the engine raises 22018 "Cannot transliterate character between character sets"~~ **FIXED 2026-09-17, and the bullet understated it - widening the probe found a SECOND wrong answer and a refusal beside it.** The engine's law, measured across 31 cells and three attachments: ASCII_VAL is the first BYTE of the operand's STORED representation, and the CHARACTER SET decides. A byte carrier's decoded char IS its byte (a NONE 0xE9 is 233 - the only case that ever agreed). A TABLED SINGLE-BYTE page answers the stored codepage byte: **a WIN1252 0x9F is 159, where fire-crab answered 376 (U+0178), a wrong value the bullet never mentioned and one with no raise anywhere near it.** A MULTIBYTE set raises 22018 *Cannot transliterate character between character sets* when the FIRST character occupies more than one byte - and only then, since `'aé'` is 97 on both. A bare literal takes the ATTACHMENT's set, which is why `ASCII_VAL('é')` raises under UTF8 and answers 195 under WIN1252 or NONE. The raise is PER ROW in DELIVERY order (under `ORDER BY ... DESC` the engine delivers the later rows first, then raises), a row the WHERE excludes never raises, and a WHERE term over a raiser raises mid-scan. Third defect, also unlisted: `ASCII_VAL(NULL)` answered a contentless 42000 where the engine answers NULL - `Expr::Null` types as Int, so the text-only fence at `type_of` refused the untyped literal (a NULL COLUMN and `CAST(NULL AS VARCHAR(4))` were never affected). The fix stamps the operand's set at resolution as `SysFn::AsciiValCs(u8)` - the `OctetLengthCs` precedent - reading it from `cmp_text_charset` and NOT `expr_value_charset`, which answers None for UTF8 and would have produced a fix that never fires on the case it was written for. `expr_no_raise`'s standing claim that ASCII_VAL never raises was false and had let a WHERE term be admitted that then answered 3 rows against the engine's raise; it is now judged by the set. `qa/serve-real-asciival.sh` 3 -> 40 checks, its fixture gaining UTF8/WIN1252/NONE/OCTETS/ASCII/CHAR columns - **the old three checks ran only over a NONE database with a plain VARCHAR(5), the single case that already agreed, and its header recorded the engine as "taking the first byte" where it in fact raises.**
- ~~**MEDIUM** (the CAST matrix and string functions a) — LPAD/RPAD with a non-literal length argument announces double the engine's maximum length (65532), above the 32765-byte VARCHAR limit.~~ **DONE 2026-09-18 - `qa/serve-real-funcwidth.sh` 59 -> 115 checks (21 flip on `c05b375`).** THE BULLET'S CENTRAL CLAIM WAS FALSE: it says "no single ceiling reproduces the table", and one does. Measured over four source charsets x four attachments (the fourth, ISO8859_1, added precisely to test the generalisation the reverted attempt got wrong), the engine's width is `chars = 65533/bpc(SOURCE)`, `bytes = min(chars, ceiling/bpc(OUT)) * bpc(OUT)`, with `ceiling` **65533 under a NONE attachment and 32765 under ANY real one** - which is why the previous attempt, moving the ceiling UNCONDITIONALLY, was right about real attachments and destroyed every NONE-attachment cell. Two sites: [text_form_m]'s Lpad/Rpad arm now counts the fallback in the SOURCE's characters (`MAX_VARCHAR_BYTES / bpc_src`, where it handed back a source-blind 65533), and [resolve_text_cs] takes the attachment-conditional ceiling. The SECOND HALF was found by the gate and diagnosed from the code, not guessed: NONE, OCTETS and CHAR-NONE sources have `charset_id <= 1`, so [build_expr_col] announces them as a PLAIN POSITIVE ttype and they never reach `cap` at all - they were still 65533 under every attachment while the UTF8 and WIN1252 sources (negative `enc_real_cs` sentinels) were already correct. Values move nowhere, and the statically sized expressions never touch the sentinel (`REPLACE`, `SUBSTRING`, `UPPER`, `||` measured identical on both servers at 7-14 characters, orders below any ceiling), which is what bounds the blast radius that broke `exprshape` last time - it is 54/0 here. **The earlier attempt and its revert are recorded below and were RIGHT to be reverted; the difference this time is that the WHOLE matrix was measured before a line was written.** The divergence is real only under a REAL attachment. Under a NONE attachment - which is what most gates use - fire-crab matches the engine cell for cell, which is why `exprshape` and `funcwidth` were green over it. THE ENGINE'S COMPUTED-PAD WIDTH DEPENDS ON THE SOURCE CHARSET **AND** THE ATTACHMENT, measured engine-only across the matrix: a UTF8 source announces 65532 under a NONE attachment, 32764 under UTF8 and **16383 under WIN1252**; a NONE source 65533 / 32765 / 32765; a WIN1252 source 65533 (cs 53) / 32764 (cs 4) / 32765 (cs 53); OCTETS 65533 / 32765 / 32765. Equivalently the CHARACTER cap is `65533/bpc_source` for some combinations and `32765/bpc_source` for others, and **no single ceiling reproduces the table** - a UTF8 source caps at 16383 characters under NONE *and* WIN1252 but at 8191 under UTF8, while a NONE source caps at 65533 under NONE and 32765 under both real attachments. THE ATTEMPT: `MAX_VARCHAR_BYTES` 65533 -> 32765, which is the `resolve_text_cs` ceiling feeding the computed-pad fallback. It made every `-ch UTF8` cell agree and REGRESSED `exprshape` from 54 OK / 0 DIFF to 50 / 5, because the previous value was exactly right for the NONE attachment that gate uses (triangulated: engine 65532 / 65533, previous binary identical, new binary 32764 / 32765). Reverted; `exprshape` is 54 / 0 again. The lesson is not subtle - I measured ONE COLUMN of the matrix under `-ch UTF8` and generalised it to the whole table. OPEN, and NOT measured: whether the engine RAISES 54000 or CAPS the declared width at the top end. Over a COLUMN source it caps (`LPAD(<utf8 col>, 32766)` announces 65532 under NONE, no raise), which sits uneasily beside the stored claim at `server.rs:67413` that `LPAD('Hi',16384,'*')` raises at prepare - plausibly a literal-source versus column-source split, and fire-crab's refusal arm keys on the SOURCE charset's bytes-per-character where everything above tracks the ATTACHMENT. `qa/serve-real-funcwidth.sh` 48 -> 59 checks now pins the NONE-attachment behaviour that the attempt broke, including the computed-length pads over UTF8 / NONE / OCTETS / CHAR sources that gate never had.
- **MEDIUM** (the CAST matrix and string functions a) — **A FOLDED SCALAR SUBQUERY SIZES A PAD FROM THE DATA.** A non-correlated scalar subquery is evaluated at PREPARE and spliced into the statement TEXT as a literal (`server.rs` ~49603 / ~49736), so by the time the width is computed it is indistinguishable from one the user wrote, and the pad is sized at that value: measured over a two-row table, `LPAD(<utf8 col>, (SELECT MAX(ID) FROM T))` announced len 28 for MAX 7 and len 4 for MIN 1, `COUNT(*)=2` gave len 8, and a NONE source gave len 7 - where the engine keeps it dynamic at 32764 / 32765. **The same statement describes differently against different data**, which is the one thing a describe must never do, and it is the NARROW direction, so a client sizing buffers from it truncates. Constant arithmetic is NOT folded (`LPAD(V, 3+4)` keeps the dynamic width on both), so this is specific to the text fold. Unlike the width law above, it is attachment-INDEPENDENT and reproduces everywhere. Recorded rather than fixed: undoing the fold for this one position needs the argument's place recovered from the pre-fold text, and widening every subquery-bearing item instead would be wrong - a whole-item scalar subquery announces its column's own type and agrees today (the `subq_items` nullability patch beside it is the model a real fix would follow). Gated as a self-expiring `differs` cell in `qa/serve-real-funcwidth.sh`.
- ~~**LOW** (CASE / COALESCE / NULLIF / IIF / DECOD) — CAST of a literal to CHARACTER SET WIN1252 transliterates where the engine byte-copies, so a CASE branch carrying it ships 1 byte instead of 2~~ **STALE, measured 2026-09-17 across all three attachments (NONE / UTF8 / WIN1252):** `CAST('é' AS VARCHAR(4) CHARACTER SET WIN1252)` agrees on its octet length, on its raw bytes read back through an OCTETS cast, inside a `CASE` branch and inside `COALESCE` - twelve cells, all agreeing. This was the LOW tier's only candidate for a wrong VALUE and it does not reproduce; the byte-copy law arrived with the `transcode_text` work that also retired the `NONE || WIN1252` bullet above.
- ~~**LOW** (scalar subqueries) — EXISTS in the select list describes non-nullable and names the column CONSTANT instead of BOOL. **HALF OF THIS CLAIM IS FALSE and the bullet stays LIVE for the other half.** **Measured 2026-09-17: the nullability half is WRONG - both servers announce `BOOLEAN Nullable`.** What survives is the NAME alone: the engine says `name: BOOL`, this server `name: CONSTANT` (alias `E` on both). It is the same leak as the recursive-CTE one below - an expression's describe carries this server's own node kind where the engine carries a chosen label or nothing - and the two should be fixed together.~~ **FIXED 2026-09-17 (`serve-real-describe` 146, 14 flip on `51a9068`) - and it was never a missing law but a TEXT MATCH.** The fold rewrites `EXISTS <marker>` to a literal and the planner patches the column back to BOOL; the patch compared the item text EXACTLY, so an alias or a leading `NOT` fell through to the literal's own CONSTANT. Measured: the engine says BOOL for `EXISTS(...)`, `EXISTS(...) AS E`, a bare `EXISTS(...) E` and `NOT EXISTS(...)` alike, in every select-list position - and `NOT EXISTS` already got the ALIAS right while the field said CONSTANT, so the two travel by different routes and only one was wrong. **The "one behaviour seen three ways" reading was itself wrong**: measurement split it into THREE separate defects with three different causes - this text match, a scope rule (below), and a union rule the recursive path never ran.
- ~~**LOW** (scalar subqueries) — ONE ROOT CAUSE with the two below: Derived table + GROUP BY: fire-crab marks the grouping column Nullable where the engine keeps the base primary key's NOT NULL. **Re-measured 2026-09-17 and LOCALISED - it is not "derived tables lose NOT NULL".** Two controls place it exactly: `SELECT ID FROM NN` agrees (`LONG` non-null on both) and `SELECT G FROM (SELECT ID AS G FROM NN) Z` **also agrees** - so a derived table DOES carry the bit through. What loses it is an EXPRESSION over a wrapped column (`G + 1` over a derived table, `ID + 1` over a CTE, `a.W * 2` in a LATERAL) and this GROUP BY key. `serve-real-notnulldesc.sh` states the law correctly in its own header ("a derived table, a CTE, DISTINCT/FIRST keep the inner bit; a GROUP BY key keeps the column's") and is green at 27 - its fixture has the right columns but only ONE derived cell, TWO CTE cells and NO LATERAL, and `serve-real-derived.sh` / `serve-real-cte.sh` contain no `SQLDA_DISPLAY` at all, so none of them can reach the shape.~~ **STRUCK 2026-09-17: the GROUP BY HEADLINE IS STALE, and what survived of it is FIXED.** Re-measured in the bullet's OWN shapes - `SELECT * FROM (SELECT ID FROM NN GROUP BY ID) Z`, the aliased key, and the key beside an aggregate - all three AGREE today (`LONG` non-null on both). The only differing cell was an EXPRESSION over the key, which is not a GROUP BY fact at all but the expression defect above, now fixed. A bullet naming two things where measurement finds one is how this list keeps over-stating itself.
- ~~**LOW** (scalar subqueries) — UNDERSTATED, and the bigger half is a WRONG ANSWER: Scalar aggregate subquery inside a derived table reports RDB$FIELD_NAME 'MAX' where the engine leaves it blank. **Re-measured 2026-09-17, and the bullet describes the lesser of two defects.** THE ENGINE REFUSES THE STATEMENT OUTRIGHT: `SELECT * FROM (SELECT (SELECT MAX(W) FROM NN) FROM RDB$DATABASE) Z` answers `-104 / Invalid command / no column name specified for column number 1 in derived table Z`, and this server ANSWERS it (20). A control pins the rule as general rather than subquery-specific: the engine refuses `SELECT * FROM (SELECT 1+1 FROM RDB$DATABASE) Z` too, and this server answers that as well - **every derived-table column must be named**. Answering where the engine raises is the class already fixed once here (COALESCE-over-multiply), so it outranks every describe entry in this tier. The name half is real but separate: with the column NAMED, the engine gives `name:` blank where this server gives `name: MAX` (alias `M` on both). **THE WRONG-ANSWER HALF IS FIXED (2026-09-17): both servers now refuse the unnamed shape, with the engine's own four-line message, its column POSITION and its derived-table ALIAS — see the DONE entry for the unnamed derived-table column. The bullet stays LIVE for the NAME half, which re-measurement WIDENS:** the leak is there with the column aliased in a derived table, over a real table, in a CTE, *and* under a DECLARED column list `Z (M)` — four shapes, not one. Two controls bound it and correct the diagnosis: a PLAIN aggregate in a derived table AGREES (`name: MAX alias: M` on BOTH servers), and so does the TOP LEVEL (`name: MAX`), so this is **not** "an aggregate leaks its kind-name" — it is a SCALAR SUBQUERY column losing its symbolic name as it crosses a derived/CTE SCOPE BOUNDARY, where the engine blanks it and this server carries it through. That puts it with the EXISTS bullet and the recursive-CTE one: ONE behaviour, now seen FOUR ways, and they should still be fixed together.~~ **THE NAME HALF IS NOW FIXED TOO (2026-09-17), and the "one behaviour" reading did not survive measurement.** The real law is a SCOPE rule with a precise trigger: **a subquery in a derived table's or CTE's SELECT LIST blanks every EXPRESSION column of that scope one level up**, while plain FIELD columns keep their names and the top level blanks nothing. It blanks a neighbour the outer never selects, so it belongs to the scope rather than the column. The trigger is the select list ALONE - a subquery in the inner `WHERE`, `HAVING`, an `EXISTS` or `IN` there, or a nested derived table in the inner `FROM` all KEEP the kind-name - and it is **not** a flattening rule, which was the obvious guess: `DISTINCT`, `GROUP BY`, an aggregate, `FIRST`, `SKIP`, `ORDER BY` and a window function are all flatten blockers and all keep it. Nine such shapes are now regression pins in `serve-real-describe`.~~ **BOTH HALVES ARE NOW CLOSED, so the bullet is struck**: the wrong-answer half by the unnamed-derived-column landing (the engine refuses the statement outright) and the name half by this one.
- ~~**HIGH** (generators) — **A GENERATOR READ THROUGH A DERIVED TABLE ANSWERS ZERO.** Found in passing while probing the `fname: None` column class for the unnamed-derived-column chunk, and deliberately NOT folded into it - `SELECT * FROM (SELECT NEXT VALUE FOR G1 AS N FROM NN) Z` answers `0, 0` where the engine answers `1, 2`, and `SELECT * FROM (SELECT GEN_ID(G1, 1) AS N FROM NN) Z` answers `<null>`. The UNNAMED forms of both now refuse on both servers (the derived-column law), so this is only reachable with the column named, which is exactly why it survived: the shape that exposes it is one step off the shape the gates wrote. **A wrong ANSWER, so it outranks every describe entry in this list and is the next candidate.** Measure the generator's value path per ROW before touching it: the top-level `SELECT NEXT VALUE FOR G1 FROM NN` was not compared here, and whether the draw happens once or per row is the whole question.~~ **FIXED 2026-09-17 (`serve-real-genrow` 56, 15 flip on `ad86de9`) - and that closing question was the right question with the wrong dichotomy: it is neither once nor per row, but ONCE PER DELIVERED REFERENCE.** Measured over four rows: one reference draws 4, two draw 8, three draw 12, a column the outer never delivers draws NOTHING, and a filter on another column does not re-evaluate - it shrinks the delivered set first (keeping 2 of 4 draws 2). The value follows OUTPUT order, so the first DELIVERED row takes the first value whatever record it came from. The cause was one unguarded `..`: `branch_rows_each`'s `Plan::Project` arm dropped `gen_cols` on the floor - the THIRD time that destructure has done so in this function family - leaving the synthetic slot unfilled, which reads as NULL and renders as 0 under `NEXT VALUE FOR`'s non-nullable announcement (580) and as `<null>` under `GEN_ID`'s (581). One cause, two appearances. Fixed by classifying the shape ([classify_derived_gen]) and, at exactly one delivered reference, materialising / filtering / sorting and THEN advancing; every other count REFUSES rather than answer a different wrong number.
- ~~**LOW** (common table expressions) — the same root cause as above: Any expression over a CTE (or derived-table) column is announced Nullable; the engine announces it NOT NULL. **Confirmed 2026-09-17**: `WITH C AS (SELECT ID FROM NN) SELECT ID + 1 AS E FROM C` announces `INT64` non-null on the engine and `INT64 Nullable` here, identically under all three attachments; the derived-table twin (`G + 1`) behaves the same. Fixing the expression case should retire this, the GROUP BY key above and the LATERAL entry below together.~~ **FIXED 2026-09-17 (`serve-real-notnulldesc` 27 -> 42, 11 flip on `6addfe3`)** - and it retired the GROUP BY entry with it, though NOT the LATERAL one, which has a different mechanism. The bit travels through a derived table, a CTE and a union already (a plain pass-through copies it explicitly); what lost it was an EXPRESSION in the OUTER query, because the synthetic view's descriptors carry `flags: 0` ([desc_of_projcol]) and [mark_not_null_cols] needs a base TABLE that a derived scope has not got - so the `is_nn` closure answered "nullable" for every field. [plan_over_source]'s expression branch now runs the existing [expr_nullable] against the INNER columns' own bits. Measured over every kind (arithmetic, negation, CAST, a function, UPPER, `||`, CASE with both branches fixed), through a union column and two deep - with the control that a nullable source stays nullable on both servers, which is why this READS the bit rather than setting it.
- ~~**LOW** (common table expressions) — Recursive CTE tree walk is emitted breadth-first; Firebird emits it depth-first~~ **FIXED 2026-09-17, and the bullet was the only LOW entry that turned out to be a WRONG ANSWER rather than a describe difference.** THE ENGINE'S LAW, measured before any code: depth-first PRE-ORDER, with roots and siblings in RECORD (storage) order - over a two-root tree it emits `1,2,4,6,3,5,7,9,10,8` where a level queue emits `1,7,2,3,9,8,4,5,10,6`, and the projected level makes it plainer (`0,1,2,3,1,2,0,1,2,1` against `0,0,1,1,1,1,2,2,2,3`). It is INVARIANT to an index on the parent column and FOLLOWS INSERTION ORDER: the same tree inserted in reverse walks `7,8,9,10,1,3,5,2,4,6`. A DIAMOND emits the doubly-reachable node TWICE, once per path; duplicates are kept adjacent; an orphan never enters; and there is NO cycle detection - a cycle simply recurses to the depth bound. **THE STORED REASONING FOR LEAVING IT WAS WRONG, and this server's own output refuted it:** the note at the refusal site argued "fire-crab has no record-number model, so a stack instead of a queue would only trade one wrong order for another" - but each expansion already SCANS the table, so its rows come back in storage order by construction (this server's level-1 rows were `2,3,9,8`, exactly the engine's sibling order). What was missing was never the sibling order but the INTERLEAVING, which is a stack over ROWS, not over levels. TWO MORE THINGS FELL OUT: `FIRST`/`SKIP`/`ROWS` over a recursive CTE was deliberately REFUSED because a queue sliced different rows than the engine - with the order correct the refusal is gone and the slices match (`FIRST 4` is `1,2,4,6`); and the depth bound is a RAISE, not a refusal - 1023 and 1024 levels answer in full, 1025 raises `SQLSTATE 54001 Too many concurrent executions of the same request`, and a cycle raises the same way, where this server answered a bare 42000 for both (now `EvalErr::TooManyClones`, reusing the engine's own `GDS_REQ_MAX_CLONES`). **THE COST IS REAL AND IS RECORDED RATHER THAN HIDDEN:** the branch is re-planned once per ROW instead of once per LEVEL, because `plan_over_source` consumes its `BoundSrc` at three shape-dependent sites and there is no single node to re-bind. Measured on a quiet box, attach amortised, repeatable to the millisecond: CHAINS are unchanged (500 → 62/59 ms, 1000 → 144/144, 2000 → 201/201, identical before and after, since one row per level was already one expansion per row), while a ~950-node BUSHY tree goes from 1 ms to 134 ms - against the engine's own 150 ms for that shape. The shape this server was fastest at was fast because it was answering in the wrong order. `qa/serve-real-recursive.sh` 43 → 58 checks; 12 flip on the previous binary, and the cells that should not move (duplicates, the 1023/1024 chains, all 43 older cells) pass on both.
- **LOW** (common table expressions) — **A recursive CTE JOINED to a table takes the JOIN's driver order, not the walk's.** Surfaced while measuring the walk above and NOT fixed there: `<walk> SELECT R.ID FROM R JOIN TREE T2 ON T2.ID = R.ID` answers `1,2,3,4,5,6,7,9,8,10` on the engine — TREE's storage order, because its optimiser drove the join from the TABLE — where fire-crab answers the CTE's own order (`1,2,4,6,3,5,7,9,10,8` now, `1,7,2,3,9,8,4,5,10,6` before). It differed before the depth-first change and differs after, so it is a separate join-driver-order question, not part of that law. ~~Unmeasured beyond this one shape.~~ **MEASURED IN FULL 2026-09-18, RECORDED NOT FIXED, and gated by eight self-expiring `differs` cells (`qa/serve-real-recursive.sh` 57 -> 73).** THE LAW: the engine DEMOTES a recursive CTE out of the driving position and drives the other stream, so rows come back grouped by THAT side's rows in ITS storage order, each followed by its matches in the walk's order. Over `JM` stored 50,10,30,20,40 against a walk of 1,2,4,7,9 the engine answers `50/9 10/1 20/1 40/2` where this server answers `10/1 20/1 40/2 50/9`. It holds with a TABLE, a DERIVED TABLE and a VIEW opposite the CTE alike, for a comma join as for `JOIN`, and in a chain the engine drives the first non-recursive stream whichever position the CTE occupies - and it diverges ONLY when the CTE is written FIRST (`JM JOIN C` already agrees). **IT IS NOT ORDER-ONLY**: `FIRST 3` answers the engine's `50,10,20` as `10,20,40` and `SKIP 2` its `20,40` as `40,50` - a DIFFERENT SET OF ROWS, silently, the same escalation the depth-first entry above hit. **THE ENGINE ONLY DRIVES THE OTHER STREAM FOR A PLAIN JOIN**, which is why a blanket fix would be a NEW wrong answer: it drives the CTE instead - and this server already agrees - when a conjunct in the WHERE or the ON names ONE of the step's two streams, or when an index serves the join column. A conjunct naming NEITHER stream (`WHERE 1=1`) does NOT move it, and a conjunct naming a THIRD stream in a chain does not either; those seven agreeing shapes are held as `both` controls beside the eight recorded ones. **WHY IT IS RECORDED RATHER THAN FIXED**: the ordering cannot live where the RIGHT-join law lives. For an INNER equi-join EVERY [join_step] call site passes a ONE-ROW accumulated side - `rows_materialised` takes its whole-acc early return only for non-LEFT/INNER kinds, `JoinCursor::fold` expands one base row at a time, and the only whole-acc INNER call is guarded by `keys.is_empty()` (a theta join) - so a sort there reorders within a single row's slice. An attempt that did exactly that was built, measured as changing NOTHING (every probe and all eight cells identical to the previous binary), and REVERTED. Ordering it correctly means carrying the other side's row index out to the three concatenation points - the streaming arm, the materialising loop and the cursor - changing a function called from twelve sites on the hottest path in the server, for a LOW-tier bullet. The gate's fixture also had to be built for this: it owned TREE but NOTHING to join it against, which is why a walk-only gate could never show who drives.
- ~~**LOW** (common table expressions) — the same leak as the EXISTS entry above: Recursive-CTE columns leak the anchor expression's node kind into the describe `name:` field (CONSTANT); the engine leaves it empty. **Confirmed 2026-09-17**: the engine gives `name:` blank with `alias: L`, this server `name: CONSTANT alias: L`, under all three attachments. Together with the EXISTS bullet (engine `BOOL`, here `CONSTANT`) and the named-derived-column case (engine blank, here `MAX`), these are one behaviour seen three ways - the describe carries this server's internal node kind where the engine carries a chosen label or nothing.~~ **FIXED 2026-09-17: A RECURSIVE CTE IS A UNION, and the engine names its columns like one.** An EXPRESSION anchor blanks, a plain FIELD anchor keeps its own - measured: `SELECT 1 AS L ...` is blank/`L` on the engine and was `CONSTANT`/`L` here, while `SELECT W AS L ...` is `W` on BOTH and always agreed. A plain `UNION` already got this right ([plan_union]'s naming pass); the recursive path took [output_cols_of]'s seed columns untouched, so the rule simply never reached it. The same predicate is now used at both sites so they cannot drift.
- ~~**LOW** (date/time arithmetic) — UNION DISTINCT with a CAST branch blanks the field name and table origin in the describe~~ **STALE, measured 2026-09-17**: `SELECT V FROM NN UNION DISTINCT SELECT CAST('x' AS VARCHAR(10)) FROM RDB$DATABASE` describes identically on both servers under all three attachments - the whole SQLDA block, name and relation included.
- **LOW** (LATERAL) — an OUTER column that is NOT NULL, used inside the lateral subquery's own expression (`FROM NN a, LATERAL (SELECT a.W * 2 AS Z FROM RDB$DATABASE) x`), is announced Nullable; the engine announces it fixed. Mechanism: the describe stand-in renders every outer reference as `CAST(NULL AS <type>)`, which is nullable by construction, so `inner_cols` cannot see that the outer column was fixed. Fixing it needs a NOT NULL stand-in of the same type and WIDTH per type (a bare literal would move the described width of text expressions), not a one-line change. The divergence is in the SAFE direction - a bit announced set that is never used - unlike announcing NOT NULL where a NULL can arrive, which desynchronises the wire. **STILL LIVE, re-measured 2026-09-17 after the expression fix landed**: `SELECT x.Z FROM NN a, LATERAL (SELECT a.ID * 2 AS Z FROM RDB$DATABASE) x` is still `Nullable` here and fixed on the engine, while `SELECT a.ID FROM NN a, LATERAL (...) x` - the outer column itself - agrees. So this is NOT the same root cause as the two struck above, which is what those bullets claimed: they were fixed by reading the inner column's bit, and this one cannot be, because `subst_lateral` renders the outer reference as a TYPED `CAST(NULL AS <type>)` whose width the describe depends on. Replacing it with a bare NOT NULL literal would move the announced width of text expressions - the failure that forced the LPAD/RPAD revert - so it needs a per-type stand-in, not a one-line change
...
- **DONE 2026-09-18 (same gate, `qa/serve-real-groupalias.sh`, four cells in section 3) — `ORDER BY` a GROUPED COLUMN THAT IS NOT IN THE SELECT LIST was refused here and answered by the engine. It now answers.** `SELECT COUNT(*) FROM T GROUP BY N ORDER BY N` - no alias anywhere - answers `2|1` on the engine and refuses here; with `N` in the select list (`SELECT N, COUNT(*) … ORDER BY N`) both answer, and so they do with an alias on the aggregate. The first filing tied this to a select-list alias and quoted its result as `1,1`; both were wrong, and a boundary probe with the alias REMOVED is what showed it. [parse_order_by_expr] resolves a bare ORDER BY name through `resolve_name` over the PROJECTION columns, so a grouped column that is not projected has no `ProjCol` to find - which is the shape to fix, and is unrelated to the aggregate-alias wrong answer recorded above. **THE SITE NAMED IN THE PRECEDING SENTENCE IS NOT THE SITE THAT FIXED IT.** The cause is one line further out: [build_group_items] DOES seed a slot for every unclaimed `key_fid`, but only inside `if !deferred.is_empty()`, so a plain `SELECT COUNT(*) FROM T GROUP BY N ORDER BY N` — nothing deferred — never got one. Fixed by seeding on demand in [plan_group]'s ORDER BY resolver, which is precisely what [resolve_having] already did: **the SECOND place the same mechanism was missing, and the first was found the same way.** The `gitems` search is bound inside its own block before the `borrow_mut`, because a `RefCell` double borrow is a RUNTIME PANIC, not a compile error — so the clean build was necessary and NOT sufficient, and only the probe (four shapes through the seeding path) could show it does not panic.
- **LOW, NEW (2026-09-18, measured)** — **a HAVING that references a SELECT-LIST ALIAS refuses on both servers, but with DIFFERENT vectors.** `SELECT COUNT(*) AS CNT FROM T GROUP BY N HAVING CNT > 1`: the engine gives SQLSTATE **42S22**, -206, *Column unknown "CNT"*; this server gives 42000 *Dynamic SQL Error*. Both correctly refuse - HAVING cannot see a select alias - so this is an error-VECTOR divergence, not a behavioural one, and ranks accordingly. **STILL OPEN, but no longer UNPINNED: section 5 of `qa/serve-real-groupalias.sh` asserts the ENGINE's -206 and this server's refusal, so if either side moves the cell goes red rather than the divergence quietly changing shape.**

- ~~**MEDIUM, NEW (2026-09-18, measured, gated by `qa/serve-real-exprvalue.sh`)** — **an EXPRESSION pattern in `HAVING` refuses, in every family, with or without an aggregate.**~~ **DONE THE SAME DAY (2026-09-18, `qa/serve-real-havingexpr.sh`, 28 cells, counted floor 28, exit 1 and 16 cells red on `c0fe691`)** - and it was WIDER than this row said: the boundary was "in HAVING the right side must be a LITERAL", so `MAX(N) = MAX(ID)`, `MAX(N) > MAX(ID) - 1`, `MAX(U) = V` and `U = V` refused too, sixteen shapes in all with no wrong answer among them. [resolve_having] folded aggregates from `rt.lhs` ONLY; it now folds and substitutes from BOTH sides and seeds a `GItem::Key` slot for every grouped column before building the synthetic view. The original text is kept because it states the shape that was measured, and the amendment states what it turned out to be. Everything below remains true as a description of the CAUSE: `HAVING U LIKE V` refuses where `WHERE U LIKE V` answers, and so do `HAVING MAX(U) LIKE MAX(V)` and `HAVING MAX(U) CONTAINING MAX(SUB)`; the LITERAL forms all answer (`HAVING MAX(U) LIKE 'caf%'`, `CONTAINING 'af'`, `STARTING WITH 'ca'`), which is what proves the boundary is the expression pattern and nothing wider. THE CAUSE IS A THIRD ROUTING SITE: `HAVING` has its own resolver, [resolve_having], which takes `Vec<Vec<RawTerm>>` and resolves against a SYNTHETIC GROUP-ROW VIEW, so it shares neither [resolve_predicate]'s routing guard (widened in chunks 37 and 38) nor [resolve_raw_cond]'s (chunk 39). Its aggregate folding reads `rt.lhs` only ([collect_aggs] at the `RawLhs::Expr` branch) and its one expression special-case matches `(RawLhs::Agg, RawKind::CmpExpr)` alone, so a pattern living in `rt.kind` is never folded or routed. Deliberately NOT half-fixed from the value-world chunk: a mistake there lands a term against the wrong synthetic slot, which is a wrong answer rather than a refusal. Two `gap` cells hold it, including the sharp no-aggregate form.
- ~~**LOW, NEW (2026-09-18, measured while narrowing the above, PRE-EXISTING and unrelated)** — `HAVING MAX(U) = MAX(V)` refuses at prepare where the engine answers it; `HAVING MAX(U) = 'cafe'` answers on both. An aggregate on BOTH sides of a HAVING comparison is the shape, and it predates every pattern chunk - found only because the narrowing probe carried it as a control.~~ **DONE 2026-09-18, in the same slice as the row above** - it was never a separate defect but the same one: an aggregate on the right was not folded into a group slot. Gated by four cells (`MAX(n) = MAX(id)`, `MAX(n) > MAX(id) - 1`, `MAX(u) = MAX(u)`, `MAX(n) <> MAX(id) - 1`), all red on `c0fe691`. **The general lesson is about the PROBE, not the fix: it was found only because the narrowing probe carried controls it did not need** - a control that never fails costs one line and is the only reason this was not left behind as "pre-existing and unrelated".
