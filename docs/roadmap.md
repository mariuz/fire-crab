...
- **MEDIUM (missing conversion, no gate green) - a CLIENT-DRIVEN `gbak -c` / `gbak -r` restore into fire-crab is not implemented.** Once the attach stopped lying (below), gbak gets much further - it op_creates the target, starts its transaction, and reads the catalog - and then dies at `op_compile` of the request GPRE built for `FOR (RDB$DATABASE) ... MODIFY`: 170 bytes of BLR whose body is `blr_label / blr_loop / blr_select / blr_receive / blr_leave / blr_handler / blr_modify`, none of which `parse_blr_request` covers, and none of which the read-only `exec_blr_stmt` could execute if they did parse. gbak reports `ERROR:Dynamic SQL Error / Exiting before completion due to errors`, rc=1. The empty shell IS created and is a real database - the engine attaches it and `gfix -v -full` is clean - it just holds no user table (`-204 Table unknown`). This is not charset-specific (a backup of a NONE-charset database fails at the same request). fire-crab's restore has always been the SERVICE path only (`isc_action_svc_restore`, `qa/serve-real-gbakrestore.sh`); `fbsvcmgr ... action_restore` through fire-crab into a fresh path still restores the rows and passes `gfix -v -full`. `blr_first` (68) was added in this chunk, which is what got the walk as far as the modify. Closing this needs the legacy BLR WRITE API (blr_store / blr_modify / blr_erase and the label-loop-select cursor protocol) and is its own chunk; it is what keeps the last 5 of `qa/serve-real-attach.sh`'s 18 checks red (the two `gbak -c`-into-a-fresh-path lines and the three `gbak -r -rep` lines).
