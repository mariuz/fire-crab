...
- **Bind-error leftovers** (a plain column against a bad text bind is DONE, `serve-real-bindconv`): an EXPRESSION left side (`DP + 0 = ?`, `ABS(I) = ?`), a JOIN ON parameter, a parameter inside an IN subquery and `CAST(? AS INTEGER)` still refuse with the generic *Dynamic SQL Error* where the engine raises the per-row conversion error; a DOUBLE column bound '1e400' / '-1e400' refuses where the engine compares against +-infinity (`DP > ?` ['-1e400'] answers every non-NULL row). Law-safe, vector and timing differ.
